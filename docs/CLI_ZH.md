...
vibe runs list --current-session --brief      # 在 Avibe Agent shell 内
vibe runs show run_abc123
vibe runs show                         # 在 Avibe Agent shell 内查看调用方 Run
```

`vibe runs list` 无过滤参数时仍保持全局列表语义；传入 `--session-id` 等参数
或 `--current-session` 才会筛选。`vibe runs show` 在 Avibe 已注入 caller
context 的 Agent run 内可以省略 run id，并默认使用 `AVIBE_RUN_ID`。
...
- 用 `--create-session` / `--create-session-per-run` 让任务使用新建 Session
- 用 `--same-scope` / `--scope-id <scopes.id>` 放置这些新建 Session
- 用 `--cwd` 指定任务创建 Session 时的工作目录
...
Session，才使用 `--fork-session <session-id>`。新 Session 默认保持源 Session
的 backend、scope 和 cwd。只有 backend 不变时，才可以通过 `--agent`、
`--model`、`--reasoning-effort` 覆盖 fork 后 Session 的 Agent、模型或推理强度；
跨 backend fork 会被拒绝。不要把 fork 参数和 `--session-id`、`--create-session`
或 `--create-session-per-run` 混用，也不要和 `--post-to` 混用。

在 Avibe 已注入 caller context 的 Agent 环境内发起异步 run 时，callback
默认指向当前调用方 Session。只有需要显式覆盖时才使用 `--callback-session-id`。
当你有意不自动回调、后续会通过 `vibe runs show` 或 runs 列表/轮询查看结果时，
使用 `--no-callback`。callback 与普通投递相互独立；system、tool call、
assistant 中间过程消息不会包含在 callback 里。
...
`--create-session`、`--same-scope`、`--scope-id` 和 `--name`。watch 与 task
使用同一套目标语义：在 Avibe 已注入 caller context 的 Agent shell 内可以省略
`--session-id`，默认回到调用方 Session；如果 watch 要创建新的可见 Session，
使用 `--create-session --same-scope` 或 `--create-session --scope-id <scopes.id>`。
如果每次触发都要创建新 Session，则用 `--create-session-per-run` 搭配同样的
scope placement 参数。需要可管理、可暂停、可查看的后台等待任务时，优先使用
`vibe watch`，不要随手起 `nohup`。
