...
通过现有 mode-0600 控制器 socket 读取当前范围内的本地记忆，或提交需要记住的长期内容——既包括用户明确要求保存的内容，也包括在设置中开启「主动记录」后，Agent 从对话以及在本机工作中主动提炼的结论（含在文件或工具输出中遇到的持久环境、账户事实）。该命令不会启动服务，也没有清空、配置、导出或删除子命令。
