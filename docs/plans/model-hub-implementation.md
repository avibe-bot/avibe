...
## Routing modes delivery - contract_version 9
...
`0de3d2f47`/`9cb9ebb53`, and atomic-admission amendment `c50bc43a5`. Its delivery ownership replaces
...
Persist `routes` as sparse manual overrides: absent means automatic, present means exact
manual intent, including empty/stale/dormant arrays. Source creation and catalog reconcile
never seed or append manual routes. Sources PUT edits backend default membership/order;
manual PUT creates intent even when equal to automatic, DELETE restores by removing the
key, and preview evaluates an isolated draft without side effects. One pure resolver owner
...
Schema/API/type/mirror closure is contract_version 9 on the complete tested head.
...
TurnProvenance accepts released versions 5 through 9 with its historical required fields;
...
the backend/model GET and the ordinary v9 envelope, never through AgentChain. Optional
...
- The terminal `contract_version` is owner-fixed at **9** in every registered version consumer:
...
| **FC-05** | `agent-supply.schema.json` | Each backend stores mode, default sources.order and a sparse map of manual {hops} overrides. Key absence inherits; key presence is exact intent, including empty. Actual effective routes own adoption and summaries; every model_supply row requires route_origin and effective chain_length, with null origin and false has_runnable_hop exactly for zero-length plans. Health remains a separate projection. New menu rows receive no key. |
| **FC-06** | `agent-chain.schema.json` | AgentChain requires manual_override and route_origin and returns effective Source/model hops in planner order. Manual arrays remain exact; empty plans have null origin. Live current/runnable/blocker/retry annotations preserve the tier and membership. API-key inventory absence alone is not model_unsupported; subscriptions retain known-model admission; explicit retirement remains a blocker. Existing healthy/cooldown/backoff/action/process precedence remains unchanged, including native_cli_unavailable and waiting/interrupted relations. |
...
and sparse manual overrides. Key absence inherits; key presence preserves exact submitted
hops, including empty and equal-to-automatic arrays. No second persisted policy flag or
separate mapping object exists. Existing supported persisted arrays, including stale and
dormant records, load without historical-authorship inference; new catalog rows have no key.
...
`contract_version: 9` under the PM ruling of 2026-08-12 00:37.
...
`contract_version: 9` under the PM ruling of 2026-08-12 00:15.
