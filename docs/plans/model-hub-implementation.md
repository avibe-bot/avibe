...
| `tests/scenarios/auth_setup/{catalog.yaml,test_auth_setup_scenarios.py}` | I3 owns AUTH-SETUP-210 after K4 and #1312 merge: block the first OAuth-start provider call, lose that caller's response, overlap a same-nonce retry that coalesces to the same terminal flow/provider, then assert provider start exactly once. Pending-start failure/task-cancellation release and committed-flow replay cover the other decision rows; I3 unit/API fixtures separately prove explicit canceled-flow replay starts no provider and expiry permits exactly one fresh start. I5 consumes this evidence but does not edit either file. **OAuth explicit-cancel revision: 2026-08-12 00:37 by PM ruling.** |
