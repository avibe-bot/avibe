...
lanes. The I1–I7 split is **tentative pending the UI lane's design-to-spec reconciliation table**;
...
| **I3 subscription custody and native import** *(tentative)* | codex | `core/handlers/model_hub/{oauth,native_oauth,revocations,migration}.py`; `tests/test_model_hub_oauth.py`; `tests/scenarios/model_hub/test_model_hub_migration_scenarios.py`; `tests/scenarios/auth_setup/catalog.yaml`; `tests/scenarios/auth_setup/test_auth_setup_scenarios.py`. The migration scenario and complete auth-setup catalog/test loop are the sole carve-outs from I5's scenario tree. The K4 increment owns AC-48's OAuth registry: it atomically claims the exact nonce/vendor/channel tuple before provider work, coalesces concurrent retries to one pending terminal result, releases failed/canceled starts after cleanup, and atomically converts success to one echoed-nonce flow. AUTH-SETUP-210 blocks the first provider call, overlaps the same-nonce retry, and proves provider invocation exactly once; I7 owns only the API/service transport boundary | K4 merged + #1312 merged |
| **I4 Sources / Gateway UI** *(tentative; one lane, two increments)* | codex | `ui/src/components/settings/models/**`; `ui/src/i18n/*.json`; `vibe/i18n/*.json`; UI tests. Both i18n trees are I4-exclusive. The current increment consumes the already-specified install/unsupported-host surfaces for AC-38/AC-43; each model row may open frame 02, render the drawing, and consume the chain read projection, but no chain-write caller is wired and Save remains disabled. Exact-chain editing and its UI tests belong to the second increment only after the G-32 specification round merges. That second increment also echoes the shared `guard_token` on confirmed mutations, but only after I7 freezes the token-bearing refusal fixtures; the current increment continues to consume master's pre-I7 no-token refusal and must not add an inactive caller field. It consumes AC-36/37/39–42/44–48 only after K3 changes their G rows from gaps to contract references; AC-49/AC-50 evidence and live-backoff copy activate only after K5 aligns the UI specification and I7 freezes their producer fixtures. This is an ordering split inside I4, not a new lane identity. **Revised 2026-08-11 18:10, 18:32, and 19:56 by PM/owner ruling.** | current increment: I1 envelopes + K4 contracts; second increment: G-32 specification round + K3 follow-up + K5 UI-spec sync + frozen I7/I2/I3 fixtures |
| **I7 contract-completion implementation (backend)** | codex | After #1312 merges, exclusive ownership transfers from I1 for `config/v2_config.py`; `core/handlers/model_hub/{service,resolver,classification,rpc,request,errors,events}.py`; `vibe/{ui_server,model_hub_client}.py`; `tests/test_model_hub_config.py`; `tests/test_model_hub_api.py`; and `tests/test_model_hub_resolution.py`. I7 owns AC-35–AC-47 and AC-49–AC-50's backend producers/tests plus AC-48's API/service half. `service.py` is the sole shared guard-plan/token owner and implements every guard-totality row, including supplied-token unforced behavior and old-token empty-plan success; route layers only transport the token. The same service/config boundary persists the server-keyed full-request Source-create claim/receipt, exactly replays its original committed envelope only on a matching digest, rejects a mismatch, and deletes the receipt with its Source. The transferred resolver/classifier/event boundary owns `retired: false` filtering and the four network-failure cells: only explicit shaped errors enter existing non-permanent Source classifications; an unclassified pre-first-byte failure creates bounded in-memory backoff; an unclassified post-first-byte interruption is event-only. I7 projects optional observation evidence without inventing facts. I7 delegates OAuth tuple claim/coalescing to I3's registry and owns only its service/API transport. Its first AC-50 mechanical action is the sole owner-authorized I2-file exception: delete the strict xfail marker from `tests/test_model_hub_l3.py::test_chain_projection_and_probe_latency_partition` in the same commit that makes it XPASS; it otherwise consumes I2 runtime phase/error facts and I3 OAuth fixtures without editing their files, and it never edits contracts, UI/i18n, scenario trees, or any I2–I5 file. **Revised 2026-08-11 through the 20:20 PM gate ruling.** | K4 merged + #1312 merged; all I1-to-I7 file transfers activate only after #1312 merges |
...
**Executor ruling (owner, 2026-08-09; I7 added 2026-08-11).** Every new I1–I7 implementation lane uses
...
must merge before I2–I5. K4 and #1312 must both merge before I7. I2, I3, and I7 may
then proceed in parallel under the exclusive file split. I4's current increment may
...
round, the K3 follow-up, and frozen I7/I2/I3 fixtures. I5 closes integration after I1–I4 and
...
the explicit I6 transfer for the release-gate pair. `tests/test_model_hub_l3.py` has one serial ownership transfer: I1 alone updates its
versioned contract/provenance fixtures on the first tested head; only after I1 merges
does I2 own the whole file. I2–I5 stop and report to the orchestrator if their work
...
For the K4 implementation batch, I7's listed files transfer only after #1312 merges.
...
`model-hub-contracts/` and the exact mechanical closure in §8. After K4 merges, those
files are read-only to I2–I7. All remaining handoff rows still must land before release;
...
## 8. Implementation acceptance criteria (AC-1–AC-50; v3/K4 addenda through 2026-08-11)
...
inventory. The current ledger has **thirty-three** criteria. AC-22 onward are
...
| **FC-02** | `mirror-registry.json` | Mechanically covers every closed enum and cross-file identity used by the final files: three protocols, backend ids, Source/model origins, exact-hop blocker/reason/detail-key/remedy pairs (including `native_cli_unavailable` and live connection backoff), credential refresh-capability branches, guard/OAuth/Source-create state machines, network phase totality, observation-evidence reasons, turn outcome/discriminator/copy-key rows, native-import actions and locale keys, event agents, the sole remaining eligibility reason, `added_to`/`adopted_by` shapes, channel defaults, the four `supply_status` labels, the derived takeover label, and UI locale homes. API-boundary-only refusal values register their contract field, named negative route test, and forbidden UI/i18n scope instead of fabricating a render consumer. The same live-input invocation proves retired route literals are absent repo-wide and path identities do not reappear in the Source-model request bodies. Every mirror names an executable relation and terminal version **5**. |
...
| **FC-05** | `agent-supply.schema.json` | Each backend stores `mode: hub | direct`, one explicit `sources.order`, and exactly one `{hops: [{source_id, model_id}, ...]}` row per menu model. `hops` is always present and may be empty; neither order has a policy discriminator. It enforces at most one native Source per backend and exposes `adopted_by: [{backend, menu_model}]` from persisted references; Source-card attribution reuses that projection. `supply_status` is the sole backend-health projection. Fresh-install construction uses Gateway; the onboarding service supplies Direct for an existing installation with no Model Hub state. No separate mapping, matching, enrollment, or takeover field exists. |
| **FC-06** | `agent-chain.schema.json` | Returns the stored exact Source/model hops in the same order with only `current`, `runnable`, closed blocker reasons (`source_missing`, `model_unsupported`, `native_cli_unavailable`, Source-state reasons, and live `models.source.backoff.connection_failed`), and retry metadata added. `health: backoff` is a Source-scoped in-memory pre-first-byte throttle with a future `retry_at`; it never persists in Source/config state and rolls up as self-healing `waiting`. The shape has no route-policy or capability-matching discriminator. Takeover derives from stored first hop plus live current position and adds no field. |
| **FC-07** | `observation-result.schema.json` + `probe-result.schema.json` | Keeps unsaved Add Source connectivity/protocol/inventory observation separate from saved-Route runtime probes. Add-flow results report classified reachability/authentication plus an observed protocol only after a real upstream response, never a credential ref. Optional evidence contains only a sanitized request label, an actually observed HTTP status, and the closed failed-discovery reason; omission forbids client fabrication. A failed observation may request a manual three-value probe-order hint but cannot save a protocol without response proof. `ProbeResult` remains the configured-Route runtime domain and uses the distinct live backoff key for unclassified pre-first-byte connection failure. |
...
| **FC-12** | `api.md` | **Revised 2026-08-11 by the owner-authorized K4 completion; guard/nonce totality corrected through 19:42 and network policy through 19:56.** Contracts Source CRUD, add-only unsaved connectivity/protocol observation and discovery, the sole saved `POST /sources/<id>/refresh` mutation, the unified `/sources/<source_id>/models` subresource for manual creation, all-inventory reasoning-list edits, discovered-model retirement, and manual deletion, the explicit backend Source-order PUT, exact route GET/PUT, mode, events/provenance, native-config import, OAuth, and Direct-mode responses. Source create returns exact `added_to` positions; Source cards use `adopted_by`; an optional nonce claim stores a server-keyed full-request digest and committed terminal receipt for exact lost-response replay. OAuth start claims its nonce/vendor/channel tuple before provider work and coalesces a concurrent retry. Unsaved observation evidence is optional and server-factual. Every Source/inventory mutation mirrors §4.5's total matrix row-for-row, including unguarded writes that cannot change `id`, `origin`, or Routes. Every nonempty forced guarded-impact plan requires the exact opaque `guard_token` from its refusal; stale nonempty plans return the same 409 family with `refusal_reason: plan_changed` and never remove an unconfirmed hop, while an empty recomputed plan follows ordinary success with force/token inert. The network totality table admits only shaped errors to existing Source classifications, gives an unclassified pre-first-byte connection failure bounded live backoff, and makes an unclassified post-first-byte interruption event-only. Forced Source/manual-model deletion preserves survivor order; discovered-model DELETE instead persists `retired: true`, retains the row through refresh, and uses the same exact-hop and protected-supply guards. OAuth-start rejects a duplicate native Source before adapter invocation. No saved `/test`, separate saved discovery, route policy, separate mapping, takeover, experimental-consent, or vendor-guessed protocol surface exists. |
...
**Implementation and test landing checklist (complete before release).** The I1–I5 and I7 PRs
collectively touch every applicable row and leave no downstream compatibility task:

| Landing point | Required result |
| --- | --- |
| `config/v2_config.py` | Final Source/model/protocol/configured-route types, one canonical validation boundary shared by every persisted-Source writer and subsequent load, singleton and exact-pair uniqueness validation, one explicit Source order per backend and one explicit hops array per menu model, persisted discovered-model retirement plus Source-create nonce/claim/receipt lifecycle, serializer completeness, fresh-install Gateway construction, and existing-install-with-no-Hub-state Direct onboarding. No policy state exists. The receipt stores only a server-keyed full-request digest and the original terminal envelope; it stores no plaintext key, disappears after pre-commit settlement or Source deletion, and survives with its Source for exact committed replay. Unclassified transport failure and live connection backoff never serialize into Source/config state. **Retirement/nonce/network revision: 2026-08-11 19:56 K4.** |
| `core/handlers/model_hub/{service,resolver,classification,errors}.py` | One §4.3 stored-chain executor and one Add Source implementation of §4.2's placement policy; the latter chooses and persists each matched hop position before returning `added_to`. Also owns the complete credential-failure matrix keyed by refresh capability, closed error classes, `adopted_by`, Source-global health, local-engine distinction, and all route/inventory guards. After #1312 merges, I7 owns `resolver.py`, `classification.py`, and `tests/test_model_hub_resolution.py`: Add-time matching, inventory membership, new-Route validation, runnability, and invocation admit only `retired: false` rows; the network classifier executes every shaped/unclassified × before/after-first-byte cell and alone owns live connection-backoff state. `service.py` is the single shared guard planner/token verifier for every matrix row; it binds normalized input, exact nonempty plan, and persisted versions, refuses an unconfirmed nonempty plan, treats a supplied token as inert when force is false, and treats force or an old token as inert after recomputation reaches an empty plan. Discovered-model DELETE persists the retirement tombstone and refresh never revives it. Source-create persists a server-keyed digest claim before observation/credential work, releases it only after retained-material reconciliation, atomically reclaims it after release, commits the original response receipt with its Source, replays only a matching request, and deletes the receipt with the Source. The observation assembler publishes only evidence it holds. Runtime never matches, places, substitutes, or writes a network health verdict. A static-key `401` performs no retry; only refresh-capable credentials receive one bounded refresh. **Retirement/nonce/guard/network totality revision: 2026-08-11 19:56 K4.** |
| `core/handlers/model_hub/{oauth,native_oauth,revocations,migration}.py` | Vendor-specific OAuth defaults, duplicate-native rejection before adapter work, all-exit transient credential cleanup, durable revoke reconciliation, pre-provider exact-tuple OAuth-start claim/coalescing/release/flow conversion and nonce echo, and the complete four-row native-config action matrix with original files untouched. Every imported Source passes the canonical final-shape validator before commit; no import writer bypasses it with direct dataclass construction. **OAuth nonce revision: 2026-08-11 19:42 K4.** |
| `core/handlers/model_hub/{rpc,request,provenance,events}.py`, `vibe/ui_server.py`, `vibe/model_hub_client.py`, `core/controller.py` | Final API envelopes/routes mirror the guarded-mutation and guard-totality matrices, including token-bearing nonempty refusal, empty-plan ordinary success, Source-delete, discovered-model retirement, whole-Route replacement, existing-chain reorder, runtime install/no-op, all three nonce-retry states, optional observation evidence, and live connection-backoff reads. `rpc.py`, `request.py`, `ui_server.py`, and `model_hub_client.py` only carry `guard_token`; they do not mint, verify, or reinterpret it. After #1312, `events.py` transfers from I1 to I7 and records each network-totality cell without turning an unclassified failure into persistent state; unclassified post-first-byte interruption is event-only. Provenance mirrors the complete frozen turn-outcome matrix including `canceled`; Direct-mode responses remain explicit. During implementation, absence of `VIBE_MODEL_HUB_ENABLED` keeps the controller/routes/UI disabled; explicit enablement must construct the v3 aggregate. This is a release-control gate, not a compatibility layer. I6 removes it only after I5 evidence is green. **K4 route/evidence/network revision: 2026-08-11 19:56.** |
...
| `core/handlers/model_hub/turn_gateway.py`, `vibe/model_hub_runtime/**` | Exact stored-hop execution, three-protocol transport, credential refresh capability, authoritative before/after-first-byte facts, pre-stream fallthrough, post-stream no replay, and local Gateway failures at any request phase excluded from Source state and fallback. I2 transports raw phase/shaped-error evidence but does not own the network taxonomy, Source decision, or live-backoff executor transferred to I7. The runtime owns the persisted install worker/lease, orphan reconciliation, host detection, and installed-state no-op without owning HTTP routes. Turn copy is selected from the authoritative outcome matrix after I7's state decision and live annotation of the same stored chain. **Install/network ownership revision: 2026-08-11 19:56 K4.** |
| `ui/src/components/settings/models/**` | Final Source/Gateway types and calls; protocol selector only after failed observation as a probe-order hint; Source cards reuse `adopted_by`; Add Source renders the stored backend Source order plus returned `added_to` positions without inferring newness from order; backend groups consume `supply_status`; takeover derives from the chain; saved Source details exposes only guarded Refresh models; Direct groups expose reversible mode actions. For frame 02, I4's current increment opens the editor from each model row, renders the drawing, and consumes the chain read projection only: it wires no chain-write caller and keeps Save disabled. Exact stored-hop/mapping editing and `guard_token` echo belong to I4's second increment after the G-32 specification round, K3 follow-up, and I7 token-bearing refusal fixtures merge. Observation evidence and live connection-backoff presentation also belong to the second increment, after K5 corrects their UI-spec presence/copy rules and I7 freezes producer fixtures. The current increment consumes master's pre-I7 refusal shape and must not add an inactive token caller. The current increment also consumes the already-specified install/unsupported-host states; the second consumes the remaining K4 projections/actions only after their named specification upgrade. **Frame-02/guard/evidence split revised 2026-08-11 through 19:56 by PM/owner ruling.** No bottom-only “new” section, policy control, “not enabled” hint, latency/last-check copy, consent/experimental surface, separate mapping object, takeover field, supplying-backends field, or vendor default protocol exists. Any new-item marker is transient presentation state, not a route field. `vendorMeta.ts` may order probes but cannot choose a saved protocol. Narrowing `SourceProtocol` must be checked against production and test files, not only the production-only `tsconfig.app.json` program. |
| `ui/src/i18n/{en,zh}.json`, `vibe/i18n/{en,zh}.json` | Exact mirrored final reason/detail and `supply_status` keys, including `native_cli_unavailable` and `route_unconfigured`; every Turn-outcome copy-matrix key; the derived takeover label; distinct Direct/Native terms; the sole Claude Hub warning; compact protocol-observation failure copy; and no retired policy, consent, experimental, or mechanism-copy keys. After K5's UI-spec sync, I4's second increment adds the distinct short connection-backoff copy for `models.source.backoff.connection_failed`; it never reuses quota-cooldown wording. A registry-generated guard compares closed outcome/discriminator, blocker/remedy, and credential-remedy relations with both locale sets. |
| `tests/test_model_hub_config.py` | The adapter parity gate at current line 159 remains exact; the mirror-registry version gate at current line 236 and provenance version gate at current line 314 use terminal value **5**; round-trip/completeness fixtures use only final Source and route shapes. It validates the server-private Source-create claim/receipt schema, rejects plaintext or unknown fields, proves claim-without-envelope versus committed-envelope shape, validates every optional-observation-evidence example/negative, and proves live backoff cannot serialize as Source state. **Receipt/evidence/network fixture revision: 2026-08-11 19:56 K4.** |
| `tests/test_model_hub_api.py` | The current line-1204 `experimental_consent_at` assertion becomes an absence assertion; API fixtures cover unique final model entries, edited-effort and retirement preservation across rediscovery, distinct add-only unsaved operations versus the sole guarded saved refresh, all-exit discovery cleanup, three protocols, observation-before-save, all three pre-observation nonce retry states, deterministic policy-chosen Add Source placement with the returned persisted position, and every guarded-mutation row. Guard fixtures cover every guard-totality row and token subcase: unforced empty/nonempty plans with no token and with an inert supplied token; forced nonempty exact/missing/stale tokens; and forced empty plans with no token or an old token. They prove every nonempty confirmed plan commits once, every nonempty unconfirmed plan returns the current token-bearing refusal without removing a hop, and every empty plan follows ordinary success without a fabricated 409. Source-create fixtures prove in-flight conflict/no work, released atomic re-claim/exactly one new attempt, committed same-digest exact replay after Route/Source changes, committed different-digest conflict for every substantive field including key bytes, and Source-delete receipt removal/reclaimability. Observation fixtures cover evidence omission/request/status/reason totality without guessed facts. Network fixtures cover all four shaped/transport × before/after-first-byte cells, bounded auto-clearing live backoff, and zero config writes for unclassified failures. The same file covers Source deletion from all backend Source orders and routes with survivor order preserved, discovered retirement, existing-chain reorder, runtime install/no-op, duplicate-native pre-adapter rejection, adoption, and absence of policy/enrollment/takeover/supplying sibling fields. **K4 totality/receipt/evidence/network revision: 2026-08-11 19:56.** |
| `tests/test_model_hub_{resolution,runtime,oauth,l3}.py` | After #1312 merges, `tests/test_model_hub_resolution.py` transfers to I7 and proves `retired: false` filtering in Add-time matching, inventory membership, new-Route validation, runnability, and invocation plus the complete network-classification/live-backoff table; I2 retains runtime/l3 files and supplies exact first-byte phase and raw shaped/transport facts without owning state decisions, while I3 retains OAuth. Together they cover sole §4.3 stored-chain consumption, no runtime matching or substitution branch, exact effort membership, every credential-capability row, configured-order fallthrough, every turn-outcome/copy row, blocked `no_candidate` detail, `native_cli_unavailable`, engine loss before/during/after streaming with no Source mutation or replay, persisted install/restart/no-op behavior, canceled provenance, vendor OAuth defaults, and OAuth nonce released/in-flight/committed totality with pre-provider claim, concurrent coalescing, cleanup release, atomic flow conversion, and echo. Until I7 lands AC-50, `test_chain_projection_and_probe_latency_partition` carries the 2026-08-11 20:20 PM-authorized strict xfail for the removed persistent network spelling; I7's first mechanical action deletes that marker in the same commit as the implementation, so an accidental early fix is a failing XPASS. I1 moves the versioned contract/provenance fixtures in `test_model_hub_l3.py` to terminal version **5** on its same tested head; after #1312 merges, I2 owns that whole file's runtime-behavior updates except that one deletion. **K4 retirement/OAuth/network ownership revision: 2026-08-11 20:20.** |
| `ui/src/components/settings/models/**/*.test.*` | No protocol control on the normal add flow, honest manual probe-order fallback, final inventory editing, visible and adjustable backend Source order plus policy-chosen Add Source placement, no position-based newness or bottom-only new section, one guarded saved refresh button, adopted/supply-status projection consumption, derived takeover versus exhausted rendering, reversible Direct mode, distinct Native copy, and no policy/not-enabled/latency/consent surface. Under the 2026-08-11 18:10 PM ruling, the current-increment frame-02 fixture proves that a model row opens the drawn editor, renders the chain read projection, keeps Save disabled, and emits no chain mutation. Exact configured-chain editing and its save/refusal/reconciliation tests are second-increment obligations after the G-32 specification round, K3 follow-up, and I7 fixtures merge; those tests echo the returned `guard_token`, replace the displayed plan on `plan_changed`, and never force without a token. After K5's UI-spec sync and I7 fixtures, the same second increment proves evidence renders only when present and live connection backoff uses its distinct short-delay copy rather than quota cooldown copy. I4 supplies a mechanical type-check gate that includes these test files despite `tsconfig.app.json` excluding them. |
| `tests/scenarios/auth_setup/{catalog.yaml,test_auth_setup_scenarios.py}` | I3 owns AUTH-SETUP-210 after K4 and #1312 merge: block the first OAuth-start provider call, lose that caller's response, overlap a same-nonce retry that coalesces to the same terminal flow/provider, then assert provider start exactly once. Failure/cancellation release and committed-flow replay cover the other two table rows. I5 consumes this evidence but does not edit either file. **Lost-start concurrency revision: 2026-08-11 19:42 by PM ruling.** |
...
plan, and returns an opaque token bound to method, target, normalized substantive input,
the exact staged mutation/guard plan, and all relevant persisted-state versions. The
fixture covers every row and token subcase of the force/plan/token totality matrix:
unforced empty success with no token and with an inert supplied token; unforced nonempty
`confirmation_required` with no token and with an inert supplied token; forced nonempty
exact-token success; forced nonempty missing-token `confirmation_required`; and forced
nonempty stale-token `plan_changed`. It also changes the world after refusal
so the forced retry carries the old token but recomputes to an empty plan; that request
must take ordinary success without a 409 or removed hop. The closed reason, error, and
guard-decision vocabularies are checked through `mirror-registry.json`.
...
`tests/test_model_hub_api.py`. After the G-32 specification round and K3's G-28
...
**Acceptance.** When SourceCreate supplies a valid `client_nonce`, the server computes a
server-keyed HMAC-SHA-256 over every canonical substantive field, including the exact
transient key bytes, and atomically persists that private claim before observation or
credential work. The digest and key never appear in an API response or log, and no
plaintext key enters the record. The committed Source echoes the nonce unchanged so
list reads expose exactly one match. One table-driven fixture covers every state/action
cell. `nonce.in_flight` returns HTTP 409 `source_nonce_conflict` before observation or
provisioning. `nonce.released` first settles AC-26 cleanup, then the same-nonce retry
atomically records its digest and performs exactly one fresh attempt. `nonce.committed`
with the same digest returns the exact stored original terminal envelope and performs
neither again; after intervening Source/Route edits, its original `source`, `added_to`,
`adopted_by`, and positions remain unchanged. A committed retry whose vendor, default-
expanded display name, canonical/default-expanded Base URL, protocol order, or key bytes
differ returns the existing HTTP 409 `source_nonce_conflict`. Source deletion removes
the receipt atomically and makes the nonce claimable again. After a simulated lost
response the client reads first; a match finishes reconciliation and a miss retries the
same nonce, so no state can dead-wait. Reconstruction reconciles retained material before
deleting an ownerless claim. Omitting the nonce preserves existing create behavior.

**Binding handoff (owner / files / activation).** After K4 and #1312 merge, I7 owns
`config/v2_config.py`, `core/handlers/model_hub/{service,rpc,request,errors}.py`,
`vibe/{ui_server,model_hub_client}.py`, and
`tests/test_model_hub_{config,api}.py` for receipt-schema validation, keyed digest,
claim/receipt persistence, cleanup/recovery, Source-delete release, and the complete
state/action AC-47 fixture. After K3 upgrades G-29 and I7 freezes same-digest exact-
receipt replay, different-digest conflict, in-flight conflict, released reclaim,
cleanup/recovery, Source-delete release, and lost-response fixtures, I4's second increment
...
response echoes it. One table-driven fixture covers all three states: released claims
start exactly once; an in-flight same-tuple retry coalesces to the same pending terminal
result without a second provider invocation; and committed retries return the same
`flow_id`, state, and presentation. A shared provider-start failure or cancellation
releases only after cleanup, and the next retry starts one fresh attempt; provider
success atomically converts the claim to the flow. A different tuple cannot resolve to
that flow. Omitting the nonce preserves existing one-action/one-start behavior.
...
once; the OAuth unit table also covers release and committed replay. At the same
activation edge, I7 owns only
`core/handlers/model_hub/{service,rpc,request,errors}.py`,
...
omitted-nonce, released/in-flight/committed, and concurrent lost-response fixtures, I4's second increment owns
...
### AC-49 — Carry honest observation evidence for §1.5 detail copy

**Acceptance.** `observation-result.schema.json` accepts an optional `evidence` object
and rejects every unknown evidence field. When present, `request` is a sanitized
`METHOD /path` probe label without host, query, credential, or response body; `status`
is an optional integer HTTP status and implies `reachable: true`; and `reason` is
present only for `discovery: failed`, using exactly `rate_limited | transport | unknown`.
An evidence-bearing discovery failure requires that reason; every other discovery value
rejects it. The server omits `evidence` when it cannot prove the facts, and clients omit
the evidence detail rather than inventing request/status/reason. One schema/API fixture
covers evidence absent, ambiguous HTTP evidence, transport evidence without status,
all three discovery reasons, reason on a non-failed discovery, status with non-true
reachability, a query-bearing request label, and unknown fields. Contract version stays 5.

**Binding handoff (owner / files / activation).** K4 freezes
`docs/plans/model-hub-contracts/{observation-result.schema.json,api.md,mirror-registry.json}`.
After K4 and #1312 merge, I7 owns the producer/API projection in
`core/handlers/model_hub/{service,rpc,request}.py`,
`vibe/{ui_server,model_hub_client}.py`, and
`tests/test_model_hub_{config,api}.py`. After K5's UI-spec follow-up changes §1.5's
request/reason presence rule and G row to this contract reference, and I7 freezes
absent/HTTP/transport/reason fixtures, I4's
second increment owns
`ui/src/components/settings/models/{types.ts,modelsApi.ts,AddApiKeyDialog.tsx,dialogFields.test.tsx}`.
I5 then consumes the frozen behavior in
`tests/scenarios/model_hub/{catalog.yaml,test_model_hub_live_resolution_scenarios.py}`.
...
phase (`before_first_byte` versus `after_first_byte`). Both shaped cells enter exactly
their existing non-permanent Source classification and unchanged recovery rule; phase
changes only retry/replay behavior. Unclassified pre-first-byte connection failure
retains Source/config bytes exactly, emits the redacted network event, and creates one
...
first byte, Source endpoint/credential replacement, or process reconstruction clears
the streak. Unclassified post-first-byte interruption emits only the event: it writes no
...
the closed reason, and a future deadline. Contract version stays 5 under the owner’s
pre-release semantic correction of 2026-08-11 19:44–19:56.

**Binding handoff (owner / files / activation).** K4 freezes
`docs/plans/model-hub-contracts/{source,agent-chain,agent-supply,probe-result}.schema.json`,
`api.md`, and `mirror-registry.json`. After K4 and #1312 merge, ownership transfers from
I1 to I7 for `core/handlers/model_hub/{service,resolver,classification,errors,events,rpc}.py`,
`vibe/{ui_server,model_hub_client}.py`, and
`tests/test_model_hub_{api,resolution}.py`; I7 alone owns the classifier, live-backoff
executor/read projection, Source no-write assertions, and event-only cell. I2 retains
...
shaped/transport and first-byte facts but does not classify or mutate Source/backoff
state. Under the 2026-08-11 20:20 PM gate ruling, I7's first mechanical action is to
delete the strict AC-50 xfail marker from
`tests/test_model_hub_l3.py::test_chain_projection_and_probe_latency_partition` in the
same commit that makes the old-spelling assertion XPASS; no other I2-file edit is
authorized. After K5 registers the distinct short-backoff copy and I7 freezes all four cells
plus reset/cap fixtures, I4's second increment owns
...
| **Assign the shared backend launch seam to an active lane**<br><br>When I1 replaces resolver/config shapes and I2 changes exact-hop transport, `modules/agents/model_hub.py` must change as well: it is the shared launch boundary for Claude, Codex, and OpenCode and currently imports and executes the runtime-matching resolver through `resolve_model_hub_turn`, `matching_sources`, and eligibility walks. None of the active I1–I5 scopes or landing rows owns this file; its only assignment is in the explicitly non-binding historical L3 table, so every active lane can complete while backend launches still use the retired routing model. Assign this shared seam and its backend call-site coverage to I1 or I2.<br><br>AGENTS.md reference: [AGENTS.md:L28-L30](https://github.com/avibe-bot/avibe/blob/0309b62d167ede209bd429afc401144c93cc677a/AGENTS.md#L28-L30) | AC-22 | `modules/agents/model_hub.py`; shared Claude/Codex/OpenCode launch call-site tests | I2 |
| **Specify the add-time model matching algorithm**<br><br>**Closed by owner ruling 2026-08-09: `matching-v1`.** The Add Source path transcribes the former resolver semantics: Claude native aliases use discovered-only inventory and the deterministic `(version_tuple, date_or_zero, model_id)` maximum; OpenCode uses exact checked identifiers or a unique bare suffix; Codex and non-native sources use literal equality. The stored hop is the concrete upstream id and runtime never repeats matching. | AC-22 | `model-hub.md` §4.2 `matching-v1`, `opencode-overlay.md`, `api.md`, `core/handlers/model_hub/{service,resolver}.py`, and backend fixtures in `tests/test_model_hub_{api,resolution}.py` | I1 |
