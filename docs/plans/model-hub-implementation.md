...
| **I7 contract-completion implementation (backend)** | codex | After #1312 merges, exclusive ownership transfers from I1 for `config/v2_config.py`; `core/handlers/model_hub/{service,resolver,rpc,request,errors}.py`; `vibe/{ui_server,model_hub_client}.py`; `tests/test_model_hub_config.py`; `tests/test_model_hub_api.py`; and `tests/test_model_hub_resolution.py`. I7 owns AC-35–AC-47's config/service/API producers and tests plus AC-48's API/service half. `service.py` is the sole shared guard-plan/token owner and implements every guard-totality row, including old-token empty-plan success; route layers only transport the token. The same service boundary implements all three same-nonce retry states, including released-claim re-acquisition and committed Source readback. The transferred resolver and resolution test own `retired: false` filtering for Add-time matching, inventory membership, and runnability. It consumes I2 runtime and I3 OAuth fixtures without editing their files, and it never edits contracts, UI/i18n, scenario trees, or any I2–I5 file. **Revised 2026-08-11 18:32 and 19:10 by PM ruling.** | K4 merged + #1312 merged; resolver ownership activates only after #1312 merges |
...
## 8. Implementation acceptance criteria (AC-1–AC-48; v3/K4 addenda through 2026-08-11)
...
| `core/handlers/model_hub/{service,resolver,classification,errors}.py` | One §4.3 stored-chain executor and one Add Source implementation of §4.2's placement policy; the latter chooses and persists each matched hop position before returning `added_to`. Also owns the complete credential-failure matrix keyed by refresh capability, closed error classes, `adopted_by`, Source-global health, local-engine distinction, and all route/inventory guards. After #1312 merges, I7 owns `resolver.py` and `tests/test_model_hub_resolution.py`: Add-time matching, inventory membership, new-Route validation, runnability, and invocation admit only `retired: false` rows. `service.py` is the single shared guard planner/token verifier for every matrix row; it binds normalized input, exact nonempty plan, and persisted versions, refuses an unconfirmed nonempty plan, and treats force or an old token as inert after recomputation reaches an empty plan. Discovered-model DELETE persists the retirement tombstone and refresh never revives it; Source-create claims a nonce before observation/credential work, releases it only after retained-material reconciliation, and atomically reclaims it on the next same-nonce retry. Runtime never matches, places, or substitutes. A static-key `401` performs no retry; only refresh-capable credentials receive one bounded refresh. **Retirement/nonce/guard totality revision: 2026-08-11 K4.** |
...
| `core/handlers/model_hub/{rpc,request,provenance,events}.py`, `vibe/ui_server.py`, `vibe/model_hub_client.py`, `core/controller.py` | Final API envelopes/routes mirror the guarded-mutation and guard-totality matrices, including token-bearing nonempty refusal, empty-plan ordinary success, Source-delete, discovered-model retirement, whole-Route replacement, existing-chain reorder, runtime install/no-op, and all three nonce-retry states. `rpc.py`, `request.py`, `ui_server.py`, and `model_hub_client.py` only carry `guard_token`; they do not mint, verify, or reinterpret it. Provenance/events mirror the complete turn-outcome matrix including `canceled`; Direct-mode responses remain explicit. During implementation, absence of `VIBE_MODEL_HUB_ENABLED` keeps the controller/routes/UI disabled; explicit enablement must construct the v3 aggregate. This is a release-control gate, not a compatibility layer. I6 removes it only after I5 evidence is green. **K4 route/guard revision: 2026-08-11.** |
...
| `tests/test_model_hub_api.py` | The current line-1204 `experimental_consent_at` assertion becomes an absence assertion; API fixtures cover unique final model entries, edited-effort and retirement preservation across rediscovery, distinct add-only unsaved operations versus the sole guarded saved refresh, all-exit discovery cleanup, three protocols, observation-before-save, all three pre-observation nonce retry states, deterministic policy-chosen Add Source placement with the returned persisted position, and every guarded-mutation row. Guard fixtures cover every guard-totality row: unforced empty/nonempty plans, token-without-force validation, forced nonempty exact/missing/stale tokens, and forced empty plans with no token or an old token. They prove every nonempty confirmed plan commits once, every nonempty unconfirmed plan returns the current token-bearing refusal without removing a hop, and every empty plan follows ordinary success without a fabricated 409. Nonce fixtures prove in-flight conflict/no work, released atomic re-claim/exactly one new attempt, and committed same-Source readback/no work. The same file covers Source deletion from all backend Source orders and routes with survivor order preserved, discovered retirement, existing-chain reorder, runtime install/no-op, duplicate-native pre-adapter rejection, adoption, and absence of policy/enrollment/takeover/supplying sibling fields. **K4 totality revision: 2026-08-11 19:10.** |
...
fixture covers every row of the force/plan/token totality matrix: unforced empty success,
unforced nonempty `confirmation_required`, token-without-force validation, forced
nonempty exact-token success, forced nonempty missing-token `confirmation_required`,
and forced nonempty stale-token `plan_changed`. It also changes the world after refusal
...
claims it before observation or credential work, and the committed Source echoes it
unchanged so list reads expose exactly one match. One table-driven fixture covers all
three same-nonce retry states. `nonce.in_flight` returns HTTP 409
`source_nonce_conflict` before observation or provisioning. `nonce.released` first
settles AC-26 cleanup, then the same-nonce retry atomically reclaims and performs exactly
one fresh observation/provisioning attempt. `nonce.committed` returns the ordinary HTTP
200 create terminal envelope with the same Source and performs neither again. After a
simulated lost response the client reads first; a match finishes reconciliation and a
miss always retries the same nonce, so no state can dead-wait. Reconstruction reconciles
retained material before releasing an ownerless claim. Omitting the nonce preserves
existing create behavior and makes no false reconciliation promise.
...
three-state AC-47 fixture, including released re-acquisition and committed readback.
After K3 upgrades G-29 and I7 freezes committed-response, in-flight-conflict,
released-reclaim, cleanup/recovery, and lost-response fixtures, I4's second increment
