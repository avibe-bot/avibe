...
| **I4 Sources / Gateway UI** *(tentative; one lane, two increments)* | codex | `ui/src/components/settings/models/**`; `ui/src/i18n/*.json`; `vibe/i18n/*.json`; UI tests. Both i18n trees are I4-exclusive. The current increment consumes the already-specified install/unsupported-host surfaces for AC-38/AC-43; each model row may open frame 02, render the drawing, and consume the chain read projection, but no chain-write caller is wired and Save remains disabled. Exact-chain editing and its UI tests belong to the second increment only after the G-32 specification round merges. That second increment echoes both refusal-plan arrays unchanged on confirmed mutations, but only after I7 freezes the plan-echo fixtures; the current increment continues to consume master's pre-I7 refusal shape and must not add inactive confirmation fields. It consumes AC-36/37/39–42/44–48 only after K3 changes their G rows from gaps to contract references; AC-50 live-backoff copy activates only after K5 aligns the UI specification and I7 freezes its producer fixtures. This is an ordering split inside I4, not a new lane identity. **Revised 2026-08-11 18:10, 18:32, 19:56, and 20:35 by PM/owner ruling.** | current increment: I1 envelopes + K4 contracts; second increment: G-32 specification round + K3 follow-up + K5 UI-spec sync + frozen I7/I2/I3 fixtures |
| **I7 contract-completion implementation (backend)** | codex | After #1312 merges, exclusive ownership transfers from I1 for `config/v2_config.py`; `core/handlers/model_hub/{service,resolver,classification,rpc,request,errors,events}.py`; `vibe/{ui_server,model_hub_client}.py`; `tests/test_model_hub_config.py`; `tests/test_model_hub_api.py`; and `tests/test_model_hub_resolution.py`. I7 owns AC-35–AC-47 and AC-50's backend producers/tests plus AC-48's API/service half. `service.py` is the sole shared guard-plan owner and implements every guard-totality row by recomputing and exactly comparing the two echoed arrays; route layers only transport those arrays. The same service/config boundary reserves Source-create nonces before work, exposes committed evidence only as `Source.client_nonce`, returns distinct in-progress/committed conflicts, and releases on AC-26 settlement or Source deletion; it stores no digest, receipt, or terminal envelope. The transferred resolver/classifier/event boundary owns `retired: false` filtering and the four network-failure cells: only explicit shaped errors enter existing non-permanent Source classifications; an unclassified pre-first-byte failure creates bounded in-memory backoff; an unclassified post-first-byte interruption is event-only. I7 delegates OAuth tuple claim/coalescing to I3's registry and owns only its service/API transport. Its first AC-50 mechanical action is the sole owner-authorized I2-file exception: delete the strict xfail marker from `tests/test_model_hub_l3.py::test_chain_projection_and_probe_latency_partition` in the same commit that makes it XPASS; it otherwise consumes I2 runtime phase/error facts and I3 OAuth fixtures without editing their files, and it never edits contracts, UI/i18n, scenario trees, or any I2–I5 file. **Revised 2026-08-11 through the 20:35 owner subtraction ruling.** | K4 merged + #1312 merged; all I1-to-I7 file transfers activate only after #1312 merges |
...
| **FC-12** | `api.md` | **Revised 2026-08-11 by the owner-authorized K4 completion; guard/nonce simplified by the 20:35 owner subtraction ruling and network policy retained from 19:56.** Contracts Source CRUD, add-only unsaved connectivity/protocol observation and discovery, the sole saved `POST /sources/<id>/refresh` mutation, the unified `/sources/<source_id>/models` subresource for manual creation, all-inventory reasoning-list edits, discovered-model retirement, and manual deletion, the explicit backend Source-order PUT, exact route GET/PUT, mode, events/provenance, native-config import, OAuth, and Direct-mode responses. Source create returns exact `added_to` positions; Source cards use `adopted_by`; an optional nonce is reserved before work and persisted on the Source, while committed retries conflict and reconcile through the ordinary Source list. OAuth start claims its nonce/vendor/channel tuple before provider work and coalesces a concurrent retry. Every Source/inventory mutation mirrors §4.5's total matrix row-for-row, including unguarded writes that cannot change `id`, `origin`, or Routes. Every nonempty forced guarded-impact plan requires exact echoes of the refusal's `would_remove_hops` and `would_interrupt`; a missing or changed echo returns the same 409 family with the new plan and never removes an unconfirmed hop, while an empty recomputed plan follows ordinary success with force/echo inert. The network totality table admits only shaped errors to existing Source classifications, gives an unclassified pre-first-byte connection failure bounded live backoff, and makes an unclassified post-first-byte interruption event-only. Forced Source/manual-model deletion preserves survivor order; discovered-model DELETE instead persists `retired: true`, retains the row through refresh, and uses the same exact-hop and protected-supply guards. OAuth-start rejects a duplicate native Source before adapter invocation. No saved `/test`, separate saved discovery, route policy, separate mapping, takeover, experimental-consent, observation-evidence detail, receipt/digest, or vendor-guessed protocol surface exists. |
...
| --- | --- |
| `config/v2_config.py` | Final Source/model/protocol/configured-route types, one canonical validation boundary shared by every persisted-Source writer and subsequent load, singleton and exact-pair uniqueness validation, one explicit Source order per backend and one explicit hops array per menu model, persisted discovered-model retirement plus the unique Source-create `client_nonce` column and in-flight reservation lifecycle, serializer completeness, fresh-install Gateway construction, and existing-install-with-no-Hub-state Direct onboarding. No receipt, digest, terminal-envelope snapshot, plaintext key, or policy state exists. A failed create releases only after AC-26 settlement; a committed nonce survives with its Source and Source deletion releases it. Unclassified transport failure and live connection backoff never serialize into Source/config state. **Retirement/nonce/network revision: 2026-08-11 20:35 K4.** |
| `core/handlers/model_hub/{service,resolver,classification,errors}.py` | One §4.3 stored-chain executor and one Add Source implementation of §4.2's placement policy; the latter chooses and persists each matched hop position before returning `added_to`. Also owns the complete credential-failure matrix keyed by refresh capability, closed error classes, `adopted_by`, Source-global health, local-engine distinction, and all route/inventory guards. After #1312 merges, I7 owns `resolver.py`, `classification.py`, and `tests/test_model_hub_resolution.py`: Add-time matching, inventory membership, new-Route validation, runnability, and invocation admit only `retired: false` rows; the network classifier executes every shaped/unclassified × before/after-first-byte cell and alone owns live connection-backoff state. `service.py` is the single shared guard planner for every matrix row; it recomputes the exact two-array plan, commits a nonempty plan only when both echoed arrays match, returns the new plan otherwise, and treats force/echo as inert after recomputation reaches an empty plan. Discovered-model DELETE persists the retirement tombstone and refresh never revives it. Source-create reserves its nonce before work, releases it only after retained-material reconciliation, atomically reclaims it after release, persists it on success, and returns committed conflict for list-based reconciliation. Runtime never matches, places, substitutes, or writes a network health verdict. A static-key `401` performs no retry; only refresh-capable credentials receive one bounded refresh. **Retirement/nonce/guard/network totality revision: 2026-08-11 20:35 K4.** |
...
| `ui/src/components/settings/models/**` | Final Source/Gateway types and calls; protocol selector only after failed observation as a probe-order hint; Source cards reuse `adopted_by`; Add Source renders the stored backend Source order plus returned `added_to` positions without inferring newness from order; backend groups consume `supply_status`; takeover derives from the chain; saved Source details exposes only guarded Refresh models; Direct groups expose reversible mode actions. For frame 02, I4's current increment opens the editor from each model row, renders the drawing, and consumes the chain read projection only: it wires no chain-write caller and keeps Save disabled. Exact stored-hop/mapping editing and exact two-array plan echo belong to I4's second increment after the G-32 specification round, K3 follow-up, and I7 refusal fixtures merge. Live connection-backoff presentation also belongs to the second increment after K5 corrects its UI-spec copy and I7 freezes producer fixtures. The current increment consumes master's pre-I7 refusal shape and must not add inactive confirmation fields. K5 separately removes the withdrawn observation request/status detail slots. The current increment also consumes the already-specified install/unsupported-host states; the second consumes the remaining K4 projections/actions only after their named specification upgrade. **Frame-02/guard split revised 2026-08-11 through 20:35 by PM/owner ruling.** No bottom-only “new” section, policy control, “not enabled” hint, latency/last-check copy, consent/experimental surface, separate mapping object, takeover field, supplying-backends field, or vendor default protocol exists. Any new-item marker is transient presentation state, not a route field. `vendorMeta.ts` may order probes but cannot choose a saved protocol. Narrowing `SourceProtocol` must be checked against production and test files, not only the production-only `tsconfig.app.json` program. |
...
| `tests/test_model_hub_config.py` | The adapter parity gate at current line 159 remains exact; the mirror-registry version gate at current line 236 and provenance version gate at current line 314 use terminal value **5**; round-trip/completeness fixtures use only final Source and route shapes. It validates Source `client_nonce` persistence/uniqueness, rejects receipt/digest fields and unregistered observation evidence, and proves live backoff cannot serialize as Source state. **Nonce/subtraction/network fixture revision: 2026-08-11 20:35 K4.** |
| `tests/test_model_hub_api.py` | The current line-1204 `experimental_consent_at` assertion becomes an absence assertion; API fixtures cover unique final model entries, edited-effort and retirement preservation across rediscovery, distinct add-only unsaved operations versus the sole guarded saved refresh, all-exit discovery cleanup, three protocols, observation-before-save, all three pre-observation nonce retry states, deterministic policy-chosen Add Source placement with the returned persisted position, and every guarded-mutation row. Guard fixtures cover every totality cell: unforced empty/nonempty plans, forced nonempty exact/missing/different plan echoes, and a forced request whose old nonempty echo recomputes to an empty plan. They prove every exactly confirmed nonempty plan commits once, every other nonempty forced plan returns the current refusal without removing a hop, and every empty plan follows ordinary success. Source-create fixtures prove in-progress conflict/no work, released atomic reserve/exactly one new attempt, committed conflict plus list lookup of exactly one nonce-bearing Source, AC-26 release, and Source-delete release/reclaimability. Observation fixtures reject request/status evidence not present in the six-field contract. Network fixtures cover all four shaped/transport × before/after-first-byte cells, bounded auto-clearing live backoff, and zero config writes for unclassified failures. The same file covers Source deletion from all backend Source orders and routes with survivor order preserved, discovered retirement, existing-chain reorder, runtime install/no-op, duplicate-native pre-adapter rejection, adoption, and absence of policy/enrollment/takeover/supplying sibling fields. **K4 totality/subtraction/network revision: 2026-08-11 20:35.** |
...
plan. The fixture covers every row of the force/plan/echo totality matrix: unforced empty
...
takes ordinary success without a 409 or removed hop. No token, digest, version receipt,
server-side confirmation state, or parallel plan-changed discriminator exists. The
closed error and guard-decision vocabularies are checked through `mirror-registry.json`.
...
After K4 and #1312 merge, I7 owns the sole shared guard planner and exact-array comparison in
...
for numbered rows, exact two-array plan echo, and replacement by a newly refused plan.
...
reserves that unique value before observation or credential work and stores no digest,
terminal envelope, or plaintext credential with it. The committed Source persists and
echoes the nonce unchanged so list reads expose exactly one match. One table-driven
...
claimable again. After a simulated lost response, in-progress waits/retries, committed
conflict reads, and a released/list-miss retry all terminate without a separate endpoint.
Reconstruction reconciles retained material before releasing an ownerless reservation.
...
`vibe/{ui_server,model_hub_client}.py`, and
`tests/test_model_hub_{config,api}.py` for nonce-column validation, reservation
persistence, cleanup/recovery, Source-delete release, and the complete state/action AC-47
fixture. After K3 upgrades G-29 and I7 freezes in-progress conflict, committed conflict
...
After K4 merges, K5 alone owns `docs/plans/model-hub-ui-spec.md` §1.5 and removes its
request/status slots under the 20:35 ruling; no backend or I4 implementation file is
activated by AC-49.
