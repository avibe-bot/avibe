...
- `touch-none` on the strip only. Capture does not enter the browser's touch-action negotiation, so wherever a touchscreen reports the desktop breakpoint the default lets the native pan claim the finger and cancel the drag part-way; measured, it stopped at 272px of an intended 368px. Scrolling anywhere outside the strip is untouched.
...
- [x] Implementation, focused tests (13 + 4 shell cases), lint, build, and real-browser evidence (8 hermetic Chromium cases, dark/light captures at 496) complete.
- [x] Review round 1: one P2 — native touch negotiation not disabled on the strip. Fixed with `touch-none` and a real CDP touch drag that fails without it.
