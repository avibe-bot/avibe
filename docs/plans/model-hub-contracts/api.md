...
| `model_hub.runtime_default_applied` | lock-owning service startup/default migration | one-time legacy upgrade and runtime-start recovery | Additive persisted marker. Missing on disk upgrades once to `enabled: true` and `runtime_default_applied: true` at service startup; ordinary config reads do not persist the migration. Later explicit Stop remains false. |
