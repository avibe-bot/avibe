...
| PUT `/api/models/agents/opencode/menu` | `{menu}` → `{agent: AgentSupply}` | Open-menu configuration. A newly checked identifier must still be supplied by current OpenCode-eligible inventory at commit time; already checked route aliases may be preserved or removed without same-name inventory. |
