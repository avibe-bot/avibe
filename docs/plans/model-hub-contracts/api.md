...
| **GET** `/api/models/agents/<backend>/chain?model=<id>` | → `AgentChain` | **v2.** The capability chain for that (agent, model) — cooling members included, flagged `runnable: false`. An empty `chain` is `ok: true` with `chain: []`, not an error. Carries `supply_state` (`ok`/`waiting`/`interrupted`) at the MODEL grain: the single answer every model-scoped consumer reads, including the probe's `supply` sibling and `TurnProvenance.model_supply_state`, so none of them consults the backend rollup for a question it cannot answer. The `waiting`/`interrupted` split is spec §4.5's predicate, stated there once — `interrupted` when at least one blocker needs the user, `waiting` only when every blocker is a cooldown. `model` is a **menu identifier** (see identifier rules below). **Hub-mode route** (07-29, review round 9): this row is defined for `mode: hub` and the client must not offer it for a Direct backend. `agent-supply.schema.json` nulls `sources`, `selected_model_id` and `model_supply` in Direct, so there is no order to walk and no default model to fill in — the honest answer is not `chain: []`, because the empty-chain rule above means 「Hub has nothing that can serve this」 and a Direct backend's native CLI may be running that model perfectly well. Presenting Hub starvation for a backend that is not on Hub is a worse failure than refusing the question. What Direct *should* answer — a documented error or a mode-specific payload — is **AC-7** in `docs/plans/model-hub-implementation.md` §8 and is not decided here. |
| **POST** `/api/models/agents/<backend>/probe` | `{model?}` → `{probe: ProbeResult}` | **v2.** One minimal dry-run request through the current chain (「试跑一次」). `model` is a **menu identifier** and defaults to `selected_model_id`. The result is NESTED under `probe`, never spread into the envelope — see below. **Hub-mode route** (07-29, review round 9), for the same reason as the chain row and one of its own: in Direct there is no `selected_model_id` to default to, and a `ProbeResult` requires a non-nullable `source_id`, which a Direct turn has no Hub source to name — the implementation would have to fabricate one to return a valid object. Scoped to `mode: hub`; the Direct representation is **AC-7**. |
...
1. every entry in an **open** menu's `menu.checked` (07-29, review round 9). Round 5
   wrote this term as 「every checked fixed-menu model」, which names state that does not
   exist: `agent-supply.schema.json` permits `menu` — and therefore `checked` — only for
   `menu_kind: open`, and gives a fixed menu `builtin_models`, which is the
   server-populated CATALOG, not a selection. Both readings of the old wording were
   wrong. Skipping the term drops real protection from the one menu kind that *does*
   persist a checked set, so deleting a source could starve a model the user ticked in
   the OpenCode menu. Treating a fixed menu's catalog as checked refuses deletes over
   dozens of models nobody selected and some the backend cannot even run, which trains
   the user to pass `force=true` reflexively and hollows out the whole guard. A fixed
   menu therefore contributes nothing here and is protected by terms 2-4 — which is
   exactly right, because a fixed-menu model the user actually selected either owns a
   mapping row, is the backend default, or is some enabled Agent's effective model.
   `menu.checked` entries are already prefixed menu identifiers (`zhipuai/glm-5.2`), so
   they enter the union in the namespace above with no translation;
...
`agents` may still be legitimately empty — an open menu's checked-but-unassigned model,
or a mapping row no Agent runs on — and after this ruling that is the only way it
empties.
