...
| `turn.request_unroutable` | `failed_terminal` | a well-formed gateway request naming a model this process scope never routed | no Source was attempted, blamed, or mutated; the request could not be bound to any route the scope prepared | `models.launch.request_unroutable`: this request's model is not routed for this session, pointing to Models. It must not reuse `request_incompatible`, whose copy denies that reselecting a Source helps |
