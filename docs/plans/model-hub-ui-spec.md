...
| G-33 | 04 add-subscription, a flow whose declaration carries a device code | anywhere to display or copy `presentation.device_code` | **The remaining contracted value has nowhere to land.** `oauth-flow.schema.json`'s Form B is `auth_url + device_code + expects none`. This round gives every non-null `presentation.instructions_key` a direct helper-line consumer, including Form B, so that former half is closed. The paste-back exhibit accepts and submits Forms A and C, but Form B submits nothing and instead needs a read-only device code the user copies out. The new input cannot render that output, so G-33 remains independently open |
...
**Held-state conservation for transient dialogs** `[derived]`. When a destructive or
credential-repair dialog opens over a rendered source state, it holds that exact origin.
取消, close, Escape, an outside press, or abandoning an F3 refusal restores the held
origin; none of those no-op exits manufactures `Ready`. The §1.6 guard, frame 11 and frame
12 cite this one rule. Only a successful mutation or a later source read may select a
different state.
...
| §1.4 | Submitting paste-back | 提交 pressed — `POST /api/models/oauth/submit` with the held `{flow_id, value}` `[contract]` | F1 → Paste-back failed | `addSub.paste.submitting` | A successful terminal dispatches by its held intent: `create` closes into 06; `reauth` → §1.11's repair terminal. An unsuccessful terminal → OAuth failed; an allowed non-terminal `{flow}` → Awaiting paste-back completion; a request failure or lost response → Paste-back failed |
| §1.4 | Awaiting paste-back completion `[contract]` | Submit, or the reconciliation read below, answered with `OAuthFlow.state` `starting`, `awaiting_action` or `verifying` | The same 2s status poll and expiry/15-minute bound as Awaiting sign-in; a failed status read changes nothing and the next tick retries it (D-16) | `addSub.paste.submitting`, `addSub.cancel` | `success` dispatches by held intent: `create` closes into 06; `reauth` → §1.11's repair terminal. `failed` / `cancelled` → OAuth failed; another non-terminal reading stays here. 取消 / close / outside press → Dismissing |
| §1.4 | Paste-back failed `[derived]` | The submit request failed or never answered | F1, in place; the input, `flow_id` and intent are kept | `addSub.paste.fail`, `addSub.retry`, `addSub.cancel` | 重试 first re-reads `GET /api/models/oauth/status/<flow_id>` (D-36): `success` dispatches by held intent to 06 or §1.11's repair terminal; `failed` / `cancelled` → OAuth failed; a non-terminal reading → Awaiting paste-back completion, **without submitting the held value again**; the read itself failing leaves this state unchanged. 取消 → Dismissing |
...
| §1.10 | Edit open | 编辑来源 chosen from 06's source overflow | F5 | `sourceDetail.edit.title`, `sourceDetail.edit.name`, `sourceDetail.edit.baseUrl`, `sourceDetail.edit.hint`, `sourceDetail.edit.cancel`, `sourceDetail.edit.save` | 保存 → Saving source; 取消 / close / outside press → dismiss unchanged |
| §1.10 | Saving source | 保存 pressed — `PATCH /api/models/sources/<id>` with the changed `display_name` and/or `base_url` `[contract]`. Clearing a previously non-null Base URL keeps 保存 enabled and normalizes the empty draft to `base_url: null`, never `""`; this restores the vendor's official endpoint `[contract]` | F3 when a Base URL change is refused; F1 otherwise → Source save failed | `sourceDetail.edit.saving` | Success → close and refresh the source projection; a guard refusal → Source save refused |
...
| §1.10 | Remove confirmation `[derived]` | 移除来源 chosen from 06's source overflow; the exact §1.6 source-detail state behind it is held | F5 — no request has been sent | `guard.title.removeSource`, `guard.hint.removeSource`, `guard.confirm.removeSource`, `guard.cancel` | 移除来源 → Removing source with the initial non-forced delete; 取消 / close / outside press restores the held §1.6 origin by the shared conservation rule |
...
| helper | The localized string named by non-null `presentation.instructions_key`; otherwise the expects-specific generic hint | no | — |
...
The helper is declaration-first as well `[contract]`: when
`presentation.instructions_key` is non-null, Forms A, B and C render that localized key
directly. Only a null key falls back to the generic helper selected by `expects`; the
client owns neither a vendor-to-instruction table nor a second paraphrase of server copy.
...
metadata is data and receives no copy key.
...
- Forms A, B and C are selected from the returned `presentation`, never a vendor table. A
  ...
  output. Submit, 2s polling, timeout, cancel and the F1–F5 treatments are otherwise §1.4
  verbatim;
...
held-state conservation rule: a no-op exit restores the exact state under the modal and
never promotes it to `Ready` `[derived]`.
