...
| M6 | R6 route replacement, or an authoritative exact ordered AgentChain from `GET /api/models/agents/<backend>/chain?model=<id>` matching submitted `hops` under D-36 | `Source[]` including `adopted_by`, complete `AgentSupply[]` including mode/model supply and any present routes projection, and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | Start AR-M as soon as ET-10/ET-11 or AR-D6 holds commit evidence. Reuse every member already settled in that attempt: Agents supplies mode/page authority only, while the exact chain endpoint is the sole D-36 equality authority. Keep R6's entire envelope when received; inference marks `removed_hops` / `interrupted` unavailable. The modal may transfer the active generation to the page without cancelling it. AP records `AgentSupply.routes` as optional and deliberately unconsumed here | AR-M consumes each successful applicable member monotonically and dispatches a successful AgentSupply immediately. Decisive Direct mode cancels/disowns every Route-chain member as not applicable; those members and their late results never enter stale/Retry. CA admits Retry only after the generation settles with applicable failures; it retries exactly that subset and never overlaps generations. Mounted completion retains the report/evidence and Done; transferred completion preserves any FF-1-valid active target and otherwise uses PF-1. ET-20 alone acknowledges/closes/transfers; no path resends or questions the route write |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or AR-D6 proves the desired Route from the exact chain endpoint while response-only arrays are unavailable; exact commit evidence is held while the active AR-M generation has pending applicable members | F2 applies only to the applicable members that AR-M3 later marks stale; the Route write/desired state is already proven | `route.refreshing`, `route.impact.done` | ET-11 or ET-17c owns the mutually exclusive entry; AR-M/ET-19/AS-5a/AS-5b/AS-6a/AS-6b and ET-20 own independent member settlement, Retry, page installation and the sole user exit |
...
| §1.2 | Route impact reported `[derived]` `[contract]` | R6 holds a successful route-replace response with non-empty `removed_hops` and/or `interrupted`; the complete returned `chain` remains held while AR-M independently settles every applicable invalidated projection | F2 only for the exact AR-M3 applicable failed subset; the write already succeeded and DP-4 owns every exit | Report block keys always; `route.refreshing` while any applicable member is pending; `route.impact.refreshFail` and `route.retry` only in the settled-failure subphase; neither retry key after AR-M2 success | ET-10, AR-M/ET-19, AS-5a/AS-5b/AS-6a/AS-6b and ET-20 own report entry, member settlement/Retry/install and the sole user exit; successful authority is installed immediately and never dismisses the report |
...
`[derived]`. It applies §4.6's stable total key to the current draft only: a hop whose
...
result remains local until 保存. The owner interpretation for frame 02 scopes §4.2/§4.6's
sole post-creation “consumer” to a server capability that reads Source order to mutate
stored Routes. This editor instead reads an already page-held display projection under an
explicit user gesture and produces only that one local draft: it never reruns placement or
matching. The per-model write body remains only explicit `hops`, and its handler cannot
observe Source order, so no second server consumer exists. This control never calls
...
machine; ET-18a and the AS dispatcher alone own cleanup, destination, focus and any next read:
...
D-36. A read attempt has one monotonically increasing generation. Its accumulator records
each member independently as pending, successful authority, failure, or not applicable;
successful authority is installed/held immediately and is never erased because a sibling
failed. Decisive evidence recomputes the applicable-member set before completeness, stale or
Retry is evaluated. A member removed from that set is disowned, not failed, and its pending,
failed or late result cannot appear in a stale/Retry subset. At most one generation owns reads
for a workflow. Starting a successor generation invalidates the prior one, and every late
result from an invalidated generation is ignored.

Evidence precedence is closed and order-independent. An exact `AgentSupply.mode: direct` or
chain `direct_mode` decides that the editor is illegal before any sibling failure. In Hub,
...
| ID / generation | Independently settled members | Decisive evidence precedence | Immediately installed / held authority | Applicable set after decisive evidence | Remaining stale or awaited members | Sibling cancellation / late result | Destination | Retry subset |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AR-D1 — initial Agents failure | Agents failed | No mode or Route evidence exists | Keep the latest installed page projection and immutable draft/stage | Agents | Failed Agents read | Settle generation; no sibling exists | Originating save failure via ET-16b | Agents only through ET-16a |
| AR-D2 — Direct AgentSupply | Agents succeeded with exact backend `mode: direct` | Mode is decisive before all Route/failure classes | AS-1 installs that AgentSupply | Agents only; Source/chain comparison is not applicable | None | Settle generation; issue no Source/chain read; ignore later obsolete results | Actual Direct page | None |
| AR-D3 — Hub AgentSupply | Agents succeeded with `mode: hub` | Hub mode permits exact reconciliation but AgentSupply proves no Route equality | AS-2 installs the Hub AgentSupply as the new reversible page origin; retain immutable draft/stage | Agents, current Sources and exact AgentChain | Current Sources and exact AgentChain | ET-17a starts both under this generation; AP-2 remains unconsumed | Originating failure, reconciliation pending | None while pending |
| AR-D4 — Hub comparison pending | AR-D3 + ET-17a Source/chain reads | No terminal choice until every member needed by a higher-precedence candidate settles | Retain AS-2 and every successful Source/chain member | Agents, current Sources and exact AgentChain | Pending or failed Source/chain slots | The generation owns both reads until AR-D5–AR-D8 or an RL disown edge | Originating failure, reconciliation pending | None while pending |
| AR-D5 — chain becomes Direct | AR-D4 + chain `direct_mode`; Source may be pending, successful or failed | `direct_mode` outranks every sibling Source result | Hold only the illegality evidence; it is not a landing payload | Fresh page Agents payload only | Page Agents read | Settle the exact-chain slot; cancel/disown Source and every remaining comparison slot; every late comparison result is ignored | DM-3 / ET-18a → page Partial | Page Agents read only |
| AR-D6 — exact chain match | AR-D4 + AgentChain pairs equal submitted; Source may be pending, successful or failed | Exact chain-endpoint equality proves commit before sibling failure | Hold AgentChain as D-36 evidence; retain AS-2's installed Hub page and any successful Source | M6 Source, Agents and Route-chain projections; reuse every settled member | Only applicable M6 members still pending/failed | AR-M adopts applicable pending/success/failed members in the same generation | Route committed, reconciling via ET-17c | Applicable M6 failed subset only |
| AR-D7 — differing chain, Source success | AR-D4 + nonmatching AgentChain + successful Source list | No commit evidence exists; complete resend authority is available | Install Source/chain as the latest reversible authority and run V5 | Agents, Sources and exact chain | None | Settle generation before ET-17d resend or ET-17e invalid result; ignore late duplicates | V5-valid → ET-17d; V5-invalid → ET-17e | New ET-16 generation only after a later failed/no-answer PUT |
| AR-D8 — comparison incomplete | AR-D4 + non-`direct_mode` chain failure, or nonmatching chain + Source failure | Failure is selectable only after every higher-precedence slot settles | Retain every successful member already installed/held | Agents, Sources and exact chain | Exactly the failed Source/chain member(s) | Settle generation; no successful sibling is rolled back | Originating failure via ET-17b | Agents-first successor through ET-16a; no timer |
| AR-M1 — applicable member succeeds | Active M6 generation + any successful Source, Agents or Route-chain member | AgentSupply mode is immediately decisive for page form. Direct also removes every Route-chain member from applicability | Install/hold that exact member; AgentSupply invokes AS-5a/AS-5b/AS-6a/AS-6b by ownership phase and mode | Hub: Source + Agents + Route chain. Direct: Source + Agents; every Route-chain slot is not applicable | Every still-pending/failed applicable member only | Direct cancels/disowns pending Route reads and ignores their late results; sibling failure cannot undo any installation | Same mounted report/committed phase, or receiving page | None while an applicable member remains pending |
| AR-M2 — all applicable members current | Active M6 generation + no pending/failed applicable member | Complete applicable surface settles reconciliation | Keep every exact applicable member and commit evidence; report-free mounted origin becomes Route committed, refreshed | The latest mode-selected set | None; not-applicable Route members are neither current nor stale | Settle generation; later duplicate/disowned result is ignored | Mounted evidence remains until ET-20, or transferred page remains | None; Retry absent |
| AR-M3 — applicable failed subset settles | Active M6 generation + no pending applicable member + one or more applicable failures | Previously successful AgentSupply/mode remains authoritative; failure affects only still-applicable members | Keep every successful applicable member installed and exact commit evidence held | The latest mode-selected set | Failed applicable member set only, visibly stale | Settle generation; later results from it are ignored | Mounted → ET-19a; transferred → ET-19c | Exact applicable failed subset through ET-19b/ET-19d |
| AR-M4 — retry applicable failed subset | Settled AR-M3 + explicit admitted Retry | No prior success is reread or discarded | Retain installed members/evidence | Recompute from held decisive mode before issuing | Prior applicable failed subset becomes pending in one successor generation | CA disables Retry until settlement; prior-generation and disowned results are ignored | Same mounted phase or receiving page | Exactly the applicable failed subset |
...
listed. A registered programmatic status may be targeted; an unregistered heading or text
node may not. PF-1 resolves post-install targets only after that gate:
...
| AR-D Source + exact-chain reads | ET-17a attaches both to the current AR-D generation | AR-D5 settles the chain evidence and cancels/disowns Source plus remaining comparison work; ET-18a performs the handoff. ET-8a/ET-8b disown the full generation; AR-D6–AR-D8 otherwise settle it | A result after disown/settlement cannot alter the selected AR outcome or installed origin |
| AR-M Source, Agents and Route-chain reads | ET-10, ET-11 or ET-17c acquires/transfers one AR-M generation; ET-19b/ET-19d acquires one failed-subset successor | ET-20 transfers rather than cancels. A decisive Direct AgentSupply in AR-M1 cancels/disowns all Route-chain slots; a successor disowns its predecessor | Ignore disowned Route results and all predecessor-generation results; only still-applicable members can settle AR-M |
...
| ET-17a — AR-D3 Hub comparison begins | AR-D3 selected after AS-2 installs the exact Hub AgentSupply | Under the same generation read current Sources and the exact chain endpoint; never inspect AP-2 for equality | Keep modal/draft; keep Retry busy/inert | Originating failure, Hub reconciliation pending over the installed Hub page; accumulator enters AR-D4 | Existing failure copy | Keep inert `route.retry` | One Source read + one exact-chain read in the current AR-D generation |
...
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain equal to submitted pairs | Hold AgentChain, mark response-only tails unavailable and transfer the same generation to AR-M | Clear comparison busy; discard obsolete retry/refusal state | Route committed, reconciling | `route.refreshing` | FF-1-valid `route.impact.done` | AR-M reads only missing/pending/failed applicable model-surface members |
...
| ET-18a — decisive `direct_mode` handoff | Loading, Saving or AR-D5 + matching DM-1/DM-2/DM-3 evidence | Error evidence selects no page form; start/transfer the page's Agents read | Close modal/selector, cancel grab, discard every draft/submitted/refusal value; cancel/disown AR-D siblings | Page §1.0 Partial | Existing Partial copy; never label Direct from the error | Stable owning backend group head | Page Agents read; every late sibling result is ignored |
| ET-18b — error-handoff Agents read failure | ET-18a Partial + last-good page projection | Classify only the read failure | Clear pending; keep last-good projection explicitly stale | Page §1.0 Partial/F2 | Existing F2 cause and page Retry | Page Retry | None until ET-18c |
| ET-18c — retry error-handoff Agents read | ET-18b + stale page projection | Reissue only `GET /api/models/agents` | Set page Retry busy; retain stale projection | Page §1.0 Partial pending | Existing F2 copy | Keep page Retry | Page Agents read; success enters AS-3/AS-4 |
...
| ET-19b — mounted failed-subset Retry | ET-19a + exact applicable failed subset + CA-M2 admission | Start AR-M4 for exactly that subset | Keep modal/evidence/successful members; mark Retry busy/inert | Same report or report-free state, retry pending | Prior report/refresh copy | Keep FF-1-valid `route.retry` | One AR-M successor generation for the applicable failed subset |
...
| ET-19d — page-owned failed-subset Retry | ET-19c + exact applicable failed subset + CA-M4 admission | Start AR-M4 for exactly that subset | Keep evidence/current members; mark page Retry busy/inert | Same receiving page, retry pending | Existing stale line | Keep FF-1-valid page Retry | One AR-M successor generation for the applicable failed subset |
...
| AS-2 — AR-D3 Hub install | AR-D3 + exact Hub AgentSupply | Install that exact Hub row as the latest reversible page authority; AP-2 is deliberately not consumed for commit comparison | Clear only Agents-member pending; retain generation/modal/draft/stage | Originating failure over actual Hub page | Existing failure copy | Keep inert `route.retry` | ET-17a Source/chain reads in the same generation |
| AS-3 — error-handoff Direct dispatch | ET-18a/ET-18c + exact AgentSupply `mode: direct` | Install that exact Direct row | Clear page-read pending and stale Hub row | Actual Direct page | Exact Direct projection | PF-1 | None |
| AS-4 — error-handoff Hub dispatch | ET-18a/ET-18c + exact AgentSupply `mode: hub` | Install that exact Hub row; never resurrect the editor | Clear page-read pending/stale marker | Actual Hub page | Exact Hub projection | PF-1 | None |
| AS-5a — mounted AR-M Direct install | Mounted evidence + any successful M6 AgentSupply member whose owning backend is `mode: direct` | Dispatch mode immediately, install the exact current Direct page behind the modal, and make all Route-chain members not applicable | Retain modal/evidence and applicable Source/Agents state; cancel/disown Route reads and remove their failed/pending slots from stale/Retry | Same mounted report/committed phase over actual Direct page | Registered report/refresh/stale block only for applicable members | Keep FF-1-valid `route.impact.done` | AR-M continues applicable pending members, or CA admits only an applicable failed subset after settlement |
| AS-5b — transferred AR-M Direct install | Transferred AR-M + any successful M6 AgentSupply member whose owning backend is `mode: direct` | Dispatch mode immediately, install the exact current Direct page, and make all Route-chain members not applicable | Retain evidence and applicable Source/Agents state; cancel/disown Route reads and remove their failed/pending slots from stale/Retry | Actual Direct page with exact applicable pending/stale markers | Exact Direct projection plus registered applicable pending/stale block | Preserve the current active page target only if FF-1-valid; otherwise PF-1 | AR-M continues applicable pending members, or CA admits only an applicable failed subset after settlement |
| AS-6a — mounted AR-M Hub install | Mounted evidence + any successful M6 AgentSupply member whose owning backend is `mode: hub` | Dispatch mode immediately and install the exact current Hub page behind the modal | AR-M retains modal/evidence and every other applicable member's independent pending/stale state | Same mounted report/committed phase over actual Hub page | Registered report/refresh/stale block for the AR-M phase | Keep FF-1-valid `route.impact.done` | AR-M continues applicable pending members, or CA admits an applicable failed-subset Retry after settlement |
| AS-6b — transferred AR-M Hub install | Transferred AR-M + any successful M6 AgentSupply member whose owning backend is `mode: hub` | Dispatch mode immediately and install the exact current Hub page | AR-M retains evidence and every other applicable member's independent pending/stale state | Actual Hub page with exact pending/stale markers | Exact Hub projection plus registered pending/stale block | Preserve the current active page target only if FF-1-valid; otherwise PF-1 | AR-M continues applicable pending members, or CA admits an applicable failed-subset Retry after settlement |
...
in an admitted row; while disabled/inert, pointer, Enter and Space all send nothing. An
unlisted async subphase for a CA-covered control is absent or inert:

| ID | Control | Async subphase | Rendered / admitted | Activation |
| --- | --- | --- | --- | --- |
| CA-D1 | Save-failure Retry | No AR-D generation active | Rendered, enabled | Emit ET-16a and atomically acquire the sole generation |
| CA-D2 | Save-failure Retry | ET-16a / AS-2 / ET-17a generation active | Rendered busy, disabled/inert | None; cannot overlap AR-D or reach a second PUT |
| CA-M1 | Mounted M6 Retry | Initial AR-M pending, AR-M4 retry pending, or AR-M2 success | Absent; if layout reserves it, disabled/inert | None |
| CA-M2 | Mounted M6 Retry | Settled ET-19a / AR-M3 applicable failed subset | Rendered, enabled | Emit ET-19b for exactly the held applicable failed subset |
| CA-M3 | Page M6 Retry | Initial/transferred AR-M pending, AR-M4 retry pending, or AR-M2 success | Absent; if layout reserves it, disabled/inert | None |
| CA-M4 | Page M6 Retry | Settled ET-19c / AR-M3 applicable failed subset | Rendered, enabled | Emit ET-19d for exactly the held applicable failed subset |
