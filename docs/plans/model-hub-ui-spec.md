...
| C15 — page-form landing evidence | Every response, terminal or absence branch that changes the mounted page form MUST name the authoritative payload or read that selects the destination; classification evidence alone can retire an illegal surface but can never stand in for the target projection | [x] DM-1–DM-3 errors close the illegal editor and transfer to the page Agents read; DM-4 consumes the exact AgentSupply already returned by ET-16. In every case only an actual `AgentSupply.mode` selects Direct or Hub | [x] R1/R2 and M0–M2 hand their exact returned or reread Source/absence evidence to the page; an error class selects no destination by itself | [x] R3/R4 and M0/M3/M4 hand their exact returned or reread Source evidence to the page; flow/error classes select failure handling, never a healthy landing | [x] N/A — the menu changes only its own reversible phase and hands one explicit local vendor selection to §1.4; no response or terminal branch changes the page form |
| C16 — event-transition totality | Every event that changes state MUST register, in one owning edge row, its complete authority dispatch, transient-state cleanup, visible feedback and destination focus; an unlisted transition is forbidden | [x] ET-1–ET-20 are the sole edge owner for frame 02; the state, V5, DM, control and keyboard registers cite them and do not define parallel destinations | [x] §1.10's state rows consume R1/R2 and M0–M2, clear their modal phase under DP-1/DP-2/DP-4, render the registered F/copy treatment and name the source overflow or receiving page as focus owner | [x] §1.11's state rows consume R3/R4 and RR/M0/M3/M4, settle flow/key transient ownership, retain the registered report/failure copy and name the invoking card or receiving projection as focus owner | [x] Closed/Open and the §1.4 handoff name the explicit vendor event, menu cleanup, visible destination and Add subscription / receiving-surface focus; no server response edge belongs to frame 13 |
...
| M6 | R6 route replacement, or an authoritative exact ordered `(source_id, model_id)` readback matching the submitted `hops` (including D-36) | The exact AgentChain, `Source[].adopted_by`, the owning backend's AgentSupply/model-supply projection and the Route-chain index; the attempt may change membership, mappings, order, runnability and current-hop presentation | After Route impact reported takes a Done-equivalent exit, before handing off a received two-empty-array success, **or before closing a matching-readback commit**, read the complete model surface. Keep R6's entire envelope when received; for readback evidence hold the matching AgentChain and mark `removed_hops` / `interrupted` unavailable. Route committed, reconciling owns every report-free pending read | A received report stays mounted with `route.impact.refreshFail`, read-only Retry and every DP-4 Done-equivalent exit. A report-free failure enters Committed projection stale with the returned or matching-readback chain held, exact empty/unavailable tail disposition retained and the last-good dependent projections marked stale. Retry repeats only M6; no path resends or questions the route write |
...
focuses its grip through ET-5. Identity mapping and an explicit cross-model mapping are
...
changed pairs. ET-17 alone owns the state edge when refreshed Hub authority invalidates a
...
| --- | --- | --- |
| RS-1 — local edit | opening AgentChain + V5-valid ordered draft | Send nothing. ET-5–ET-7 own draft/focus transitions; Save is enabled only when the ordered pair array differs from the opening origin; empty is allowed |
| RS-2 — first save | one immutable submitted `hops` array | ET-9 sends the complete array without `force` or plan echoes and transfers focus/ownership to Saving |
...
| RS-5 — confirm | the immutable submitted `hops` + both exact arrays from the currently displayed refusal | ET-13 verifies the exact held plan, then ET-9 re-sends with `force: true`, `would_remove_hops` and `would_interrupt`; confirmation never creates a new mutation body |
...
| RS-8 — non-guard failure / no answer | `direct_mode`, another shaped rejection, or no response evidence | ET-14/ET-15/ET-18 are the sole destination owner. No-answer reconciliation starts only from ET-16; no prose branch may resend first |
...
machine; ET-18 alone owns cleanup, destination, focus and any next read:

| ID | Evidence producer | What the evidence proves | Landing payload available here | Edge owner |
| --- | --- | --- | --- | --- |
| DM-1 | Opening `GET /api/models/agents/<backend>/chain?model=<id>` returns `direct_mode` | The Route surface is illegal at this observation point | None — the standard error envelope carries no `AgentSupply` | ET-18 starts/transfers the mounted page's existing Agents read |
| DM-2 | Route `PUT` returns `direct_mode` | No Route commit evidence exists and the editor is now illegal | None — the error may not label the row or page Direct | ET-18 starts/transfers the same Agents read |
| DM-3 | D-36 or another post-failure chain read returns `direct_mode` | The held attempt has no useful Route landing projection | None — no second chain or Source read can manufacture one | ET-18 starts/transfers the same Agents read |
| DM-4 | ET-16's first reconciliation read returns the exact backend `AgentSupply` with `mode: direct` | Direct is the authoritative current backend mode, so no Route read or V5 validation is legal | The returned AgentSupply row itself; it is installed exactly and no second Agents read is sent | ET-18 consumes this payload directly |
...
`mode` alone selects Direct or Hub — including a concurrent switch back to Hub. DM-4 skips
...
and mutates nothing. ET-13 owns both confirming and abandoning that refusal.
...
shared hop and SupplyGap blocks, while an empty member skips only its block. A report's
完成-equivalent exit then starts M6. If both arrays are empty, Route committed,
reconciling owns the immediately started M6 read and every DP-4 exit while it is pending.
...
array and whether the attempt was initial or confirmed. On explicit 重试, ET-16 first reads
`GET /api/models/agents`; `mode: direct` is DM-4 and ends Route reconciliation, while only
`mode: hub` authorizes ET-17's Source/chain reads. This table classifies evidence;
ET-16/ET-17 remain the only edge owner:

| Readback evidence | What it proves | Owning edge |
| --- | --- | --- |
| Exact backend AgentSupply reads `mode: direct` | The Route surface is not legal and this same row is the complete landing payload | DM-4 / ET-18; issue no Source or chain read |
| Exact backend AgentSupply reads `mode: hub` | Route reconciliation is legal, but no commit verdict exists yet | ET-17 reads current Sources and the exact `(backend, menu_model)` Route |
| Ordered Route pairs equal submitted `hops`, including empty-to-empty | The desired Route is committed. `current`, health and supply annotations are irrelevant to equality | ET-17 → ET-11; hold the AgentChain, mark response-only tails unavailable and run M6 |
| Ordered Route pairs differ and refreshed V5 remains valid | The read proves only the current origin and a valid resend basis | ET-17 → ET-9 on this explicit Retry; a confirmed resend keeps the exact held refusal arrays |
| Ordered Route pairs differ and refreshed V5 invalidates any new/changed row | The read proves the current origin but not a legal resend | ET-17 → Invalid after refresh; keep and mark every offending row, send no PUT |
| The required AgentSupply, Source or Route read fails | No landing, commit verdict or valid resend basis was proved by that stage | ET-16/ET-17 stays in the originating failure state; Retry restarts at AgentSupply and no timer runs |
| A post-Hub chain read returns `direct_mode` | The earlier Hub evidence was superseded before the chain read; the error has no landing payload | DM-3 / ET-18; close, discard and transfer to the page Agents read |

ET-8 owns every non-mutating exit. In Route save outcome unknown it hands the exact row to
...
column below; **an unlisted transition is forbidden**:
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ET-1 — open model row | Mounted §1.1 row + exact `(backend, menu_model)` | No projection is inferred; start the exact chain GET | Close any row menu; no draft/grab exists | Loading route | `route.loading` | `route.cancel` | Dialog-owned chain GET |
| ET-2 — opening chain success | Loading route + returned AgentChain | Validate envelope, then install its exact pair array as origin and draft | Clear loading only | Ready | Normal frame copy | Keep `route.cancel`; payload arrival does not steal focus | None |
| ET-3 — opening non-direct failure | Loading route + held page row; `direct_mode` is reserved for ET-18 | Classify shaped/transport failure only after excluding DM-1 | Clear loading; retain the independent page row unchanged | Route unread | `route.fail.read` | `route.retry` | None until explicit Retry |
...
| ET-5 — Add selector event | Ready, Dirty or Invalid after refresh + current V5 authorities | Open from current V5 candidates; confirm revalidates the selected exact pair before append | Escape clears selector selection; confirm closes selector | Same state on open/Escape; confirm reclassifies as Ready, Dirty or still Invalid | Selector keys; invalid lines remain until repaired | First candidate on open, `route.addHop` on Escape, new grip on confirm | None |
| ET-6 — local add/remove/move/sort | Ready, Dirty or Invalid after refresh + origin/draft | Re-run V5 after the one local mutation; a no-op retains the prior class | Finish pointer operation; removal drops any state owned by that row | Ready when equal to origin; Dirty when changed and valid; Invalid after refresh only while a previously marked offending row remains | Normal copy plus `route.invalidAfterRefresh` on every remaining invalid row | Changed/new grip; removal chooses successor, predecessor or `route.addHop` | None |
| ET-7 — keyboard grab lifecycle | Ready, Dirty or Invalid after refresh + focused grip + pre-grab order | No server read; ungrabbed Arrow/Home/End moves only the roving focus, while grabbed Arrow/Home/End updates the draft and re-runs V5 | Space drops in place and clears `aria-grabbed`; Escape cancels and restores pre-grab order; Tab/Shift+Tab first drops in place and clears `aria-grabbed`, then moves | Ready when the settled draft equals origin; Dirty when it differs and is valid; Invalid after refresh while a marked offending row remains; never leave a grabbed owner behind | Existing live-region announcement; invalid row feedback remains | Grip while grabbed; after Tab, the ordinary next/previous enabled control | None |
| ET-8 — reversible exit | Loading route, Route unread, Ready, Dirty, Invalid after refresh, Route save rejected or Route save outcome unknown + registered origin/evidence | No mutation; discard any dialog-owned late read after close | Close selector; cancel any grab to pre-grab order; discard local/submitted draft | Close to held page row; unknown outcome alone marks it Chain unresolved | Exact prior row, or Chain unresolved for unknown outcome | Invoking model row | D-35 page owner only for Chain unresolved |
| ET-9 — send/resend Route PUT | V5-valid Dirty draft, or exact held guard plan | First save is non-forced; confirmed save echoes immutable draft + both current arrays with `force: true` | Close selector; drop any grab before freezing the submitted array | Saving | `route.saving` | Stable progress status container, programmatically focusable but not actionable | One owned Route PUT |
| ET-10 — R6 success with impact | Saving + exact R6 envelope with either array nonempty | Consume every R6 member before cardinality selects presentation | Release PUT ownership; discard any refusal preview, retain response evidence | Route impact reported | Impact title/detail and each nonempty report block | `route.impact.done` | None until Done starts M6 |
| ET-11 — report-free commit evidence | Saving + R6 two-empty envelope, or ET-17 matching AgentChain with tails unavailable | Preserve exact response/readback evidence, then enter M6 | Release PUT/reconciliation ownership; no origin can be restored | Route committed, reconciling | `route.refreshing` | `route.impact.done` | M6 starts immediately |
| ET-12 — guarded 409 | Saving + immutable draft + complete current refusal arrays | Guard code first, then both arrays; a changed forced plan replaces the old plan | Release PUT ownership; discard a superseded plan only after replacement is held | Route save refused | Shared Qp6FI copy and current arrays | `guard.cancel`, the non-destructive choice | None until explicit confirm |
| ET-13 — guard action | Route save refused + immutable draft/current arrays | Cancel consumes no authority; confirm revalidates the exact echoes before handing the send to ET-9 | Cancel drops the refusal only; confirm clears it as ET-9 freezes the same immutable draft and arrays | Cancel → Dirty; confirm → ET-9/Saving | Normal draft on cancel; `route.saving` on confirm | `route.save` on cancel; stable progress status on confirm | PUT only after confirm; its later response is dispatched by ET-10/ET-12/ET-14/ET-15/ET-18 |
| ET-14 — definitive non-guard rejection | Saving + submitted draft; `direct_mode` is reserved for ET-18 | Exclude guard and DM before classifying the shaped rejection | Release PUT ownership; retain draft/stage for Retry | Route save rejected | `route.fail.save` | `route.retry` | None until Retry |
| ET-15 — no response evidence | Saving + submitted draft + initial/confirmed stage | No response fact is promoted to commit or refusal | Release PUT ownership; retain immutable attempt evidence | Route save outcome unknown | `route.fail.unconfirmed` | `route.retry` | None until Retry |
| ET-16 — reconciliation begins / AgentSupply dispatch | Route save rejected or outcome unknown + held draft/stage | **First** read `GET /api/models/agents`; failure stays, exact `mode: direct` is DM-4/ET-18, only exact `mode: hub` authorizes ET-17 | Keep error body and draft while pending; Retry is `aria-busy`; direct mode discards editor transients in ET-18 | Same failure while pending/failed; ET-18 on direct; otherwise ET-17 continues | Existing failure copy while pending; F1 on read failure | Keep `route.retry` | Agents read, then ET-17 only for Hub |
| ET-17 — Hub Source/chain reconciliation | Originating failure + exact Hub AgentSupply + held draft/stage | Read Sources and exact chain; `direct_mode` outranks V5; failure stays; equal pairs → ET-11; different + V5-valid → ET-9; different + refreshed-invalid new/changed row → Invalid after refresh | On direct discard through ET-18; on invalid discard submitted attempt/refusal plan but keep editable rows; on resend freeze the same stage | ET-18, originating failure, ET-11, ET-9 or Invalid after refresh respectively | Existing failure, refreshing, saving, or `route.invalidAfterRefresh` on every offending row | Owning backend group head for ET-18; Retry on failure; Done on commit; progress on resend; on invalid, make the first offending row roving and focus its remove action | No read on invalid; M6/PUT only in named branches |
| ET-18 — direct-mode landing handoff | Any Route phase + DM-1/DM-2/DM-3 error, or ET-16's exact DM-4 AgentSupply | Error evidence never selects a page form; DM-1–DM-3 transfer to existing page Agents read, while DM-4 installs its exact row immediately | Close selector/modal, cancel grab, discard every draft/submitted/refusal value and never resend | Page §1.0 Partial while an error-triggered read is pending; then actual Direct or Hub by returned `mode`; DM-4 lands directly | Existing Partial/F2 only; never label Direct from an error | Owning backend group head, which stays mounted across Direct/Hub forms; page Retry remains reachable on failure | Agents read/retry only for DM-1–DM-3; none for DM-4 |
| ET-19 — M6 settlement | Route impact reported, Route committed/reconciling or M6-stale state + exact commit/report evidence | Done on a report starts M6; success installs complete surface; failure cannot negate commit | Preserve report/evidence through failure; reconstruct no report array | Success closes to current row; report failure stays mounted; report-free failure → Committed projection stale | `route.impact.refreshFail` only on failure; prior report/refresh copy remains | `route.impact.done` remains when mounted; current model row on success | M6; read-only Retry repeats only M6 |
| ET-20 — committed exit / Retry | Mounted report, reconciling or stale state + exact commit evidence | Retry invokes only M6; Done/close/Escape/outside never invokes mutation | Exit transfers an in-flight M6 owner or carries stale evidence; never restores origin | Receiving page with current or explicitly stale projection | Held report/evidence plus stale marker and refresh-failure line as applicable | Receiving model row | Page-owned pending M6 or later read-only M6 Retry |
...
| --- | --- | --- | --- | --- |
| Loading route | 取消 button + title-bar close | `route.cancel` | Enabled while the opening GET owns no mutation | One modal tab stop; activation/close emits ET-8 |
| Route unread | 重试 | `route.retry` | Enabled | First command in Tab order; activation emits ET-4 |
| Route unread | 取消 button + title-bar close | `route.cancel` | Enabled | Normal Tab order; activation/close emits ET-8 |
| Ready / Dirty / Invalid after refresh | 添加一跳 | `route.addHop`; disabled explanation `route.add.none` | Enabled exactly when V5 exposes a candidate | Normal Tab order; activation emits ET-5 |
| Ready / Dirty / Invalid after refresh + Add selector | Source/model candidate region | `route.add.source`, `route.add.model` | Labels render while open; candidate focus exists while a valid pair remains | Roving candidate keys emit ET-5; Tab reaches confirmation |
| Ready / Dirty / Invalid after refresh + Add selector | 添加 confirmation | `route.add.confirm` | Enabled only for the selected exact V5-valid pair | Activation emits ET-5 |
| Ready / Dirty / Invalid after refresh | 按来源顺序重排 | `route.reorder` | Enabled; stable-sort no-op remains valid | Normal Tab order; activation emits ET-6 |
| Ready / Dirty / Invalid after refresh | Each hop grip | `route.grip` | Rendered for every hop; exactly one grip is the roving list tab stop | Space/Arrow/Home/End/Tab/Escape emit ET-7 |
| Ready / Dirty / Invalid after refresh | Each hop remove action | `route.removeHop` | Rendered for every hop; the current roving row's action joins Tab order | Activation emits ET-6; ET-7 guarantees no grabbed state reaches it |
...
| Ready / Dirty / Invalid after refresh | 取消 button + title-bar close | `route.cancel` | Enabled | Normal Tab order; activation/close emits ET-8 |
| Ready / Dirty / Invalid after refresh | 保存 | `route.save` | Ready and Invalid: disabled. Dirty: enabled only when changed and V5-valid. Invalid Save uses all rendered invalid lines as `aria-describedby` | Activation when enabled emits ET-9 |
| Saving | Owned progress status; no action control | `route.saving` | Stable programmatic focus target; every command/dismissal is inert while PUT is owned | No actionable tab stop; response emits ET-10–ET-15/ET-18 |
| Route save refused | 仍要保存 | `guard.confirm.saveRoute` | Enabled for exact current arrays | Shared Qp6FI Tab order; activation emits ET-13/ET-9 |
| Route save refused | 取消 + title-bar close | `guard.cancel` | Enabled | First focusable command; activation/close emits ET-13 |
| Route save rejected / outcome unknown | 重试 | `route.retry` | Enabled; `aria-busy` only while ET-16/ET-17 owns reads | First command in Tab order; activation emits ET-16 |
| Route save rejected / outcome unknown | 取消 button + title-bar close | `route.cancel` | Enabled | Normal Tab order; activation/close emits ET-8 |
| Route committed, reconciling / Route impact reported | 完成 + title-bar close | `route.impact.done` | Enabled before, during and after the registered M6 read | First command in Tab order; activation/close emits ET-19/ET-20 |
| Route impact reported after M6 failure | 重试 | `route.retry` | Enabled in addition to every Done-equivalent exit | Normal Tab order; activation emits ET-20 |
| Report-free Committed projection stale | 重试 | `route.retry` | Enabled while stale projection is mounted | Normal Tab order; activation emits ET-20 |
| Report-free Committed projection stale | 完成 + title-bar close | `route.impact.done` | Enabled while producer dialog remains mounted | Activation/close emits ET-20 |
...
events only. ET-5–ET-9 and ET-20 remain the sole owner of cleanup, destination and entry
focus:

| Key | Emitted event |
| --- | --- |
| `Tab` / `Shift+Tab` | Ungrabbed: move through enabled controls and wrap in the modal. Grabbed: emit ET-7, which first drops in place and clears `aria-grabbed`, then performs the same move |
| `Space` on a grip | Emit ET-7 start/drop |
| `↑` / `↓`, `Home` / `End` on a grip | Emit ET-7 row movement; ungrabbed moves the roving focus, grabbed moves the draft row |
| `Escape` | Grabbed → ET-7 cancel/restore; selector open → ET-5 close; otherwise emit ET-8 or ET-20 for the mounted phase. Saving emits nothing |
| `Enter` / `Space` on a remove action | Emit ET-6 removal |
| `Enter` / `Space` on 添加一跳 or selector confirmation | Emit ET-5 open/append |
| `Enter` / `Space` on 保存, 重试, guard primary or 完成 | Emit the ET row cited by that focused control; Enter never submits implicitly from a row/selector |

Ordinals renumber contiguously after ET-6/ET-7. Grab state and every new position use the
...
evidence whenever one of those branches changes the page form; a classifier alone can
retire a surface but cannot project its destination. C16 makes every state-changing event
account for authority dispatch, transient cleanup, visible feedback and destination focus
in one owning edge row. None can be inferred from the reversible no-op rule.
