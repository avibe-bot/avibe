...
| G-34 | §1.4 / §1.11, E6 materialization failure | the contracted error-envelope shape that carries any committed, non-reconstructible impact from that failed materialization attempt | **The code is contracted and the impact envelope is not.** `api.md` names `discovery_failed` and `migration_item_conflict` as terminal materialization errors, which is enough to stop the poll and start the registered attempt-scope read, but it freezes no error-response member that can be consumed as impact evidence. Runtime or client fields are not wire authority and this UI register does not read them backwards into a contract. R5 therefore reserves the consumer without naming, rendering or discarding a guessed member; K4 owns freezing the envelope before that placeholder can be upgraded |
...
| C13 — terminal / absence attempt scope | Every terminal code and authoritative subject absence after a producer attempt MUST account for that attempt's full invalidation scope before visible handoff; no caller may bypass the registered reconciliation owner | [x] Every `source_not_found` and inferred deletion enters M0; M2 is M0's deletion-attempt reconciliation | [x] Shared §1.4 create E4 owns no pre-existing Source scope, while create E6 re-reads `Source[]`; RR-5 covers reauth `failed`, `cancelled`, `flow_expired`, `flow_not_found` and E6, and E7 enters M0 with the complete native scope when applicable | [x] N/A — the menu has made no producer attempt and owns no terminal code |
...
| R5 | §1.4 / §1.11 E6 materialization error | `[contract-gap]` G-34 | Shape pending; no member name is registered | Reserve the attempt's error-envelope consumer. Until K4 freezes the shape, render no guessed impact member and do not classify a runtime/client field as wire evidence. E6 still stops polling and RR-5 still reconciles the complete attempt scope; after the freeze, every contracted member must be added here with a render, hold, transfer or irrelevant-with-reason disposition. |
...
| RR-5 | Status/submit returns E6's `discovery_failed` / `migration_item_conflict`; or reauth returns terminal `failed` / `cancelled`, E8 `flow_not_found`, or E9 `flow_expired` | none / exact Source projection | create / reauth | Create re-reads the Source list. Reauth re-reads its complete producer-attempt scope **before** failure handoff: M3's full model surface for `native_cli`, including every same-backend sibling, and the registered selected-Source scope for Hub. E8/E9 cannot use a forgotten or expired binding as Source-presence evidence. E2 has no terminal evidence and keeps the scope with the bounded poll | No successful repair tail exists. For reauth the exact Source is absent, present, or still unread when reconciliation fails; a present reread is current projection evidence, not a substitute success envelope. E6's error-envelope impact disposition remains R5/G-34 until K4 freezes it | Stop polling and use the intent-specific §1.4 failure row. Perform the registered read: absent or E7 → M0 / §1.6 Source gone; present → OAuth failed in front of the reconciled projection; unread → OAuth failed with that read pending, whose Retry repeats **only** RR-5 before it may resend. E2 OAuth-status outages, including `engine_down`, stay in the bounded poll instead |
...
| §1.4 | OAuth materialization failed `[derived]` `[contract]` `[contract-gap]` G-34 | Status or submit returned E6's `discovery_failed` or `migration_item_conflict` after this dialog acquired the flow. Authorization entered materialization and the affected source projection may already have changed; no other named code enters this state. R5 reserves, but does not guess, the uncontracted error-envelope impact consumer | F1 in place; polling stops immediately | `addSub.error.finalize` for `intent: create`, `addSub.error.finalizeReauth` for `intent: reauth`, `addSub.retry` | RR-5 refreshes the attempt scope immediately: create re-reads the source list; reauth re-reads M3's complete model surface and applies M0 / Source gone precedence. Keep the failure surface in front of that projection. Until G-34 closes, name or render no error-envelope impact member. 重试 performs fresh acquisition with the held intent/channel body; 取消 → Dismissing |
...
| §1.11 | Repair failed `[derived]` | The pre-flow reauth request or credential replacement failed before its terminal could be confirmed; the repair intent, channel acknowledgement, exact origin status and any typed key remain held | F1, on the repair surface | `upstream.repair.fail`, `upstream.retry` | RR-6–RR-9 first perform the producer-attempt read C10 registers: complete model surface for lost native acquisition or uncertain credential replacement, held Source id for Hub reauth. Then absent → M0 / Source gone; a held `needs_action`/`error` origin that is now clear → RR-7's M3/M4 handoff before rendering the reread Source as repaired; a still-blocked origin → remain here; any origin already non-blocked → remain regardless of present snapshot, because health is not mutation evidence. 重试 repeats the held producer. After a flow is held, E4 is create-only; E8/E9 run RR-5 before OAuth failed / Source gone, E6 uses R5/G-34 and OAuth materialization failed, and E2 remains inconclusive. Every branch preserves `intent: reauth` |
...
| E6 — materialization-only failure | Exactly `discovery_failed` or `migration_item_conflict` | This code can arise only after terminal authorization entered materialization, which may already have changed Source state and dependent supply | Stop immediately → OAuth materialization failed; RR-5 reads the attempt scope | R5 / G-34 reserves the error-envelope consumer without guessing its members. `create` refreshes the source list; `reauth` refreshes M3's complete model surface, including same-backend native siblings, before M0 Source-gone precedence |
...
carries what the server now says about the affected source. R5/G-34 is the explicit
exception to envelope-member totality until K4 freezes the materialization-error shape;
no implementation may fill it from runtime inference.
...
- Forms A, B and C are selected from a **non-terminal** returned `presentation`, never a
  ...
  E3a/E3b distinguish action-required from accepted paste input, E8 `flow_not_found` and E9
  `flow_expired` / terminal failure first run RR-5's attempt-scope read, and only E6's
  `discovery_failed` / `migration_item_conflict` stops immediately and refreshes M3's complete
  model surface; R5/G-34 holds its error-envelope consumer pending K4 rather than guessing;
