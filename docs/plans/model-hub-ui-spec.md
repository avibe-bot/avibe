...
| C14 — non-mutating failure exit | Every modal failure state MUST offer an exit that sends no mutation; a committed-state exit preserves write evidence and marks unread dependent projections stale rather than restoring the origin | [x] Read/rejected and settled-nonmatch failures may abandon to latest held authority through ET-8a; unknown-outcome exit uses ET-8b to hand only its exact Route evidence to Chain unresolved; both disown modal-only reads. Committed exits preserve evidence | [x] Edit/remove failures cancel to their held draft/origin; Committed projection stale has Done-equivalent dismissal in addition to read-only Retry | [x] OAuth/repair failures can dismiss through their registered cleanup or Done-equivalent committed exit; neither path resends repair | [x] The menu's Escape / outside exit sends nothing and restores Add subscription focus |
...
| C16 — atomic event-transition totality | Every state-changing event MUST register one atomic owner for authority dispatch, cleanup, visible feedback, destination focus and next owner; unlisted transitions are forbidden. ET owns gestures/non-RO single producers, AR owns mode-neutral composite-read member settlement and prerequisite-gated acquisition, RO owns exact-chain admission and Route-outcome settlement independently from page form, LF/AS own landing read-set/install actions, PF-1/FF-1 own focus eligibility and fallback, and CA alone admits **and dispatches** async controls. One row cannot group events whose outcomes differ. Every prerequisite failure must settle its acquired member while deferring ineligible dependants; CA predicates must be a positive exhaustive/mutually-exclusive phase partition; every rendered dismissal must map to exactly one edge; every post-install focus transfer must exhaust PF-1's FF-1-filtered candidates | [x] AR records `(workflow generation, member, acquisition eligibility, observation epoch, member settlement, required causal-frontier set, decisive page evidence)`: `deferred-by-prerequisite` members are neither pending, failed, stale nor completion obligations, and successful Agents mode alone activates their applicable frontiers. RO separately records the complete `Route outcome × owner × observation legality` product; no producer identity or sibling settlement participates in an outcome edge. Mounted RO-O delegates its admitted member result to AR-D, while the RO owner-factor table settles page-session results and names cleanup, destination, feedback, focus and next owner. LF closes each returned mode's companion read set and every Hub post-Source success invokes the same RO hook. RL closes every started read with generation, cancellation/disown owner and late-result disposition; AP cites every consumed wire member's route/schema provenance. CA-D positively partitions every exact async phase so one row or none, never two, can emit; all rendered dismissal gestures use the registered commit-class × owner × busy edge. PF-1 filters active target, exact row, exact group and destination-page control through FF-1 in order. Visible feedback cites a registered key or closed copy block | [x] §1.10's state rows consume R1/R2 and M0–M2, clear their modal phase under DP-1/DP-2/DP-4, render the registered F/copy treatment and name the source overflow or receiving page as focus owner | [x] §1.11's state rows consume R3/R4 and RR/M0/M3/M4, settle flow/key transient ownership, retain the registered report/failure copy and name the invoking card or receiving projection as focus owner | [x] Closed/Open and the §1.4 handoff name the explicit vendor event, menu cleanup, visible destination and Add subscription / receiving-surface focus; no server response edge belongs to frame 13 |
...
| §1.2 | Route save outcome unresolved `[derived]` | The route write initially produced no exact Route authority: either no HTTP/API answer or DM-2's shaped `direct_mode` response. The exact submitted draft and whether this was the non-forced or confirmed stage remain held while RO is `unknown`; an exact nonmatching chain moves the same mounted geometry to `observed-nonmatch` while Source settles. RO owns the outcome independently from owner/page form; Direct changes only observation legality | F1; D-36 permits no blind resend | Mounted: `route.fail.unconfirmed` while unknown, `route.fail.reconcileRead` after a read failure, `route.retry` by CA phase, and `route.cancel`. Page-owned Hub failure reuses the row's Chain-unresolved/D-35 read treatment; page-owned Direct makes no false Route claim | ET-15/DM-2, RO/AR-D/ET-16, LF/AS, ET-17 and ET-18a own entry, owner/legality changes and D-36 reconciliation. Every mounted `unknown` dismissal uses ET-8b's evidence transfer; every mounted `observed-nonmatch` dismissal uses ET-8a's abandonment, including Source pending/failed/retrying. CA-D's positive partition admits exactly one applicable Retry; a page-owned read failure admits only D-35. Reload drops client evidence and never resends |
...
| Mounted editor + nonmatch | Set outcome `observed-nonmatch` and keep latest Hub/chain authority; do not wait to settle the outcome and do not yet infer Source success | While Source is pending keep existing failure copy; Source success runs V5; Source failure uses `route.fail.reconcileRead` without changing outcome. Every non-busy subphase retains ET-8a's reversible abandonment | Product table selects: pending → wait; current Source + valid → CA-D7/ET-17d; invalid → ET-17e; Source failed → CA-D5/ET-17g only; ET-8a may instead disown Source work and close without resend |
...
| RT-18 — CA-D positive phase partition | Table-drive initial idle/busy, generic-subset idle/busy, Source-only idle/busy, explicit-resend idle, plus invalid and successful products | Each of the seven named families selects exactly its CA-D row; invalid/successful selects zero enabled rows. ET-17b cannot also admit CA-D1, ET-17f cannot also admit CA-D1/D3, and every busy phase emits nothing for pointer/Enter/Space |
| RT-19 — settled nonmatch dismissal | Exercise `observed-nonmatch` while Source is pending, after its failure, and during a Source-only read Retry; invoke Cancel/title-close/Escape/outside | Every gesture takes ET-8a: disown all AR-D Source work, ignore late results, discard draft/comparison, reveal the latest installed authority and send no read/mutation. The same gestures from `unknown` take ET-8b instead; Saving remains inert and committed stays ET-20 |
...
ET-8a owns ordinary and settled-noncommit reversible exits, including every mounted
`observed-nonmatch` subphase. ET-8b alone handles RO `unknown` and
...
definitive rejection or settled nonmatch ET-8a likewise reveals only the latest authority
the dialog holds and disowns every modal-only AR-D read.
...
| ET-8a — ordinary reversible / settled-noncommit exit | Loading route, Route unread, Ready, Dirty, Invalid after refresh, Route save rejected, or mounted `observed-nonmatch` with no mutation response owned + latest installed authoritative page projection | Send no mutation; disown the current RL opening generation and the full AR-D generation, including a pending/retrying Source read, before close | Close selector; restore/cancel any grab; discard local/submitted draft and settled nonmatch comparison; ignore every late result from both disowned generation families | Reveal the latest installed page authority; before reconciliation this is the held opening projection, after AS-2 it is that newer Hub projection | Exact latest projection; retain its existing unresolved marker if any. No mutation Retry or false commit/non-commit report follows | Preserve an FF-1-valid active page target, otherwise PF-1 after the modal target unmounts | None; an already-unresolved row retains D-35 |
| ET-8b — unknown-outcome exit | RO is `(unknown, mounted editor, Hub-observable \| Direct-suspended)` + immutable attempt evidence + latest installed page authority | Send no mutation; change owner to page session. Disown every modal-only AR-D Agents/Source generation. If Hub-observable, transfer the current exact-chain generation when it is already the one legal RO-O observation, otherwise disown it and acquire exactly one page-owned successor; if Direct-suspended, disown illegal chain work and send no read | Discard modal-only transients; retain submitted pairs/stage, unknown outcome and legality. Every disowned Agents/Source/chain late result is ignored; only the transferred/acquired page-owned chain generation survives | Latest installed Hub projection with its exact row marked Chain unresolved while RO-O reads, or actual Direct page with suspended evidence held off-row | Hub read failure uses the existing Chain-unresolved/D-35 read treatment; Hub nonmatch is silent abandonment with no mutation Retry; Direct renders no false Route verdict | Preserve an FF-1-valid page target, otherwise PF-1 after modal close | Hub → exactly one page-owned RO-O; Direct → RO-S until any LF-H post-Source hook re-enters RO-O |
...
| ET-15 — no response evidence | Saving + submitted draft + initial/confirmed stage | Set RO outcome `unknown`; infer neither commit nor refusal | Release PUT ownership; retain immutable attempt evidence | Route save outcome unresolved | `route.fail.unconfirmed` | `route.retry` | None until Retry |
| ET-16a — begin failure reconciliation | Route save rejected or outcome unresolved in CA-D1 initial idle + held draft/stage | Start one AR-D generation with `GET /api/models/agents` | Keep error body/draft; CA-D2 owns focus transfer plus Retry busy/inert admission | Same failure, reconciliation pending | Existing failure copy | CA-D2 handoff | One AR-D Agents read; no second generation is admitted |
...
The seven named phase families below cover initial idle, initial busy, generic subset idle,
generic subset busy, Source-only idle, Source-only busy and explicit resend idle; invalid or
successful products select no enabled CA-D row:

| ID | Control | Async subphase | Rendered / admitted | Synchronous focus handoff | Activation |
| --- | --- | --- | --- | --- | --- |
| CA-D1 | Save-failure Retry | **Initial idle**: ET-14/ET-15 origin or ET-16b, with RO `unknown`/`rejected`, no AR-D AgentSupply landing, no settled Source/chain subset and no generation active | Rendered, enabled | None while admitted | Emit ET-16a and atomically acquire the sole initial Agents generation |
| CA-D2 | Save-failure Retry | **Initial busy**: ET-16a Agents, then AS-2/ET-17a Source+chain generation active before an ET-17b/ET-17f/ET-17d/ET-17e terminal phase | Rendered busy, disabled/inert | If Retry is active, move first to enabled Cancel, otherwise enabled title-close, before changing admission | None; cannot overlap AR-D or reach a second PUT |
| CA-D3 | Failed-read subset Retry | **Generic subset idle**: ET-17b holds an exact failed Source/chain subset, RO is not `observed-nonmatch` + Source-failed, and no member is pending | Rendered, enabled | None while admitted | Emit ET-17h for exactly the failed read subset; never reread a settled success or resend the mutation |
...
| CA-D5 | Source-only reconciliation Retry | **Source-only idle**: ET-17f holds `observed-nonmatch` + failed Source and no member is pending | Rendered, enabled | None while admitted | Emit ET-17g for Sources only |
| CA-D6 | Source-only reconciliation Retry | **Source-only busy**: ET-17g's Source successor is active while `observed-nonmatch` remains settled | Rendered busy, disabled/inert | If Retry is active, move first to enabled Cancel, otherwise enabled title-close, before changing admission | None; cannot reread Agents/chain or overlap a Source generation |
| CA-D7 | Explicit mutation Retry after comparison | **Explicit resend idle**: `observed-nonmatch` + frontier-satisfying Source success + V5-valid draft, with no read pending | Rendered, enabled | On activation mount and focus the programmatically focusable `route.saving` status before Retry becomes inert | Emit ET-17d; this is the only post-comparison row that resends PUT |
...
| Route save rejected / outcome unresolved | 重试 | `route.retry` | CA-D1–D7's positive phase partition is the sole owner, including mounted `observed-nonmatch`. Initial, exact generic-subset, Source-only and explicit-resend idle phases each enable only their row; their busy twin is disabled/inert; invalid/successful phases enable none | First command in Tab order only when enabled; activation delegates to the current CA row |
| Route save rejected / mounted `observed-nonmatch` | 取消 button | `route.cancel` | Enabled whenever no mutation response is owned; a reconciliation read may be disowned | Normal Tab order; activation emits ET-8a |
| Route save rejected / mounted `observed-nonmatch` | Title-bar close | `route.cancel` | Enabled whenever no mutation response is owned; a reconciliation read may be disowned | Independent Tab stop; activation emits ET-8a |
| Route save outcome unresolved with RO `unknown` | 取消 button | `route.cancel` | Enabled whenever no mutation response is owned; a reconciliation read may be transferred/disowned | Normal Tab order; activation emits ET-8b |
| Route save outcome unresolved with RO `unknown` | Title-bar close | `route.cancel` | Enabled whenever no mutation response is owned; a reconciliation read may be transferred/disowned | Independent Tab stop; activation emits ET-8b |
...
| Noncommitted `rejected` or `observed-nonmatch`, including Source pending/failed/retrying | Mounted editor | None or disownable AR-D read generation | ET-8a | ET-8a | ET-8a | ET-8a |
| Noncommitted `unknown`, including AR-D pending/failed/retrying | Mounted editor | None or transferable/disownable AR-D read generation | ET-8b | ET-8b | ET-8b | ET-8b |
