...
good dependent projections marked stale, under the page's own `toast.refreshFailed` line when a
first-paint member is what failed. Its
...
stale region's `sourceDetail.retry`, and only when a first-paint member is what failed
`[derived]`.
