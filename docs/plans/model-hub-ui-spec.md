...
installed/held immediately; each install and final settlement reapplies PF-1 so an opener
removed by reconciliation cannot strand focus. Direct mode makes Route-chain members not
applicable and disowns their reads; it does not turn them into stale failures. M6 never
...
AS-5a/AS-6a reapply PF-1 after each relevant install. The event table remains the sole destination
...
| Route committed, reconciling / refreshed / Route impact reported / page-owned AR-M stale | No modal control | `common.saved` / existing page retry copy | The editor is already closed; only the receiving page renders pending/stale state | Focus remains page-owned and PF-1 is reapplied after installs/settlement |
