...
specification is deleted, and the current export remains the visual authority. §1.2
registers only its controls' semantics and the interaction states that cannot live in a
drawing; §1.1 keeps the same boundary for its legend note and the two model-row keys. The
...
| G-18 | 05 add-by-key, 拉取型号 and the observation 添加 runs before it saves — **registered against Source observation** | ~~the route that carries a non-persisting observation of a source that does not exist yet~~ nothing | `POST /api/models/sources/observe` accepts `{vendor, base_url?, key, protocol_order?}` and returns `SourceObservation` without persisting a Source or returning a credential reference. §1.5 consumes its closed outcome/reachability/authentication/protocol/discovery/models facts. |
...
| C7 — authoritative field validation | Register every editable field against the authority that normalizes or rejects it; no field may rely on a generic request failure as its validator | [x] V5 consumes exact-pair uniqueness, server eligibility and callable-inventory authority while permitting unchanged stale pairs | [x] V1/V2 register every frame-11 field, including the complete Base URL normalizer | [x] V3/V4 register the replacement key and paste-back value; the shared reauth acknowledgement is the contracted literal `true`, not a free-form draft | [x] N/A — vendor rows emit closed enum values and expose no editable field |
...
| C14 — non-mutating failure exit | Every modal failure state MUST offer an exit that sends no mutation; a committed-state exit preserves write evidence and marks unread dependent projections stale rather than restoring the origin | [x] Read/rejected failures may cancel to held authority; unknown-outcome exit hands the row to Chain unresolved; committed exits preserve evidence | [x] Edit/remove failures cancel to their held draft/origin; Committed projection stale has Done-equivalent dismissal in addition to read-only Retry | [x] OAuth/repair failures can dismiss through their registered cleanup or Done-equivalent committed exit; neither path resends repair | [x] The menu's Escape / outside exit sends nothing and restores Add subscription focus |
| C15 — page-form landing evidence | Every response, terminal or absence branch that changes the mounted page form MUST name the authoritative payload or read that selects the destination. Successful authority is consumed monotonically: sibling failure cannot erase it, exact mode evidence cannot be overwritten, and a later failure cannot roll the page back to an older projection | [x] AR is the sole composite-read result owner. It records each member's observation epoch against every causal frontier that can invalidate that member, dispatches each successful AgentSupply for mode/page authority immediately, uses only exact AgentChain GET equality as D-36 Route evidence, and recalculates the applicable member set after either AgentSupply Direct or chain `direct_mode`. LF is the closed landing read-set owner: exact Direct retains every settled independent Source authority; each exact Hub landing installs AgentSupply immediately, establishes a new Source landing frontier and cannot call Sources current until a successor Source observation satisfies it. DM errors retire an illegal editor but never invent a landing payload; PF-1 is fallback-only after installation | [x] R1/R2 and M0–M2 hand their exact returned or reread Source/absence evidence to the page. Their complete-surface results dispatch returned mode before installing a page form; an error class selects no destination by itself | [x] R3/R4 and M0/M3/M4 hand their exact returned or reread Source evidence to the page. Their complete-surface results dispatch returned mode before installing a page form; flow/error classes select failure handling, never a healthy landing | [x] N/A — the menu changes only its own reversible phase and hands one explicit local vendor selection to §1.4; no response or terminal branch changes the page form |
| C16 — atomic event-transition totality | Every state-changing event MUST register one atomic owner for authority dispatch, cleanup, visible feedback, destination focus and next owner; unlisted transitions are forbidden. ET owns gestures/non-RO single producers, AR owns mode-neutral composite-read member settlement, RO owns exact-chain admission and Route-outcome settlement independently from page form, LF/AS own landing read-set/install actions, PF-1/FF-1 own focus eligibility and fallback, and CA alone admits **and dispatches** async controls. One row cannot group events whose outcomes differ | [x] AR records `(workflow generation, member, observation epoch, member settlement, required causal-frontier set, decisive page evidence)`, the frontier-satisfaction relation, applicable-member function, installed authority, remaining stale members and retry subset. RO separately records the complete `Route outcome × owner × observation legality` product; no producer identity or sibling settlement participates in an outcome edge. Mounted RO-O delegates its admitted member result to AR-D, while the RO owner-factor table settles page-session results and names cleanup, destination, feedback, focus and next owner. LF closes each returned mode's companion read set and every Hub post-Source success invokes the same RO hook. RL closes every started read with generation, cancellation/disown owner and late-result disposition; AP cites every consumed wire member's route/schema provenance. CA enforces at most one active attempt, is the sole pointer/Enter/Space activation dispatcher for its rows and synchronously hands focus to the destination phase's first FF-1-valid owner before an active control becomes disabled/inert. Visible feedback cites a registered key or closed copy block | [x] §1.10's state rows consume R1/R2 and M0–M2, clear their modal phase under DP-1/DP-2/DP-4, render the registered F/copy treatment and name the source overflow or receiving page as focus owner | [x] §1.11's state rows consume R3/R4 and RR/M0/M3/M4, settle flow/key transient ownership, retain the registered report/failure copy and name the invoking card or receiving projection as focus owner | [x] Closed/Open and the §1.4 handoff name the explicit vendor event, menu cleanup, visible destination and Add subscription / receiving-surface focus; no server response edge belongs to frame 13 |
...
| R6 | §1.2 exact Route replacement | `model-hub.md` Source-mutation matrix, `mutation.route_replace` | `chain`, `removed_hops`, `interrupted` | Hold `chain` as the authoritative post-commit AgentChain and write evidence. Render each non-empty `removed_hops` and `interrupted` block in Route impact reported; an empty member skips only its own block. If both arrays are empty, skip the report but retain both exact empty values. M6 owns every Route-derived projection before handoff. |
...
| M6 | R6 route replacement, or an authoritative exact ordered AgentChain from `GET /api/models/agents/<backend>/chain?model=<id>` matching submitted `hops` under D-36 | `Source[]` including `adopted_by`, complete `AgentSupply[]` including mode/model supply and any present routes projection, and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | R6 receipt or AR-D6's matching exact-chain observation establishes a Route-commit frontier required by every applicable member and starts AR-M. The returned/read chain self-satisfies Route at that frontier. Earlier Source/Agents successes remain installed as latest-known authority but require successor observations acquired after that frontier. Acquire Agents first; its exact payload self-satisfies Agents and, when Hub, adds a Hub-landing frontier required by Sources. Only then acquire the Source successor, so its generation satisfies CF-R and the exact landing's CF-H without a predictably obsolete parallel read. Any already-held Source acquired before that landing remains latest-known only. Keep R6's entire envelope when received; inference marks `removed_hops` / `interrupted` unavailable. AP records `AgentSupply.routes` as optional and deliberately unconsumed here | AR-M consumes each successful applicable member monotonically and closes only when each member's observation acquisition is causally at-or-after every frontier that applies to that member. Either exact AgentSupply Direct or chain `direct_mode` takes the same decisive edge: retain every successful independent member, disown only pending/not-applicable Route work, and acquire an authoritative Agents landing read whose exact returned mode alone selects the page form. CA admits Retry only after settlement, for exactly the applicable failed subset, and never overlaps generations. Mounted completion retains report/evidence and Done; transferred completion preserves any FF-1-valid active target and otherwise uses PF-1. ET-20 alone acknowledges/closes/transfers; no path resends or questions the route write |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or AR-D6 proves the desired Route from the exact chain endpoint while response-only arrays are unavailable; exact commit evidence is held while the active AR-M generation has pending applicable members or a member whose observation has not satisfied every required causal frontier | F2 applies only to the applicable members that AR-M3 later marks stale; the Route write/desired state is already proven | `route.refreshing`, `route.impact.done` | ET-11 or ET-17c owns mutually exclusive entry; AR-M/AR-MD, ET-18aM/ET-18aP, ET-19, AS and ET-20 own member epochs/frontiers, Direct handoff, Retry, page installation and the sole user exit |
| §1.2 | Route committed, refreshed `[derived]` | AR-M2 completed a mounted report-free M6 generation with every applicable member satisfying all causal frontiers and installed its exact page behind the still-mounted modal; commit evidence remains held until acknowledgement | F5 — no request remains and no impact report exists | `route.impact.title`, `route.impact.done` | ET-20 is the only exit. The page projection is already installed, and modal close invokes PF-1 because Done unmounts |
| §1.2 | Route save refused `[derived]` `[contract]` | A non-forced or forced save returned `source_last_supplier` with the current nonempty `would_interrupt` plan and complete `would_remove_hops` projection | F3 — shared `Qp6FI`; G-23 remains the drawing gap for the detailed SupplyGap block | `guard.title.saveRoute`, `guard.subtitle.saveRoute`, `guard.confirm.saveRoute`, `guard.label`, `guard.count`, `guard.hop.position`, `guard.hint.interrupt`, `guard.gap.label`, `guard.gap.subject`, `guard.gap.agents`, `gateway.modelCount`, `guard.cancel` | ET-12, ET-13a and ET-13b separately own plan replacement, cancellation and confirmation |
| §1.2 | Route save rejected `[derived]` | The server definitively rejected the route write outside the guarded-plan and `direct_mode` branches, so this attempt did not commit | F1, in place | `route.fail.save`, `route.fail.reconcileRead` after a failed AR-D member, `route.retry`, `route.cancel` | ET-14, AR-D/ET-16, AS-1/AS-2, ET-17, ET-18a and ET-8a own entry, reconciliation, every outcome and the non-mutating exit. CA admits the initial Agents reconciliation or only the exact settled failed read subset; no active generation overlaps |
| §1.2 | Route save outcome unknown `[derived]` | The route write produced no exact Route authority: either no HTTP/API answer or DM-2's shaped `direct_mode` response. The exact submitted draft and whether this was the non-forced or confirmed stage remain held. RO owns the outcome independently from owner/page form; Direct changes only observation legality | F1; D-36 permits no blind resend | Mounted: `route.fail.unconfirmed`, `route.fail.reconcileRead`, `route.retry`, `route.cancel`. Page-owned Hub failure reuses the row's Chain-unresolved/D-35 read treatment; page-owned Direct makes no false Route claim | ET-15/DM-2, RO/AR-D/ET-16, LF/AS, ET-17, ET-18a and ET-8b own entry, owner/legality changes and D-36 reconciliation. CA admits mutation Retry only for mounted Hub-observable nonmatch after V5; a page-owned read failure admits only D-35's read retry. Reload drops client evidence and never resends |
| §1.2 | Route impact reported `[derived]` `[contract]` | R6 holds a successful route-replace response with non-empty `removed_hops` and/or `interrupted`; the complete returned `chain` remains held while AR-M independently settles every applicable member after all of that member's required causal frontiers | F2 only for the exact AR-M3 applicable failed subset; the write already succeeded and DP-4 owns every exit | Report block keys always; `route.refreshing` while any applicable member is pending; `route.impact.refreshFail` and `route.retry` only in the settled-failure subphase; neither retry key after AR-M2 success | ET-10, AR-M/AR-MD, ET-18aM, ET-19, AS and ET-20 own report entry, member settlement/Direct handoff/Retry/install and the sole user exit; successful authority is installed immediately and never dismisses the report |
...
| §1.2 / §1.6 / §1.9 / §1.10 / §1.11 | Committed projection stale `[derived]` | M0 has authoritative Source absence; an M1/M2/M3/M4 mutation or R3 terminal has commit evidence and its complete read failed; M5 holds committed mode-switch AgentSupply and its Source read failed; or M6/AR-M3 holds Route commit evidence with a settled applicable failed member subset. Hold the exact success envelope when received; inference holds its authoritative subject and marks response-only members unavailable. Every successful applicable projection stays installed | F2 — the operation-specific sentence names only the failed read/member, never the completed write, authoritative absence or successful sibling | M0/M2 `sourceDetail.remove.impact.refreshFail`; M1 `sourceDetail.edit.impact.refreshFail`; M3/M4 `upstream.repair.impact.refreshFail`; M5 `upstream.unread`; M6 `route.impact.refreshFail`; the admitted owning Retry and, where mounted, its Done key | M0–M4 render committed Source/absence with last-good dependent projections stale; M5 renders held AgentSupply with Source stale; M6 renders every applicable AR-M success exactly and marks only AR-M3's applicable failed subset stale. Direct mode removes Route-chain members from the set rather than marking them stale. M0–M5 Retry repeats their owning read; M6 Retry repeats only the applicable failed subset through CA-M2/CA-M4. Success updates only its member, another applicable failure stays. DP-4 exits and page navigation remain legal. Never resend mutation, invent impact, discard evidence, roll back installed authority or describe the write/absence as uncertain (C6/C9/C10/C14/C15) |
...
carries row-grain configuration/runnability facts but no current hop. The one read that carries hops at all is
...
**When that read answers, `current` names the serving hop exactly** `[contract]`.
...
The normal dialog contains one ordered list of exact hops, a grip and ordinal on each row,
...
`GET /api/models/agents/<backend>/chain?model=<id>`. The draft is only the ordered
projection of `AgentChain.chain` to `{source_id, model_id}`; live `channel`, `health`,
`runnable`, `reason`, `retry_at`, `current` and `supply_state` remain read annotations and
are never echoed into the write. The returned projection becomes the exact reversible
origin. All local edits operate on a copy, so 取消 can restore that value without another
authority. Empty `hops` is a valid explicit unconfigured Route, not absence of a Route.
...
AgentSupply eligibility plus Source inventory, are grouped by Source, name the exact
upstream model id, and exclude duplicate pairs. Choosing one appends that exact pair and
focuses its grip through ET-5c. Identity mapping and an explicit cross-model mapping are
...
| --- | --- | --- |
| RS-1 — local edit | opening AgentChain + V5-valid ordered draft | Send nothing. ET-5a–ET-7f own the atomic selector/draft/focus transitions; Save is enabled only when the ordered pair array differs from the opening origin; empty is allowed |
| RS-2 — first save | one immutable submitted `hops` array | ET-9a sends the complete array without `force` or plan echoes and transfers focus/ownership to Saving |
...
| RS-4 — guarded refusal | HTTP 409 `source_last_supplier` + complete `would_remove_hops` and nonempty `would_interrupt` | ET-12 renders shared `Qp6FI`. The refusal persisted nothing and the draft remains unchanged. G-23 still records the missing drawn SupplyGap block; this section registers the data/copy consumer but invents no block geometry |
| RS-5 — confirm | the immutable submitted `hops` + both exact arrays from the currently displayed refusal | ET-13b verifies the exact held plan and re-sends it with `force: true`, `would_remove_hops` and `would_interrupt`; confirmation never creates a new mutation body |
...
| RS-8 — non-guard failure / no answer | `direct_mode`, another shaped rejection, or no response evidence | ET-14/ET-15/ET-18a are the sole destination owner. No-answer reconciliation starts only from ET-16a; no prose branch may resend first |
...
| DM-2 | Route `PUT` returns `direct_mode` | The editor is now illegal; no exact Route authority exists, so RO sets `outcome = unknown` for the submitted pairs even though the response is shaped | None — the error may not label the row or page Direct | ET-18a starts/transfers the same Agents read and changes only ownership/legality around that retained unknown attempt |
| DM-3 | D-36 or another post-failure chain read returns `direct_mode` | Exact Route observation is temporarily illegal; this changes RO legality to Direct-suspended and does **not** settle the held Route outcome | None — no second chain or Source read can manufacture one | ET-18a starts/transfers the same Agents read; RO retains every other axis until a producer-independent Hub post-Source hook makes observation legal |
| DM-4 | ET-16a's first reconciliation read returns the exact backend `AgentSupply` with `mode: direct` | Direct is the authoritative current backend mode, so no Route read or V5 validation is legal now; mode still proves neither Route commit nor non-commit | The returned AgentSupply row itself; it is installed exactly and no second Agents read is sent | AS-1 consumes this payload; RO derives its outcome from held Route evidence, never from this mode producer |
...
same RO `unknown` outcome. No mode evidence alone repeats or settles that mutation.
...
both refusal arrays confirms a nonempty recomputed plan. For `mutation.route_replace`,
...
`would_interrupt` refuses. Every 409 therefore names a current nonempty interruption plan
and mutates nothing. ET-13a and ET-13b separately own abandoning and confirming that refusal.
...
response. In every case the exact evidence and modal stay mounted while AR-M has pending
applicable members. Each successful applicable member is installed/held immediately; a
...
mounted, and a report-free commit enters Committed projection stale only for its applicable
failed subset.
...
observation epoch, member settlement, required causal-frontier set, decisive page evidence)`;
RO's separate product key is `(Route outcome, owner, observation legality)`. Page-form
authority, member settlement and Route-mutation outcome are orthogonal: a mode result may
settle and install the page while the Route outcome stays unknown, and a Source failure may
not rewrite an exact Route result. Each independently acquired member records pending,
...
applicable-member set before completeness, stale or Retry is evaluated. A member removed
from that set is disowned, not failed, and only pending work is cancelled; a success already
received remains installed. At most one generation per member owns reads for a workflow.
...
compares the submitted ordered pairs for D-36, including empty-to-empty. A matching chain
starts M6 with response-only tails unavailable; a nonmatching chain plus current Sources may
authorize one resend. The latest installed authoritative page projection becomes the
...
| LF-H | Any exact returned `AgentSupply.mode: hub`, including ordinary M5 PATCH, DM, AR, AS and later page refresh producers | Install that AgentSupply immediately, establish a new Hub-landing frontier for Sources and require one Source observation acquired at or after it before calling the Hub landing current. Outside proven M6 this is M5's landing-owned Source read. Inside M6, a Source generation already acquired after this exact landing may satisfy it; a pre-landing Source success/failure remains latest-known only and is superseded by exactly one successor read | A frontier-satisfying Source failure keeps AgentSupply installed, marks only Sources stale and offers only the owning Source read Retry. Every frontier-satisfying Source success invokes one common post-Source hook: set `legality = Hub-observable`; if RO is `(unknown, page session, Hub-observable)` and owns no exact-chain generation, acquire exactly one through RO-O before any Route resend |

**RO is the Route-outcome product reducer** `[derived]`. It is independent from LF's
page-form settlement. Its state is exactly `outcome × owner × legality`, where outcome is
`unknown | committed | observed-nonmatch | rejected`, owner is `mounted editor | page
session`, and legality is `Hub-observable | Direct-suspended`. The immutable submitted
pairs/stage and workflow generation are evidence attached to `unknown`; no producer name is
part of the state or a transition predicate:

| Axis | Values | Only authority that changes it | Invariant |
| --- | --- | --- | --- |
| Route outcome | `unknown`, `committed`, `observed-nonmatch`, `rejected` | No response **or DM-2 shaped `direct_mode`** without exact Route authority → `unknown`; R6 or an exact matching chain → `committed`; an exact nonmatching chain → `observed-nonmatch`; a shaped non-guard/non-Direct rejection → `rejected` | Mode, modal dismissal and read transport failure cannot change this axis |
| Owner | `mounted editor`, `page session` | ET-8b or ET-18a dismissal/handoff changes mounted editor → page session; no transition silently remounts the draft | Owner changes neither outcome nor legality; reload alone discards page-session evidence and never sends a mutation |
| Observation legality | `Hub-observable`, `Direct-suspended` | LF-H's successful post-Source hook → Hub-observable; any exact Direct evidence → Direct-suspended | Legality changes neither outcome nor owner; Direct cancels only illegal chain work and never the held evidence |

The product's legal work and settlement are total:

| ID | Product predicate | Exact work owner | Result / visible disposition |
| --- | --- | --- | --- |
| RO-O — observable unknown | `(unknown, mounted editor, Hub-observable)` or `(unknown, page session, Hub-observable)` with no current exact-chain generation | Admit exactly one exact-chain observation for the held pairs. Mounted entry is acquired/settled as an AR-D member; page-session entry is acquired by the producer-independent LF-H post-Source hook and settled by RO's owner table. A second Hub payload cannot acquire a sibling generation | Match → `committed` and M6. Nonmatch → `observed-nonmatch` immediately: mounted AR-D then derives wait/V5/Source-only Retry from its independent Source slot; page-session RO treats the dismissed change as abandoned, keeps the exact Hub projection and exposes no mutation Retry. Read failure preserves `unknown` and admits only a read retry owned by that same surface |
| RO-S — suspended | `(unknown, *, Direct-suspended)` | Retain generation/pairs/stage across Direct/Hub page-form transitions; cancel/disown only an illegal exact-chain generation | Expose no mutation Retry and make no commit/non-commit claim. The next LF-H post-Source success changes only legality, then RO-O acquires exactly one observation |
| RO-K — settled | `committed`, `observed-nonmatch` or `rejected` with either owner/legality | No reconciliation read follows solely from RO. `committed` enters M6; mounted `observed-nonmatch` may use the registered V5-gated explicit resend; page-session `observed-nonmatch` is abandoned; `rejected` keeps its ordinary failure treatment | Mode changes and dismissal preserve the settled outcome. Reload may discard page-session evidence but never resends |
...
| Owner + result | Cleanup / destination | Visible feedback / focus | Next owner |
| --- | --- | --- | --- |
| Mounted editor + match | Set outcome `committed`, establish CF-R and keep the modal mounted in Route committed, reconciling | `route.refreshing`; focus `route.impact.done` | M6/AR-M; acquire every member successor required by CF-R |
| Mounted editor + nonmatch | Set outcome `observed-nonmatch` and keep latest Hub/chain authority; do not wait to settle the outcome and do not yet infer Source success | While Source is pending keep existing failure copy; Source success runs V5; Source failure uses `route.fail.reconcileRead` without changing outcome | Product table selects: pending → wait; current Source + valid → CA-D7/ET-17d; invalid → ET-17e; Source failed → CA-D5/ET-17g only |
| Mounted editor + read failure | Keep `unknown`, settle the exact-chain generation and retain draft/latest Hub authority | `route.fail.reconcileRead`; focus the admitted `route.retry` | A newly admitted mounted RO-O read only |
| Page session + match | Set outcome `committed`, establish CF-R, retain exact Hub page and install commit evidence without remounting the editor | Page Chain-unresolved marker clears; preserve current FF-1-valid page target or PF-1 | M6 as page-owned reconciliation; acquire the Agents-first frontier sequence before settlement |
| Page session + nonmatch | Set outcome `observed-nonmatch`, clear the unresolved marker and discard submitted pairs/stage as explicitly abandoned | Existing exact Hub row; preserve current FF-1-valid page target or PF-1. No failure or mutation Retry renders | None |
| Page session + read failure | Keep `unknown`, settle the generation and leave the exact row Chain unresolved | Existing D-35/Chain-unresolved treatment; preserve current FF-1-valid page target or PF-1 | D-35 re-expansion may acquire one page-owned RO-O read |

RO lives only for the mounted page session. Reload is the sole implicit evidence drop and
never authorizes a blind resend or a long-lived reconciliation ledger.
...
| AP-3 | `AgentChain.chain[*].source_id/model_id` | `model-hub-contracts/agent-chain.schema.json`; exact chain GET (`api.md` route table) | `chain` and each pair identity are required. This endpoint is the sole lost-response equality authority for the edited Route |
...
| AP-5 | R6 `chain`, `removed_hops`, `interrupted` | route-replace success schema/mutation matrix; exact chain PUT (`api.md` route table) | All returned members are consumed. D-36 inference has no response envelope, so both response-only impact arrays are marked unavailable |

| ID / workflow generation | Independent reducer edge + observation epoch | Required frontiers / Route outcome | Decisive page evidence | Immediately installed / held authority | Applicable set after decisive evidence | Awaited / stale members | Cancellation / late result | Destination / Retry subset |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AR-D1 — initial Agents failure | Agents fails before any exact landing | No M6 frontier exists; Agents settles failed. Preserve the prior RO outcome | No mode or Route evidence exists | Keep latest installed page projection and immutable draft/stage | Agents | Failed Agents read | Settle generation; no sibling exists | ET-16b; Retry Agents only through ET-16a |
| AR-D2 — Direct AgentSupply | Agents succeeds with exact backend `mode: direct` | No M6 frontier exists. Preserve RO outcome and owner, changing only legality to Direct-suspended | Mode is decisive for page form only and is already the authoritative landing read | AS-1/LF-D installs that AgentSupply; if RO outcome is unknown retain its Route generation, submitted pairs/stage and every settled Source | Agents for landing; an unknown Route observation is suspended, not settled or failed | None while Direct; a suspended attempt is visibly non-retryable | Cancel/disown only illegal chain work; ignore that generation's late result; retain the complete RO product state | Actual Direct page; no mutation Retry. Any later LF-H post-Source hook re-evaluates RO-O from product state |
| AR-D3 — Hub AgentSupply | Agents succeeds with exact `mode: hub` | The landing self-satisfies Agents and establishes a Hub-landing frontier required by Source; no Route-commit frontier exists yet | Hub permits exact reconciliation but proves no Route equality | AS-2/LF-H installs Hub as reversible page origin; retain immutable draft/stage | Agents, Sources and exact AgentChain | One Source generation acquired after this landing + exact AgentChain | ET-17a acquires both after the landing in this workflow generation; AP-2 stays unconsumed | Originating failure, pending; no Retry while active |
| AR-D4 — Hub members pending | AR-D3 plus independently settling Source and exact-chain members | Each member settles only its own slot; no sibling result changes RO | No terminal choice until a Route edge or read failure is selected | Retain AS-2 and install/hold every successful member | Agents, Sources and exact AgentChain | Pending or failed Source/chain slots | Own both reads until AR-D5–AR-D9 or RL disown | Derive the destination only from the orthogonal product table below |
| AR-D5 — chain becomes Direct | Chain settles `direct_mode`; Source may be pending, successful or failed in its own epoch | No Route-commit frontier. Preserve RO outcome/owner and change only legality to Direct-suspended | Chain Direct takes the common Direct page-form edge but proves no mutation outcome | Hold illegality evidence and every successful Source/Agents member; if outcome is unknown retain its generation/pairs/stage. The error is no landing payload | Source + authoritative landing Agents; an unknown exact Route observation is suspended while Direct | Landing Agents read unless the decisive member itself was Agents | Preserve settled Source; disown only pending comparison work; ignore its late results; keep the complete RO product state | ET-18a family → page Partial or retained committed modal; Retry only landing reads, never the mutation. Any later LF-H post-Source hook re-evaluates RO-O |
| AR-D6 — exact chain match / commit frontier | Exact chain matches submitted pairs, regardless of Source settlement or response order | Immediately set RO outcome `committed` and establish the Route-commit frontier. The matching chain self-satisfies Route | Exact equality proves commit before any sibling result | Hold AgentChain as D-36 evidence; keep earlier successes installed as latest-known only | M6 Source, Agents and Route-chain projections | Acquire an Agents successor first; after its exact landing, acquire the Source successor required by CF-R and any CF-H | Supersede/disown a predecessor member only when its successor is acquired; ignore predecessor late results | ET-17c → M6; later Source success/failure cannot change `committed`; Retry only a frontier-satisfying failed subset |
| AR-D7 — exact chain nonmatch | Exact chain differs from submitted pairs, regardless of Source settlement or response order | Immediately set RO outcome `observed-nonmatch`; no commit frontier exists | Exact inequality settles only the Route outcome and never selects page form | Hold exact AgentChain as authoritative comparison; preserve independently held Source/Agents | Agents, Sources and exact chain | Only the Source slot may remain pending or failed | Settle/disown only the chain generation; a Source result cannot rewrite `observed-nonmatch` | Destination/action comes only from the product table below; never reread Agents/chain solely because Source failed |
| AR-D8 — exact chain read failure | Non-Direct exact-chain observation fails | Preserve the prior RO outcome; transport failure never changes it. No commit frontier exists | No new Route authority | Retain every successful Source/Agents member already installed/held | Agents, Sources and exact chain | Chain failed plus any still-pending/failed Source | Settle only chain failure; no success or RO axis is rolled back | After all pending members settle, ET-17b; Retry only the exact failed read subset, never an already-successful member |
| AR-D9 — Source member settles, mode-neutral | The post-Hub-landing Source generation returns success or failure before or after AR-D6/AR-D7/AR-D8 | Success or failure settles only Source at its acquisition epoch and changes no RO axis. If a later commit/Hub frontier applies, acquire a successor instead of treating this slot as current/stale | Source carries no page-form or Route-outcome evidence | Install a success monotonically; mark only a frontier-satisfying failure stale | The mode-selected set is unchanged | Source alone becomes current, failed or pending for a newer frontier | Chain settlement cannot undo Source; Source settlement cannot undo chain outcome | Re-evaluate the product table. A Source-only failure Retry never rereads Agents or exact chain |
| AR-M1 — applicable member settles, mode-neutral | Active M6 + any applicable Source, Agents or Route-chain member returns success or failure | Settle only that member. It closes its slot only if its acquisition epoch satisfies every required frontier; otherwise retain it as latest-known and acquire one successor | Only an exact successful Agents mode payload may select LF/AS page form. Source success/failure, Agents failure and non-Direct Route settlement carry no page-form evidence | Install/hold a success monotonically; mark only a frontier-satisfying failed member stale. Successful Agents invokes LF/AS; every other result updates only its member authority/staleness | Recompute from the already-installed decisive mode; under Direct, Source + Agents remain applicable and Route is not applicable; under Hub, Source + Agents + Route remain applicable | Every applicable member missing a frontier-satisfying observation plus the exact frontier-satisfying failed subset | Sibling failure cannot undo a success; ignore predecessor-epoch results | Keep the currently installed Direct/Hub page and mounted/transferred ownership unchanged. A late Direct-mode Source result settles only Source and never resurrects Hub; after all required frontiers settle AR-M2 or AR-M3 alone selects completion/Retry |
| AR-MD — decisive Direct | Active M6 + exact AgentSupply `mode: direct` **or** Route-chain `direct_mode` | The Route-commit frontier remains held; Route slots are provisionally not applicable until the landing Agents member settles | Both producers take this one reducer edge; the landing AgentSupply then recomputes applicability from its exact mode | Preserve every successful independent member. A current Direct AgentSupply self-satisfies Agents; chain Direct acquires that landing read through ET-18aM/ET-18aP | Landing Direct: Source + Agents, Route not applicable. Landing Hub: Source + Agents + a newly acquired Route chain | Landing Agents when chain supplied Direct; if it returns Hub, Sources also requires its new landing frontier and Route becomes applicable | Cancel/disown only prior pending Route work and ignore its late results; never discard a successful Source/Agents member | Mounted report remains mounted or receiving page enters Partial until AS dispatch; Retry only the current applicable failed subset |
| AR-M2 — all applicable frontiers satisfied | Active M6 + no pending/failed applicable member and every member has an observation satisfying all frontiers required by that member | Complete applicable surface settles reconciliation | The latest exact mode-selected set controls page form | Keep every exact applicable member and commit evidence | Latest mode-selected set | None; not-applicable Route members are neither current nor stale | Settle generation; ignore duplicate/disowned/predecessor results | Mounted evidence remains until ET-20 or transferred page remains; Retry absent |
| AR-M3 — applicable failed subset settles | Active M6 + no pending/unsatisfied applicable member + one or more applicable failures acquired after all frontiers required by their members | Commit frontier held; applicable generation settles partially current | Successful mode/authority remains decisive | Keep every successful applicable member installed and commit evidence held | Latest mode-selected set | Exact failed applicable subset, visibly stale | Settle; ignore later results from it | Mounted → ET-19a; transferred → ET-19c; Retry exact failed subset |
| AR-M4 — retry applicable failed subset | Settled AR-M3 + explicit admitted Retry | All causal frontiers remain held; successor epochs are pending | No prior success is reread or discarded | Retain installed members/evidence | Recompute from held decisive mode before issuing | Prior applicable failed subset becomes pending | CA first hands focus off, then disables Retry; predecessor/disowned results are ignored | Same mounted/receiving phase; Retry exactly the failed subset |

AR-D presentation and action eligibility are derived only after its independent reducers run:

| Route outcome / chain slot | Source slot | Derived destination / admitted work |
| --- | --- | --- |
| `unknown` or `rejected`; chain pending | Pending, successful or failed | Remain in AR-D4; an unresolved higher-precedence Route observation forbids mutation Retry |
| `unknown` or `rejected`; chain failed | No pending sibling | ET-17b retains every exact success and retries only the failed Source/chain subset; RO is unchanged |
| `observed-nonmatch` | Pending | Keep the exact comparison and wait only for Source; do not reread Agents/chain |
| `observed-nonmatch` | Successful after the current Hub-landing frontier | Run V5. Valid admits ET-17d's explicit mutation Retry; invalid enters ET-17e |
| `observed-nonmatch` | Failed after the current Hub-landing frontier | ET-17f keeps `observed-nonmatch` and exact chain authority, marks only Sources stale and admits only ET-17g's Source read Retry |
| `committed` | Any pre-frontier Source settlement | Enter ET-17c/M6 immediately; the commit frontier requires a successor Source generation, so the earlier result cannot complete or fail M6 |
...
| RT-8 — DM-2 unknown crosses ordinary M5 | Route PUT returns shaped `direct_mode`; LF-D installs Direct; the ordinary §1.9 mode PATCH later returns exact Hub | DM-2 creates `(unknown, page session, Direct-suspended)` before modal handoff. M5/LF-H closes Source, the common hook changes legality and RO-O sends exactly one chain GET: match enters M6; nonmatch settles page-owned abandonment; neither branch resends PUT |
| RT-9 — dismissed Hub unknown settles page-owned | Lose a Route PUT response in Hub and dismiss through ET-8b; exercise exact-chain match, nonmatch and read failure | ET-8b changes only owner. Match enters M6; nonmatch becomes page-session `observed-nonmatch` with no editor or mutation Retry; read failure remains page-session `unknown` and admits only one exact-chain read retry |
...
| RT-14 — nonmatch then Source failure | Exact chain first settles `observed-nonmatch`; the frontier-satisfying Source read then fails | Preserve the exact nonmatch and chain authority, mark only Sources stale, and admit only ET-17g's Source Retry. Agents/chain are neither reread nor reset to `unknown` |
| RT-15 — Source failure then nonmatch | The frontier-satisfying Source read fails first; exact chain then settles nonmatch | Reach the identical product as RT-14: `observed-nonmatch` + failed Source, with the same retained authority, feedback and Source-only Retry. Response order changes no reducer axis |
...
attached; it never reconstructs the opening row after AS-2 installed newer authority. In a
definitive rejection ET-8a likewise reveals only the latest authority the dialog holds.
ET-8b transfers or acquires exactly one page-owned RO-O read when Hub-observable; neither
path starts a sibling read or resends on dismissal.
...
Route mutation outcome, independently from LF/AS page-form landing. This table alone owns user
...
be targeted; an unregistered heading or text node may not. PF-1 resolves post-install targets
only after that gate:

| Active target after install/close | Installed page form | Exact model row after refreshed collapse/rendering | PF-1 output |
| --- | --- | --- | --- |
| Mounted, focusable and admitted | Any | Any | Preserve that exact active target; background settlement does not move focus |
| Fails FF-1 | Direct | Not rendered by definition | Admitted stable backend group head |
| Fails FF-1 | Hub | The exact `(backend, menu_model)` row is rendered and focusable | That model row |
| Fails FF-1 | Hub | The exact row is absent, collapsed or not focusable | Admitted stable backend group head |
...
| RO exact-chain observation | ET-16a acquires one mounted RO-O generation; after every LF-H Source success, the common hook acquires one page-owned RO-O generation exactly when the product is `(unknown, page session, Hub-observable)` | ET-8a disowns a `rejected` mounted generation. ET-8b changes only owner and, when already Hub-observable, transfers/acquires the page-owned successor. ET-18a/AS-1 change only legality to Direct-suspended and cancel the now-illegal request | Ignore results after explicit disown or successor acquisition. Direct preserves outcome/owner/pairs/stage; a later Hub post-Source hook may acquire exactly one legal successor. Page-owned nonmatch settles abandonment and cannot call editor ET-17d |
| AR-D Source + exact-chain reads | ET-17a attaches both to the current AR-D workflow generation after AR-D3's Hub landing, recording a distinct member epoch for each. ET-17g acquires a Source-only successor after AR-D9 failure; ET-17h acquires exactly the generic failed read subset | AR-D5 settles Direct evidence and disowns only work made illegal before ET-18a. AR-D6 immediately settles `committed` and establishes CF-R; AR-D7 immediately settles `observed-nonmatch`; AR-D8 settles only chain failure; AR-D9 independently settles Source. ET-8a/ET-8b disown the full mounted workflow as registered | A result after member disown/supersession cannot alter any settled axis. Route match/nonmatch is never rewritten by Source; Source is never rewritten by Route. An installed observation predating a later CF-R/CF-H remains latest-known but cannot satisfy that frontier |
| AR-M Source, Agents and Route-chain reads | ET-10/ET-11 establishes CF-R and acquires Agents first; ET-17c transfers CF-R into the same sequence. Every LF-H establishes CF-H before acquiring/accepting its Source successor. ET-19b/ET-19d acquires one failed-subset successor | ET-20 transfers rather than cancels. AR-MD from either Agents Direct or chain `direct_mode` preserves successful independent members, cancels/disowns prior pending Route slots and uses ET-18aM/ET-18aP when a fresh landing Agents read is required. Landing Direct keeps Route disowned; landing Hub establishes CF-H, then acquires one Source successor and makes Route applicable. A successor disowns its predecessor member generation | Ignore disowned and predecessor-epoch results. An applicable member settles AR-M only when its acquisition epoch satisfies all of its CF-R/CF-H requirements |
...
| ET-2 — opening chain success | Loading route + returned AgentChain | Validate the envelope, then install its exact pair array as origin and draft | Clear loading only | Ready | Normal frame copy | Keep `route.cancel`; payload arrival does not steal focus | None |
...
| ET-5a — open Add selector | Ready, Dirty or Invalid after refresh + current V5 authorities | Materialize current V5 candidates | Clear any prior selector selection | Same editor state + selector open | Selector keys; existing invalid lines remain | First exact candidate | None |
...
| ET-5d — move selector candidate focus | Open selector + current candidate | Send/read nothing; choose the next exact candidate | Replace only the selector's roving selection | Same open selector | Existing selector copy | Newly selected candidate | None |
| ET-6a — remove one row | Ready, Dirty or Invalid after refresh + exact row/draft | Remove locally, then re-run V5 | Drop selector/grab state owned by that row | Ready if equal to origin; Dirty if changed and valid; Invalid only while a marked row remains | Normal copy plus every remaining invalid line | First FF-1-valid target: successor grip; predecessor grip; when empty `route.addHop` only if V5 admits it; then `route.reorder`; finally enabled `route.cancel` | None |
...
| ET-8a — ordinary reversible exit | Loading route, Route unread, Ready, Dirty, Invalid after refresh or Route save rejected + latest installed authoritative page projection | Send no mutation; disown the current RL opening generation and the full AR-D generation before close | Close selector; restore/cancel any grab; discard local/submitted draft; ignore every late result from both disowned generation families | Reveal the latest installed page authority; before reconciliation this is the held opening projection, after AS-2 it is that newer Hub projection | Exact latest projection; retain its existing unresolved marker if any | Preserve an FF-1-valid active page target, otherwise PF-1 after the modal target unmounts | None; an already-unresolved row retains D-35 |
| ET-8b — unknown-outcome exit | RO is `(unknown, mounted editor, Hub-observable | Direct-suspended)` + immutable attempt evidence + latest installed page authority | Send no mutation; change only owner to page session. If Hub-observable, transfer/acquire exactly one page-owned RO-O exact-chain generation; if Direct-suspended, send no read | Discard modal-only transients; retain generation, submitted pairs/stage, unknown outcome and legality. Disown only a predecessor read when the page-owned successor is acquired | Latest installed Hub projection with its exact row marked Chain unresolved while RO-O reads, or actual Direct page with suspended evidence held off-row | Hub read failure uses the existing Chain-unresolved/D-35 read treatment; Hub nonmatch is silent abandonment with no mutation Retry; Direct renders no false Route verdict | Preserve an FF-1-valid page target, otherwise PF-1 after modal close | Hub → page-owned RO-O; Direct → RO-S until any LF-H post-Source hook re-enters RO-O |
| ET-9a — first Route PUT | V5-valid Dirty draft | Freeze and send the complete array without `force` or plan echoes | Close selector; settle any grab | Saving | `route.saving` | Programmatically focusable progress status | One owned Route PUT |
| ET-10 — R6 success with impact | Saving + exact R6 envelope with either array nonempty | Consume every R6 member, establish CF-R, mount its report and start AR-M with the Agents member first | Release PUT/refusal ownership; retain the entire envelope | Route impact reported with AR-M pending | Impact title/detail and each nonempty block | `route.impact.done` | Agents landing read; AR-M/LF dispatch then acquires Sources under Direct or after CF-H under Hub, plus any newly applicable Route member |
| ET-11 — two-empty R6 commit | Saving + exact R6 envelope whose two impact arrays are empty | Preserve the exact envelope, establish CF-R and start AR-M with the Agents member first | Release PUT ownership; no origin can be restored | Route committed, reconciling | `route.refreshing` | `route.impact.done` | Agents landing read; AR-M/LF dispatch then acquires Sources under Direct or after CF-H under Hub, plus any newly applicable Route member |
| ET-12 — guarded 409 | Saving + immutable draft + complete current refusal arrays | Validate guard code and both arrays; a later 409 atomically replaces both | Release PUT ownership only after the current plan is held | Route save refused | Shared Qp6FI copy and current arrays | `guard.cancel` | None until explicit user action |
...
| ET-13b — confirm guard and send | Route save refused + immutable draft/current arrays | Revalidate exact echoes, then send the same draft with `force: true` and both arrays | Drop the rendered refusal as the immutable values freeze into the request | Saving | `route.saving` | Programmatically focusable progress status | One owned Route PUT |
| ET-14 — definitive non-guard rejection | Saving + submitted draft; DM-2 is reserved for ET-18a | Exclude guard and `direct_mode`, then classify the shaped rejection | Release PUT ownership; retain draft/stage | Route save rejected | `route.fail.save` | `route.retry` | None until Retry |
| ET-15 — no response evidence | Saving + submitted draft + initial/confirmed stage | Set RO outcome `unknown`; infer neither commit nor refusal | Release PUT ownership; retain immutable attempt evidence | Route save outcome unknown | `route.fail.unconfirmed` | `route.retry` | None until Retry |
| ET-16a — begin failure reconciliation | Route save rejected or outcome unknown + held draft/stage + CA-D1 admission | Start one AR-D generation with `GET /api/models/agents` | Keep error body/draft; CA-D2 owns focus transfer plus Retry busy/inert admission | Same failure, reconciliation pending | Existing failure copy | CA-D2 handoff | One AR-D Agents read; no second generation is admitted |
...
| ET-17b — settled AR-D read failure | The orthogonal AR-D product has no pending member and at least one failed Source/chain slot, except the dedicated `observed-nonmatch` + Source failure row ET-17f | Preserve every settled RO axis and successful member; do not restart an already-authoritative Agents/chain read | Clear active generation/busy; retain draft and latest installed page projection | Originating failure | `route.fail.reconcileRead` | FF-1-valid `route.retry` | CA-D3 admits a successor for exactly the failed Source/chain subset; CA-D4 owns its pending phase |
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain equal to submitted pairs | Set RO outcome `committed`; hold AgentChain, mark response-only tails unavailable, establish CF-R and transfer the workflow to AR-M immediately without waiting for Source | Clear comparison busy; discard obsolete mutation Retry/refusal state; retain earlier successes as latest-known but frontier-insufficient | Route committed, reconciling | `route.refreshing` | FF-1-valid `route.impact.done` | Acquire Agents first; LF-H then establishes CF-H and acquires Sources. Keep the matching chain self-satisfied |
| ET-17d — observed nonmatch + current Source, valid | AR-D7 holds `observed-nonmatch`; AR-D9 holds Source success satisfying current CF-H; refreshed V5 remains valid | Freeze the held initial/confirmed stage and send its registered PUT body only on explicit user Retry | Settle AR-D; retain the latest installed page authority and exact confirmed arrays when required | Saving | `route.saving` | CA-D7's synchronous handoff to the programmatically focusable progress status | One owned Route PUT; no reconciliation generation remains active |
| ET-17e — observed nonmatch + current Source, invalid | AR-D7 holds `observed-nonmatch`; AR-D9 holds Source success satisfying current CF-H; refreshed V5 invalidates a new/changed pair | Install refreshed Source/chain authority; send no PUT | Settle AR-D; clear submitted/refusal state; retain editable rows and AS-2's latest Hub page | Invalid after refresh | `route.invalidAfterRefresh` on every offender | First offending row's FF-1-valid Remove action | None |
| ET-17f — observed nonmatch + Source failure | AR-D7 holds `observed-nonmatch` and AR-D9 settles a Source failure satisfying current CF-H, in either response order | Preserve the exact chain comparison and outcome; classify only Sources stale | Clear active generation/busy; retain draft, AgentSupply and exact chain authority | Originating failure with Source stale | `route.fail.reconcileRead` | FF-1-valid `route.retry` | CA-D5 admits only ET-17g; Agents/chain and the mutation are not eligible |
| ET-17g — Source-only reconciliation Retry | ET-17f + CA-D5 admission | Reissue only `GET /api/models/sources` under the same latest CF-H; preserve `observed-nonmatch` and exact chain | Retain all settled authority; CA-D6 owns focus handoff plus Retry busy/inert admission | Same failure, Source reconciliation pending | Existing failure copy | CA-D6 handoff | One Source successor; success re-evaluates V5 for ET-17d/ET-17e, failure returns ET-17f |
...
| ET-18a — reversible decisive `direct_mode` handoff | Loading, Saving or AR-D5 + matching DM-1/DM-2/DM-3 evidence and no CF-R commit frontier | Error evidence selects no page form; retain every successful independent member and start/transfer the page's Agents read. DM-2 first sets RO outcome `unknown`; every handoff changes owner to page session and legality to Direct-suspended without changing any already-known outcome | Close modal/selector and cancel grab. Discard editable presentation; for unknown retain generation/submitted pairs/stage outside the modal. Disown only pending illegal Route siblings | Page §1.0 Partial over every latest successful Source/Agents projection | Existing Partial copy; never label Direct from the error or claim a Route outcome | Stable owning backend group head | Page Agents read; result enters LF-D/LF-H. LF-H's post-Source hook re-evaluates RO-O; every late disowned sibling result is ignored |
...
| ET-19a — mounted AR-M3 failed subset | Mounted report/committed modal + AR-M3 settled generation | Failure cannot negate commit or any successful installed member | Clear generation pending; retain modal/evidence and mark only the applicable failed member set stale | Same impact report, or report-free Committed projection stale | `route.impact.refreshFail` plus prior report/refresh copy | FF-1-valid `route.retry` | None until CA-M2 admits ET-19b |
| ET-19b — mounted failed-subset Retry | ET-19a + exact applicable failed subset + CA-M2 admission | Start AR-M4 for exactly that subset | Keep modal/evidence/successful members; CA-M1 owns focus transfer plus Retry busy/inert admission | Same report or report-free state, retry pending | Prior report/refresh copy | CA-M1 handoff | One AR-M successor generation for the applicable failed subset |
| ET-19c — transferred AR-M3 failed subset | ET-20 receiving page + AR-M3 settled generation | Failure cannot negate commit or any successful installed member | Clear generation pending; retain evidence and mark only applicable failed members stale | Receiving page with partial current authority | `route.impact.refreshFail` | Preserve current page focus only if FF-1-valid; otherwise use that page's registered fallback | None until CA-M4 admits ET-19d |
| ET-19d — page-owned failed-subset Retry | ET-19c + exact applicable failed subset + CA-M4 admission | Start AR-M4 for exactly that subset | Keep evidence/current members; CA-M3 owns focus transfer plus Retry busy/inert admission | Same receiving page, retry pending | Existing stale line | CA-M3 handoff | One AR-M successor generation for the applicable failed subset |
| ET-20 — committed Done-equivalent exit | Mounted report, reconciling, refreshed or stale phase + exact commit evidence | Done, title-close, Escape and outside send no mutation/read | Close modal and transfer any active AR-M generation/evidence; never restore origin | Receiving page with exact installed successes plus pending/stale markers for remaining applicable members | Held evidence plus the registered pending/stale marker as applicable | PF-1 after modal Done unmounts, over the exact installed page | Transferred AR-M if active; otherwise none, or later CA-M4/ET-19d after failure |
| AS-1 — AR-D2 Direct install | AR-D2 + exact AgentSupply `mode: direct` | LF-D installs that exact Direct row before any Source/chain read | Settle the Agents member and close modal. Preserve RO outcome, change owner to page session and legality to Direct-suspended; retain generation/pairs/stage exactly when outcome is unknown | Actual Direct page; an unknown outcome remains page-session-owned and suspended | Exact Direct projection; mode makes no Route claim | PF-1 because the modal target unmounts | No read while Direct; any later LF-H post-Source hook re-evaluates RO-O from axes, not producer |
...
| AS-3 — reversible error-handoff Direct dispatch | ET-18a/ET-18c + exact AgentSupply `mode: direct` | LF-D installs that exact Direct row and keeps every successful independent Source authority already carried | Clear page-read pending and stale Hub row; retain RO outcome/owner and set legality Direct-suspended | Actual Direct page | Exact Direct projection | Preserve an FF-1-valid page target or use PF-1 | No companion read; an M6 landing read dispatches through AS-5a/AS-5b instead |
| AS-4 — reversible error-handoff Hub dispatch | ET-18a/ET-18c + exact AgentSupply `mode: hub` | LF-H installs that exact Hub row, keeps successful independent authority and starts M5's Source read; never resurrect the editor | Clear Agents pending; retain the old Source list explicitly stale until M5 settles and retain all RO evidence | Actual Hub page with Sources pending/stale until LF-H closes | Exact Hub projection plus `upstream.unread` only after Source failure | Preserve an FF-1-valid page target or use PF-1 | M5 Source success invokes the common LF-H post-Source hook; M6 landing uses AS-6a/AS-6b instead |
| AS-5a — mounted AR-M Direct install | Mounted evidence + current successful M6 AgentSupply `mode: direct`, whether it supplied AR-MD or answered ET-18aM | Dispatch mode immediately, install the exact current Direct page behind the modal, and make all Route-chain members not applicable | Retain modal/evidence and applicable Source/Agents state; cancel/disown pending Route reads and remove their failed/pending slots from stale/Retry | Same mounted report/committed phase over actual Direct page | Registered report/refresh/stale block only for applicable members | Keep FF-1-valid `route.impact.done` | AR-M continues applicable pending members, or CA admits only an applicable failed subset after settlement |
| AS-5b — transferred AR-M Direct install | Transferred AR-M + current successful M6 AgentSupply `mode: direct`, whether it supplied AR-MD or answered ET-18aP | Dispatch mode immediately, install the exact current Direct page, and make all Route-chain members not applicable | Retain evidence and applicable Source/Agents state; cancel/disown pending Route reads and remove their failed/pending slots from stale/Retry | Actual Direct page with exact applicable pending/stale markers | Exact Direct projection plus registered applicable pending/stale block | Preserve the current active page target only if FF-1-valid; otherwise PF-1 | AR-M continues applicable pending members, or CA admits only an applicable failed subset after settlement |
| AS-6a — mounted AR-M Hub install | Mounted evidence + successful M6 AgentSupply `mode: hub`, including the landing read after chain Direct | Dispatch mode immediately, install the exact Hub page behind the modal and establish CF-H for Sources; the payload self-satisfies Agents | Retain modal/evidence and independent authority. Acquire one Source successor unless its generation was acquired after this exact landing. If this is AR-MD's landing read, recompute Route as applicable and acquire one new post-landing chain epoch | Same mounted report/committed phase over actual Hub page | Registered report/refresh/stale block for the AR-M phase | Keep FF-1-valid `route.impact.done` | AR-M continues every member missing its required frontier, including Sources and any newly applicable chain, or CA admits a failed subset after settlement |
| AS-6b — transferred AR-M Hub install | Transferred AR-M + successful M6 AgentSupply `mode: hub`, including the landing read after chain Direct | Dispatch mode immediately, install the exact Hub page and establish CF-H for Sources; the payload self-satisfies Agents | Retain evidence and independent authority. Acquire one Source successor unless its generation was acquired after this exact landing. If this is AR-MD's landing read, recompute Route as applicable and acquire one new post-landing chain epoch | Actual Hub page with exact pending/stale markers | Exact Hub projection plus registered pending/stale block | Preserve the current active page target only if FF-1-valid; otherwise PF-1 | AR-M continues every member missing its required frontier, including Sources and any newly applicable chain, or CA admits a failed subset after settlement |
...
| --- | --- | --- | --- | --- | --- |
| CA-D1 | Save-failure Retry | No AR-D generation active | Rendered, enabled | None while admitted | Emit ET-16a and atomically acquire the sole generation |
| CA-D2 | Save-failure Retry | ET-16a / AS-2 / ET-17a generation active | Rendered busy, disabled/inert | If Retry is active, move first to enabled Cancel, otherwise enabled title-close, before changing admission | None; cannot overlap AR-D or reach a second PUT |
| CA-D3 | Failed-read subset Retry | ET-17b holds an exact failed Source/chain subset and no member is pending | Rendered, enabled | None while admitted | Emit ET-17h for exactly the failed read subset; never reread a settled success or resend the mutation |
| CA-D4 | Failed-read subset Retry | CA-D3 successor generation pending | Rendered busy, disabled/inert | If Retry is active, move first to enabled Cancel, otherwise enabled title-close, before changing admission | None; cannot overlap the successor generation |
| CA-D5 | Source-only reconciliation Retry | ET-17f holds `observed-nonmatch` + failed Source and no member is pending | Rendered, enabled | None while admitted | Emit ET-17g for Sources only |
| CA-D6 | Source-only reconciliation Retry | ET-17g Source successor pending | Rendered busy, disabled/inert | If Retry is active, move first to enabled Cancel, otherwise enabled title-close, before changing admission | None; cannot reread Agents/chain or overlap a Source generation |
| CA-D7 | Explicit mutation Retry after comparison | `observed-nonmatch` + frontier-satisfying Source success + V5-valid draft, with no read pending | Rendered, enabled | On activation mount and focus the programmatically focusable `route.saving` status before Retry becomes inert | Emit ET-17d; this is the only post-comparison row that resends PUT |
| CA-M1 | Mounted M6 Retry | Initial AR-M pending, AR-M4 retry pending, or AR-M2 success | Absent; if layout reserves it, disabled/inert | If Retry is active, move first to enabled `route.impact.done`, otherwise its enabled title-close, before changing admission | None |
| CA-M2 | Mounted M6 Retry | Settled ET-19a / AR-M3 applicable failed subset | Rendered, enabled | None while admitted | Emit ET-19b for exactly the held applicable failed subset |
| CA-M3 | Page M6 Retry | Initial/transferred AR-M pending, AR-M4 retry pending, or AR-M2 success | Absent; if layout reserves it, disabled/inert | If Retry is active, move first to the receiving page's registered FF-1-valid fallback before changing admission | None |
| CA-M4 | Page M6 Retry | Settled ET-19c / AR-M3 applicable failed subset | Rendered, enabled | None while admitted | Emit ET-19d for exactly the held applicable failed subset |
...
| Ready / Dirty / Invalid after refresh + Add selector | Source/model candidate region | `route.add.source`, `route.add.model` | Labels render while open; candidate focus exists while a valid pair remains | Roving candidate keys emit ET-5d; Escape emits ET-5b; Tab reaches confirmation |
...
| Route save rejected / outcome unknown | 重试 | `route.retry` | CA-D1/D2 own initial reconciliation; CA-D3/D4 own exact failed-read-subset Retry; CA-D5/D6 own `observed-nonmatch` + Source-failure Source-only Retry; CA-D7 alone admits the V5-valid explicit mutation Retry | First command in Tab order only when enabled; activation delegates to the current CA row |
| Route save rejected | 取消 button | `route.cancel` | Enabled | Normal Tab order; activation emits ET-8a |
| Route save rejected | Title-bar close | `route.cancel` | Enabled | Independent Tab stop; activation emits ET-8a |
| Route save outcome unknown | 取消 button | `route.cancel` | Enabled | Normal Tab order; activation emits ET-8b |
| Route save outcome unknown | Title-bar close | `route.cancel` | Enabled | Independent Tab stop; activation emits ET-8b |
...
| Route impact reported / report-free stale | 重试 | `route.retry` | CA-M2 renders/enables it only after ET-19a settled with an applicable failed subset; CA-M1 makes it absent/inert during initial/retry pending and after success | Normal Tab order only when enabled; activation delegates to the current CA row |
| Receiving page with transferred stale subset | 重试 | `route.retry` | CA-M4 renders/enables it only after ET-19c settled with an applicable failed subset; CA-M3 makes it absent/inert during initial/retry pending and after success | Page Tab order only when enabled; activation delegates to the current CA row |

**Dismissal is closed by phase** `[derived]`. The four dismissal affordances are distinct
inputs but share an ET edge only when their cleanup, destination, focus and next owner are
identical. An entry absent from this matrix is forbidden; selector/grab Escape takes its
more-specific ET-5b/ET-7e edge before this phase matrix.

| Modal phase | Cancel button | Title close | Escape | Outside press |
| --- | --- | --- | --- | --- |
| Loading / unread / Ready / Dirty / Invalid / definitive rejection | ET-8a | ET-8a | ET-8a | ET-8a |
| Route save outcome unknown | ET-8b | ET-8b | ET-8b | ET-8b |
| Route save refused | ET-13a | ET-13a | ET-13a | ET-13a |
| Saving | Inert / absent | Inert / absent | Inert | Inert |
| Committed / report / mounted stale | ET-20 Done-equivalent | ET-20 | ET-20 | ET-20 |
...
| Candidate Arrow / `Home` / `End` | ET-5d moves selector focus only |
...
unlike persisted `Source.adopted_by`. **No frame in this document
...
order, and ⑤'s whole observation would re-hit the same endpoint with the same key — in
⑤ the new information is elapsed time, which the user supplies by waiting rather than
by editing. A build that pre-selects a segment, or that enables ④'s 重试 before a pick,
or that promotes ⑤'s whole-observation retry to full mint, is not deviating cosmetically; it has
...
reason it takes a string rather than 「拉到 0 个型号」. It is also where ④′ lands when
  the hint identifies the interface, and where ⑤′'s 重试 lands when the repeated observation
...
| `fail.auth` | 鉴权失败 | Authentication failed |
...
| `undetermined.detail` | 返回结构对不上任何一种已知接口 | The response shape matches no interface we know |
...
moment a hop it named entered cooldown. `standby` and unadopted `active`
...
For a page-owned RO `unknown` whose exact-chain observation failed, this same re-expansion
is the only read retry: it reacquires one RO-O generation when Hub-observable, retains the
submitted pairs/stage and sends no mutation. A nonmatching answer settles page-owned
abandonment and removes that retry; Direct-suspended keeps the evidence but sends nothing.
...
and the exact submitted ordered pair array. Only the exact chain `GET` is presence-total for
the Route under comparison: its ordered `(source_id, model_id)` pairs equal to that array
prove the desired Route committed, including empty-to-empty. `AgentSupply.routes` is a real
...
comparison. Mode and Route outcome are orthogonal: Direct mode makes that exact read
temporarily illegal but proves neither commit nor non-commit. RO therefore retains its
`outcome × owner × legality` product plus generation/submitted pairs/stage across Direct/Hub
...
read enters M6; a nonmatch after dismissal settles abandonment without resend; a failed read
retains evidence and permits only a chain-read retry. Reload discards this client-held
evidence and never permits a blind resend or creates a persistent reconciliation ledger. In
the mounted editor, exact nonmatch settles `observed-nonmatch` independently: a sibling
Source failure preserves it and admits only a Source read retry before V5, never a repeated
Agents/chain read. A differing chain permits one resend only on the user's Retry after the
frontier-satisfying Source comparison succeeds; a failed exact-chain read permits none. Either a received
