...
| §1.2 | Route save outcome unknown `[derived]` | The route write returned no HTTP/API answer; the exact submitted draft and whether this was the non-forced or confirmed stage remain held | F1; D-36 permits no blind resend | `route.fail.unconfirmed`, `route.fail.reconcileRead` after AR-D1/AR-D8, `route.retry`, `route.cancel` | ET-15, AR-D/ET-16, AS-1/AS-2, ET-17, ET-18a and ET-8b own entry, D-36 reconciliation, every outcome and the Chain-unresolved exit. CA admits Retry only without an active generation |
...
machine; the ET-18a family and the AS dispatcher alone own cleanup, destination, focus and
any next read:
...
| DM-2 | Route `PUT` returns `direct_mode` | No Route commit evidence exists and the editor is now illegal | None — the error may not label the row or page Direct | ET-18a starts/transfers the same Agents read |
| DM-3 | D-36 or another post-failure chain read returns `direct_mode` | The held attempt has no useful Route landing projection | None — no second chain or Source read can manufacture one | ET-18a starts/transfers the same Agents read |
| DM-4 | ET-16a's first reconciliation read returns the exact backend `AgentSupply` with `mode: direct` | Direct is the authoritative current backend mode, so no Route read or V5 validation is legal | The returned AgentSupply row itself; it is installed exactly and no second Agents read is sent | AS-1 consumes this payload directly |
...
explicitly stale, and Retry repeats only `GET /api/models/agents`; the held Source list is
unchanged. A successful read installs the exact returned `AgentSupply` row, whose actual
`mode` alone selects Direct or Hub through AS-3/AS-4 — including a concurrent switch back to Hub. DM-4 skips
that redundant read because it already carries the exact landing payload. No branch
resurrects the Route draft, repeats the mutation or derives a page form from an error.
...
**Lost-response and M6 reads use one attempt-result algebra** `[contract]` `[derived]`
D-36. The reducer key is `(workflow generation, member, observation epoch, settlement,
decisive evidence)`. Each independently acquired member records pending, successful
...
chain `direct_mode` takes the same Direct reducer edge before any sibling failure: retain
every successful independent Source/Agents member, cancel/disown only pending Route work,
and acquire one authoritative Agents landing read. That read's exact returned mode alone
selects Direct or Hub; the decisive error itself is no landing payload. In Hub,
...
| ID / workflow generation | Independently settled member + observation epoch | Commit barrier / settlement | Decisive evidence precedence | Immediately installed / held authority | Applicable set after decisive evidence | Awaited / stale members | Cancellation / late result | Destination / Retry subset |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| AR-D2 — Direct AgentSupply | Agents succeeds in the pre-barrier epoch with exact backend `mode: direct` | No Route commit barrier is needed; the current Agents member settles | Mode is decisive before all Route/failure classes and is already the authoritative landing read | AS-1 installs that AgentSupply | Agents only; Source/chain comparison is not applicable | None | Settle generation; issue no Source/chain read; ignore obsolete results | Actual Direct page; no Retry |
...
| AR-D5 — chain becomes Direct | Chain settles `direct_mode`; Source may be pending, successful or failed in its own epoch | No Route commit barrier; Direct settles Route as not applicable | Chain Direct takes the common Direct edge | Hold illegality evidence and every successful Source/Agents member; the error is no landing payload | Source + authoritative landing Agents; Route is not applicable | Landing Agents read unless the decisive member itself was Agents | Preserve successful Source; disown only pending comparison work; ignore its late results | ET-18a family → page Partial or retained committed modal; Retry only the landing Agents read after failure |
...
composite-read generation and selects its cited ET/AS handoff; this table alone owns user
gestures, single producer answers and the atomic cleanup/destination/focus/read effects of
those handoffs. AS rows install only the exact projection AR selected. V5/DM only classify
evidence, and CA/control/keyboard registers only admit and emit events. Every transition MUST
...
The AS rows are the one AgentSupply page-form dispatcher. Every complete-surface read that
can change the page shape enters one of them before installing a destination: Direct always
installs the exact Direct payload, while only Hub may install a Hub form. No ET row may
...
| AR-D Agents read | ET-16a acquires one AR-D generation | ET-8a/ET-8b disown the full AR-D generation; ET-18a settles it on Direct evidence | Ignore every result after those edges or a later ET-16a successor |
...
| Error-handoff Agents read | ET-18a acquires the page-owned generation; ET-18c settles the failed generation and acquires a successor | Page replacement or a successor disowns the prior generation | Ignore every prior-generation result; a current success alone enters AS-3/AS-4 |
...
| ET-8b — unknown-outcome exit | Route save outcome unknown + immutable attempt evidence + latest installed page authority | Send no mutation/read; disown the full dialog-owned AR-D generation | Discard modal transients, ignore every late result from that generation, and transfer the uncertain attempt marker | Latest installed page projection with its exact row marked Chain unresolved | `route.fail.unconfirmed` hands off to that row's unresolved treatment | Preserve an FF-1-valid page target, otherwise PF-1 after modal close | D-35 page owner |
...
| ET-18a — reversible decisive `direct_mode` handoff | Loading, Saving or AR-D5 + matching DM-1/DM-2/DM-3 evidence and no commit barrier | Error evidence selects no page form; retain every successful independent member and start/transfer the page's Agents read | Close modal/selector, cancel grab, discard every draft/submitted/refusal value; disown only pending AR-D/Route siblings | Page §1.0 Partial over every latest successful Source/Agents projection | Existing Partial copy; never label Direct from the error | Stable owning backend group head | Page Agents read; every late disowned sibling result is ignored |
...
| ET-18b — reversible error-handoff Agents read failure | ET-18a Partial + last-good page projection | Classify only the read failure | Clear pending; keep last-good projection explicitly stale; CA-P1 admits page Retry | Page §1.0 Partial/F2 | Existing F2 cause and page Retry | FF-1-valid page Retry | None until ET-18c; M6 landing-read failure instead remains in AR-M and settles through ET-19a/ET-19c |
| ET-18c — retry error-handoff Agents read | ET-18b + stale page projection + CA-P1 admission | Reissue only `GET /api/models/agents` | Retain stale projection; CA-P2 owns focus transfer plus Retry busy/inert admission | Page §1.0 Partial pending | Existing F2 copy | CA-P2 handoff | Page Agents read; success enters AS-3/AS-4 |
...
| AS-1 — AR-D2 Direct install | AR-D2 + exact AgentSupply `mode: direct` | Install that exact Direct row before any Source/chain read | Settle AR-D; close modal and discard Route transients | Actual Direct page | Exact Direct projection | PF-1 because the modal target unmounts | None |
...
| AS-3 — reversible error-handoff Direct dispatch | ET-18a/ET-18c + exact AgentSupply `mode: direct` | Install that exact Direct row; keep any successful independent Source authority already carried | Clear page-read pending and stale Hub row | Actual Direct page | Exact Direct projection | Preserve an FF-1-valid page target or use PF-1 | None; an M6 landing read dispatches through AS-5a/AS-5b instead |
| AS-4 — reversible error-handoff Hub dispatch | ET-18a/ET-18c + exact AgentSupply `mode: hub` | Install that exact Hub row; keep successful independent authority and never resurrect the editor | Clear page-read pending/stale marker | Actual Hub page | Exact Hub projection | Preserve an FF-1-valid page target or use PF-1 | None; an M6 landing read dispatches through AS-6a/AS-6b instead |
...
**CA is the sole async control-admission matrix** `[derived]`. It covers controls whose
...
reconciliation or M6 generation for the workflow. A covered control emits its ET event only
in an admitted row; while disabled/inert, pointer, Enter and Space all send nothing. Before
...
| Route save rejected / outcome unknown | 重试 | `route.retry` | CA-D1 enabled only with no AR-D generation; CA-D2 renders it busy and disabled/inert throughout ET-16a/AS-2/ET-17a ownership | First command in Tab order only when enabled; only CA-D1 activation emits ET-16a |
...
| Route impact reported / report-free stale | 重试 | `route.retry` | CA-M2 renders/enables it only after ET-19a settled with an applicable failed subset; CA-M1 makes it absent/inert during initial/retry pending and after success | Normal Tab order only when enabled; activation emits ET-19b and retains the modal |
| Receiving page with transferred stale subset | 重试 | `route.retry` | CA-M4 renders/enables it only after ET-19c settled with an applicable failed subset; CA-M3 makes it absent/inert during initial/retry pending and after success | Page Tab order only when enabled; activation emits ET-19d |
...
| `Enter` / `Space` on 重试 | Only an enabled CA row emits: Route unread → ET-4; CA-D1 → ET-16a; CA-M2 → ET-19b; CA-M4 → ET-19d. Busy/inert retry controls emit nothing |
...
comparison. A differing chain permits one resend only on the user's Retry after the
