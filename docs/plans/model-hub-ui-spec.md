...
previews that plan through existing Restore/Undo/Save/guard, never a saveable empty Manual
state. Undo restores the previous unsaved draft; the saved nonempty override stays untouched
until confirmed successful Save, which closes the editor and reports `common.saved` through the
shared toast. Backend changes
...
| C6 — committed-report exit | Once a mutation has committed, every surface that mounts a report makes every dismissal path that report's Done-equivalent exit and may neither restore a pre-write origin nor discard held response or D-36 commit evidence. A page-owned committed state may intentionally mount no Done-equivalent; it retains the evidence in the receiving page and exposes only the registered read-only Retry for an acquired failed subset. A failed projection member adds Retry only for that subset and MUST NOT remove a mounted Done-equivalent where the surface has one | [x] A successful route save closes the editor immediately after R6 or exact D-36 commit evidence is installed and emits the shared `common.saved` toast. M6 retains the exact response members page-side, installs successful members monotonically, marks only acquired failures stale and exposes Retry through the receiving page; no route impact report or Done-equivalent remains mounted | [x] N/A — a committed save or removal mounts no report: frame 11's guard already put the exact plan to the user and took the destructive press for it, so the commit announces itself in one toast and holds nothing open. Received and inferred evidence still survives M1/M2 read failure through Committed projection stale | [x] Repair impact retains R3/R4's distinct envelopes through M3/M4 read failure, and Repair unresolved retains R3 through M3; Retry is additive and no path restores the invoking origin | [x] N/A — the menu commits nothing and owns no response report |
...
| --- | --- | --- | --- | --- |
| R1 | §1.10 source metadata `PATCH` | `model-hub.md` Source-mutation matrix, `mutation.source_metadata` | `source`, `removed_hops`, `interrupted` | `source` is held as the updated Source projection. `removed_hops` and `interrupted` are read scope, not a block to render: each names the Route chains M1 must reconcile, and the guard already put that same plan to the user before the write. M1 owns every invalidated projection behind the commit's toast. |
| R2 | §1.10 source `DELETE` | `model-hub.md` Source-mutation matrix, `mutation.source_delete` | `removed_hops`, `interrupted` | Each array names the Route chains M2 must reconcile; cardinality selects no state, because frame 11's refusal already put that plan to the user and took the forced press for it. The absent `source` is intentional because the Source was deleted. M2 owns the complete post-delete projection read on every path. |
...
| R6 | §1.2 Route save or Restore | `model-hub.md` Source-mutation matrix, `mutation.route_replace` / `mutation.route_restore` | `chain`, `removed_hops`, `interrupted` | Hold `chain` as the authoritative post-commit AgentChain and write evidence. Retain each non-empty `removed_hops` and `interrupted` array in page-owned M6; the arrays are reconciliation evidence and are not mounted in a post-save route dialog. If both arrays are empty, retain both exact empty values. M6 owns every Route-derived projection before the editor closes. |
...
| --- | --- | --- | --- | --- |
| M0 | Exact Source absence from `GET /api/models/sources` or a source-bound `source_not_found`; M2 supplies the same evidence after deletion | `Source[]`, every backend's Source order in `AgentSupply[]`, and every Route chain | Remove the exact Source locally, drop its overlay, retain the absence evidence and read the complete model surface before declaring dependent projections current. Every §1.6/§1.10/§1.11 caller enters this owner; M2's post-delete read is this same M0 reconciliation and may hand in its already-settled result rather than duplicate it | Enter Committed projection stale with the Source absence and last good dependent projections explicitly stale; the page's `toast.refreshFailed` names that read. Retry repeats only M0, while DP-4's committed exits remain legal. None resends a mutation, restores the Source, invents impact or questions the absence |
| M1 | R1 source metadata / Base URL, or D-36 reread matching every requested normalized field | `Source[]`; a Base URL inventory change may also change `AgentSupply[]` runnability and Route chains through the guarded cascade | Behind the commit's own announcement — for a received success of either impact cardinality **and for an inferred commit** — read the complete model surface. No user step stands between the write and this read. Keep R1's exact envelope when received; for inference hold the reread Source and mark `removed_hops` / `interrupted` unavailable | Enter Committed projection stale with the exact evidence held: only the failed members are marked stale and Retryable, and `toast.refreshFailed` names that read. Nothing reports the save itself as unconfirmed |
| M2 | R2 source deletion, or D-36 reread proving the exact Source absent | `Source[]`, every backend's Source order in `AgentSupply[]`, and every Route chain | Immediately on every committed path — received arrays of either cardinality or an inferred commit — remove the exact Source locally and enter M0 behind the removal's own announcement. This is M0's deletion-attempt complete-surface read, not a second owner. Keep received arrays when available and otherwise mark both response-only arrays unavailable | Enter Committed projection stale through M0 with the exact evidence held: only the failed members are marked stale and Retryable, and `toast.refreshFailed` names that read. Nothing reports the deletion as unconfirmed |
...
failed subset. Compose that subset into the receiving Sources/Gateway cards'
stale-data and Retry presentation independently of the latest region-read kind,
without replacing the actual authoritative read or hiding newly observed data.
Page-form selection uses that same composed view so a direct/empty landing
...
| DP-4 — committed evidence | Repair impact reported; Repair unresolved and their Committed projection stale states. Page-owned Route M6 and committed Source save/removal are deliberately excluded: they retain evidence in the receiving page and have no modal report or Done-equivalent | A report/result runs its registered Done exit. A report-free pending read exposes the same Done command; Committed projection stale keeps read-only Retry beside it | Not rendered | Close, Escape and outside press are always the Done-equivalent committed exit, including while a report-free read is pending or the projection is stale | Never restore the pre-write origin. Preserve the exact success envelope, D-36 chain/Source/absence evidence or authoritative direct absence; a pending exit transfers the owning read to the receiving page, while a stale exit carries the evidence there, marks unread dependent projections stale and leaves its refresh-failure line visible. Retry repeats only the registered M read. No exit questions or repeats the mutation, invents an unavailable member, or treats a read failure as failed commit |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or AR-D6 proves the desired Route from the exact chain endpoint while response-only arrays are unavailable; exact commit evidence is held by page-owned M6 while the active AR-M generation has an acquired member pending, a ready member awaiting acquisition, or an acquired member whose observation has not satisfied every required causal frontier. Mode-dependent deferred members are not pending obligations. The editor closes immediately after the evidence is accepted | F2 applies only to acquired members that AR-M3 marks stale; an Agents-first failure makes Agents the exact stale/Retry subset while Source/Route remain deferred. The Route write/desired state is already proven | `common.saved` toast, then the existing page stale/Retry treatment | ET-11 or ET-17c owns entry; AR-M/AR-MD, ET-18aM/ET-18aP, ET-19, AS and ET-20 own member eligibility/epochs/frontiers, Direct handoff, Retry and page installation. Background installs preserve the current FF-1-valid focus; PF-1 runs only when that focus disappears or becomes ineligible, and the pending landing target survives failed subsets. No modal exit remains |
| §1.2 | Route committed, refreshed `[derived]` | AR-M2 completed a page-owned M6 generation with every activated applicable member satisfying all causal frontiers and no member deferred behind an unresolved prerequisite; it installed the exact page projection and clears the transient route reconciliation status | F5 — no request remains and no impact report is rendered | Existing page projection; the success toast has already announced the commit | ET-20 records settlement and clears the pending landing target after the final focus decision; there is no mounted route dialog or Done action |
| §1.2 | Route save refused `[derived]` `[contract]` | A non-forced save returned the current exact refusal plan: nonempty manual PUT guards interruption; Restore/last-hop removal additionally guards effective hop removal. A forced save never lands here — RS-6 resends it | F3 — shared `Qp6FI`, an independent modal over the unchanged draft; G-23 remains the drawing gap for the detailed SupplyGap block | `guard.title.saveRoute`, `guard.subtitle.saveRoute`, `guard.confirm.saveRoute`, `guard.label`, `guard.count`, `guard.hop.position`, `guard.hint.interrupt`, `guard.gap.label`, `guard.gap.subject`, `guard.gap.agents`, `gateway.modelCount`, `guard.cancel` | ET-12, ET-13a and ET-13b separately own the refusal, cancellation and confirmation |
...
| §1.2 | Route commit evidence with impact `[derived]` `[contract]` | R6 holds a successful route-replace or route-restore response with non-empty `removed_hops` and/or `interrupted`; the complete returned `chain` remains held by page-owned M6 while AR-M independently settles every activated member after all of that member's required causal frontiers. The impact arrays are retained as reconciliation evidence, not mounted as a second dialog after save | F2 only for the exact AR-M3 acquired failed subset; deferred companions are neither stale nor Retryable. The write already succeeded and the page owns every retry | `common.saved` toast followed by the existing page stale/Retry treatment; route impact blocks and `route.impact.done` are not rendered | ET-10, AR-M/AR-MD, ET-18aM, ET-19, AS and ET-20 own evidence retention, acquisition eligibility/member settlement/Direct handoff/Retry/install and focus preservation/fallback. The editor closes at commit and has no sole modal exit |
...
| §1.4 | Default | A frame 13 vendor row has supplied the vendor | F5 | `addSub.title` … `addSub.hint.chatgpt` | The recommended option is pre-selected; selecting the other replaces it. 去登录 sends `POST /api/models/oauth/start` and opens no browser context (PD-1); RR-1/RR-2 classify any accepted flow **before** presentation: non-terminal enters PD-2, then `presentation.expects: none` → Awaiting sign-in, a paste presentation in E3a `starting` / `awaiting_action` → Awaiting paste-back, and a paste presentation already in E3b `verifying` → Awaiting paste-back completion; each renders the link and copy control when `presentation.auth_url` is non-null now or on a later flow read. Terminal → immediate status read with no form reopened. Refused because that backend already holds its one `native_cli` Source `[contract]` → Already bound, and any other answer that is not a flow → Start failed. No acquisition, poll, re-render or reconciliation ever opens a browser context `[derived]` |
...
| §1.10 | Source save failed `[derived]` | The `PATCH` failed or never answered | F1, in the edit dialog | `sourceDetail.edit.fail`, `sourceDetail.retry` | 重试 first re-reads `GET /api/models/sources` by the held source id (D-36): the requested normalized fields already present are authoritative commit evidence → Source save committed with the reread Source held and response-only `removed_hops` / `interrupted` explicitly unavailable; source absent → §1.6 Source gone; otherwise re-send → Saving source |
| §1.10 | Source save committed `[derived]` `[contract]` | R1 holds a successful metadata response, or D-36 inferred the commit | F2 for M1's complete-surface read; the write already succeeded and the authoritative response is held | `sourceDetail.edit.settlement.title`; on a failed read `toast.refreshFailed` and `sourceDetail.retry` | Close the dialog, return to 01 with the returned Source installed, and announce the commit exactly once in the page toast. **No report is mounted and no exit is owed** `[derived]`: the same `removed_hops` / `interrupted` plan was already put to the user by frame 11's guard and accepted by the destructive press, so restating it afterwards would ask a settled question again. The impact is instead M1's read scope, which runs behind the toast. A failed read enters Committed projection stale for only the members that failed; nothing there questions the save |
...
| §1.10 | Source remove failed `[derived]` | Either delete request failed or never answered | F1, in place | `sourceDetail.remove.fail`, `sourceDetail.retry` | 重试 re-reads `GET /api/models/sources` by the held id (D-36): absence is authoritative delete-commit evidence → Source removal committed with both response-only impact arrays explicitly unavailable; present → re-send the same stage, non-forced before refusal and forced after one |
| §1.10 | Source removal committed `[derived]` `[contract]` | R2 holds a successful delete response, or D-36 proved the exact Source absent; the source is already gone | F2 for M2's complete-surface read; the read cannot negate the held delete response | `sourceDetail.remove.settlement.title`; on a failed read `toast.refreshFailed` and `sourceDetail.retry` | Drop the detail into 01, remove the exact Source locally and announce the removal exactly once in the page toast. **No report is mounted and no exit is owed** `[derived]`: the refusal that preceded the forced delete already showed this plan and the second destructive press accepted it. The authoritative success arrays are still newer evidence than that preview, and they are consumed as M2's read scope rather than rendered again. A failed read enters Committed projection stale through M0 for only the members that failed; it never says the committed deletion is unconfirmed |
...
| Relation membership | For every routeable model in the current backend catalog, use its fresh exact AgentChain regardless of Automatic, Manual or Passthrough origin. Deduplicate by Source/backend pair, including collapsed model rows. An unread chain may retain known manual membership, but never infer inherited membership from default order or stale chains. Dormant overrides outside the catalog create no relation. |
| Passthrough ink (owner amendment 2026-09-07) | An actively supplying relation whose contributing fresh chains are all Passthrough uses the existing Passthrough badge `--gold-ink` token and active wire width, with a matching EN/ZH legend. A mixed Automatic/Manual/Passthrough relation retains normal supply ink. Takeover, paused supply and connected-but-unused classification keep their existing precedence and treatment; responsive wire visibility and shared anchors are unchanged. |
...
| RS-6 — plan changed | a forced request returns another guarded 409 | The user already answered the one question, so the same intent is resent forced with the recomputed arrays echoed, a bounded number of times, without reopening the guard; a plan that never settles ends as that attempt's terminal refusal |
...
**R6 and M6 own the success before the editor closes** `[contract]` `[derived]`.
The returned `chain` is the post-write projection and commit evidence. Later reads may
update live projections, but never replace that held response evidence while page-owned M6
is active. Each non-empty `removed_hops` / `interrupted` array remains exact reconciliation
evidence; it is not remounted as a Route impact dialog after save. ET-10 starts page-owned
M6 for an R6 response with impact; ET-11 starts the same read for a two-empty R6 envelope,
while ET-17c starts it when the exact chain endpoint is the first matching authority after a
lost response. The editor closes as soon as the commit evidence is accepted, emits the shared
`common.saved` toast, and returns focus through PF-1. Every successful acquired member is
installed/held immediately; background installs preserve the current FF-1-valid focus and
use PF-1 only when that focus disappears or becomes ineligible. The pending landing target
remains through failed subsets so a later successful Retry can resolve the fallback. Direct
mode makes Route-chain members not applicable and disowns their reads; it does not turn them
into stale failures. M6 never
resends or questions the route write, and it exposes any acquired failed subset through the
receiving page's existing Retry treatment.
...
| --- | --- | --- | --- | --- |
| Mounted editor + match | Install exact matching AgentChain | Set settlement `terminal-committed`, establish CF-R, start page-owned M6 and close the editor | `common.saved` toast; PF-1 after close | M6/AR-M; acquire every member successor required by CF-R |
...
target or makes it ineligible; otherwise focus stays where the user moved it. Page-owned
AS-5a/AS-6a preserve the current FF-1-valid focus after each relevant install and invoke
PF-1 only when that focus disappears or becomes ineligible. The event table remains the sole destination
...
| ET-10 — R6 success with impact | Saving + exact R6 envelope with either array nonempty | Consume every R6 member, establish CF-R, start page-owned AR-M with Agents acquired and Source/Route applicability deferred, and close the editor | Release mutation/refusal ownership; retain the entire envelope and held self-satisfying chain in page-owned M6 | Page-owned Route reconciliation | `common.saved` toast; page stale/Retry copy only if an acquired member later fails | PF-1 after close; preserve current FF-1-valid focus on installs and use PF-1 only if it disappears | Agents landing read; mode then acquires Sources under Direct or after CF-H under Hub and activates/drops the held Route evidence |
| ET-11 — two-empty R6 commit | Saving + exact R6 envelope whose two impact arrays are empty | Preserve the exact envelope, establish CF-R, start page-owned AR-M with Agents acquired and Source/Route applicability deferred, and close the editor | Release mutation ownership; retain the held self-satisfying chain in page-owned M6; no origin can be restored | Route committed, reconciling on the page | `common.saved` toast; no mounted `route.refreshing` or Done state | PF-1 after close; preserve current FF-1-valid focus on installs and retain the landing target through failed Retry | Agents landing read; mode then acquires Sources under Direct or after CF-H under Hub and activates/drops the held Route evidence |
| ET-12 — guarded 409 | Saving + immutable draft + complete current refusal arrays from a non-forced save | Validate guard code and both arrays | Release mutation ownership only after the current plan is held | Route save refused | Independent shared Qp6FI modal over the unchanged draft, current arrays | `guard.cancel` | None until explicit user action |
...
| ET-13b — confirm guard and send | Route save refused + immutable draft/current arrays | Revalidate exact echoes, then send the same draft with `force: true` and both arrays | The mounted guard turns busy as the immutable values freeze into the request, and stays until the answer lands | Saving | Busy guard confirmation | The busy guard | One owned Route PUT or DELETE matching the immutable intent, plus RS-6's bounded forced resends |
...
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain whose manual_override matches normalized submitted intent | The exact AgentChain is already installed; record `matching@epoch`, settle the attempt `terminal-committed`, mark response-only tails unavailable, establish CF-R and transfer to page-owned M6 while closing the editor | Clear comparison busy; discard obsolete read Retry state; retain earlier successes as latest-known but frontier-insufficient | Page-owned Route committed, reconciling | `common.saved` toast; page stale/Retry copy only if an acquired member later fails | PF-1 after close; preserve current FF-1-valid focus on installs and retain the landing target through failed Retry | Acquire Agents first; LF-H then establishes CF-H and acquires Sources. Keep the matching chain self-satisfied |
...
| ET-18aM — page-owned M6 chain-Direct handoff | Page-owned AR-MD selected from chain `direct_mode` after the editor has already closed | Keep CF-R/commit evidence and successful independent members; start the landing Agents read without deriving a page form from the error | Disown only prior pending Route work; the editor is already gone | Page §1.0 Partial over the last installed page | Existing Partial copy plus held committed evidence | Reapply PF-1 after the landing install | Landing Agents dispatches mode; Hub establishes CF-H and acquires every required Source/Route successor, Direct does not |
...
| ET-19a — page-owned AR-M3 failed subset | Page-owned AR-M3 settled generation | Failure cannot negate commit or any successful installed member | Clear generation pending; retain evidence and mark only the acquired failed member set stale. Mode-dependent deferred members remain unrendered/non-stale | Page with partial current authority | `route.impact.refreshFail` plus the existing page Retry | Preserve current page focus only if FF-1-valid; otherwise use PF-1 | None until CA-M2 admits ET-19b |
| ET-19b — page-owned failed-subset Retry | ET-19a + exact acquired failed subset + CA-M2 admission | Start AR-M4 for exactly that subset; an Agents-only Retry leaves companions deferred until its success dispatches mode | Keep evidence/successful members; CA-M1 owns Retry busy/inert admission | Same page projection, retry pending | Prior page copy | CA-M1 handoff | One AR-M successor generation for the acquired failed subset; successful Agents activates/acquires applicable companions once |
...
| ET-20 — page-owned committed settlement | Page-owned reconciling, refreshed or stale phase + exact commit evidence | No modal action is emitted; M6 settles the active generation and preserves the exact evidence | Keep page ownership; never restore the origin or create a second dialog | Receiving page with exact installed successes plus markers only for acquired pending/stale members; deferred companions stay unrendered until their prerequisite activates them | Held evidence plus the registered pending/stale marker as applicable | Preserve current FF-1-valid focus after each install and at final settlement; use PF-1 only if it disappears; clear the pending landing target after the final decision | Transferred AR-M if active; otherwise none, or later CA-M4/ET-19d after failure |
...
| AS-5a — page-owned AR-M Direct install | Page-owned evidence + current successful M6 AgentSupply `mode: direct`, whether it supplied AR-MD or answered ET-18aM | Dispatch mode immediately, install the exact current Direct page, activate Source when it was deferred and make all Route-chain members not applicable | Retain evidence and acquired Source/Agents state; cancel/disown pending Route reads and remove their failed/pending slots from stale/Retry | Actual Direct page | Registered acquired pending/stale block only for applicable members | Reapply PF-1 after installation | AR-M acquires/continues activated pending members, or CA admits only an acquired failed subset after settlement |
...
| CA-M1 | Page-owned M6 Retry | An acquired AR-M member is pending, AR-M4 Retry is pending, or AR-M2 succeeded; deferred companions alone never make Retry busy | Absent; if layout reserves it, disabled/inert | Preserve the receiving page's FF-1-valid focus before changing admission | None |
| CA-M2 | Page-owned M6 Retry | Settled ET-19a / AR-M3 exact acquired failed subset, including Agents-only with companions deferred | Rendered and enabled on the affected page projection | None while admitted | Emit ET-19b for exactly the held acquired failed subset |
| CA-M3 | Page M6 Retry | An acquired transferred AR-M member is pending, AR-M4 Retry is pending, or AR-M2 succeeded; deferred companions alone never make Retry busy | Absent; if layout reserves it, disabled/inert | Preserve the receiving page's registered FF-1-valid fallback before changing admission | None |
...
| Route committed, reconciling / refreshed / page-owned AR-M stale | No modal control | `common.saved` / existing page retry copy | The editor is already closed; only the receiving page renders pending/stale state | Preserve the current FF-1-valid focus during installs/settlement; use PF-1 only when it disappears or becomes ineligible |
| Route report-free stale | 重试 | `route.retry` | CA-M2 renders/enables it only after ET-19a settles an acquired failed subset, including Agents alone while companions are deferred; CA-M1 makes it absent/inert during acquired pending/retry and after success | Page Tab order only when enabled; activation delegates to the current CA row |
...
| `committed`: page-owned reconciling / stale | No route modal; the receiving page owns M6 | None or transferable AR-M read generation | No route dismissal edge | No route dismissal edge | No route dismissal edge | No route dismissal edge |
...
| `Enter` / `Space` on guard primary | ET-13b |
...
| `impact.done` `[legacy/unrendered]` | 保存成功后不显示 | Retained for compatibility; not rendered after a successful route save |
...
| authorization link `[derived]` | The exact non-null `presentation.auth_url` | yes | Open the same provider URL in a new context, or copy it with the adjacent copy control, without starting another flow (PD-2) |
...
The provider handoff is the user's `[contract]` `[derived]`. No transition opens a browser
context: PD-1 sends the start without one, and PD-2 renders the exact URL as a link with a
copy control once the **non-terminal** accepted flow first carries `presentation.auth_url`.
The user opens the provider page by activating that link, or copies the URL into whichever
browser holds the right account — the one path that works the same in every browser, popup
policy and desktop shell. RR-2 routes an already-terminal acquisition straight to status and
PD-3 renders no link. The URL is server-declared; the client keeps no vendor-to-**OAuth**-URL
table, and the static top-up/support register above is never consulted for this transition.
Acquisition, polling, re-rendering or reconciling a submit never **auto-opens** the
authorization URL; the user may activate or copy the held link at any time. A submit response is not assumed terminal,
...
| --- | --- | --- | --- |
| PD-1 | A create or reauth flow-producing control is activated | Send the start/reauth request and open **no** browser context: the provider page is the user's to open | Nothing is rendered yet. A refused/failed acquisition has nothing to close |
| PD-2 | Acquisition or a later read of the active non-terminal flow first carries non-null `presentation.auth_url` | Open nothing automatically — not on acquisition, polling, submit or reconciliation. Never derive or rewrite the URL | Keep the exact URL as a visible link with an adjacent copy control for the lifetime of the active form. Activating the link opens the same URL in a new context without restarting the flow; copying lets the user paste it into any browser |
| PD-3 | RR-2 receives a terminal flow, or an active flow terminates | Status-read or settle immediately; a terminal flow owns no provider handoff | Render no form and no stale authorization link |
...
| PD-5 | An active paste form in E3a carries `presentation.auth_url: null` | The one held flow owns a 2s status read under the existing dialog bound. Keep reading while the required URL is null; never start a second flow or open a browser context. E2 stays inconclusive, and the evidence-class matrix owns every other answer | Keep the held form and draft. The first read with a non-null URL renders PD-2's link and copy control and pauses this delivery-only poll while E3a still waits for explicit submit |
...
A success removes the source and reconciles through M2 whatever its impact
cardinality; the removal says so once, in the page toast. A guarded `409` fills the
...
holds the returned `removed_hops` and `interrupted` and reconciles against them rather than
against the preview; a second client may have changed chains or protected Agent selections
between the two requests. What it does not do is put them back on screen. The user was shown
that plan by the guard and pressed the destructive button for it, so a post-commit report
would restate a settled decision as if it were still open — and the one command such a report
can offer, 完成, is a step the user owes nothing for. The arrays are consumed as M2's read
scope instead: it reads Sources, Agents/source orders and Route chains together, behind a
single `remove.settlement.title` toast, on both the two-empty and the non-empty path. R1
applies the same rule to a Base-URL save: the returned `source` is the next projection, the
impact arrays scope M1's complete-surface read, and `edit.settlement.title` announces it.
...
than empty and no impact is invented. If any of these reads fails, C9 enters
Committed projection stale with exactly the returned or inferred write evidence and the last
good dependent projections marked stale, under the page's own `toast.refreshFailed` line. Its
Retry repeats the same M1/M2 read and cannot repeat the edit or deletion. The failure is a
read verdict and says only that: the commit itself is never described as unconfirmed, and no
state holds the user to acknowledging it `[derived]`.
...
Removing source owns a request. A committed save or removal mounts nothing to dismiss: it
closes into 01 and announces itself once in the page toast, so §1.10 registers no DP-4 report
of its own. Committed projection stale is the one committed state it can still reach, and it
carries read-only Retry plus the stale-dependent-projection marker on the receiving surface
without taking Cancel semantics, repeating the committed mutation or restoring the held
...
The announcement states the **outcome**, not the flow it arrived through `[derived]`. The two
part company on exactly one path: an edit whose Source turns out to be absent is still the
edit flow, but nothing was updated, so it announces §1.6's `sourceDetail.gone` rather than
`edit.settlement.title` — the same fact its panel is showing — and the dialog stays open on
that absence instead of closing behind a save that did not happen. A deletion answered
`source_not_found` keeps `remove.settlement.title`: already absent is the outcome that flow
asked for. Only a removal closes into 01.
...
metadata is data and receives no copy key. A committed mutation adds one announcement per
outcome and nothing else; the hop / `SupplyGap` row copy it would have reused stays registered
for §1.11's repair report, which still has one:

| Key under `models.hub.sourceDetail.*` | 中文 | English | Outcome |
| --- | --- | --- | --- |
| `edit.settlement.title` `[derived]` `[contract]` | 供应商已更新 | The source was updated | `updated` |
| `remove.settlement.title` `[derived]` `[contract]` | 供应商已移除 | The source was removed | `removed` |
| `gone` — §1.6's own key, reused `[derived]` | 这个供应商已经不在了。 | This provider is no longer available. | `gone` |
...
Each is a page toast, not a dialog: it states what happened and is gone, because the impact
it would otherwise repeat was already confirmed before the write. `gone` is the one line the
toast shares with a surface — the open panel says the same thing — so the two never disagree.
A failed surface read after any of these outcomes reuses the page's own
`models.hub.toast.refreshFailed` (刷新失败,请重试 / Couldn't refresh, please retry) beside the
stale region's `sourceDetail.retry` `[derived]`.
...
- Forms A, B and C are selected from a **non-terminal** returned `presentation`, never a
  vendor table. PD-1–PD-3 open no browser context; the non-null `auth_url` is the
  user's handoff, rendered as a visible link with a copy control. PD-5 reads a held paste flow
  every 2 seconds while that required URL is null, and its first non-null value renders that
  link without starting another flow or opening a context. `expects` selects the branch,
