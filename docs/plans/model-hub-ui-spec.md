...
candidate, which is also its selected pair: ET-5a selects the first candidate as it receives
focus, and every ET-5d roving-focus move atomically replaces both values. Pointer activation
...
through ET-5c. Identity mapping and an explicit cross-model mapping are
...
| RT-21 — selector selection owner | Open Add selector, then move by Arrow/Home/End and pointer before confirmation | ET-5a selects the first focused candidate; every ET-5d move atomically makes the new active candidate the selected pair. `route.add.confirm` derives enabled state only from that owner and ET-5c appends exactly it |
...
| ET-5d — choose another Add candidate | Open selector + current active/selected candidate | Send/read nothing; Arrow/Home/End or pointer activation atomically makes the destination candidate both active and selected | Replace the one prior active/selected owner | Same open selector | Existing selector copy | Newly active/selected candidate | None |
...
| Ready / Dirty / Invalid after refresh + Add selector | Source/model candidate region | `route.add.source`, `route.add.model` | Labels render while open; exactly one valid pair is the active/selected candidate | Arrow/Home/End or pointer activation emits ET-5d; Escape emits ET-5b; Tab reaches confirmation |
...
| Candidate Arrow / `Home` / `End`, or pointer activation | ET-5d atomically moves the selector's one active/selected owner |
