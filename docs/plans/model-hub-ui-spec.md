...
metadata is data and receives no copy key. A committed mutation adds one announcement each
and nothing else; the hop / `SupplyGap` row copy it would have reused stays registered for
§1.11's repair report, which still has one:

| Key under `models.hub.sourceDetail.*` | 中文 | English |
| --- | --- | --- |
| `edit.settlement.title` `[derived]` `[contract]` | 供应商已更新 | The source was updated |
| `remove.settlement.title` `[derived]` `[contract]` | 供应商已移除 | The source was removed |
...
it would otherwise repeat was already confirmed before the write. A failed surface read after
either commit reuses the page's own `models.hub.toast.refreshFailed` (刷新失败,请重试 /
Couldn't refresh, please retry) beside the stale region's `sourceDetail.retry` `[derived]`.
