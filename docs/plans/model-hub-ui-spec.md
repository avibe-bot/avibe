...
| C3 — focus return target | Name the mounted control that receives focus when a transient surface closes; a named target is invalid unless it is mounted after the destination projection is installed. A background install preserves the active target while it remains mounted and invokes PF-1 only if the install removes it | [x] A no-op exit reveals the latest installed authoritative page projection, which is the opening row only until a newer reconciliation payload is installed. A modal-closing landing uses PF-1 because the modal target unmounts; transferred M6 preserves the user's current page focus unless refresh removes it. Mounted M6 keeps Done | [x] A no-op close returns to the source overflow trigger. A committed page landing uses PF-1 after installation; an in-modal receiving surface keeps its registered mounted focus | [x] A no-op repair exit returns to the invoking card, vendor-observation action or menu control. A committed page landing uses PF-1 after installation; an in-modal result keeps its registered mounted focus | [x] Escape, outside dismissal and a no-op return from 04 restore Add subscription; a committed 04 exit hands focus to its named receiving surface |
...
| M6 | R6 route replacement, or the earliest authoritative exact ordered `(source_id, model_id)` readback matching submitted `hops`: either Hub `AgentSupply.routes[menu_model].hops` or AgentChain under D-36 | `Source[]` including `adopted_by`, complete `AgentSupply[]` including mode/model supply/routes, and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | Start AR-M as soon as ET-10/ET-11 or AR-D holds commit evidence. Reuse every member already settled in that attempt: an exact matching Agents payload is both commit evidence and the current AgentSupply/Route projection, so only missing members are read. Keep R6's entire envelope when received; inference marks `removed_hops` / `interrupted` unavailable. The modal may transfer the active generation to the page without cancelling it | AR-M consumes each successful member monotonically, dispatches a successful AgentSupply immediately, and marks only failed members stale. CA admits Retry only after the generation settles with failures; it retries exactly that failed subset and never overlaps generations. Mounted completion retains the report/evidence and Done; transferred completion preserves any still-mounted active page target and uses PF-1 only when refresh removed it. ET-20 alone acknowledges/closes/transfers; no path resends or questions the route write |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or AR-D3/AR-D6 proves the desired Route while response-only arrays are unavailable; exact commit evidence is held while the active AR-M generation has pending members | F2 applies only to the members that AR-M3 later marks stale; the Route write/desired state is already proven | `route.refreshing`, `route.impact.done` | ET-11 or AR-D3/ET-17c owns the mutually exclusive entry; AR-M/ET-19/AS-5a/AS-5b/AS-6a/AS-6b and ET-20 own independent member settlement, Retry, page installation and the sole user exit |
| §1.2 | Route committed, refreshed `[derived]` | AR-M2 completed a mounted report-free M6 generation with every member current and installed its exact page behind the still-mounted modal; commit evidence remains held until acknowledgement | F5 — no request remains and no impact report exists | `route.impact.title`, `route.impact.done` | ET-20 is the only exit. The page projection is already installed, and modal close invokes PF-1 because Done unmounts |
...
| §1.2 | Route save rejected `[derived]` | The server definitively rejected the route write outside the guarded-plan and `direct_mode` branches, so this attempt did not commit | F1, in place | `route.fail.save`, `route.retry`, `route.cancel` | ET-14, AR-D/ET-16, AS-1/AS-2a/AS-2b, ET-17, ET-18a and ET-8a own entry, reconciliation, every outcome and the non-mutating exit. CA admits Retry only without an active generation |
| §1.2 | Route save outcome unknown `[derived]` | The route write returned no HTTP/API answer; the exact submitted draft and whether this was the non-forced or confirmed stage remain held | F1; D-36 permits no blind resend | `route.fail.unconfirmed`, `route.retry`, `route.cancel` | ET-15, AR-D/ET-16, AS-1/AS-2a/AS-2b, ET-17, ET-18a and ET-8b own entry, D-36 reconciliation, every outcome and the Chain-unresolved exit. CA admits Retry only without an active generation |
| §1.2 | Route impact reported `[derived]` `[contract]` | R6 holds a successful route-replace response with non-empty `removed_hops` and/or `interrupted`; the complete returned `chain` remains held while AR-M independently settles every invalidated projection | F2 only for the exact AR-M3 failed subset; the write already succeeded and DP-4 owns every exit | Report block keys always; `route.refreshing` while any member is pending; `route.impact.refreshFail` and `route.retry` only in the settled-failure subphase; neither retry key after AR-M2 success | ET-10, AR-M/ET-19, AS-5a/AS-5b/AS-6a/AS-6b and ET-20 own report entry, member settlement/Retry/install and the sole user exit; successful authority is installed immediately and never dismisses the report |
...
| §1.2 / §1.6 / §1.9 / §1.10 / §1.11 | Committed projection stale `[derived]` | M0 has authoritative Source absence; an M1/M2/M3/M4 mutation or R3 terminal has commit evidence and its complete read failed; M5 holds committed mode-switch AgentSupply and its Source read failed; or M6/AR-M3 holds Route commit evidence with a settled failed member subset. Hold the exact success envelope when received; inference holds its authoritative subject and marks response-only members unavailable. Every successful projection stays installed | F2 — the operation-specific sentence names only the failed read/member, never the completed write, authoritative absence or successful sibling | M0/M2 `sourceDetail.remove.impact.refreshFail`; M1 `sourceDetail.edit.impact.refreshFail`; M3/M4 `upstream.repair.impact.refreshFail`; M5 `upstream.unread`; M6 `route.impact.refreshFail`; the admitted owning Retry and, where mounted, its Done key | M0–M4 render committed Source/absence with last-good dependent projections stale; M5 renders held AgentSupply with Source stale; M6 renders every AR-M success exactly and marks only AR-M3's failed subset stale. M0–M5 Retry repeats their owning read; M6 Retry repeats only the failed subset through CA-M2/CA-M4. Success updates only its member, another failure stays. DP-4 exits and page navigation remain legal. Never resend mutation, invent impact, discard evidence, roll back installed authority or describe the write/absence as uncertain (C6/C9/C10/C14/C15) |
...
AR-D3 starts it from the earliest matching AgentSupply Route, while ET-17c starts it only
when a later AgentChain is the first matching authority. In every case the exact evidence and
modal stay mounted while AR-M has pending members. Each successful member is installed/held
immediately; a successful AgentSupply installs its exact page behind the modal and leaves Done
focused even if a sibling later fails. No settlement acknowledges, closes or transfers the
report. ET-20's Done-equivalent family is the only user-initiated
...
mounted, and a report-free commit enters Committed projection stale only for its failed subset.
...
each member independently as pending, successful authority, or failure; successful authority
is installed/held immediately and is never erased because a sibling failed. At most one
generation owns reads for a workflow. Starting a successor generation invalidates the prior
one, and every late result from an invalidated generation is ignored.
...
`AgentSupply.routes[menu_model].hops` is the exact persisted Route projection: equality with
the submitted ordered pairs is the earliest D-36 commit evidence, including empty-to-empty.
It starts M6 immediately with response-only tails unavailable and no later chain observation
may redefine that attempt. Only a nonmatching Hub Route authorizes the Source + exact-chain
stage; that stage waits for every result needed by its candidate outcome before choosing a
failure or resend. The latest installed authoritative page projection becomes the reversible
exit origin, so later failure cannot reveal an older opening snapshot.

| ID / generation | Independently settled members | Decisive evidence precedence | Immediately installed / held authority | Remaining stale or awaited members | Sibling cancellation / late result | Destination | Retry subset |
| --- | --- | --- | --- | --- | --- | --- | --- |
| AR-D1 — initial Agents failure | Agents failed | No mode or Route evidence exists | Keep the latest installed page projection and immutable draft/stage | Agents | Settle generation; no sibling exists | Originating save failure via ET-16b | Agents only through ET-16a |
| AR-D2 — Direct AgentSupply | Agents succeeded with exact backend `mode: direct` | Mode is decisive before all Route/failure classes | AS-1 installs that AgentSupply | None | Settle generation; issue no Source/chain read; ignore later obsolete results | Actual Direct page | None |
| AR-D3 — earliest Hub match | Agents succeeded with `mode: hub` and exact `routes[menu_model].hops == submitted` | Persisted Route equality proves commit before a later chain read | AS-2a installs the Hub AgentSupply; hold its exact Route as D-36 evidence and mark response-only tails unavailable | Only M6 members not already supplied by this Agents response | Do not start ET-17; AR-M adopts the current generation and ignores any obsolete chain result | Route committed, reconciling | M6 failed subset only |
| AR-D4 — Hub mismatch opens comparison | Agents succeeded with `mode: hub` and a nonmatching exact Route | Hub mode permits reconciliation but mismatch proves neither failure nor resend basis | AS-2b installs the Hub AgentSupply as the new reversible page origin; retain immutable draft/stage | Current Source list and exact AgentChain | ET-17a starts both under this generation | Originating failure, reconciliation pending | None while pending |
| AR-D5 — chain becomes Direct | AR-D4 + chain `direct_mode`; Source may be pending, success or failure | `direct_mode` outranks every sibling Source result | Hold only the illegality evidence; it is not a landing payload | Fresh page Agents payload | Cancel/disown Source; any Source result or failure arriving later is ignored | DM-3 / ET-18a → page Partial | Page Agents read only |
| AR-D6 — later chain match | AR-D4 + AgentChain pairs equal submitted; Source may be pending, success or failure | Exact Route equality proves commit before sibling failure | Hold AgentChain as D-36 evidence; retain AS-2b's installed Hub page; successful Source is reused | Pending/failed Source plus any other M6 member not already current | AR-M adopts pending/success/failed members in the same generation; no sibling can overwrite commit | Route committed, reconciling via ET-17c | M6 failed subset only |
| AR-D7 — differing chain, Source success | AR-D4 + nonmatching AgentChain + successful Source list | No commit evidence exists; the complete comparison authority is available | Install Source/chain as the latest reversible authority and run V5 | None | Settle generation before ET-17d resend or ET-17e invalid result; ignore late duplicates | V5-valid → ET-17d; V5-invalid → ET-17e | New ET-16 generation only after a later failed/no-answer PUT |
| AR-D8 — comparison incomplete | AR-D4 + non-`direct_mode` chain failure, or nonmatching chain + Source failure | Failure is selectable only after every higher-precedence slot for this generation settled | Retain every successful member already installed/held | Exactly the failed Source/chain member(s) | Settle generation; no successful sibling is rolled back | Originating failure via ET-17b | Agents-first successor through ET-16a; no timer |
| AR-M1 — member settles successfully | Active M6 generation + any successful Source, Agents or Route-index member | A successful AgentSupply's `mode` is immediately decisive for page form even while siblings remain pending/failed | Install/hold that exact member; AgentSupply invokes AS-5a/AS-5b/AS-6a/AS-6b by ownership phase and mode | Every still pending/failed member only | Keep same generation; later sibling failure cannot undo this installation | Same mounted report/committed phase, or receiving page | None while any member remains pending |
| AR-M2 — all members current | Active M6 generation + no pending/failed member | Complete current surface settles reconciliation | Keep every exact member and commit evidence; report-free mounted origin becomes Route committed, refreshed | None | Settle generation; later duplicate result is ignored | Mounted evidence remains until ET-20, or transferred page remains | None; Retry absent |
| AR-M3 — failed subset settles | Active M6 generation + no pending member + one or more failed members | Previously successful AgentSupply/mode remains authoritative; failure affects only named members | Keep every successful member installed and exact commit evidence held | Failed member set only, visibly stale | Settle generation; later results from it are ignored | Mounted → ET-19a; transferred → ET-19c | Exact failed subset only through ET-19b/ET-19d |
| AR-M4 — retry failed subset | Settled AR-M3 + explicit admitted Retry | No prior success is reread or discarded | Retain installed members/evidence | Prior failed subset becomes pending in one successor generation | CA disables Retry until that generation settles; prior-generation results are ignored | Same mounted phase or receiving page | Exactly the failed subset |
...
attached; it never reconstructs the opening row after AS-2a/AS-2b installed newer authority. In a
...
**PF-1 is the sole post-install focus fallback** `[derived]`. After installation, first
preserve the active target if it remains mounted. Only an install or modal close that removes
that target invokes the fallback, after the rendered target set is known:

| Active target after install/close | Installed page form | Exact model row after refreshed collapse/rendering | PF-1 output |
| --- | --- | --- | --- |
| Still mounted | Any | Any | Preserve that exact active target; background settlement does not move focus |
| Removed | Direct | Not rendered by definition | Stable backend group head |
| Removed | Hub | The exact `(backend, menu_model)` row is rendered | That model row |
| Removed | Hub | The exact row is absent or collapsed | Stable backend group head |
...
target; otherwise focus stays where the user moved it. Mounted AS-5a/AS-6a always leave Done
focused. The event table remains the sole destination owner. Its **Visible feedback** cell MUST cite a key in the copy register below or name an
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ET-1 — open model row | Mounted §1.1 row + exact `(backend, menu_model)` | Start the exact chain GET; infer no projection | Close the row menu; no draft/grab exists | Loading route | `route.loading` | `route.cancel` | Dialog-owned chain GET |
...
| ET-4 — unread Retry | Route unread + exact identity/page row | Reissue only the opening chain GET | Clear the old error | Loading route | `route.loading` | `route.cancel` | Dialog-owned chain GET |
...
| ET-8a — ordinary reversible exit | Loading route, Route unread, Ready, Dirty, Invalid after refresh or Route save rejected + latest installed authoritative page projection | Send no mutation; invalidate any dialog-owned AR-D generation | Close selector; restore/cancel any grab; discard local/submitted draft; ignore every late result from the invalidated generation | Reveal the latest installed page authority; before reconciliation this is the held opening projection, after AS-2b it is that newer Hub projection | Exact latest projection; retain its existing unresolved marker if any | Preserve a still-mounted active page target, otherwise PF-1 after the modal target unmounts | None; an already-unresolved row retains D-35 |
| ET-8b — unknown-outcome exit | Route save outcome unknown + immutable attempt evidence + latest installed page authority | Send no mutation/read; invalidate any dialog-owned AR-D generation | Discard modal transients, ignore every late result from that generation, and transfer the uncertain attempt marker | Latest installed page projection with its exact row marked Chain unresolved | `route.fail.unconfirmed` hands off to that row's unresolved treatment | Preserve a still-mounted page target, otherwise PF-1 after modal close | D-35 page owner |
...
| ET-16b — AR-D1 failure handoff | AR-D1 selected after the Agents member failed | Do not choose a page form | Clear active generation/busy; retain error body/draft and latest page authority | Originating failure | F1 read cause | `route.retry` | None until a newly admitted ET-16a |
| ET-17a — AR-D4 Hub comparison begins | AR-D4 selected from AS-2b's nonmatching exact Hub AgentSupply | Under the same generation read current Sources and exact chain | Keep modal/draft; keep Retry busy/inert | Originating failure, Hub reconciliation pending over the installed Hub page | Existing failure copy | Keep `route.retry` | One Source read + one exact-chain read in the current AR-D generation |
| ET-17b — AR-D8 incomplete comparison | AR-D8 selected after every higher-precedence slot settled | No commit verdict or resend basis exists | Clear active generation/busy; retain every successful authority/draft and the latest installed page projection | Originating failure | F1 read cause | `route.retry` | None until a newly admitted ET-16a |
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain equal to submitted pairs | Hold AgentChain, mark response-only tails unavailable and transfer the same generation to AR-M | Clear comparison busy; discard obsolete retry/refusal state | Route committed, reconciling | `route.refreshing` | `route.impact.done` | AR-M reads only missing/pending/failed model-surface members |
...
| ET-19a — mounted AR-M3 failed subset | Mounted report/committed modal + AR-M3 settled generation | Failure cannot negate commit or any successful installed member | Clear generation pending; retain modal/evidence and mark only the failed member set stale | Same impact report, or report-free Committed projection stale | `route.impact.refreshFail` plus prior report/refresh copy | `route.retry` | None until CA-M2 admits ET-19b |
| ET-19b — mounted failed-subset Retry | ET-19a + exact failed subset + CA-M2 admission | Start AR-M4 for exactly that subset | Keep modal/evidence/successful members; mark Retry busy/inert | Same report or report-free state, retry pending | Prior report/refresh copy | Keep `route.retry` | One AR-M successor generation for the failed subset |
| ET-19c — transferred AR-M3 failed subset | ET-20 receiving page + AR-M3 settled generation | Failure cannot negate commit or any successful installed member | Clear generation pending; retain evidence and mark only failed members stale | Receiving page with partial current authority | `route.impact.refreshFail` | Preserve current page focus | None until CA-M4 admits ET-19d |
| ET-19d — page-owned failed-subset Retry | ET-19c + exact failed subset + CA-M4 admission | Start AR-M4 for exactly that subset | Keep evidence/current members; mark page Retry busy/inert | Same receiving page, retry pending | Existing stale line | Keep page Retry | One AR-M successor generation for the failed subset |
| ET-20 — committed Done-equivalent exit | Mounted report, reconciling, refreshed or stale phase + exact commit evidence | Done, title-close, Escape and outside send no mutation/read | Close modal and transfer any active AR-M generation/evidence; never restore origin | Receiving page with exact installed successes plus pending/stale markers for remaining members | Held evidence plus the registered pending/stale marker as applicable | PF-1 after modal Done unmounts, over the exact installed page | Transferred AR-M if active; otherwise none, or later CA-M4/ET-19d after failure |
...
| AS-2a — AR-D3 matching Hub install | AR-D3 + exact Hub AgentSupply whose persisted Route equals submitted pairs | Install that exact Hub row and hold the matching Route as commit evidence | Transfer AR-D to AR-M; retain modal and discard obsolete retry/refusal state | Route committed, reconciling over actual Hub page | `route.refreshing` | `route.impact.done` | AR-M reads only model-surface members not settled by this Agents response |
| AS-2b — AR-D4 nonmatching Hub install | AR-D4 + exact Hub AgentSupply whose persisted Route differs | Install that exact Hub row as the latest reversible page authority | Clear only Agents-member pending; retain generation/modal/draft/stage | Originating failure over actual Hub page | Existing failure copy | Keep inert `route.retry` | ET-17a Source/chain reads in the same generation |
...
| AS-5a — mounted AR-M Direct install | Mounted evidence + any successful M6 AgentSupply member whose owning backend is `mode: direct` | Dispatch mode immediately and install the exact current Direct page behind the modal | AR-M retains modal/evidence and every other member's independent pending/stale state | Same mounted report/committed phase over actual Direct page | Registered report/refresh/stale block for the AR-M phase | Keep `route.impact.done` | AR-M continues pending members, or CA admits failed-subset Retry after settlement |
| AS-5b — transferred AR-M Direct install | Transferred AR-M + any successful M6 AgentSupply member whose owning backend is `mode: direct` | Dispatch mode immediately and install the exact current Direct page | AR-M retains evidence and every other member's independent pending/stale state | Actual Direct page with exact pending/stale markers | Exact Direct projection plus registered pending/stale block | Preserve the current active page target if still mounted; otherwise PF-1 | AR-M continues pending members, or CA admits failed-subset Retry after settlement |
| AS-6a — mounted AR-M Hub install | Mounted evidence + any successful M6 AgentSupply member whose owning backend is `mode: hub` | Dispatch mode immediately and install the exact current Hub page behind the modal | AR-M retains modal/evidence and every other member's independent pending/stale state | Same mounted report/committed phase over actual Hub page | Registered report/refresh/stale block for the AR-M phase | Keep `route.impact.done` | AR-M continues pending members, or CA admits failed-subset Retry after settlement |
| AS-6b — transferred AR-M Hub install | Transferred AR-M + any successful M6 AgentSupply member whose owning backend is `mode: hub` | Dispatch mode immediately and install the exact current Hub page | AR-M retains evidence and every other member's independent pending/stale state | Actual Hub page with exact pending/stale markers | Exact Hub projection plus registered pending/stale block | Preserve the current active page target if still mounted; otherwise PF-1 | AR-M continues pending members, or CA admits failed-subset Retry after settlement |
...
| CA-D2 | Save-failure Retry | ET-16a / AS-2a / AS-2b / ET-17a generation active | Rendered busy, disabled/inert | None; cannot overlap AR-D or reach a second PUT |
...
| CA-M2 | Mounted M6 Retry | Settled ET-19a / AR-M3 failed subset | Rendered, enabled | Emit ET-19b for exactly the held failed subset |
...
| CA-M4 | Page M6 Retry | Settled ET-19c / AR-M3 failed subset | Rendered, enabled | Emit ET-19d for exactly the held failed subset |
...
| Route save rejected / outcome unknown | 重试 | `route.retry` | CA-D1 enabled only with no AR-D generation; CA-D2 renders it busy and disabled/inert throughout ET-16a/AS-2a/AS-2b/ET-17a ownership | First command in Tab order only when enabled; only CA-D1 activation emits ET-16a |
...
| Route impact reported / report-free stale | 重试 | `route.retry` | CA-M2 renders/enables it only after ET-19a settled with a failed subset; CA-M1 makes it absent/inert during initial/retry pending and after success | Normal Tab order only when enabled; activation emits ET-19b and retains the modal |
| Receiving page with transferred stale subset | 重试 | `route.retry` | CA-M4 renders/enables it only after ET-19c settled with a failed subset; CA-M3 makes it absent/inert during initial/retry pending and after success | Page Tab order only when enabled; activation emits ET-19d |
...
| `Escape` | Grabbed → ET-7e cancel/restore; selector open → ET-5b; ordinary reversible phase → ET-8a or ET-8b; committed phase → ET-20; Saving emits nothing |
...
alone may retire an illegal surface but cannot project its destination. PF-1 preserves a
still-mounted active target and chooses a rendered fallback only after installation removes
that target. C16 gives each gesture or single producer edge to ET, composite settlement to
AR, pure page installation to AS, post-install fallback to PF-1 and asynchronous control
admission to CA. Each atomic owner accounts for cleanup, visible feedback, destination focus
and next owner; different outcomes cannot share a row, and visible feedback without a
registered key or named copy block is incomplete. None can be inferred from the reversible
no-op rule.
...
and the exact submitted ordered pair array. The first authoritative Hub Route projection
whose ordered `(source_id, model_id)` pairs equal that array proves the desired Route
committed: prefer `AgentSupply.routes[menu_model].hops` when the Agents read already carries
it, otherwise use the later chain `GET`. Runtime annotations do not participate in either
comparison, and a later chain observation cannot redefine an attempt already proved by the
AgentSupply Route. A differing Route permits one resend only on the user's Retry after the
registered comparison settles; a failed read permits none. Either a received R6 success or
that earliest inferred commit enters M6 before visible handoff, and inference marks the
response-only `removed_hops` / `interrupted` members unavailable.
