...
| --- | --- | --- | --- | --- |
| M1 | R1 source metadata / Base URL | `Source[]`; a Base URL inventory change may also change `AgentSupply[]` runnability and Route chains through the guarded cascade | After a non-empty impact report's Done-equivalent exit, or before handing off an impact-free success, read the complete model surface. Keep R1's `source` and arrays until it settles | A report stays rendered with `sourceDetail.edit.impact.refreshFail`; an impact-free path keeps the returned Source and last good Agent/chain projections under F2 |
| M2 | R2 source deletion | `Source[]`, every backend's Source order in `AgentSupply[]`, and every Route chain | After the report's Done-equivalent exit, read the complete model surface before returning to 01. With two empty arrays, remove the exact Source locally and run the same read without inventing an impact report | A report stays rendered with `sourceDetail.remove.impact.refreshFail`. The empty-impact path keeps the committed Source absent and the last good dependent projections under F2 |
| M3 | R3 OAuth reauth terminal | the returned Source, its Agent-supply/runnability projection and every chain that names its exact source/model pairs | After Repair impact reported's Done-equivalent exit, or before handing off an empty-impact terminal, read the complete model surface. Keep `source`, `recovered` and `interrupted_pairs` as response evidence until it settles | A report stays rendered with `upstream.repair.impact.refreshFail`; an empty-impact path keeps the returned Source and last good dependent projections under F2 |
| M4 | R4 credential replacement | the returned Source plus every Agent-supply and Route-chain projection changed by `removed_hops` / `interrupted` | Use the same complete-surface timing as M3, while retaining R4's standard mutation envelope | A report stays rendered with `upstream.repair.impact.refreshFail`; an empty-impact path keeps the returned Source and last good dependent projections under F2 |
...
| DP-4 — committed report | Source save impact reported, Source removal impact reported, Repair impact reported | Run the report's registered Done exit and its M1/M2/M3/M4 projection read | Not rendered | Exactly the same operation as Done | Never restore the pre-write origin. Preserve the authoritative arrays until the complete read succeeds; a projection-read failure keeps them rendered and cannot negate the held write response |
...
| `sugad` source bar | 36×36 identity tile, source name, **state dot + state label** (使用中 in the drawn state) + 型号列表更新于 {{time}}, mono `host · N 个型号` | source state `[spec §4.5]` | 重新拉取 / 添加模型 / source overflow `[frame 11]` | Refetch / append an editable row / open 编辑来源 · 移除来源 |
...
transition that sends `{acknowledge_irreversible: true}` `[contract]`. A Hub source sends
`{}` directly because the contract does not require that acknowledgement.
...
questions the credential write; an empty-impact success holds the returned Source while
the same complete read settles.
