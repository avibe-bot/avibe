...
| C3 — focus return target | Name the mounted control that receives focus when a transient surface closes | [x] A no-op exit returns to the invoking model row; a committed exit focuses that row after its current Hub projection is installed or marked stale. AS-1/AS-3/AS-5 instead focus the stable backend group head because an actual Direct landing may unmount the model row | [x] A no-op close returns to the source overflow trigger; a committed exit hands focus to its named receiving surface | [x] A no-op repair exit returns to the invoking card, vendor-observation action or menu control; a committed exit hands focus to the returned source projection | [x] Escape, outside dismissal and a no-op return from 04 restore Add subscription; a committed 04 exit hands focus to its named receiving surface |
...
| C15 — page-form landing evidence | Every response, terminal or absence branch that changes the mounted page form MUST name the authoritative payload or read that selects the destination; every complete-surface read dispatches its returned `AgentSupply.mode` before installing a form, and classification evidence alone can retire an illegal surface but can never stand in for the target projection | [x] DM-1–DM-3 errors close the illegal editor and transfer to the page Agents read; DM-4 consumes the exact AgentSupply returned after ET-16a. AS-1–AS-6 are the only mode dispatcher for that read, its error-handoff reread and every M6 success: Direct installs exact payload and focuses the group head; only Hub installs/focuses the model row | [x] R1/R2 and M0–M2 hand their exact returned or reread Source/absence evidence to the page. Their complete-surface results dispatch the returned `AgentSupply.mode` before installing a page form; an error class selects no destination by itself | [x] R3/R4 and M0/M3/M4 hand their exact returned or reread Source evidence to the page. Their complete-surface results dispatch the returned `AgentSupply.mode` before installing a page form; flow/error classes select failure handling, never a healthy landing | [x] N/A — the menu changes only its own reversible phase and hands one explicit local vendor selection to §1.4; no response or terminal branch changes the page form |
| C16 — atomic event-transition totality | Every state-changing event MUST register, in one owning atomic edge row, its complete authority dispatch, transient-state cleanup, visible feedback and destination focus; an unlisted transition is forbidden. One row MUST NOT contain two user events whose cleanup, destination, focus or next owner differs | [x] The ET and AS rows are the sole edge owner for frame 02. Selector, row edit, grab, guard, reconciliation, M6 and dismissal events are split by phase; the state, V5, DM, control and keyboard registers cite them and define no parallel destination | [x] §1.10's state rows consume R1/R2 and M0–M2, clear their modal phase under DP-1/DP-2/DP-4, render the registered F/copy treatment and name the source overflow or receiving page as focus owner | [x] §1.11's state rows consume R3/R4 and RR/M0/M3/M4, settle flow/key transient ownership, retain the registered report/failure copy and name the invoking card or receiving projection as focus owner | [x] Closed/Open and the §1.4 handoff name the explicit vendor event, menu cleanup, visible destination and Add subscription / receiving-surface focus; no server response edge belongs to frame 13 |
...
| M6 | R6 route replacement, or an authoritative exact ordered `(source_id, model_id)` readback matching the submitted `hops` (including D-36) | The exact AgentChain, `Source[].adopted_by`, the owning backend's AgentSupply/model-supply projection and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | Start the complete model-surface read as soon as ET-10 or ET-11 holds commit evidence. Keep R6's entire envelope when received; for readback evidence hold the matching AgentChain and mark `removed_hops` / `interrupted` unavailable. The report or report-free committed modal stays mounted while the read runs; a Done-equivalent exit may transfer that exact read owner to the page without cancelling it | ET-19a keeps a failed read in the same mounted report or report-free stale phase with `route.impact.refreshFail`; ET-19b Retry retains that phase and evidence while repeating only M6. Every successful read first enters AS-5/AS-6's shared AgentSupply dispatcher. ET-20 alone owns user-initiated committed exits; no path resends, questions the route write or invents impact |
...
M6 as the report mounts; ET-11 starts the same read as the report-free committed phase
mounts. In both cases the exact evidence and modal stay mounted while M6 is pending.
ET-20's Done-equivalent family is the only user-initiated ownership-transfer exit; it may
hand an in-flight M6 read to the page but never starts, cancels or repeats it.
...
| Ordered Route pairs equal submitted `hops`, including empty-to-empty | The desired Route is committed. `current`, health and supply annotations are irrelevant to equality | ET-17c → ET-11; hold the AgentChain, mark response-only tails unavailable and run M6 |
...
installs the exact Direct payload and focuses the stable backend group head; only Hub may
install and focus the model row. No ET row may repeat or bypass that mode decision.
...
| ET-6b — pointer move one row | Ready, Dirty or Invalid after refresh + exact row/draft | Apply the one pointer move, then re-run V5 | Finish the pointer operation | Ready if equal to origin; Dirty if changed and valid; existing Invalid while marked rows remain | Live-region position announcement plus invalid lines | Moved row's grip | None |
| ET-6c — global draft sort | Ready, Dirty or Invalid after refresh + exact draft and page-held Source order | Apply the registered stable local sort once, then re-run V5; no-op is valid | Finish only the sort gesture | Ready if equal to origin; Dirty if changed and valid; existing Invalid while marked rows remain | Normal copy plus invalid lines; announce changed/no-op result | `route.reorder`, including after a no-op | None |
...
| ET-7b — begin keyboard grab | Ready, Dirty or Invalid after refresh + focused grip/order | Snapshot pre-grab order; send/read nothing | Set only this grip `aria-grabbed=true` | Same editor state + grabbed row | Grab announcement | Same grip | None |
| ET-7c — move grabbed row | Grabbed row + pre-grab order/current draft | Arrow/Home/End mutates the draft and re-runs V5 | Keep the same grab owner | Same grabbed phase over reclassified draft | Position announcement plus invalid lines | Same grip at its new position | None |
| ET-7d — drop grabbed row | Grabbed row + current draft | Send/read nothing; settle current order | Clear `aria-grabbed` and pre-grab snapshot | Ready, Dirty or Invalid by settled V5 result | Drop announcement plus invalid lines | Dropped row's grip | None |
| ET-7e — cancel keyboard grab | Grabbed row + pre-grab order | Restore the exact snapshot; send/read nothing | Clear `aria-grabbed` and snapshot | Prior Ready, Dirty or Invalid class | Cancel announcement plus prior invalid lines | Same row's restored grip | None |
| ET-7f — Tab from grabbed row | Grabbed row + current draft | Settle current order and re-run V5 before focus movement | Clear `aria-grabbed` and snapshot, then perform ordinary Tab/Shift+Tab | Ready, Dirty or Invalid by settled V5 result | Drop announcement plus invalid lines | Ordinary next/previous enabled modal control | None |
...
| ET-11 — report-free commit evidence | Saving + R6 two-empty envelope, or ET-17c matching AgentChain with tails unavailable | Preserve exact evidence and start M6 immediately | Release PUT/reconciliation ownership; no origin can be restored | Route committed, reconciling | `route.refreshing` | `route.impact.done` | M6 complete-surface read |
...
| ET-20 — committed Done-equivalent exit | Mounted report, reconciling or stale phase + exact commit evidence | Done, title-close, Escape and outside send no mutation/read | Close modal and transfer any pending M6 owner/evidence; never restore origin | Receiving page with current or explicitly pending/stale projection | Held evidence plus pending/stale marker as applicable | Receiving model row under the held Hub projection | Page-owned pending M6, or later ET-19d Retry |
| AS-1 — ET-16 Direct dispatch | ET-16a + exact AgentSupply `mode: direct` | Install that exact Direct row before any Source/chain read | Clear busy; close modal and discard all Route transients | Actual Direct page | Exact Direct projection | Stable backend group head | None |
...
| AS-3 — error-handoff Direct dispatch | ET-18a/ET-18c + exact AgentSupply `mode: direct` | Install that exact Direct row | Clear page-read pending and stale Hub row | Actual Direct page | Exact Direct projection | Stable backend group head | None |
| AS-4 — error-handoff Hub dispatch | ET-18a/ET-18c + exact AgentSupply `mode: hub` | Install that exact Hub row; never resurrect the editor | Clear page-read pending/stale marker | Actual Hub page | Exact Hub projection | Invoking model row | None |
| AS-5 — M6 Direct success | Mounted or ET-20-transferred M6 + complete returned surface whose owning AgentSupply is `mode: direct` | Dispatch mode first, then install the exact complete Direct surface | Clear M6 pending/stale; close any mounted modal; retain commit evidence in page history | Actual Direct page | Exact complete surface | Stable backend group head | None |
| AS-6 — M6 Hub success | Mounted or ET-20-transferred M6 + complete returned surface whose owning AgentSupply is `mode: hub` | Dispatch mode first, then install the exact complete Hub surface | Clear M6 pending/stale; close any mounted modal; retain commit evidence in page history | Actual Hub page | Exact complete surface | Current model row | None |
...
| Route committed, reconciling / Route impact reported / mounted M6 stale | 完成 | `route.impact.done` | Enabled before, during and after M6 | First command in Tab order; activation emits ET-20 |
| Route committed, reconciling / Route impact reported / mounted M6 stale | Title-bar close | `route.impact.done` | Enabled before, during and after M6 | Independent Tab stop; activation emits ET-20 |
...
illegal surface but cannot project its destination. C16 makes every state-changing event
...
None can be inferred from the reversible no-op rule.
