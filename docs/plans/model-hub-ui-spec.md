...
hold it for a named receiving state, name the other section that consumes it, or mark it
irrelevant with the reason. Empty arrays are omitted because they carry zero report rows;
an unmentioned member is never silently discarded. The frame-11/frame-12 producers use
this one matrix rather than restating the rule in each state row:

| ID | Producer | Contracted success members | Registered disposition |
| --- | --- | --- | --- |
...
**Status-read matrix — transport silence and a server answer are different evidence**
`[contract]` `[derived]`. This is the one matrix used by Awaiting sign-in, Awaiting
...
| Terminal `success` envelope | Authorization and materialization completed | Stop | `flow` supplies terminal state and intent; `create` then consumes `source`, `added_to`, `adopted_by`; `reauth` consumes R3's `source`, `recovered`, `interrupted_pairs` |
| Any other server-named safe failure, for example `discovery_failed` | The server answered for this attempt; authorization may have succeeded before materialization failed | Stop immediately → OAuth materialization failed | `create` refreshes the source list; `reauth` refreshes the held source id, because its supply/state may already have changed |

A single **transport** failure therefore does not end the flow — the poll is bounded by
the clock, not by the first blip. A server-named safe failure is the opposite: treating it
as silence leaves the dialog polling while the server may already have cleared a reauth
source's discovered supply. The intent-specific failure copy says which journey failed,
and the refreshed projection carries what the server now says about the affected source.
...
A success removes the source; empty impact completes directly, while non-empty impact is
reported below. A guarded `409` fills the same dialog with the server's staged consequence,
...
holds and renders the returned `removed_hops` and `interrupted` before any list reread; a
...
完成. R1 applies the same response-total rule to a Base-URL save: the returned `source` is
the next projection, but non-empty impact evidence is shown first.
...
array renders that `source` immediately. `recovered` is explicitly not a second UI branch:
it reports whether the prior state was blocked, while `source.state` is the complete current
projection. The report reuses `guard.gap.subject` and `guard.gap.agents` because the payload
is the same noun, rather than creating a second spelling for it.
