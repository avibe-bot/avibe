...
| M5 | §1.9 successful `direct` → `hub` mode `PATCH`, D-36's held-origin AgentSupply read proving that switch committed, or LF-H receiving exact Hub AgentSupply after a Direct-shaped/unknown landing origin | `Source[]`; the qualifying mode transition may have created the backend's singleton native Source, while the returned or reread `AgentSupply` is already authoritative for mode/backend supply | Hold that exact `AgentSupply` as write evidence where a mode mutation is proven and otherwise as landing evidence; read `GET /api/models/sources` before declaring the Hub landing current. A successful read hands both current projections to 01 and never reconstructs a Source from AgentSupply | Keep the exact AgentSupply installed, mark only the last-good Source list stale and render `upstream.unread` with read-only `upstream.retry`; Retry repeats only M5's Source read. Ordinary navigation remains legal. The failure neither resends/questions a proven mode switch nor settles/questions RO's independent Route outcome |
...
| §1.2 | Route save outcome unknown `[derived]` | The route write returned no HTTP/API answer; the exact submitted draft and whether this was the non-forced or confirmed stage remain held. RO owns the outcome independently from the mounted page form; Direct suspends exact Route observation instead of settling or discarding it | F1; D-36 permits no blind resend | `route.fail.unconfirmed`, `route.fail.reconcileRead` after AR-D1/AR-D8, `route.retry`, `route.cancel` | ET-15, RO/AR-D/ET-16, LF/AS, ET-17, ET-18a and ET-8b own entry, page-session suspension/resumption, D-36 reconciliation and the Chain-unresolved exit. CA admits mutation Retry only while Hub makes observation legal and no generation is active; reload drops the client evidence and never resends |
...
| DM-2 | Route `PUT` returns `direct_mode` | The editor is now illegal; this mode evidence does **not** prove whether the Route request committed | None — the error may not label the row or page Direct | ET-18a starts/transfers the same Agents read; RO preserves an unknown attempt when no mutation response exists |
| DM-3 | D-36 or another post-failure chain read returns `direct_mode` | Exact Route observation is temporarily illegal; this does **not** settle the held Route attempt | None — no second chain or Source read can manufacture one | ET-18a starts/transfers the same Agents read; RO-S suspends the unknown outcome until a Hub landing |
| DM-4 | ET-16a's first reconciliation read returns the exact backend `AgentSupply` with `mode: direct` | Direct is the authoritative current backend mode, so no Route read or V5 validation is legal now; mode still proves neither Route commit nor non-commit | The returned AgentSupply row itself; it is installed exactly and no second Agents read is sent | AS-1 consumes this payload; only an unknown origin enters RO-S, while a definitive rejection remains rejected |
...
resurrects the Route draft or derives a page form from an error. A Route write with no response
is different: RO may resume its exact readback after a later Hub landing, but no mode evidence
alone repeats or settles that mutation.
...
`[derived]` D-36. The reducer key is `(page-session workflow generation, member,
observation epoch, member settlement, decisive page evidence, Route outcome)`. Page-form
authority and Route-mutation outcome are orthogonal: a mode result may settle and install the
page while the Route outcome stays unknown. Each independently acquired member records pending, successful
...
payloads; they do not independently decide companion projection freshness:

| ID | Exact landing evidence | Closed read set and installation | Failure / next owner |
| --- | --- | --- | --- |
| LF-D | Exact returned `AgentSupply.mode: direct` | Install that AgentSupply immediately; retain every already-settled independent Source authority. Route reads are illegal while Direct, but RO-S evidence remains page-session-owned | No companion read. A later authoritative Hub landing enters LF-H and, if RO-S exists, RO-R |
| LF-H | Exact returned `AgentSupply.mode: hub` from any DM/AR/AS origin | Install that AgentSupply immediately and require one current Source member before calling the Hub landing current. Outside proven M6 this is M5's one Source read; inside M6 the already-owned barrier-or-later Source member satisfies the same requirement without a duplicate read | Source failure keeps AgentSupply installed, marks only Sources stale and offers only the owning Source read Retry. After Source success, RO-S resumes exactly once through RO-R before any Route resend |

**RO is the Route-outcome reducer** `[derived]`. It is independent from LF's page-form
settlement and lives only for the mounted page session; reload drops the client-held D-36
evidence and never authorizes a blind resend or a long-lived reconciliation ledger:

| ID | Route outcome + page form | Held evidence / legal work | Settlement |
| --- | --- | --- | --- |
| RO-U — unknown, observable | Lost Route response while Hub makes the exact chain GET legal | Hold workflow generation, immutable submitted pairs/stage and unknown marker; AR-D owns the ordinary exact readback | Matching → AR-D6 commit barrier/M6. Nonmatching + current Sources → V5, then only ET-17d may resend. Failed read → AR-D8; no resend |
| RO-S — observation suspended | RO-U receives exact Direct mode or chain `direct_mode` before Route outcome settles | Install LF-D page authority; retain the generation, submitted pairs/stage and unknown marker across Direct/Hub page-form transitions. Cancel/disown the illegal chain generation only; do not classify commit/non-commit and do not expose a mutation Retry while Direct | Remain suspended until the first authoritative LF-H landing; page navigation may continue. Reload discards this client evidence and permits no blind resend |
| RO-R — resumed once | First authoritative LF-H landing while RO-S is held, after LF-H's Source read succeeds | Acquire exactly one successor exact-chain generation using the held pairs; keep the current Hub page installed. Further Hub payloads cannot start a sibling read | Match → AR-D6/M6. Nonmatch → V5 before ET-17d may resend. Failure → AR-D8. Any new Direct evidence returns to RO-S without settling the attempt |
| RO-K — known | R6 response or AR-D6 exact match proves commit; AR-D7 exact nonmatch proves the submitted value absent at that observation | Commit evidence enters M6; nonmatch may authorize only the V5-gated user Retry already registered | Settle/drop suspended evidence only at this exact Route authority, explicit user abandonment of an observable noncommitted comparison, or page-session reload |
...
| AR-D2 — Direct AgentSupply | Agents succeeds in the pre-barrier epoch with exact backend `mode: direct` | No Route commit barrier; member settles. Preserve the origin's independent Route outcome: definitive rejection stays rejected; only unknown enters RO-S | Mode is decisive for page form only and is already the authoritative landing read | AS-1/LF-D installs that AgentSupply; for an unknown origin retain its Route generation, submitted pairs/stage and every settled Source | Agents for landing; an unknown Route observation is suspended, not settled or failed | None while Direct; a suspended attempt is visibly non-retryable | Cancel/disown only illegal chain work; ignore that generation's late result; retain RO-S only for an unknown origin | Actual Direct page; no mutation Retry. If RO-S exists, the first authoritative Hub landing enters LF-H then RO-R |
...
| AR-D5 — chain becomes Direct | Chain settles `direct_mode`; Source may be pending, successful or failed in its own epoch | No Route commit barrier; member settles. Preserve the origin's independent Route outcome: only an unknown attempt enters RO-S | Chain Direct takes the common Direct page-form edge but proves no mutation outcome | Hold illegality evidence and every successful Source/Agents member; when origin is unknown also retain its generation/pairs/stage. The error is no landing payload | Source + authoritative landing Agents; an unknown exact Route observation is suspended while Direct | Landing Agents read unless the decisive member itself was Agents | Preserve successful Source; disown only pending comparison work; ignore its late results; keep RO-S only for an unknown origin | ET-18a family → page Partial or retained committed modal; Retry only landing reads, never the mutation. If RO-S exists, LF-H later enters RO-R |
| AR-D6 — exact chain match / barrier | Exact chain matches submitted pairs; pre-barrier Source/Agents may already have settled | This matching observation establishes the commit barrier; its chain is M6-current | Exact equality proves commit before sibling failure | Hold AgentChain as D-36 evidence; keep every pre-barrier success installed as latest-known only | M6 Source, Agents and Route-chain projections | Acquire one barrier-or-later epoch for every applicable pre-barrier Source/Agents success or pending/failure; the barrier chain is current | Supersede/disown each pre-barrier member generation only when its successor is acquired; ignore predecessor late results | ET-17c → M6; Retry only a settled barrier-or-later failed subset |
| AR-D7 — differing chain, Source success | Nonmatching chain + successful current Source in the same pre-barrier generation | No barrier; both required comparison members settle | No commit evidence; resend authority is complete | Install Source/chain as latest reversible authority and run V5 | Agents, Sources and exact chain | None | Settle before ET-17d/ET-17e; ignore duplicates | V5-valid → ET-17d; invalid → ET-17e; later failed PUT starts a new AR-D generation |
| AR-D8 — comparison incomplete | Non-Direct chain failure, or nonmatch + Source failure | No barrier; every higher-precedence candidate has settled | Failure is now selectable | Retain every successful member already installed/held | Agents, Sources and exact chain | Exact failed member set | Settle; no success is rolled back | ET-17b; Retry begins Agents-first successor through ET-16a |
| AR-M1 — applicable Hub member settles | Active M6 + Source, Hub Agents or Route-chain member observed at/barrier-or-later | Barrier already held; member is M6-current only when its epoch is not earlier | Hub keeps the complete applicable set | Install/hold that exact member; Hub Agents invokes AS-6a/AS-6b | Source + Agents + Route chain | Every still-pending/failed applicable barrier-or-later member | Sibling failure cannot undo installation; ignore predecessor-epoch results | Mounted report/committed phase or receiving Hub page; no Retry while pending |
...
| RT-8 — suspended unknown later matches | Lose a Route PUT response, observe exact Direct, then later receive authoritative Hub and an exact chain equal to held submitted pairs | LF-D installs Direct while RO-S retains the attempt. LF-H closes its Source read set, RO-R sends one chain GET, equality enters AR-D6's commit barrier/M6 and no PUT is resent |
| RT-9 — suspended unknown later differs | The same sequence, but RO-R reads a different exact chain | Keep the current Hub projection; run V5 with the fresh Source/chain authorities. Only a valid explicit user Retry may resend through ET-17d; invalid/failed comparison never resends |
...
Neither path starts a second read owner or resends on dismissal.
...
| AR-D Agents read | ET-16a acquires one AR-D generation; RO-R acquires exactly one successor after LF-H closes its Source read | ET-8a disowns a definitive-rejection generation. ET-8b transfers unknown evidence to RO rather than destroying it; ET-18a/AS-1 suspend its Route outcome on Direct evidence | Ignore results after explicit disown or successor acquisition. Direct cancels illegal Route work but preserves RO-S's generation/pairs/stage until Hub resumes observation |
...
| Error-handoff Agents read | ET-18a acquires the page-owned generation; ET-18c settles the failed generation and acquires a successor | Page replacement or a successor disowns the prior generation | Ignore every prior-generation result; a current success alone enters LF-D/LF-H and its AS install |
...
| ET-8b — unknown-outcome exit | Route save outcome unknown + immutable attempt evidence + latest installed page authority | Send no mutation/read; transfer the dialog generation to the page-session RO owner | Discard modal-only transients; retain generation, submitted pairs/stage and unknown marker. Disown only any currently pending request; a later legal successor is RO-R's alone | Latest installed page projection with its exact row marked Chain unresolved, or actual Direct page with RO-S held off-row | `route.fail.unconfirmed` hands off to the Hub row's unresolved treatment; Direct renders no false Route verdict | Preserve an FF-1-valid page target, otherwise PF-1 after modal close | Hub → D-35/RO-U; Direct → RO-S until LF-H resumes once |
...
| ET-15 — no response evidence | Saving + submitted draft + initial/confirmed stage | Promote no response fact to commit/refusal | Release PUT ownership; retain immutable attempt evidence | Route save outcome unknown | `route.fail.unconfirmed` | `route.retry` | None until Retry |
...
| ET-18a — reversible decisive `direct_mode` handoff | Loading, Saving or AR-D5 + matching DM-1/DM-2/DM-3 evidence and no commit barrier | Error evidence selects no page form; retain every successful independent member and start/transfer the page's Agents read. Preserve the origin's independent Route outcome; only unknown enters RO-S | Close modal/selector and cancel grab. Discard editable presentation; for an unknown origin retain generation/submitted pairs/stage outside the modal. Disown only pending illegal Route siblings | Page §1.0 Partial over every latest successful Source/Agents projection | Existing Partial copy; never label Direct from the error or claim a Route outcome | Stable owning backend group head | Page Agents read; result enters LF-D/LF-H. Every late disowned sibling result is ignored |
...
| ET-18b — reversible error-handoff Agents read failure | ET-18a Partial + last-good page projection + optional RO-S evidence | Classify only the read failure | Clear pending; keep last-good projection explicitly stale and retain RO-S unchanged; CA-P1 admits page Retry | Page §1.0 Partial/F2 | Existing F2 cause and page Retry | FF-1-valid page Retry | None until ET-18c; M6 landing-read failure instead remains in AR-M and settles through ET-19a/ET-19c |
| ET-18c — retry error-handoff Agents read | ET-18b + stale page projection + CA-P1 admission | Reissue only `GET /api/models/agents` | Retain stale projection and RO-S evidence; CA-P2 owns focus transfer plus Retry busy/inert admission | Page §1.0 Partial pending | Existing F2 copy | CA-P2 handoff | Page Agents read; success enters LF-D/LF-H and its AS install |
...
| AS-1 — AR-D2 Direct install | AR-D2 + exact AgentSupply `mode: direct` | LF-D installs that exact Direct row before any Source/chain read | Settle the Agents member and close modal. Preserve a definitive rejection unchanged; for an unknown origin transfer its Route generation/pairs/stage into RO-S instead of discarding them | Actual Direct page; an unknown origin additionally holds the suspended Route outcome at page-session grain | Exact Direct projection; mode makes no Route claim | PF-1 because the modal target unmounts | No read while Direct; when RO-S exists, first authoritative LF-H landing enters RO-R |
...
| AS-3 — reversible error-handoff Direct dispatch | ET-18a/ET-18c + exact AgentSupply `mode: direct` | LF-D installs that exact Direct row and keeps every successful independent Source authority already carried | Clear page-read pending and stale Hub row; retain RO-S unchanged when present | Actual Direct page | Exact Direct projection | Preserve an FF-1-valid page target or use PF-1 | No companion read; an M6 landing read dispatches through AS-5a/AS-5b instead |
| AS-4 — reversible error-handoff Hub dispatch | ET-18a/ET-18c + exact AgentSupply `mode: hub` | LF-H installs that exact Hub row, keeps successful independent authority and starts M5's Source read; never resurrect the editor | Clear Agents pending; retain the old Source list explicitly stale until M5 settles and retain RO-S when present | Actual Hub page with Sources pending/stale until LF-H closes | Exact Hub projection plus `upstream.unread` only after Source failure | Preserve an FF-1-valid page target or use PF-1 | M5 Source read; success enters RO-R exactly once when RO-S exists. M6 landing uses AS-6a/AS-6b instead |
...
temporarily illegal but proves neither commit nor non-commit. RO-S therefore keeps the
generation, submitted pairs/stage and unknown evidence at mounted-page-session grain across
Direct/Hub forms; the first authoritative Hub landing closes LF-H's Source read and then
RO-R resumes exactly one chain GET. Reload discards this client-held evidence and never
permits a blind resend or creates a persistent reconciliation ledger. A differing chain permits one resend only on the user's Retry after the
