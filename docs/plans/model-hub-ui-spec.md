...
| G-13 | 03 order drawer, chains that already exist — **registered by the global priority save** | ~~an action that re-applies the current order to chains built before it~~ ~~the route behind it~~ | `POST /api/models/agents/<backend>/chains/reorder` is the sole explicit operation that stores a supplied Source order and reorders every stored Route by it. The combined mutation is idempotent, adds/removes/remaps nothing and has no guard; a failure leaves the drawer draft untouched for retry. |
...
extra: §1.3's 保存顺序 sends the same complete order to the atomic chains/reorder
operation, so retrying the save lands the same result without a persisted half-state.
...
| §1.3 | Saving | 保存顺序 pressed — send `{order: string[]}` to `POST /api/models/agents/<backend>/chains/reorder`, which stores the order and applies it to existing Routes in one mutation `[contract]` | F1 | `order.save`, `order.fail.save`, `order.retry` | Success → close; failure → keep every move held and 重试 the same save |
...
| §1.5 | ① Default | Dialog opened; Base URL or API key empty | F5 | `addKey.title`, `addKey.subtitle`, `addKey.field.name`, `addKey.field.baseUrl`, `addKey.field.baseUrl.hint`, `addKey.field.apiKey`, `addKey.field.apiKey.reveal`, `addKey.field.apiKey.conceal`, `addKey.field.protocol`, `addKey.protocol.auto`, `addKey.protocol.idleHint`, `addKey.protocol.manual`, `addKey.detect`, `addKey.cancel` | Interface type is a dimmed result area, not a required input. The collapsed disclosure `addKey.protocol.manual` holds Auto plus the three concrete interfaces as a probe constraint, never proof. Auto carries no glyph; a concrete option carries its protocol-family glyph. 检测 is disabled. 取消 → dismiss |
...
upstream model id, and exclude duplicate pairs. The selector owns exactly one active
...
| RS-8 — shaped rejection | a non-guard, non-`direct_mode` error response | ET-14 settles that attempt terminal-rejected. Retry is a new explicit workflow: revalidate the retained draft, freeze a new submitted generation and send one ordinary **non-forced** PUT; any new guard starts its own Qp6FI phase. It never runs D-36 or reuses prior force/plan evidence |
| RS-9 — no response / no Route authority | transport no-answer or `direct_mode` without an exact Route result | ET-15/ET-18a set attempt settlement `unknown`. D-36 may read only: matching proves commit; nonmatching or failed observation never proves the old PUT terminal and never emits a recovery PUT. The user may abandon; any later edit begins a distinct workflow from newly read authority |
...
starts M6 with response-only tails unavailable. A nonmatching chain is installed as the
...
`Hub-observable | Direct-suspended`. The immutable submitted pairs/stage and workflow
...
| --- | --- | --- | --- |
| RO-R — terminal rejection | `(terminal-rejected, mounted editor, *)` | No D-36 read is legal. CA-R1 may admit an explicit Retry only while the retained draft passes ordinary V5; activation freezes a **new** workflow generation and sends its first PUT | The old attempt remains terminal-rejected and is never resumed. Invalid V5 disables Retry and renders its ordinary row feedback; Cancel abandons without mutation |
...
| ET-14 — definitive non-guard rejection | Saving + submitted draft; DM-2 is reserved for ET-18a | Exclude guard and `direct_mode`, then settle this attempt `terminal-rejected` | Release PUT ownership; retain only the locally validated draft for an explicit new workflow | Route save rejected | `route.fail.save` | FF-1-valid `route.retry` | None; rejection never enters D-36 |
| ET-14a — explicit new attempt after rejection | Route save rejected + retained draft + CA-R1 admission | Re-run ordinary V5; when valid create a new workflow generation and send the ordinary non-forced PUT body. The prior terminal attempt supplies no request identity, force flag, plan echo or replay right | Clear old rejection feedback only as the new generation acquires PUT ownership | Saving | `route.saving` | Programmatically focusable progress status | One new owned Route PUT; any returned guard begins a new Qp6FI phase |
| ET-15 — no response evidence | Saving + submitted draft + initial/confirmed stage | Set attempt settlement `unknown`; infer neither commit nor refusal | Release PUT ownership; retain immutable attempt evidence | Route save outcome unresolved | `route.fail.unconfirmed` | `route.retry` | None until a read-only Retry |
...
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain equal to submitted pairs | The exact AgentChain is already installed; record `matching@epoch`, settle the attempt `terminal-committed`, mark response-only tails unavailable and establish CF-R before transferring to AR-M | Clear comparison busy; discard obsolete read Retry state; retain earlier successes as latest-known but frontier-insufficient | Route committed, reconciling | `route.refreshing` | FF-1-valid `route.impact.done` | Acquire Agents first; LF-H then establishes CF-H and acquires Sources. Keep the matching chain self-satisfied |
...
**It governs global route priority** `[spec]`. `POST /api/models/agents/<backend>/chains/reorder`
stores the supplied complete order and applies the current Source-order sequence to every
stored Route with a stable total sort, without adding, removing or remapping a hop
`[contract]`. The combined mutation is atomic; if it fails, the drawer keeps the draft and
retries the same save, so no partial success is presented as complete.
...
at all** `[contract]`. The drawer sends the complete order to the explicit
`POST /api/models/agents/<backend>/chains/reorder` route. That response is the
authoritative `AgentSupply` echo after the order has been stored and existing Routes
have been stably reordered in one mutation. A source removed from the priority list
remains a valid Route hop when it is still present in a chain; the reorder operation only
places listed Sources first and preserves every hop pair. There is nothing for a guard to
refuse, and a failed request leaves the draft available for retry.
...
The `chains/reorder` request persists the complete Source order and applies it to every
existing Route using the stable sort defined in `model-hub.md` §4.6. Therefore the first
source is tried first and the next priority is used automatically for fallback-eligible
quota, rate-limit, server, authentication, and network failures. If the request fails,
the drawer keeps the same draft and retries it; no partial priority state is visible.
...
did not happen and nothing about why. The save sends the complete order to the atomic
reorder operation, while the drawer keeps every move the user made, and the frozen rule against enumerating request-level failure
...
applies it to existing Route hops (§4.6), so the first listed upstream is tried first and
the next priority is used automatically for fallback-eligible quota, rate-limit, server,
...
- **Empty order, held-out sources remaining** `[derived]`: the *ordered* section renders
  ...
  stops being supplied. This save stores the order and stably reorders existing
...
- Field order is 名称(可选) → Base URL → API Key → 接口类型. Interface type is a
  **detection-result area**, not a required input. Idle is dimmed 自动探测 +
  `addKey.protocol.idleHint`; Ready is the same row undimmed; Detecting is a spinner
  with `addKey.protocol.detecting`; Identified is a mint strip naming the proved
  protocol (protocol-family glyph before the label) and the model count — never the
  model names.
...
- Manual override is a collapsed disclosure `addKey.protocol.manual`. A concrete
  ...
  in place; retry stays disabled while Auto is selected.
...
| Interface type | result area last: idle/ready 自动探测 row, detecting spinner, identified mint strip, or ④'s gold strip; protocol-family glyphs on every concrete protocol name | observation result; disclosure selection is a probe constraint | disclosure yes; ④ selector yes | Open `addKey.protocol.manual`; a concrete choice constrains the next 检测 / ④ 重试 |
...
| persist strip state ②″ | spinner, 保存中…, 正在保存这个来源 | in-flight persist | none — cancel blocked | — |
...
optional `display_name`, normalized `base_url`, the selected `protocol` constraint when
...
models and health. Thus ④'s pick remains a probe constraint rather than authority, and ⑤'s
...
| `saving` | 保存中… | Saving… |
| `saving.detail` | 正在保存这个来源 | Saving this source |
...
executes the exact hop order stored for each model. The drawer's atomic chains/reorder
save is the explicit bridge: it stores the priority and applies it to existing Routes
without changing hop membership or mappings. This decision assigns each surface
