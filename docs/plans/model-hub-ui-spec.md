...
| M6 | R6 route replacement, or an authoritative exact ordered AgentChain from `GET /api/models/agents/<backend>/chain?model=<id>` matching submitted `hops` under D-36 | `Source[]` including `adopted_by`, complete `AgentSupply[]` including mode/model supply and any present routes projection, and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | R6 receipt or AR-D6's matching exact-chain observation establishes the attempt's commit barrier and starts AR-M. The barrier-defining returned/read chain is current at that barrier. A Source or Agents success observed before it remains monotonically installed as latest-known authority but cannot satisfy M6-current; every still-applicable such member acquires one barrier-or-later observation epoch. Keep R6's entire envelope when received; inference marks `removed_hops` / `interrupted` unavailable. AP records `AgentSupply.routes` as optional and deliberately unconsumed here | AR-M consumes each successful applicable member monotonically but declares it M6-current only at the barrier-or-later epoch. Either exact AgentSupply Direct or chain `direct_mode` takes the same decisive edge: retain every successful independent member, disown only pending/not-applicable Route work, and acquire an authoritative Agents landing read whose exact returned mode alone selects the page form. CA admits Retry only after settlement, for exactly the applicable failed subset, and never overlaps generations. Mounted completion retains report/evidence and Done; transferred completion preserves any FF-1-valid active target and otherwise uses PF-1. ET-20 alone acknowledges/closes/transfers; no path resends or questions the route write |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or AR-D6 proves the desired Route from the exact chain endpoint while response-only arrays are unavailable; exact commit evidence is held while the active AR-M generation has pending applicable members or pre-barrier successes awaiting their required barrier-or-later epoch | F2 applies only to the applicable members that AR-M3 later marks stale; the Route write/desired state is already proven | `route.refreshing`, `route.impact.done` | ET-11 or ET-17c owns mutually exclusive entry; AR-M/AR-MD, ET-18aM/ET-18aP, ET-19, AS and ET-20 own member epochs, Direct handoff, Retry, page installation and the sole user exit |
| §1.2 | Route committed, refreshed `[derived]` | AR-M2 completed a mounted report-free M6 generation with every applicable member current and installed its exact page behind the still-mounted modal; commit evidence remains held until acknowledgement | F5 — no request remains and no impact report exists | `route.impact.title`, `route.impact.done` | ET-20 is the only exit. The page projection is already installed, and modal close invokes PF-1 because Done unmounts |
...
| §1.2 | Route save rejected `[derived]` | The server definitively rejected the route write outside the guarded-plan and `direct_mode` branches, so this attempt did not commit | F1, in place | `route.fail.save`, `route.fail.reconcileRead` after AR-D1/AR-D8, `route.retry`, `route.cancel` | ET-14, AR-D/ET-16, AS-1/AS-2, ET-17, ET-18a and ET-8a own entry, reconciliation, every outcome and the non-mutating exit. CA admits Retry only without an active generation |
...
observation epoch, member settlement, decisive page evidence)`; RO's separate product key is
`(Route outcome, owner, observation legality)`. Page-form authority, member settlement and
Route-mutation outcome are orthogonal: a mode result may settle and install the page while
the Route outcome stays unknown. Each independently acquired member records pending, successful
authority, failure or not applicable plus the epoch at which that observation began. A
successful result is installed/held immediately and is never erased because a sibling
failed. R6 receipt or the first matching exact-chain observation establishes the commit
barrier: pre-barrier successes remain latest-known installed authority, but no such member
can satisfy M6-current. Every member still applicable after the barrier must have one
barrier-or-later observation generation; the barrier-defining returned/read chain itself
satisfies that condition. Decisive evidence then recomputes the applicable-member set before
completeness, stale or Retry is evaluated. A member removed from that set is disowned, not
failed, and only pending work is cancelled; a success already received remains installed.
At most one generation per member owns reads for a workflow. Starting its successor
invalidates the prior member generation, and every late result from an invalidated or
disowned generation is ignored.
...
error itself is no landing payload. LF-H's Source member settles before the Hub projection is
called current. In Hub,
...
| LF-H | Any exact returned `AgentSupply.mode: hub`, including ordinary M5 PATCH, DM, AR, AS and later page refresh producers | Install that AgentSupply immediately and require one current Source member before calling the Hub landing current. Outside proven M6 this is M5's one Source read; inside M6 the already-owned barrier-or-later Source member satisfies the same requirement without a duplicate read | Source failure keeps AgentSupply installed, marks only Sources stale and offers only the owning Source read Retry. Every successful current Source settlement invokes one common post-Source hook: set `legality = Hub-observable`; if RO is `(unknown, page session, Hub-observable)` and owns no exact-chain generation, acquire exactly one through RO-O before any Route resend |
...
| --- | --- | --- | --- |
| RO-O — observable unknown | `(unknown, mounted editor, Hub-observable)` or `(unknown, page session, Hub-observable)` with no current exact-chain generation | Admit exactly one exact-chain observation for the held pairs. Mounted entry is acquired/settled as an AR-D member; page-session entry is acquired by the producer-independent LF-H post-Source hook and settled by RO's owner table. A second Hub payload cannot acquire a sibling generation | Match → `committed` and M6. Nonmatch → `observed-nonmatch`: mounted AR-D runs V5 and may expose ET-17d; page-session RO treats the dismissed change as abandoned, keeps the exact Hub projection and exposes no mutation Retry. Read failure preserves `unknown` and admits only a read retry owned by that same surface |
...
| --- | --- | --- | --- |
| Mounted editor + match | Set outcome `committed`, establish AR-D6 barrier and keep the modal mounted in Route committed, reconciling | `route.refreshing`; focus `route.impact.done` | M6/AR-M |
| Mounted editor + nonmatch | Set outcome `observed-nonmatch`, keep latest Hub authority and run V5 without closing the editor | V5-valid keeps registered failure/draft copy with admitted Retry; invalid uses `route.invalidAfterRefresh` and its first FF-1-valid Remove action | Valid → ET-17d only on explicit user Retry; invalid → ET-17e; no automatic PUT |
...
| Page session + match | Set outcome `committed`, retain exact Hub page and install commit evidence without remounting the editor | Page Chain-unresolved marker clears; preserve current FF-1-valid page target or PF-1 | M6 as page-owned reconciliation |
...
| ID / workflow generation | Independently settled member + observation epoch | Commit barrier / Route outcome | Decisive page evidence | Immediately installed / held authority | Applicable set after decisive evidence | Awaited / stale members | Cancellation / late result | Destination / Retry subset |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AR-D1 — initial Agents failure | Agents failed in the pre-barrier reconciliation epoch | No commit barrier; Agents settles failed | No mode or Route evidence exists | Keep latest installed page projection and immutable draft/stage | Agents | Failed Agents read | Settle generation; no sibling exists | ET-16b; Retry Agents only through ET-16a |
| AR-D2 — Direct AgentSupply | Agents succeeds in the pre-barrier epoch with exact backend `mode: direct` | No Route commit barrier; member settles. Preserve RO outcome and owner, changing only legality to Direct-suspended | Mode is decisive for page form only and is already the authoritative landing read | AS-1/LF-D installs that AgentSupply; if RO outcome is unknown retain its Route generation, submitted pairs/stage and every settled Source | Agents for landing; an unknown Route observation is suspended, not settled or failed | None while Direct; a suspended attempt is visibly non-retryable | Cancel/disown only illegal chain work; ignore that generation's late result; retain the complete RO product state | Actual Direct page; no mutation Retry. Any later LF-H post-Source hook re-evaluates RO-O from product state |
| AR-D3 — Hub AgentSupply | Agents succeeds in the pre-barrier epoch with `mode: hub` | No commit barrier yet; Agents settles successful | Hub permits exact reconciliation but proves no Route equality | AS-2 installs Hub as reversible page origin; retain immutable draft/stage | Agents, current Sources and exact AgentChain | Current Sources + exact AgentChain | ET-17a acquires both in this workflow generation; AP-2 stays unconsumed | Originating failure, pending; no Retry while active |
| AR-D4 — Hub comparison pending | AR-D3 plus independently settling pre-barrier Source/chain members | No commit barrier until an exact match; wait for every higher-precedence member | No terminal choice yet | Retain AS-2 and install/hold every successful member | Agents, current Sources and exact AgentChain | Pending or failed Source/chain slots | Own both reads until AR-D5–AR-D8 or RL disown | Originating failure, pending; no Retry while active |
| AR-D5 — chain becomes Direct | Chain settles `direct_mode`; Source may be pending, successful or failed in its own epoch | No Route commit barrier; member settles. Preserve RO outcome/owner and change only legality to Direct-suspended | Chain Direct takes the common Direct page-form edge but proves no mutation outcome | Hold illegality evidence and every successful Source/Agents member; if outcome is unknown retain its generation/pairs/stage. The error is no landing payload | Source + authoritative landing Agents; an unknown exact Route observation is suspended while Direct | Landing Agents read unless the decisive member itself was Agents | Preserve successful Source; disown only pending comparison work; ignore its late results; keep the complete RO product state | ET-18a family → page Partial or retained committed modal; Retry only landing reads, never the mutation. Any later LF-H post-Source hook re-evaluates RO-O |
| AR-D6 — exact chain match / barrier | Exact chain matches submitted pairs; pre-barrier Source/Agents may already have settled | Set RO outcome `committed`; this matching observation establishes the commit barrier and its chain is M6-current | Exact equality proves commit before sibling failure | Hold AgentChain as D-36 evidence; keep every pre-barrier success installed as latest-known only | M6 Source, Agents and Route-chain projections | Acquire one barrier-or-later epoch for every applicable pre-barrier Source/Agents success or pending/failure; the barrier chain is current | Supersede/disown each pre-barrier member generation only when its successor is acquired; ignore predecessor late results | ET-17c → M6; Retry only a settled barrier-or-later failed subset |
| AR-D7 — differing chain, Source success | Nonmatching chain + successful current Source in the same pre-barrier generation | Set RO outcome `observed-nonmatch`; no barrier exists and both required comparison members settle | No commit evidence; only mounted owner may expose V5-gated resend authority | Install Source/chain as latest reversible authority and run V5 | Agents, Sources and exact chain | None | Settle before ET-17d/ET-17e; ignore duplicates | Mounted V5-valid → ET-17d; invalid → ET-17e. Page-session owner instead uses RO-O's abandonment row and never reaches AR-D7 |
| AR-D8 — comparison incomplete | Non-Direct chain failure, or nonmatch + Source failure | Keep RO outcome `unknown`; no barrier exists and every higher-precedence candidate has settled | Failure is now selectable | Retain every successful member already installed/held | Agents, Sources and exact chain | Exact failed member set | Settle; no success is rolled back | ET-17b; Retry begins Agents-first successor through ET-16a |
| AR-M1 — applicable member settles, mode-neutral | Active M6 + any applicable Source, Agents or Route-chain member returns success or failure at/barrier-or-later | Barrier already held; a successful member is M6-current only when its epoch is not earlier; a failure settles only that member at its epoch | Only an exact successful Agents mode payload may select LF/AS page form. Source success/failure, Agents failure and non-Direct Route settlement carry no page-form evidence | Install/hold a success monotonically; mark only a failed member stale. Successful Agents invokes LF/AS; every other result updates only its member authority/staleness | Recompute from the already-installed decisive mode; under Direct, Source + Agents remain applicable and Route is not applicable; under Hub, Source + Agents + Route remain applicable | Every still-pending applicable member plus the exact settled failed subset | Sibling failure cannot undo a success; ignore predecessor-epoch results | Keep the currently installed Direct/Hub page and mounted/transferred ownership unchanged. A late Direct-mode Source success/failure settles only Source and never resurrects Hub; after all pending settle AR-M2 or AR-M3 alone selects completion/Retry |
| AR-MD — decisive Direct | Active M6 + current AgentSupply `mode: direct` **or** Route-chain `direct_mode` | Barrier remains held; Route slots are provisionally not applicable until the landing Agents member settles | Both producers take this one reducer edge; the landing AgentSupply then recomputes applicability from its exact mode | Preserve every successful independent member. A current Direct AgentSupply is itself the authoritative landing member; chain Direct acquires that read through ET-18aM/ET-18aP | Landing Direct: Source + Agents, Route not applicable. Landing Hub: Source + Agents + a newly acquired post-landing Route chain | Landing Agents when chain supplied Direct; then Route chain too if that AgentSupply returns Hub | Cancel/disown only prior pending Route work and ignore its late results; never discard a successful Source/Agents member | Mounted report remains mounted or receiving page enters Partial until AS dispatch; Retry only the current applicable failed subset |
| AR-M2 — all applicable members current | Active M6 + no pending/failed applicable member and every member has barrier-or-later authority | Complete applicable surface settles reconciliation | The latest exact mode-selected set controls page form | Keep every exact applicable member and commit evidence | Latest mode-selected set | None; not-applicable Route members are neither current nor stale | Settle generation; ignore duplicate/disowned/predecessor results | Mounted evidence remains until ET-20 or transferred page remains; Retry absent |
| AR-M3 — applicable failed subset settles | Active M6 + no pending applicable member + one or more barrier-or-later applicable failures | Barrier held; applicable generation settles partially current | Successful mode/authority remains decisive | Keep every successful applicable member installed and commit evidence held | Latest mode-selected set | Exact failed applicable subset, visibly stale | Settle; ignore later results from it | Mounted → ET-19a; transferred → ET-19c; Retry exact failed subset |
| AR-M4 — retry applicable failed subset | Settled AR-M3 + explicit admitted Retry | Barrier retained; successor epochs are pending | No prior success is reread or discarded | Retain installed members/evidence | Recompute from held decisive mode before issuing | Prior applicable failed subset becomes pending | CA first hands focus off, then disables Retry; predecessor/disowned results are ignored | Same mounted/receiving phase; Retry exactly the failed subset |
...
| --- | --- | --- |
| RT-1 — inferred commit after early reads | AR-D Source and Hub Agents settle before the lost PUT commits; the later exact-chain read matches | Install the early successes as latest-known only, establish the matching-chain barrier, and acquire one post-barrier Source and Agents epoch before AR-M2 |
...
| RT-11 — every Hub producer runs one hook | Table-drive every authoritative Hub landing producer registered now (ordinary M5 PATCH, DM, AR, AS and page refresh) plus a fixture producer added to the table | After its current Source settlement, each invokes the same product-state hook. With `(unknown, page session, Hub-observable)` exactly one RO-O chain generation starts; without that state none starts. Producer identity is neither input nor branch |
...
| AR-D Source + exact-chain reads | ET-17a attaches both to the current AR-D workflow generation, recording a distinct member epoch for each | AR-D5 settles the chain evidence, preserves any successful Source, and disowns only pending comparison work before ET-18a. AR-D6 establishes the commit barrier and acquires barrier-or-later successor epochs for every still-applicable pre-barrier member; ET-8a/ET-8b disown the full workflow; AR-D7/AR-D8 otherwise settle it | A result after member disown/supersession cannot alter the selected AR outcome; successful authority received before a decisive edge stays installed but cannot satisfy a later M6 barrier |
| AR-M Source, Agents and Route-chain reads | ET-10/ET-11 starts the barrier generation; ET-17c transfers its commit barrier and acquires every required barrier-or-later member successor; ET-19b/ET-19d acquires one failed-subset successor | ET-20 transfers rather than cancels. AR-MD from either Agents Direct or chain `direct_mode` preserves successful independent members, cancels/disowns prior pending Route slots and uses ET-18aM/ET-18aP when a fresh landing Agents read is required. Landing Direct keeps Route disowned; landing Hub acquires a new Route member epoch after that Agents observation. A successor disowns its predecessor member generation | Ignore disowned Route and predecessor-epoch results; only applicable barrier-or-later members can settle AR-M |
...
| ET-10 — R6 success with impact | Saving + exact R6 envelope with either array nonempty | Consume every R6 member, mount its report and start one AR-M generation | Release PUT/refusal ownership; retain the entire envelope | Route impact reported with AR-M pending | Impact title/detail and each nonempty block | `route.impact.done` | AR-M model-surface members |
| ET-11 — two-empty R6 commit | Saving + exact R6 envelope whose two impact arrays are empty | Preserve the exact envelope and start one AR-M generation | Release PUT ownership; no origin can be restored | Route committed, reconciling | `route.refreshing` | `route.impact.done` | AR-M model-surface members |
...
| ET-17b — AR-D8 incomplete comparison | AR-D8 selected after every higher-precedence slot settled | No commit verdict or resend basis exists | Clear active generation/busy; retain every successful authority/draft and the latest installed page projection | Originating failure | `route.fail.reconcileRead` | FF-1-valid `route.retry` | None until a newly admitted ET-16a |
| ET-17c — AR-D6 matching chain proves commit | AR-D6 selected from an exact AgentChain equal to submitted pairs | Set RO outcome `committed`; hold AgentChain, mark response-only tails unavailable, establish its observation as the commit barrier and transfer the workflow to AR-M | Clear comparison busy; discard obsolete retry/refusal state; retain pre-barrier successes as latest-known but not M6-current | Route committed, reconciling | `route.refreshing` | FF-1-valid `route.impact.done` | Acquire one barrier-or-later read for every applicable Source/Agents member observed before the barrier; keep the matching chain itself current |
| ET-17d — AR-D7 valid comparison resends | AR-D7 selected after nonmatching chain + Source success and refreshed V5 remains valid | Freeze the held initial/confirmed stage and send its registered PUT body | Settle AR-D; retain the latest installed page authority and exact confirmed arrays when required | Saving | `route.saving` | Programmatically focusable progress status | One owned Route PUT; no reconciliation generation remains active |
| ET-17e — AR-D7 invalid comparison | AR-D7 selected after nonmatching chain + Source success and refreshed V5 invalidates a new/changed pair | Install refreshed Source/chain authority; send no PUT | Settle AR-D; clear submitted/refusal state; retain editable rows and AS-2's latest Hub page | Invalid after refresh | `route.invalidAfterRefresh` on every offender | First offending row's FF-1-valid Remove action | None |
| ET-18a — reversible decisive `direct_mode` handoff | Loading, Saving or AR-D5 + matching DM-1/DM-2/DM-3 evidence and no commit barrier | Error evidence selects no page form; retain every successful independent member and start/transfer the page's Agents read. DM-2 first sets RO outcome `unknown`; every handoff changes owner to page session and legality to Direct-suspended without changing any already-known outcome | Close modal/selector and cancel grab. Discard editable presentation; for unknown retain generation/submitted pairs/stage outside the modal. Disown only pending illegal Route siblings | Page §1.0 Partial over every latest successful Source/Agents projection | Existing Partial copy; never label Direct from the error or claim a Route outcome | Stable owning backend group head | Page Agents read; result enters LF-D/LF-H. LF-H's post-Source hook re-evaluates RO-O; every late disowned sibling result is ignored |
| ET-18aM — mounted M6 chain-Direct handoff | Mounted report/committed modal + AR-MD selected from chain `direct_mode` | Keep the barrier/evidence and successful independent members; start the landing Agents read without deriving a page form from the error | Retain modal and enabled Done; disown only prior pending Route work | Same mounted report/committed phase over last installed page until AS-5a/AS-6a | Existing report/refresh copy | Keep FF-1-valid `route.impact.done` | Landing Agents dispatches mode; Hub then acquires one new Route epoch, Direct does not |
| ET-18aP — transferred M6 chain-Direct handoff | Receiving page + AR-MD selected from chain `direct_mode` | Keep the barrier/evidence and successful independent members; start the page-owned landing Agents read | Retain installed page members; disown only prior pending Route work | Page §1.0 Partial over the last installed page | Existing Partial copy plus held committed evidence | Preserve current FF-1-valid page target, otherwise page fallback | Landing Agents dispatches mode; Hub then acquires one new Route epoch, Direct does not |
...
| AS-2 — AR-D3 Hub install | AR-D3 + exact Hub AgentSupply | Install that exact Hub row as the latest reversible page authority; AP-2 is deliberately not consumed for commit comparison | Clear only Agents-member pending; retain generation/modal/draft/stage and CA-D2 admission | Originating failure over actual Hub page | Existing failure copy | Preserve CA-D2's FF-1-valid owner | ET-17a Source/chain reads in the same generation |
...
| AS-6a — mounted AR-M Hub install | Mounted evidence + current successful M6 AgentSupply `mode: hub`, including the landing read after chain Direct | Dispatch mode immediately and install the exact current Hub page behind the modal | Retain modal/evidence and independent authority. If this is AR-MD's landing read, recompute Route as applicable and acquire one new post-landing chain epoch | Same mounted report/committed phase over actual Hub page | Registered report/refresh/stale block for the AR-M phase | Keep FF-1-valid `route.impact.done` | AR-M continues current applicable members, including the new chain, or CA admits a failed subset after settlement |
| AS-6b — transferred AR-M Hub install | Transferred AR-M + current successful M6 AgentSupply `mode: hub`, including the landing read after chain Direct | Dispatch mode immediately and install the exact current Hub page | Retain evidence and independent authority. If this is AR-MD's landing read, recompute Route as applicable and acquire one new post-landing chain epoch | Actual Hub page with exact pending/stale markers | Exact Hub projection plus registered pending/stale block | Preserve the current active page target only if FF-1-valid; otherwise PF-1 | AR-M continues current applicable members, including the new chain, or CA admits a failed subset after settlement |
...
| Route save rejected / outcome unknown | 重试 | `route.retry` | CA-D1 enabled only with no AR-D generation; CA-D2 renders it busy and disabled/inert throughout ET-16a/AS-2/ET-17a ownership | First command in Tab order only when enabled; activation delegates to the current CA row |
...
forms. Every authoritative Hub landing closes LF-H's Source read and invokes the same
producer-independent hook; `(unknown, page session, Hub-observable)` acquires exactly one
chain GET. A matching read enters M6; a nonmatch after dismissal settles abandonment without
resend; a failed read retains evidence and permits only a chain-read retry. Reload discards
this client-held evidence and never permits a blind resend or creates a persistent
reconciliation ledger. A differing chain in the still-mounted editor permits one resend only on the user's Retry after the
registered Source + chain comparison settles; a failed read permits none. Either a received
