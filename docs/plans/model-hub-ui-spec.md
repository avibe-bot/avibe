...
| C15 — page-form landing evidence | Every response, terminal or absence branch that changes the mounted page form MUST name the authoritative payload or read that selects the destination; every complete-surface read dispatches its returned `AgentSupply.mode` before installing a form, and classification evidence alone can retire an illegal surface but can never stand in for the target projection | [x] DM-1–DM-3 errors close the illegal editor and transfer to the page Agents read; DM-4 consumes the exact AgentSupply returned after ET-16a. The AS rows are the only mode dispatcher for that read, its error-handoff reread and every M6 success. PF-1 is the only post-install page-focus resolver; a mounted M6 success retains Done instead of invoking page focus | [x] R1/R2 and M0–M2 hand their exact returned or reread Source/absence evidence to the page. Their complete-surface results dispatch the returned `AgentSupply.mode` before installing a page form, then PF-1 resolves page focus; an error class selects no destination by itself | [x] R3/R4 and M0/M3/M4 hand their exact returned or reread Source evidence to the page. Their complete-surface results dispatch the returned `AgentSupply.mode` before installing a page form, then PF-1 resolves page focus; flow/error classes select failure handling, never a healthy landing | [x] N/A — the menu changes only its own reversible phase and hands one explicit local vendor selection to §1.4; no response or terminal branch changes the page form |
| C16 — atomic event-transition totality | Every state-changing event MUST register, in one owning atomic edge row, its complete authority dispatch, transient-state cleanup, visible feedback and destination focus; an unlisted transition is forbidden. One row MUST NOT contain two user events whose cleanup, destination, focus or next owner differs. Every visible-feedback value MUST cite a registered key or name an existing registered copy block | [x] The ET and AS rows are the sole edge owner for frame 02. Selector, row edit, grab, guard, reconciliation, M6 and dismissal events are split by phase; mounted and transferred M6 landings are separate. The Visible feedback column binds every value to the Route copy register or a named existing block; the state, V5, DM, control and keyboard registers define no parallel destination | [x] §1.10's state rows consume R1/R2 and M0–M2, clear their modal phase under DP-1/DP-2/DP-4, render the registered F/copy treatment and name the source overflow or receiving page as focus owner | [x] §1.11's state rows consume R3/R4 and RR/M0/M3/M4, settle flow/key transient ownership, retain the registered report/failure copy and name the invoking card or receiving projection as focus owner | [x] Closed/Open and the §1.4 handoff name the explicit vendor event, menu cleanup, visible destination and Add subscription / receiving-surface focus; no server response edge belongs to frame 13 |
...
| M6 | R6 route replacement, or an authoritative exact ordered `(source_id, model_id)` readback matching the submitted `hops` (including D-36) | The exact AgentChain, `Source[].adopted_by`, the owning backend's AgentSupply/model-supply projection and the Route-chain index; the attempt may change membership, mappings, order, runnability, current-hop presentation and page form | Start the complete model-surface read as soon as ET-10, ET-11 or ET-17c holds commit evidence. Keep R6's entire envelope when received; for readback evidence hold the matching AgentChain and mark `removed_hops` / `interrupted` unavailable. The report or report-free committed modal stays mounted while the read runs; a Done-equivalent exit may transfer that exact read owner to the page without cancelling it | ET-19a keeps a failed read in the same mounted report or report-free stale phase with `route.impact.refreshFail`; ET-19b Retry retains that phase and evidence while repeating only M6. Mounted success enters AS-5a/AS-6a, which install the exact page behind the retained modal and keep Done focused; transferred success enters AS-5b/AS-6b and resolves page focus through PF-1. ET-20 alone acknowledges/closes/transfers; no path resends, questions the route write or invents impact |
...
| §1.2 | Route committed, reconciling `[derived]` | R6 success carries two empty impact arrays, or an authoritative matching readback proves the desired Route while response-only arrays are unavailable; the exact returned/readback AgentChain and tail disposition are held while M6 reads the complete model surface | F2 → Committed projection stale; the Route write/desired state is already proven | `route.refreshing`, `route.impact.done` | ET-11 or ET-17c owns the mutually exclusive entry; ET-19a/ET-19b, AS-5a/AS-5b/AS-6a/AS-6b and ET-20 own M6 failure/Retry/success and the sole user exit |
| §1.2 | Route committed, refreshed `[derived]` | AS-5a or AS-6a completed a mounted report-free M6 read and installed its exact page behind the still-mounted modal; the commit evidence remains held until acknowledgement | F5 — no request remains and no impact report exists | `route.impact.title`, `route.impact.done` | ET-20 is the only exit. The page projection is already installed, and PF-1 resolves focus only after the modal closes |
...
| §1.2 | Route save rejected `[derived]` | The server definitively rejected the route write outside the guarded-plan and `direct_mode` branches, so this attempt did not commit | F1, in place | `route.fail.save`, `route.retry`, `route.cancel` | ET-14, ET-16a/ET-16b, AS-1/AS-2, ET-17a–ET-17e, ET-18a and ET-8a own entry, AgentSupply-first reconciliation, every outcome and the non-mutating exit |
| §1.2 | Route save outcome unknown `[derived]` | The route write returned no HTTP/API answer; the exact submitted draft and whether this was the non-forced or confirmed stage remain held | F1; D-36 permits no blind resend | `route.fail.unconfirmed`, `route.retry`, `route.cancel` | ET-15, ET-16a/ET-16b, AS-1/AS-2, ET-17a–ET-17e, ET-18a and ET-8b own entry, AgentSupply-first D-36 reconciliation, every outcome and the Chain-unresolved exit |
| §1.2 | Route impact reported `[derived]` `[contract]` | R6 holds a successful route-replace response with non-empty `removed_hops` and/or `interrupted`; the complete returned `chain` is also held while M6 reads the complete model surface | F2 for M6; the write already succeeded and DP-4 owns every exit | `route.impact.title`, `route.impact.detail`, `route.impact.done`, `route.impact.refreshFail`, `sourceDetail.impact.removedHops`, `sourceDetail.impact.interruptedModels`, `guard.hop.position`, `guard.gap.subject`, `guard.gap.agents`, `gateway.modelCount`, `route.retry` | ET-10, ET-19a/ET-19b, AS-5a/AS-6a and ET-20 own report entry focus, mounted M6 failure/Retry/success and the sole user exit; success never dismisses the report |
...
| §1.2 / §1.6 / §1.9 / §1.10 / §1.11 | Committed projection stale `[derived]` | M0 has authoritative Source absence; an M1/M2/M3/M4/M6 mutation or R3 terminal has commit evidence and its required complete model-surface read failed; or M5 holds the committed mode-switch AgentSupply and its Source-list read failed. Hold the exact success envelope when one arrived; for direct absence or D-36 inference, hold the authoritative reread subject and mark response-only members unavailable. In every case keep the last good dependent projections | F2 — the selected operation-specific sentence names only the stale read, never the completed write or authoritative absence | M0/M2 `sourceDetail.remove.impact.refreshFail`; M1 `sourceDetail.edit.impact.refreshFail`; M3/M4 `upstream.repair.impact.refreshFail`; M5 `upstream.unread`; M6 `route.impact.refreshFail`; the owning read's Retry and, where a dialog/report remains mounted, its operation Done key | M0–M4 render the committed Source or absence with last-good Agent/order/chain projections marked stale; M5 renders the held AgentSupply with the last-good Source list marked stale; M6 renders the held returned/readback chain with Source adoption, Agent supply and Route-index projections marked stale. 重试 repeats only the owning M read; success hands off current projections, another failure stays. Done, close, Escape and outside press remain legal DP-4 exits for a mounted producer, while page-grain stale projections keep ordinary navigation. Never resend mutation, invent impact, discard evidence, restore origin or describe the write/absence as uncertain (C6/C9/C10/C14) |
...
ET-17c alone starts it for matching readback evidence. In every case the exact evidence and
modal stay mounted while M6 is pending. A successful read installs its exact page behind a
mounted modal, clears pending and leaves Done focused; it does not acknowledge, close or
transfer the report. ET-20's Done-equivalent family is the only user-initiated
...
M6's read failure never questions the write: a report stays mounted, and a report-free
commit enters Committed projection stale with its exact chain evidence.

**Lost-response reconciliation compares the stored value, after page mode is dispatched**
`[contract]` `[derived]` D-36. Route save outcome unknown holds the immutable submitted
array and whether the attempt was initial or confirmed. On explicit 重试, ET-16a first reads
`GET /api/models/agents`; AS-1 consumes `mode: direct` and ends Route reconciliation, while
AS-2 alone authorizes ET-17a's Source/chain reads for `mode: hub`. This table classifies
evidence; the cited ET/AS row remains the only edge owner:

| Readback evidence | What it proves | Owning edge |
| --- | --- | --- |
| Exact backend AgentSupply reads `mode: direct` | The Route surface is not legal and this same row is the complete landing payload | DM-4 / AS-1; issue no Source or chain read |
| Exact backend AgentSupply reads `mode: hub` | Route reconciliation is legal, but no commit verdict exists yet | AS-2 installs the Hub row behind the modal; ET-17a reads current Sources and the exact `(backend, menu_model)` Route |
| Ordered Route pairs equal submitted `hops`, including empty-to-empty | The desired Route is committed. `current`, health and supply annotations are irrelevant to equality | ET-17c alone holds the AgentChain, marks response-only tails unavailable and starts M6 |
| Ordered Route pairs differ and refreshed V5 remains valid | The read proves only the current origin and a valid resend basis | ET-17d sends the registered initial/confirmed body on this explicit Retry; a confirmed resend keeps the exact held refusal arrays |
| Ordered Route pairs differ and refreshed V5 invalidates any new/changed row | The read proves the current origin but not a legal resend | ET-17e → Invalid after refresh; keep and mark every offending row, send no PUT |
| The AgentSupply read fails | No page form or Route-reconciliation basis was proved | ET-16b stays in the originating failure state; Retry restarts at ET-16a and no timer runs |
| The required Source or Route read fails | No commit verdict or valid resend basis was proved | ET-17b stays in the originating failure state; Retry restarts at ET-16a and no timer runs |
| A post-Hub chain read returns `direct_mode` | The earlier Hub evidence was superseded before the chain read; the error has no landing payload | DM-3 / ET-18a; close, discard and transfer to the page Agents read |

ET-8a owns ordinary non-mutating exits. ET-8b alone handles Route save outcome unknown and hands the exact row to
§1.1 Chain unresolved; in a definitive rejection it restores only the latest chain the
dialog actually holds. Neither path presents an obsolete opening chain as current, starts a
second read owner or resends on dismissal.

**Frame-02 event-transition totality is the sole edge owner** `[contract]` `[derived]`.
The state register names what is mounted; V5 and DM classify evidence; the control and
keyboard registers name static reachability and emitted events. Only this table may choose
a destination. Every event that changes state MUST match exactly one row and fill every
column below; **an unlisted transition is forbidden**. Rows are atomic by construction: a
row may group equivalent gestures such as Done/close/Escape/outside only when cleanup,
destination, focus and next owner are identical for all of them.
...
**PF-1 is the sole post-install page-focus resolver** `[derived]`. It runs only after the
exact destination projection is installed and the rendered target set is known. Naming a
control before that check is not a focus contract:

| Installed page form | Exact model row after refreshed collapse/rendering | PF-1 output |
| --- | --- | --- |
| Direct | Not rendered by definition | Stable backend group head |
| Hub | The exact `(backend, menu_model)` row is rendered | That model row |
| Hub | The exact row is absent or collapsed | Stable backend group head |

Every AS page landing and ET-20 cites PF-1. An M6 success whose committed modal is still
mounted is not a page-focus event: AS-5a/AS-6a leave focus on Done, and PF-1 runs only if
ET-20 later closes to that installed page. The event table remains the sole destination
...
| ET-8a — ordinary reversible exit | Loading route, Route unread, Ready, Dirty, Invalid after refresh or Route save rejected + held page origin | Send no mutation; discard any dialog-owned late read after close | Close selector; restore/cancel any grab; discard local/submitted draft | Exact held page row | Exact prior row; retain its pre-existing unresolved marker if any | Invoking model row | None; a row already unresolved retains its existing D-35 owner |
| ET-8b — unknown-outcome exit | Route save outcome unknown + immutable attempt evidence | Send/read nothing | Discard modal transients but transfer the uncertain attempt marker | Page row marked Chain unresolved | `route.fail.unconfirmed` hands off to the row's unresolved treatment | Invoking model row | D-35 page owner |
...
| ET-10 — R6 success with impact | Saving + exact R6 envelope with either array nonempty | Consume every R6 member, mount its report and start M6 immediately | Release PUT/refusal ownership; retain the entire envelope | Route impact reported with M6 pending | Impact title/detail and each nonempty block | `route.impact.done` | M6 complete-surface read |
| ET-11 — two-empty R6 commit | Saving + exact R6 envelope whose two impact arrays are empty | Preserve the exact envelope and start M6 immediately | Release PUT ownership; no origin can be restored | Route committed, reconciling | `route.refreshing` | `route.impact.done` | M6 complete-surface read |
...
| ET-16a — begin failure reconciliation | Route save rejected or outcome unknown + held draft/stage | Read `GET /api/models/agents` first | Keep error body/draft; set Retry `aria-busy` | Same failure, reconciliation pending | Existing failure copy | Keep `route.retry` | Agents read |
| ET-16b — AgentSupply read failure | ET-16a pending + held draft/stage | Classify the read failure without choosing a page form | Clear busy; retain error body/draft | Originating failure | F1 read cause | `route.retry` | None until ET-16a Retry |
| ET-17a — Hub Source/chain read begins | AS-2's exact Hub AgentSupply + originating failure/draft/stage | Read current Sources and exact chain | Keep modal/draft; keep Retry busy | Originating failure, Hub reconciliation pending | Existing failure copy | Keep `route.retry` | Source + exact chain reads |
| ET-17b — Hub reconciliation read failure | ET-17a pending + exact Hub AgentSupply | Classify the failed Source/chain stage | Clear busy; retain every held authority/draft | Originating failure | F1 read cause | `route.retry` | None until ET-16a Retry |
| ET-17c — matching Route proves commit | ET-17a + exact AgentChain equal to submitted pairs | Hold AgentChain, mark response-only tails unavailable and start M6 | Clear reconciliation busy; discard obsolete retry/refusal state | Route committed, reconciling | `route.refreshing` | `route.impact.done` | M6 complete-surface read |
| ET-17d — differing valid Route resends | ET-17a + different AgentChain + refreshed V5-valid draft | Freeze the held initial/confirmed stage and send its registered PUT body | Clear reconciliation busy; retain exact confirmed arrays when that stage requires them | Saving | `route.saving` | Programmatically focusable progress status | One owned Route PUT |
| ET-17e — differing invalid Route | ET-17a + different AgentChain + refreshed V5-invalid new/changed pair | Install refreshed origin/authorities; send no PUT | Clear busy and submitted/refusal state; retain editable rows | Invalid after refresh | `route.invalidAfterRefresh` on every offender | First offending row's reachable Remove action | None |
| ET-18a — `direct_mode` error handoff | Loading route, Saving or ET-17a Hub reconciliation + the matching DM-1/DM-2/DM-3 error | Error evidence selects no page form; start/transfer the page's Agents read | Close modal/selector, cancel grab, discard every draft/submitted/refusal value | Page §1.0 Partial | Existing Partial copy; never label Direct from the error | Stable owning backend group head | Page Agents read |
...
| ET-19a — mounted M6 failure | Route impact reported or Route committed/reconciling + exact evidence + failed M6 | Failure cannot negate commit or reconstruct a response member | Clear M6 pending; retain modal and all evidence | Same impact report, or report-free Committed projection stale | `route.impact.refreshFail` plus prior report/refresh copy | `route.retry` | None until ET-19b |
| ET-19b — mounted M6 Retry | ET-19a mounted failure + exact evidence | Repeat only M6 | Keep modal/evidence; set Retry busy | Same report or report-free state, M6 pending | Prior report/refresh copy | Keep `route.retry` | M6 complete-surface read |
| ET-19c — transferred M6 failure | ET-20 receiving page + transferred pending M6/evidence | Failure cannot negate commit | Clear pending; retain transferred evidence and stale projections | Receiving page, dependent projections stale | `route.impact.refreshFail` | Current page focus remains | None until ET-19d |
| ET-19d — page-owned M6 Retry | ET-19c page-stale evidence | Repeat only M6 | Keep evidence/stale projection; set page Retry busy | Same receiving page pending | Existing stale line | Keep page Retry | M6 complete-surface read |
| ET-20 — committed Done-equivalent exit | Mounted report, reconciling, refreshed or stale phase + exact commit evidence | Done, title-close, Escape and outside send no mutation/read | Close modal and transfer any pending M6 owner/evidence; never restore origin | Receiving page with current or explicitly pending/stale projection | Held evidence plus the registered pending/stale marker as applicable | PF-1 over the exact currently installed page projection | Page-owned pending M6 if one exists; otherwise none, or later ET-19d Retry after failure |
| AS-1 — ET-16 Direct dispatch | ET-16a + exact AgentSupply `mode: direct` | Install that exact Direct row before any Source/chain read | Clear busy; close modal and discard all Route transients | Actual Direct page | Exact Direct projection | PF-1 | None |
| AS-2 — ET-16 Hub dispatch | ET-16a + exact AgentSupply `mode: hub` | Install that exact Hub row behind the held modal | Clear only Agents-read pending; retain modal/draft/stage | Originating failure over actual Hub page | Existing failure copy | Keep `route.retry` | ET-17a Source/chain reads |
...
| AS-5a — mounted M6 Direct success | Mounted Route impact report or report-free committed modal + complete returned surface whose owning AgentSupply is `mode: direct` | Dispatch mode first and install the exact complete Direct page behind the modal | Clear only M6 pending/stale; retain modal and exact commit/report evidence | Impact origin stays Route impact reported; report-free origin enters Route committed, refreshed, both over the actual Direct page | Impact origin keeps its registered report; report-free origin replaces `route.refreshing` with `route.impact.title` | Keep `route.impact.done` | None until ET-20 |
| AS-5b — transferred M6 Direct success | ET-20-transferred M6 + complete returned surface whose owning AgentSupply is `mode: direct` | Dispatch mode first and install the exact complete Direct page | Clear page-owned M6 pending/stale; retain commit evidence in page history | Actual Direct page | Exact complete Direct surface | PF-1 | None |
| AS-6a — mounted M6 Hub success | Mounted Route impact report or report-free committed modal + complete returned surface whose owning AgentSupply is `mode: hub` | Dispatch mode first and install the exact complete Hub page behind the modal | Clear only M6 pending/stale; retain modal and exact commit/report evidence | Impact origin stays Route impact reported; report-free origin enters Route committed, refreshed, both over the actual Hub page | Impact origin keeps its registered report; report-free origin replaces `route.refreshing` with `route.impact.title` | Keep `route.impact.done` | None until ET-20 |
| AS-6b — transferred M6 Hub success | ET-20-transferred M6 + complete returned surface whose owning AgentSupply is `mode: hub` | Dispatch mode first and install the exact complete Hub page | Clear page-owned M6 pending/stale; retain commit evidence in page history | Actual Hub page | Exact complete Hub surface | PF-1 | None |
...
| Route save rejected / outcome unknown | 重试 | `route.retry` | Enabled; busy only while ET-16a/AS-2/ET-17a owns reads | First command in Tab order; activation emits ET-16a |
...
| Route committed, reconciling / refreshed / Route impact reported / mounted M6 stale | 完成 | `route.impact.done` | Enabled before, during and after M6 | First command in Tab order; activation emits ET-20 |
| Route committed, reconciling / refreshed / Route impact reported / mounted M6 stale | Title-bar close | `route.impact.done` | Enabled before, during and after M6 | Independent Tab stop; activation emits ET-20 |
| Route impact reported / report-free stale after M6 failure | 重试 | `route.retry` | Enabled with exact commit evidence held | Normal Tab order; activation emits ET-19b and retains the modal |
| Receiving page after transferred M6 failure | 重试 | `route.retry` | Enabled beside the stale dependent projection | Page Tab order; activation emits ET-19d |
...
| `Enter` / `Space` on 重试 | Route unread → ET-4; save failure → ET-16a; mounted M6 failure → ET-19b; transferred page failure → ET-19d |
...
evidence whenever one of those branches changes the page form: every complete-surface read
dispatches its returned mode before installation, while a classifier alone may retire an
illegal surface but cannot project its destination; PF-1 then selects only a focus target
that the installed projection actually renders. C16 makes every state-changing event
account for authority dispatch, transient cleanup, visible feedback, destination focus and
next owner in one atomic edge row; user events with different outcomes cannot share a row,
and visible feedback without a registered key or named copy block is incomplete. None can
be inferred from the reversible no-op rule.
...
and the exact submitted ordered pair array, so its chain `GET` can prove the desired
Route committed when the returned `(source_id, model_id)` projection is equal. Runtime
annotations do not participate in that comparison. A differing chain permits one resend
only on the user's Retry; a failed read permits none. Either a received R6 success or
that inferred commit enters M6 before visible handoff, and inference marks the
