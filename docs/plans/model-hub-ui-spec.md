...
the surface around it. Every new frame column MUST check all nine rows below. `[x] N/A` is
...
| C6 — committed-report exit | Once a mutation has committed, every dismissal path is the report's Done-equivalent exit and may neither restore a pre-write origin nor discard held response evidence | [x] Save/remove impact reports retain their envelopes through M1/M2's complete-surface read; a failed read keeps the report and does not question the write | [x] Repair impact retains R3/R4's distinct envelopes through M3/M4; no path restores the invoking card/menu origin | [x] N/A — the menu commits nothing and owns no response report |
...
| C8 — acknowledgement consequence coverage | For every capability that requires acknowledgement, register every applicable channel, the exact confirmed request value and the channel-specific consequences shown before the irreversible boundary | [x] The existing overflow reauth action transfers its exact Source/channel to §1.11's shared confirmation; frame 11 adds no alternate shortcut | [x] Hub and `native_cli` both confirm and send literal `true`; both render the common irreversible warning, while native also renders the immediate sibling-source outage warning | [x] N/A — vendor selection starts create intent and crosses no existing-credential boundary |
| C9 — empty-impact reconciliation failure | For every committed mutation that can skip its report because all impact arrays are empty, register the complete-read failure state, held write evidence and read-only Retry | [x] M1/M2 enter Committed projection stale with the updated Source or committed absence held; Retry repeats only their complete-surface read | [x] M3/M4 enter the same state with the returned Source and exact empty envelope held; Retry repeats only their complete-surface read | [x] N/A — the menu commits nothing and invalidates no projection |
...
response proves the write; it does not make every read model that write changed disappear.
...
arrays while that read runs. A failed read invokes F2 for the stale projection and never
turns a held success into an unconfirmed write:

| ID | Committed producer | Invalidated projections | Required reconciliation | Failure disposition |
| --- | --- | --- | --- | --- |
| M1 | R1 source metadata / Base URL | `Source[]`; a Base URL inventory change may also change `AgentSupply[]` runnability and Route chains through the guarded cascade | After a non-empty impact report's Done-equivalent exit, or before handing off an impact-free success, read the complete model surface. Keep R1's `source` and arrays until it settles | A report stays rendered with `sourceDetail.edit.impact.refreshFail`; an impact-free failure enters Committed projection stale with the returned Source and exact empty arrays held |
| M2 | R2 source deletion | `Source[]`, every backend's Source order in `AgentSupply[]`, and every Route chain | After the report's Done-equivalent exit, read the complete model surface before returning to 01. With two empty arrays, remove the exact Source locally and run the same read without inventing an impact report | A report stays rendered with `sourceDetail.remove.impact.refreshFail`; an impact-free failure enters Committed projection stale with the exact Source absence and empty arrays held |
| M3 | R3 OAuth reauth terminal | the returned Source, its Agent-supply/runnability projection and every chain that names its exact source/model pairs | After Repair impact reported's Done-equivalent exit, or before handing off an empty-impact terminal, read the complete model surface. Keep `source`, `recovered` and `interrupted_pairs` as response evidence until it settles | A report stays rendered with `upstream.repair.impact.refreshFail`; an empty-impact failure enters Committed projection stale with the returned Source, `recovered` and empty `interrupted_pairs` held |
| M4 | R4 credential replacement | the returned Source plus every Agent-supply and Route-chain projection changed by `removed_hops` / `interrupted` | Use the same complete-surface timing as M3, while retaining R4's standard mutation envelope | A report stays rendered with `upstream.repair.impact.refreshFail`; an empty-impact failure enters Committed projection stale with the returned Source and exact empty standard envelope held |
...
| --- | --- | --- | --- | --- |
| RR-1 | OAuth acquisition returns `flow.state` `starting`, `awaiting_action` or `verifying` | none for create; exact Source projection for reauth | create / reauth | PD-1 transfers the click-owned browser context to the flow; PD-2 navigates it whenever the active flow first carries non-null `auth_url` and keeps the visible fallback. Select the server-declared presentation form and enter the 2s status-read machine. The acquired flow owns cancellation. |
| RR-2 | OAuth acquisition returns any terminal flow | same as RR-1 | create / reauth | Do **not** open or reconstruct a presentation form. PD-3 closes the unused context and immediately reads `GET /api/models/oauth/status/<flow_id>`; only that route materializes and returns the terminal tail. E2 remains inconclusive under §1.4's evidence-class matrix. |
...
| RR-5 | Status/submit returns `failed` / `cancelled`, or E6's `discovery_failed` / `migration_item_conflict` | none / exact Source projection | create / reauth | Stop polling and use the intent-specific §1.4 failure row. E6 re-reads the list for create or the held Source id for reauth; E2 outages, including `engine_down`, stay in the bounded poll instead. Source absence takes §1.6 precedence. |
...
| DP-4 — committed evidence | Source save impact reported, Source removal impact reported, Repair impact reported and Committed projection stale | A report runs its registered Done exit; Committed projection stale retries only its M1/M2/M3/M4 projection read | Not rendered | A report uses the same operation as Done; Committed projection stale has no dismissal exit while the required read is stale | Never restore the pre-write origin. Preserve the exact success envelope until the complete read succeeds; a projection-read failure keeps its report or empty-impact evidence rendered and cannot negate the held write response |
...
| §1.4 | Awaiting sign-in | An acquired flow carries `presentation.expects: none` and no `device_code`; Form B remains G-33. `GET /api/models/oauth/status/<flow_id>` is polled every 2s until the §1.4 evidence-class matrix selects an exit `[contract]` | → OAuth failed / OAuth materialization failed. E2 transport/outage evidence, including `engine_down`, is inconclusive and the next 2s tick retries under the same bound (D-16); E4 stops at OAuth failed; only E6's two materialization codes stop at OAuth materialization failed | `addSub.signIn`; PD-4 resolves `presentation.instructions_key` and uses the device-code helper on null or lookup failure | PD-2 keeps the authorization link actionable. E5 `success` → `intent: create` closes **into 06 for the source that terminal names** with `added_to` and `adopted_by` in hand; `intent: reauth` → §1.11's R3 repair terminal `[contract]`; E4 → OAuth failed; E7 → Source gone; the polling bound passes with no terminal reading → OAuth failed — the bound is `OAuthFlow.expires_at` when the flow carries one `[contract]` and 15 minutes from acquisition when it does not `[derived]`; dismissed any of the three ways → Dismissing |
| §1.4 | Awaiting paste-back | The acquired flow carries `presentation.expects: paste_code` or `paste_callback_url` `[contract]`; the 04 paste-back exhibit replaces the option body and keeps the `flow_id` | F5 until submit | `addSub.paste.title.code`, `addSub.paste.title.callbackUrl`, `addSub.paste.subtitle`, `addSub.paste.label.code`, `addSub.paste.label.callbackUrl`, `addSub.paste.placeholder.code`, `addSub.paste.placeholder.callbackUrl`; PD-4 resolves `presentation.instructions_key`, with `addSub.paste.hint.code` / `addSub.paste.hint.callbackUrl` on null or lookup failure; `addSub.paste.submit`, `addSub.cancel` | PD-2 keeps the server-declared authorization link actionable. V4 gates 提交 → Submitting paste-back; 取消 / close / outside press → Dismissing (C7/DP-3) |
| §1.4 | Submitting paste-back | 提交 pressed — `POST /api/models/oauth/submit` with the held `{flow_id, value}` `[contract]` | F1 → Paste-back failed for E2; only E6 → OAuth materialization failed | `addSub.paste.submitting` | E5 dispatches by held intent: `create` closes into 06; `reauth` → §1.11's R3 repair terminal. E4 → OAuth failed; E3 → Awaiting paste-back completion; E2 transport/no answer → Paste-back failed; E7 → Source gone |
| §1.4 | Awaiting paste-back completion `[contract]` | Submit, or the reconciliation read below, answered with `OAuthFlow.state` `starting`, `awaiting_action` or `verifying` | The same §1.4 evidence-class matrix and expiry/15-minute bound as Awaiting sign-in: E2, including `engine_down`, continues; only E6 stops at materialization failure | `addSub.paste.submitting`, `addSub.cancel` | E5 dispatches by held intent: `create` closes into 06; `reauth` → §1.11's R3 repair terminal. E4 → OAuth failed; E3 stays here; E6 → OAuth materialization failed; E7 → Source gone. 取消 / close / outside press → Dismissing |
| §1.4 | Paste-back failed `[derived]` | The submit request returned E2 evidence — transport/no answer or an outage such as `engine_down` — so its flow outcome is unconfirmed | F1, in place; the input, `flow_id` and intent are kept | `addSub.paste.fail`, `addSub.retry`, `addSub.cancel` | 重试 first re-reads `GET /api/models/oauth/status/<flow_id>` (D-36): E5 dispatches by held intent to 06 or §1.11's R3 repair terminal; E4 → OAuth failed; E6 → OAuth materialization failed; E3 → Awaiting paste-back completion, **without submitting the held value again**; E2 leaves this state unchanged; E7 → Source gone. 取消 → Dismissing |
...
| §1.11 | Repair impact reported `[derived]` `[contract]` | R3 holds a successful OAuth repair with non-empty `interrupted_pairs`, or R4 holds a successful key replacement with non-empty `removed_hops` and/or `interrupted`; the complete returned `source` is held | F2 for M3/M4's complete-surface read; the successful response is already in hand and DP-4 owns every exit | `upstream.repair.impact.title`, `upstream.repair.impact.detail`, `upstream.repair.impact.refreshFail`, `sourceDetail.impact.removedHops`, `sourceDetail.impact.interruptedModels`, `guard.hop.position`, `guard.gap.subject`, `guard.gap.agents`, `gateway.modelCount`, `upstream.repair.impact.done`, `upstream.retry` | Render every non-empty response array under its matching hop or SupplyGap block. 完成, close, Escape or outside press run M3/M4's complete model-surface read; success returns with the current surface, failure keeps the report and changes the exit to 重试. The reread never replaces the held response evidence or restores the invoking origin |
| §1.10 / §1.11 | Committed projection stale `[derived]` | An M1–M4 mutation succeeded with every impact array empty, then its required complete model-surface read failed. The exact empty success envelope, committed Source update or absence, and last good dependent projections are held | F2 — the selected operation-specific refresh-failure sentence names the stale read, never the completed write | M1 `sourceDetail.edit.impact.refreshFail`; M2 `sourceDetail.remove.impact.refreshFail`; M3/M4 `upstream.repair.impact.refreshFail`; the matching `sourceDetail.retry` or `upstream.retry` | Render the committed Source or absence with the last good Agent/order/chain projections and put focus on 重试. 重试 repeats only the owning M1/M2/M3/M4 complete-surface read; success hands off its current projections, and another failure stays here. Never resend the mutation, invent impact rows, discard the held envelope or describe the write as uncertain (C6/C9) |
| §1.11 | Repair failed `[derived]` | The pre-flow reauth request or credential replacement failed before its terminal could be confirmed; the repair intent, channel acknowledgement, exact origin status and any typed key remain held | F1, on the repair surface | `upstream.repair.fail`, `upstream.retry` | RR-6–RR-9 own the reconciliation read by held source id (D-36): absent → Source gone; a held `needs_action`/`error` origin that is now clear → render the reread Source as repaired; a still-blocked origin → remain here; any origin that was already non-blocked → remain here regardless of the present snapshot, because health is not mutation evidence. 重试 repeats the held producer — reauth with the same channel body, or credential replacement with the same key. After a flow is held, E4 uses §1.4 OAuth failed and E6 uses §1.4 OAuth materialization failed; E2 remains inconclusive in the bounded poll. Every branch preserves `intent: reauth` |
...
| `upstream.repair.reauthConfirm.detail` `[derived]` `[contract]` | 开始登录后无法回滚。如果这次登录没有完成,这个来源会保持需要处理,直到重新登录。 | Once sign-in starts, it cannot be rolled back. If this sign-in does not complete, this source remains in needs action until you sign in again. |
| `upstream.repair.reauthConfirm.detail.nativeImpact` `[derived]` `[contract]` | 旧的本机登录会立即失效,共用它的每个来源都会不可用。完成后只恢复这一个,其它来源需要各自重新登录。 | The old local sign-in stops working immediately, and every source sharing it becomes unavailable. Finishing restores only this source; the others each need their own sign-in. |
...
again; the user may still activate the held link. A submit response is not assumed terminal: `starting`,
...
| E3 — non-terminal flow | Non-terminal `{flow}` | The flow exists and is not done | Continue the same poll without auto-opening `auth_url` or re-submitting a held paste value | None |
| E4 — terminal absence/failure | Terminal `failed` / `cancelled`, or `flow_not_found` / `flow_expired` | This flow cannot produce a later success terminal | Stop → OAuth failed | Retry preserves the held `create` / `reauth` intent |
...
E2 is bounded by the dialog clock, not by the first outage. E6 is the deliberately closed
...
If either impact-free read fails, C9 enters Committed projection stale with that exact write
evidence and the last good dependent projections; its only recovery action retries the same
M1/M2 read and cannot repeat the edit or deletion.
...
Both Hub and `native_cli` 重新授权 first open the same registered acknowledgement state
`[contract]`. Both render `upstream.repair.reauthConfirm.detail` and only 继续登录 sends
the literal `{acknowledge_irreversible: true}`. The native channel additionally renders
`upstream.repair.reauthConfirm.detail.nativeImpact` before that boundary because starting
one CLI login immediately makes every Source sharing it unavailable and success restores
only the selected Source. The Hub channel does not claim that sibling-wide effect.
...
- Forms A, B and C are selected from a **non-terminal** returned `presentation`, never a
  ...
  while only E6's `discovery_failed` / `migration_item_conflict` stops immediately and
  refreshes this held source projection;
...
`source`, `recovered` and `interrupted_pairs`; each non-empty pair is a `SupplyGap`: backend
and protected menu model, followed by the named-Agent line only when `agents` is non-empty.
...
questions the credential write. An empty-impact success holds the exact empty envelope and
returned Source while the same complete read settles; failure enters the shared Committed
projection stale state, whose Retry repeats only that read (C9).
...
C7. The acknowledgement also cites C8 for channel consequence coverage, and empty-impact
repair cites C9 for its committed read-failure state; none can be inferred from the
...
完成 and never restore the invoking origin. Committed projection stale instead focuses its
...
settle each step it promised. §1.5's ⑦ and §1.4's *Start failed* hold nothing the server
...
Writing the rule once also stops the register from re-deciding it per row: the same
question has now arrived at four states in three sections, and it has exactly two
answers.
