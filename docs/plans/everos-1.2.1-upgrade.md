...
- Memory launches on EverOS 1.2.1 as its initial supported state. There is no
  data-migration or cross-version provider-root compatibility work in scope.
...
- Define the initial project-aware store schema with non-null `project_ref` on
  queue rows.
...
- `CaptureAdmission` validates it and the store persists `project_ref` on each
  queue row (part of the fresh v2 schema — no migration of old rows).
