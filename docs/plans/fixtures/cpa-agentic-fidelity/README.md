...
final system scope. The focused suite contains 206 tests.
...
| Multi-turn loop | `_run_case` observed `first_turn.tool_calls` with nonempty unique call IDs before any follow-up request, `_validate_first`: `no_premature_tool_outputs`, plus `_validate_second`: `no_followup_tool_calls`, `tool_outputs`, and `_tool_output_tuples` independent tuple-candidate recognition, empty/malformed identity rejection, and exact complete call-ID/result tuple-set comparison |
