...
gates but had reasoning absent and its final exact tuple failed; its final system
scope and tool outputs passed. The parallel case had reasoning absent and failed
both parse gates while retaining stream order, final system scope, exact tuple,
and tool outputs.
