...
Responses-to-Messages single case returned no tool call and therefore skipped
its follow-up, and no fallback was used. Messages-to-Responses single failed
both parse gates and its final exact-tuple gate while its parallel case passed
every gate. Both Responses-to-Messages first responses lacked reasoning; the
parallel final failed system marker, scope, and exact-tuple gates. Both
Messages-to-Chat cases failed the first and final parse gates with thinking
present; the single final also failed its exact-tuple gate. Both
Chat-to-Messages cases lacked reasoning and failed final system marker, scope,
and exact-tuple gates; the parallel case also failed both parse gates.
...
| Streaming tool fragments | `_parse_anthropic_stream`, `_parse_responses_stream`, `_parse_chat_stream`, and `_stream_order_ok` opening/terminal envelopes, error/sentinel events, block/delta snapshot compatibility, item/choice/output-index/ID continuity, argument fragments, and monotonic lifecycle checks |
