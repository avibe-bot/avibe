...
| B7 | sole owned default source + exact one-hop manual route, verified by reads → delete source → guard 409 → echo plan → force | actual inherited supply gap; the one delete confirm is the whole decision, so the 409 is resent forced with its plan echoed without reopening the dialog, and the commit announces itself in one toast; malformed echo rejected; canonical route and exact captured default order restored independently (only the owned deleted ID may be omitted) | assert |
...
| D3 | hop0 → 429 before first output | same-turn failover to hop1; `switch` event; hop0 initial local cooldown 60–72s; chat copy unchanged (silent) | assert |
...
| D8 | cooldown `retry_at` elapses | hop0 becomes eligible; only recognized output or valid empty completion clears its state and emits `recover` | unit/loopback covered; local Incus outstanding |
| D9 | all temporary hops keep failing | one closed recovery-exhausted result (Codex 400, Claude/OpenCode 424); localized final copy and original upstream provenance | gateway/scenario covered; local Incus outstanding |
...
| G2 | Sources PUT and compatibility chains/reorder change defaults | same effective guard plan and canonical projection; manual arrays unchanged; omitted reorder order is read-only. Default-routing UI Save automatically echoes one exact refusal plan without a second confirmation; changed-plan refusal keeps the draft, and lost responses reconcile before any new write | assert |
...
| RM-5 | Effective removal wire confirmation is exact and atomic for defaults/Restore/inventory/catalog/Source writes | Refuse, alter state, retry stale plan, confirm current plan; no partial writes. Default-routing Save performs one automatic exact-plan confirmation with no second user action, stops on a changed-plan refusal and reconciles lost responses before replay; Restore/inventory/catalog/Source UI confirmations remain. Empty PUT and DELETE have identical effective-removal/supply guards, idempotency, admission/lease and rollback behavior; only nonempty manual PUT retains the visible-edit interruption-only guard. A cascade removing the final hop drops its override before recomputing inherited impact; pure reorders preserve nonempty manual arrays and need no confirmation |
...
| --- | --- |
| G2/RM-5: one-click defaults, bounded exact-plan handshake and lost-response reconciliation | `ui/src/components/settings/models/SourceOrderDrawer.test.tsx` |
