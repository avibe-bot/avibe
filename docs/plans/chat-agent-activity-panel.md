...
The two sources are merged into one chronological timeline by **parsed** datetimes,
not raw strings: `messages` writes `...+00:00` (microseconds) while `agent_events`
writes `...Z` (whole seconds), and a lexical sort interleaves them wrong (`.` <
`Z`). Show-Page `assistant` marks (`metadata.source == 'show_page'`) are excluded
from activity. The scan is bounded to the recent tail (most-recent 500 messages /
2000 tool-call events) — a documented cap aligned with the Chat's ~300-row retained
window, not a silent truncation.
...
trailing "Interrupted" chip. Presentation is `ActivityCard` / `ActivityChip` in
