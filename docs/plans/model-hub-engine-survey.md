...
| --- | --- | --- | --- |
| Anthropic Messages | OpenAI Responses | 200 for both two-turn cases; **NO-GO** | r44: the single retained reasoning only in the first turn and failed both parse gates, final system scope, tool-output correlation, and the exact tuple. The parallel retained reasoning but failed both parse/order gates |
| OpenAI Responses | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r44: the single passed both parse gates but lacked reasoning in both turns and failed final system scope and the exact tuple. The parallel lacked reasoning in both turns, failed both parse/order gates, and failed final system scope, tool-output correlation, and the exact tuple |
| Anthropic Messages | OpenAI Chat Completions | 200 for both two-turn cases; **NO-GO** | r44: the single retained reasoning but failed both parse gates, leaked protected reasoning into final visible text, and failed the final exact tuple. The parallel retained reasoning but failed both parse/order gates |
| OpenAI Chat Completions | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r44: the single passed both parse gates but lacked reasoning in both turns and failed final system scope and the exact tuple. The parallel lacked reasoning, failed both parse gates and first-turn stream ordering, and failed final system scope |
...
latest complete matrix attempt (r44); the direction decisions remain anchored
...
| Multi-turn loop | Assistant tool call followed by a tool result and a final answer | No tool result appears before execution; the call id links the returned result to the same call and the second turn sees the tool output | Engine-core translator plus caller adapter | r44 independently recognized malformed tuple candidates and rejected empty identities; final exact tuples failed in five of eight cases, while Messages -> Responses, Messages -> Chat, and Chat -> Messages parallel preserved the exact complete set |
| Streaming text | Short streamed answer | Every data event is strict UTF-8 JSON, wire sequence and ordering are monotonic, lifecycle transitions are unique, Anthropic `message_delta` precedes `message_stop`, done/content-part snapshots agree with deltas/items, per-item output snapshots agree with their own deltas, and the stream terminates with the protocol's done event | Engine-core stream translator | r44: Messages -> Responses, Responses -> Messages, and Messages -> Chat parallel failed both parse/order gates; Chat -> Messages parallel failed both parse gates and first-turn ordering |
| Streaming tool fragments | Stream a tool call with split arguments | At least two nonempty fragments reassemble into exactly one valid JSON argument object and one call id; typed error/discriminator events cannot be accepted as completion | Engine-core stream translator | r44 gated Anthropic, Responses, and Chat fragment counts. Complete first-turn arguments reconstructed in all four parallel cases, but Messages -> Responses, Responses -> Messages, and Messages -> Chat failed both parse/order gates; Chat -> Messages failed both parse gates and first-turn ordering |
| System prompt | System/instructions text containing a marker plus a conflicting user marker | Exact system marker remains system-scoped and the exact user conflict marker is absent | Engine-core translator | r44 final system scope passed in Messages -> Responses parallel and both Messages -> Chat cases, and failed in Messages -> Responses single, both Responses -> Messages cases, and both Chat -> Messages cases |
| Thinking/reasoning | Anthropic `thinking` budget and OpenAI `reasoning.effort` | Requested signal is observable or an explicit loss is recorded; readable reasoning, Anthropic signatures/redacted data, and Responses encrypted content are all protected from visible-text leakage | Engine-core translator; upstream model semantics | r44 observed reasoning in seven of eight Anthropic -> OpenAI turns, confirmed its absence in all eight OpenAI -> Anthropic turns, and passed normalized full-value/meaningful-excerpt leakage checks on 15 of 16 returned turns; the Messages -> Chat single final turn exposed a meaningful protected excerpt |
...
`usage.completion_tokens_details.reasoning_tokens` signal. The r44 matrix
...
data, and Responses encrypted content. It passed that gate on 15 of 16 returned
turns; the Messages -> Chat single final turn exposed a meaningful protected
excerpt. The corrected parser still observed no positive signal in any of the
...
1. **Messages -> Responses.** POST `/v1/messages` with one function tool and
   ...
   projected Anthropic thinking block to contain a nonempty signature. In r44
   the single retained reasoning only in the first turn and failed both parse
   gates, final system scope, tool-output correlation, and the exact tuple. The
   parallel retained reasoning but failed both parse/order gates. The direction
   remains NO-GO because both cases fail the closed matrix.
2. **Responses -> Messages.** POST `/v1/responses` with `instructions`, two
   ...
   `function_call_output`; repeat with `stream: true`. In r44 the single
...
3. **Messages -> Chat Completions.** Repeat the Messages single-tool request
   ...
   projected response. In r44 the single retained reasoning but failed both
   parse gates, leaked protected reasoning into final visible text, and failed
   the final exact tuple. The parallel retained thinking but failed both
   parse/order gates.
4. **Chat Completions -> Messages.** POST `/v1/chat/completions` with an
   ...
   requested reasoning signal is absent. In r44 the single passed both parse
   ...
   gates, first-turn stream ordering, and final system scope.
...
separate from the primary-model result. The latest closed-matrix rerun (r44)
exercised all eight cases through the loopback CPA after rejecting malformed
output tuples, requiring at least two nonempty Anthropic, Responses, and Chat
argument deltas per streamed function call, requiring explicit empty Anthropic
thinking/signature opening fields, rejecting duplicate Responses
`in_progress` events, and requiring Anthropic `message_delta` before
`message_stop`. All eight cases completed both turns with HTTP 200. No
fallback was used. The causal gate passed all returned first turns, and the
normalized full-value/meaningful-excerpt reasoning leakage gate passed on 15 of
16 returned turns; the Messages -> Chat single final turn exposed a meaningful
protected excerpt. A mixed primary/fallback run in
...
semantic evidence.
The focused probe suite contains 198 tests.
...
of eight r44 turns but still failed parser gates in several cases and exposed a
meaningful protected reasoning excerpt in one final answer. All eight returned
