...
| --- | --- | --- | --- |
| Anthropic Messages | OpenAI Responses | 200 for both two-turn cases; **NO-GO** | r21: the single first/final responses failed parsing and its final exact call-ID/result tuple; the parallel case passed first and final parsing but failed final system scope and exact tuple |
| OpenAI Responses | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r21: both cases lacked the requested reasoning signal; final system scope and exact call-ID/result tuples failed while tool outputs passed |
| Anthropic Messages | OpenAI Chat Completions | 200 for both two-turn cases; **NO-GO** | r21: both first/final responses failed parsing with thinking present and both final exact tuples failed |
| OpenAI Chat Completions | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r21: both cases lacked reasoning; the single final system scope and tuple failed, while the parallel case failed both parse gates, final system scope, and exact tuple but passed tool outputs |
...
latest complete rerun (r21); the direction decisions remain anchored by the repeated
...
| Multi-turn loop | Assistant tool call followed by a tool result and a final answer | Tool call id links the result to the same call; the second turn sees the tool output | Engine-core translator plus caller adapter | r21 exact tuples failed Messages -> Responses single and parallel, Messages -> Chat single and parallel, and Chat -> Messages single and parallel; Responses -> Messages single and parallel also failed reasoning-presence gates |
| Streaming text | Short streamed answer | Every data event is strict UTF-8 JSON, wire sequence and ordering are monotonic, done/content-part snapshots agree with deltas/items, per-item output snapshots agree with their own deltas, and the stream terminates with the protocol's done event | Engine-core stream translator | r21: Messages -> Responses and Responses -> Messages parallel streams passed parser/order gates; Messages -> Chat parallel failed parse/order gates; Chat -> Messages parallel passed ordering but failed parse gates |
| Streaming tool fragments | Stream a tool call with split arguments | Fragments reassemble into exactly one valid JSON argument object and one call id; typed error/discriminator events cannot be accepted as completion | Engine-core stream translator | Argument reconstruction passed in all four parallel stream cases; r21 parse/order gates still rejected Messages -> Chat parallel and Chat -> Messages parallel streams |
| System prompt | System/instructions text containing a marker plus a conflicting user marker | Exact system marker remains system-scoped and the exact user conflict marker is absent | Engine-core translator | r21 passed both Messages -> Responses cases and both Messages -> Chat cases; final system scope failed in both Responses -> Messages cases and both Chat -> Messages cases |
| Thinking/reasoning | Anthropic `thinking` budget and OpenAI `reasoning.effort` | Requested signal is observable or an explicit loss is recorded; Anthropic thinking retains its signature and is never treated as user text; Responses reasoning encrypted content is stable across added/done snapshots | Engine-core translator; upstream model semantics | r21 confirmed absent reasoning in both OpenAI -> Anthropic directions; unsigned Anthropic thinking remained in Messages -> Responses single and both Messages -> Chat cases |
...
1. **Messages -> Responses.** POST `/v1/messages` with one function tool and
   ...
   projected Anthropic thinking block to contain a nonempty signature; r21's
   single first and final responses failed parsing with thinking present and its
   final exact call-ID/result tuple, while the parallel case passed both parse
   gates but failed final system scope and its exact tuple. The pair remains NO-GO.
2. **Responses -> Messages.** POST `/v1/responses` with `instructions`, two
   ...
   `function_call_output`; repeat with `stream: true`. Both r21 first responses
   lacked the requested reasoning signal; final system scope and exact
   call-ID/result tuples failed while tool outputs passed, with the reasoning loss independently
...
3. **Messages -> Chat Completions.** Repeat the Messages single-tool request
   ...
   projected response. Both r21 first and final responses failed parsing with
...
4. **Chat Completions -> Messages.** POST `/v1/chat/completions` with an
   ...
   requested reasoning signal is absent. In r21 both cases lacked reasoning;
   the single final system scope and tuple failed, while the parallel case
   failed both parse gates, final system scope, and its exact tuple, but passed
   tool outputs.
...
separate from the primary-model result. The latest gate-complete rerun (r21)
...
The focused probe suite contains 153 tests.
