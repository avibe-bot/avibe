...
| --- | --- | --- | --- |
| Anthropic Messages | OpenAI Responses | 200 for both two-turn cases; **NO-GO** | r20: the single first/final responses failed parsing and its final exact call-ID/result tuple; the parallel case passed first parsing but failed final parsing and its final exact tuple |
| OpenAI Responses | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r20: both cases lacked the requested reasoning signal; final system scope, tool outputs, and exact call-ID/result tuples failed |
| Anthropic Messages | OpenAI Chat Completions | 200 for both two-turn cases; **NO-GO** | r20: both first/final responses failed parsing with thinking present and both final exact tuples failed |
| OpenAI Chat Completions | Anthropic Messages | 200 for both two-turn cases; **NO-GO** | r20: both cases lacked reasoning; the single final system scope and tuple failed, while the parallel case failed both parse gates and final system scope but passed its final tuple and tool outputs |
...
latest complete rerun (r20); the direction decisions remain anchored by the repeated
...
| Multi-turn loop | Assistant tool call followed by a tool result and a final answer | Tool call id links the result to the same call; the second turn sees the tool output | Engine-core translator plus caller adapter | r20 exact tuples failed Messages -> Responses single and parallel, Messages -> Chat single and parallel, Chat -> Messages single, and Responses -> Messages single and parallel; they passed only Chat -> Messages parallel. Responses -> Messages also failed reasoning-presence gates |
| Streaming text | Short streamed answer | Every data event is strict UTF-8 JSON, wire sequence and ordering are monotonic, done/content-part snapshots agree with deltas/items, per-item output snapshots agree with their own deltas, and the stream terminates with the protocol's done event | Engine-core stream translator | r20: Messages -> Responses and Responses -> Messages parallel streams passed parser/order gates; Messages -> Chat parallel failed parse/order gates; Chat -> Messages parallel passed ordering but failed parse gates |
| Streaming tool fragments | Stream a tool call with split arguments | Fragments reassemble into exactly one valid JSON argument object and one call id; typed error/discriminator events cannot be accepted as completion | Engine-core stream translator | Argument reconstruction passed in all four parallel stream cases; r20 parse/order gates still rejected Messages -> Chat parallel and Chat -> Messages parallel streams |
| System prompt | System/instructions text containing a marker plus a conflicting user marker | Exact system marker remains system-scoped and the exact user conflict marker is absent | Engine-core translator | r20 passed both Messages -> Responses cases and both Messages -> Chat cases; final system scope failed in both Responses -> Messages cases and both Chat -> Messages cases |
| Thinking/reasoning | Anthropic `thinking` budget and OpenAI `reasoning.effort` | Requested signal is observable or an explicit loss is recorded; Anthropic thinking retains its signature and is never treated as user text; Responses reasoning encrypted content is stable across added/done snapshots | Engine-core translator; upstream model semantics | r20 confirmed absent reasoning in both OpenAI -> Anthropic directions; unsigned Anthropic thinking remained in Messages -> Responses single and both Messages -> Chat cases |
...
1. **Messages -> Responses.** POST `/v1/messages` with one function tool and
   ...
   projected Anthropic thinking block to contain a nonempty signature; r20's
   single first and final responses failed parsing with thinking present and its
   final exact call-ID/result tuple, while the parallel case passed parsing but
   failed its final exact tuple. The pair remains NO-GO.
2. **Responses -> Messages.** POST `/v1/responses` with `instructions`, two
   ...
   `function_call_output`; repeat with `stream: true`. Both r20 first responses
   lacked the requested reasoning signal; final system scope, tool outputs, and
   exact call-ID/result tuples failed, with the reasoning loss independently
...
3. **Messages -> Chat Completions.** Repeat the Messages single-tool request
   ...
   projected response. Both r20 first and final responses failed parsing with
   thinking present, and both final exact tuples failed.
4. **Chat Completions -> Messages.** POST `/v1/chat/completions` with an
   ...
   requested reasoning signal is absent. In r20 both cases lacked reasoning;
   the single final system scope and tuple failed, while the parallel case
   failed both parse gates and final system scope but passed its final tuple and
   tool outputs.
...
separate from the primary-model result. The latest gate-complete rerun (r20)
...
The focused probe suite contains 150 tests.
