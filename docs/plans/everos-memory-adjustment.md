...
→ resolve ambiguous live claims: every pre-quiescence capture row whose
    ...
    state, journal operation_id) and keep the original row in the
    snapshot-covered outbox until the rotation either completes (the
    row is then resolved through the rotation-time audit) or rolls back
    (the row's `manual_required` status survives the rollback because
    it lives in the replay store). This guarantees that no ambiguous
    live claim is silently forced into either branch of the binary
    drain; ambiguous rows are non-replayable until audited resolution
    by an operator, exactly as §10.1 requires.
...
The rotation replay store is a separate SQLite database outside `state/memory/memory.sqlite` (`core/memory/store.py:24`), so a snapshot of the durable capture queue never covers quarantined rows. The store lives at `state/memory/rotation_replay.sqlite` and is excluded from the rotation snapshot by the snapshot tool (it does not appear in the file manifest, the checksum pass, or the `sqlite3 backup` walk). It carries the same row identity as `memory_capture_queue` (`source_message_digest`, `principal_id`, `project_ref`, `session_id`, `epoch`, `payload_text`, `payload_attachments`, `provider_timestamp_ms`) plus seven rotation-specific columns: `replay_state ∈ {pending_rotation_replay, in_flight, delivered_pending_rotation_commit, manual_required, settled, unknown}`, a client-generated `attempt_id` (Avibe-owned, minted by the rotation tool before the request is sent), the new sidecar's `add_request_id` (EverOS-owned, populated only from the response envelope), the journal `operation_id` of the rotation that produced it, and the audit pointer for any `manual_required` decision. **`attempt_id` and `add_request_id` are distinct identities with distinct lifetimes**: `attempt_id` is a UUIDv7 minted by the rotation tool, written **before** the replay `/add` is submitted to EverOS in the same SQLite transaction that records the lease, and used by Avibe's own audit, retry, and resolution logic; `add_request_id` is generated by EverOS and is only readable from the response envelope parsed in `core/memory/everos.py:193-203`. The pre-send write cannot carry `add_request_id` because EverOS has not yet returned one, and the existing sidecar route guard `core/memory/sidecar.py:221-228` rejects an added client request-ID field on the `/add` payload (the validator requires the exact key set `{session_id, app_id, project_id, messages}` and the message dict's exact key set `{sender_id, role, timestamp, content}`), so the plan cannot smuggle a client-supplied request-id into the payload under the no-EverOS-modification constraint. The rotation tool writes `attempt_id` and `replay_state = in_flight` together, sends the `/add` with no client request-id, then on response records `add_request_id` from the parsed envelope. If Avibe crashes between the request leaving and the acknowledgement parsing, the row remains `in_flight` with a known `attempt_id` but `add_request_id IS NULL`; the row is recovered to `manual_required` on the next boot unless a provider receipt proves the write was not committed; boot recovery never auto-replays an `in_flight` row. The `unknown` value is reserved for rows whose lease expired without a clear outcome.
...
- The rotation replay store is **not** wiped by `restart` of the live capture queue. `clear` of the live queue, however, is a destructive operator action that explicitly removes every Avibe-owned local Memory data (`docs/plans/memory-plugin-system.md:290-305`) and is the only sanctioned path to remove quarantined rows. The maintenance fence prevents concurrent access but does not make commits across the two separate SQLite files atomic — SQLite's WAL (`core/memory/store.py:1188`) only orders writes within a single database file, and an attached multi-database transaction cannot provide cross-file atomicity — so a crash between the live-queue delete and the replay-store delete would otherwise let boot recovery recreate memory the user just cleared. `clear` therefore uses a **two-phase journaled completion** instead of a single fenced transaction:
  ...
  4. **Crash recovery.** On boot, the recovery path reads the clear journal. If an open `clear_in_progress` row exists, the boot refuses to feed any `pending_rotation_replay` rows from the replay store into the live-queue feed (replaying quarantined rows against a state that the user already cleared is exactly the failure mode the journal exists to prevent), and instead surfaces the open clear plan on the processing-record page with an explicit "recovery needed" indicator. An operator command decides:
     - `fenced clear --resume`: re-execute the destructive deletes against the recorded digests; if divergence is detected (a row missing or extra in either store relative to the recorded digest), resume refuses to silently complete and surfaces the divergence. On a successful resume, phase 3 closes the journal normally.
     - `fenced clear --abort`: restore both databases from the snapshot directory recorded in the journal, mark the journal row `clear_aborted_at_ms`, and drop the journal row. The snapshots make abort a real rollback rather than a best-effort guess — the live-queue and replay-store are each replaced with the journal-persisted SQLite file, the snapshot's checksum is re-verified at restore time, and any row that the journal's digest recorded as present at `clear_started_at_ms` is restored to its pre-clear state. Abort is **not** allowed after phase 3 has dropped the snapshot directory (a completed clear is not undoable through this path; the operator who issued the completed clear can only invoke it again with the same destructive semantics, never restore deleted rows). The journal is the single source of truth for crash-safe completion across both databases; a `clear` that never wrote the open plan is treated as if `clear` never started (boot recovery proceeds normally).
...
- On rotation success, the rotation tool drains the replay store: rows that committed through the normal success-settlement path are removed. Rows that the success-settlement path itself rejected (rare; a row whose replay ack parsed but whose settlement path returned a failure) are **not** left as bare `pending_rotation_replay` rows for the next rotation while intake reopens — the same no-newer-first invariant that applies to a rollback drain applies here: the row is an already-accepted capture for the same human user, and stranding it while newer captures proceed would let the user's visible queue advance past a still-pending memory. Such a row is therefore **moved into the live capture queue's ordered retry feed ahead of any new captures** for the same principal and carries a leased `manual_required` reservation that pauses new capture `/add` for that principal until the reservation resolves. The reservation is operator-visible on the processing-record page. The reservation follows the same audited resolution path as a fresh `manual_required` (rf1/rf2/rf3) — rf1 promotes the row to a normal retry that then transitions back through success settlement; rf2 disposes it (operator chose to drop); rf3 disposes it (operator chose to refuse). Rows that reach the ordinary retry path on rf1 keep the same `attempt_id` so the audit chain is continuous. The row stays ahead of new captures for the same principal until the reservation clears. The reservation across unrelated principals does not block each other. `manual_required` rows that were already `manual_required` before rotation success do not return to `pending_rotation_replay` automatically; they wait for the audited resolution.
