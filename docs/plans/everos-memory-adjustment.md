...
- `CaptureAccepted` and `CaptureDuplicate` both carry the **trusted bounded-wait target** the caller needs to issue a later `freshness=bounded` `recall` against this specific capture: `(session_ref, target_generation, target_watermark_ms)`. The fields are derived from the durable row at enqueue time (`MemoryStore.enqueue_request()` assigns the per-session `epoch` and durable `generation` from the coordinator state and the row's `provider_timestamp_ms` is what the bounded wait will compare to `confirmed_watermark_ms` per §9.1). The current `CaptureAccepted` / `CaptureDuplicate` types in `core/memory/types.py:140-146` carry only `status` and so cannot supply these fields; the plan therefore extends both variants with the trusted target. A `CaptureDuplicate` reuses the original row's target verbatim (the original durable row already pinned the bounded-wait target when it was first enqueued) — the caller does not need to look it up by the stable capture identity, the receipt carries it. The same target is what §9.1's bounded predicate polls against, so the caller can issue `recall(RecallRequest(mode, ..., freshness=bounded, wait_scope=session_ref, target_generation, target_watermark_ms))` immediately after `capture` returns without any additional lookup.
...
{avibe_home}/state/memory/clear_journal.sqlite
{avibe_home}/state/memory/clear-journal/
...
**Open clear-recovery state must be included under the same consistency fence.** The runtime backup set includes `state/memory/clear_journal.sqlite` and the per-clear snapshot directory at `state/memory/clear-journal/`, taken under the same per-root maintenance lock and SQLite snapshot semantics as the live capture queue and the rotation replay store. A backup that snapshots the live queue and replay store while a `clear_in_progress` row is open would otherwise produce an **inconsistent restore candidate**: the captured databases reflect the pre-clear state, but the captured clear journal would record a `clear_in_progress` plan that no longer matches the captured provider data — restoring it would either replay a destructive delete against an already-restored pre-clear queue (silently destroying the user's memory the backup was meant to protect) or surface a recovery-needed indicator that contradicts the captured provider data. The backup tool therefore **refuses to create a restore candidate** while any `clear_in_progress` row is open in the journal; the backup job surfaces `backup_blocked_clear_in_progress` and the operator must either resume / abort the open clear first or wait for the clear to complete naturally. Once the clear journal has no open plan, the backup proceeds and includes the immutable `clear_completed` audit rows under the same fence so a later dispute about whether a clear actually ran can be answered from the backup itself. The per-clear snapshot directory at `state/memory/clear-journal/<journal_id>/` is included verbatim — its files are disposable (only useful during an open clear), so its inclusion does not enlarge the restore surface; it is included for the audit case where an in-flight clear must be replayed / aborted against the captured pre-clear databases. Restore semantics follow §8.3.1: an open clear in the restored journal triggers recovery-needed, never silent delete.
...
→ resolve ambiguous live claims: every pre-quiescence capture row whose
    state is `in_flight` / `unknown` / `manual_required` MUST reach an
    audited terminal decision before the drain begins. The plan does not
    modify EverOS and therefore has no receipt lookup to disambiguate a
    row that EverOS may have committed; treating an `in_flight` row as
    committed and removing it can lose an add that never landed, while
    treating it as uncommitted and moving it to `pending_rotation_replay`
    can duplicate an add already preserved in the legacy-root snapshot.
    The accepted resolution paths are: (a) wait for the row to clear via
    a recorded provider ack (success settlement) — bounded by a short
    drain deadline, after which the row is treated as `unknown`; or (b)
    for any row that does not clear within the deadline, copy the row
    into the rotation replay store as `manual_required` with the
    per-row evidence (capture_id, attempt_id, lease_expiry, observed
    state, journal operation_id) **and transition the original row in
    the snapshot-covered outbox to a non-deliverable source-row state
    `cleared_to_replay_store`**. The `cleared_to_replay_store` row is
    excluded from the binary drain's two branches (it is neither
    committed to the legacy root nor replayed against the rebuilt
    root); it carries an explicit pointer to the
    `manual_required` replay-store row so a later operator can resolve
    both halves with the same rf1/rf2/rf3 decision. The
    `cleared_to_replay_store` row is terminal from the live-queue's
    perspective: it stays in the snapshot-covered outbox for the
    duration of the rotation, but its presence in the snapshot means
    the snapshot is recording that "this row's durable home is the
    replay store, not the legacy root" — the snapshot does not try to
    re-deliver it. After the rotation completes successfully, the
    `cleared_to_replay_store` row is deleted from the live queue (its
    replay-store counterpart survives the snapshot by design); after a
    rollback, the `cleared_to_replay_store` row survives the rollback
    (its replay-store counterpart also survives) and is deleted once
    the operator resolves the matching `manual_required` row. Keeping
    both halves in sync through the rotation is what makes the
    rotation-time audit see the same row twice (once as
    `cleared_to_replay_store` in the live queue, once as
    `manual_required` in the replay store) and lets the operator's
    decision apply to both. This guarantees that no ambiguous live
    claim is silently forced into either branch of the binary drain;
    ambiguous rows are non-replayable until audited resolution by an
    operator, exactly as §10.1 requires.
→ quiesce writers under the lock:
    ...
    - drain the Avibe outbox: every pre-quiescence capture row whose
      state is `settled` / `delivered` / `unknown` (with provider ack) has
      been committed to the legacy root and is therefore inside the
      snapshot window; every row that reached the binary-drain step
      already had its ambiguous variant copied into the replay store as
      `manual_required` per the prior step, so the drain's two branches
      are now unambiguous: either the row is acknowledged (and stays
      inside the snapshot window) or it is in the replay store as a
      non-replayable `manual_required` row (and is therefore NOT inside
      the snapshot window). No row may remain in the snapshot-covered
      outbox unless it has already been committed to the legacy root,
      and no row may be added to the snapshot-covered outbox after the
...
→ **advance and persist the fence epoch** (the per-root durable epoch
    token recorded in `memory_meta` / the per-session durable `epoch`
    column family) **before the rebuilt sidecar starts**: the rotation
    tool writes a fresh epoch value, increments the per-session fence
    token, and refuses to start the new sidecar until the new epoch is
    durable. Every provider call and every settlement path **must
    verify** the active epoch before issuing `/add` / `/flush` /
    `/search` / `/get` and before recording success. Old-epoch late
    writes — a worker or callback that resumes after cutover carrying
    the pre-rotation epoch — are rejected at the adapter layer with
    `stale_epoch`, surfaced on the processing-record page, and
    transitioned to `manual_required` rather than silently mutating the
    rebuilt root. The advance is what makes the cutover boundary
    unambiguous: a call observed against the rebuilt sidecar carries
    the new epoch, a call that still carries the old epoch is provably
    pre-cutover and must not reach the new provider root. The advance
    happens once at this step; the old epoch token is retained in the
    rotation journal for audit only and is never accepted again
→ start new sidecar
...
→ replay acknowledgements do not finalize the row into the existing
    ...
    `add_request_id` and carries the **new** epoch in its lease metadata
    so a stale-epoch worker cannot finalize settlement for the new root;
    the `payload_text` / `payload_attachments` columns stay populated,
    and the row remains in the rotation replay store
...
→ on lock retention: when a rotation, clear, or rebuild terminal holds
    the maintenance lock because its capability verification or sidecar
    restart failed, the lock is **explicitly transferred or released
    to an authorized recovery operation** rather than left to gate the
    operator path. Lock retention records in the rotation journal carry
    three additional fields: `retained_owner` (the failed terminal's
    session reference), `retained_reason` (`sidecar_restart_failed`,
    `capability_verification_failed`, `snapshot_restore_failed`,
    `quarantine_transfer_aborted`), and `retained_at_ms` (timestamp the
    lock was retained). A recovery operation acquires the retained lock
    by presenting the same principal that initiated the original
    operation **and** a valid operator-scoped approval token stored in
    `state/memory/recovery_tokens/<operation_id>.json`; without both,
    the recovery operation is rejected with `recovery_unauthorized`
    rather than allowed to queue behind the failed terminal and starve.
    The recovery operation records its own `recovery_owner` and
    `recovery_lease_expires_at_ms` in the journal, takes ownership of
    the fail-closed fence (so any newly arriving capture row sees the
    same fence as the failed terminal), and proceeds through the
    §8.3 cancel-and-restore or rebuild arm of its choice. The journal
    state, snapshot metadata, and any lease tokens stay visible to the
    recovery operation; only the lock ownership field changes. If the
    recovery operation also fails, the lock is retained again with
    `retained_reason=recovery_failed` and the original lease is
    extended by a bounded recovery-lease window (default 6h, configurable
    in `state/memory/recovery_policy.json`); outside that window, the
    runtime falls back to the §15 Manual Resolution acceptance gate
...
→ on rotation-failure terminal: post-snapshot failure with verified
    snapshot restore completed ⇒ **mint and persist a third
    rollback epoch** that is distinct from both the pre-rotation
    epoch and the advanced post-cutover epoch; migrate the
    restored outbox rows, the replay-store rows, and any captured
    per-session fence tokens to the rollback epoch. The advanced epoch
    that was supposed to mark the rebuilt sidecar is recorded in
    the rotation journal as `rolled_back_epoch` (audit only) and
    is never accepted again; the pre-rotation epoch is recorded as
    `pre_rotation_epoch` (audit only) and is also never accepted
    again. Only the rollback epoch is live. Every restored row's
    `attempt_id` / lease metadata is re-stamped with the rollback
    epoch before delivery resumes. **Why a third epoch and not the
    pre-rotation epoch**: re-admitting the pre-rotation epoch
    would let the pre-rotation callbacks this fence was added to
    reject reach the provider again, which is exactly the failure
    mode the advance was meant to prevent. **Why a third epoch and
    not the advanced post-cutover epoch**: keeping the advanced
    epoch would mark every restored queue row as `stale_epoch` and
    block legitimate post-rollback delivery. The third epoch is
    therefore a fresh, audit-traceable identifier distinct from the
    two it sits between. The migration is **journaled across the
    three durable stores** (memory.sqlite, rotation_replay.sqlite,
    the owned provider root) rather than committed in a single
    transaction, because the plan itself notes that cross-file WAL
    commits are not atomic (§8.3.1 Clear all). The migration runs
    as four subphases: (a) write `rollback_epoch_migration_in_progress`
    into the rotation journal with the rollback epoch value;
    (b) re-stamp restored outbox rows in memory.sqlite to the
    rollback epoch; (c) re-stamp replay-store rows in
    rotation_replay.sqlite to the rollback epoch; (d) re-stamp
    any per-session fence tokens that live alongside the provider
    root. Each subphase records its own completion timestamp in
    the journal. Boot recovery scans the journal: if
    `rollback_epoch_migration_in_progress` is open, recovery does
    not let any sidecar or worker issue provider calls until the
    migration completes; if a subphase is missing, recovery re-runs
    that subphase against the relevant store until all three are
    durable. The migration is idempotent on `(session_ref, store)`:
    re-stamping a row that already carries the rollback epoch is a
    no-op. The rollback epoch is only marked live after all three
    subphases are durable; until then, the live epoch token is the
    pre-rotation epoch and any provider call is rejected with
    `migration_pending`. After the rollback epoch is durable and
    the restored outbox has been migrated, restart the old sidecar
    against the restored provider root, verify capability, **and only
    then** release the maintenance lock and reopen capture
    acceptance. The lock and the provider-call fence are held
    continuously through the sidecar restart and capability
    verification — releasing the lock before the restored sidecar is
    live and capability-verified would let another mutually exclusive
    operation (clear, rebuild, artifact cutover) enter while
    rollback is still opening the same provider root, which is the
    exact failure mode the §8.2 key-only rollback sequence already
    guards against. The §8.3 rotation-failure terminal uses the same
    safe order: fence held → restart sidecar → verify capability →
    release lock → reopen intake. If the restart or the capability
    verification fails, the lock stays held and the runtime falls
    back to the §8.3 retain-lock arm below.
...
The rotation replay store is a separate SQLite database outside `state/memory/memory.sqlite` (`core/memory/store.py:24`), so a snapshot of the durable capture queue never covers quarantined rows. The store lives at `state/memory/rotation_replay.sqlite` and is excluded from the rotation snapshot by the snapshot tool (it does not appear in the file manifest, the checksum pass, or the `sqlite3 backup` walk). It carries the same row identity as `memory_capture_queue` (`source_message_digest`, `principal_id`, `project_ref`, `session_id`, `epoch`, `payload_text`, `payload_attachments`, `provider_timestamp_ms`) plus ten rotation-specific columns, named explicitly below so a future implementer can read the table directly:

```text
rotation_replay row columns:
  source_message_digest      -- from memory_capture_queue (PK component)
  principal_id               -- from memory_capture_queue (PK component)
  project_ref                -- from memory_capture_queue (PK component)
  session_id                 -- from memory_capture_queue (PK component)
  epoch                      -- from memory_capture_queue (PK component)
  payload_text               -- from memory_capture_queue
  payload_attachments        -- from memory_capture_queue
  provider_timestamp_ms      -- from memory_capture_queue
  replay_state               -- enum {pending_rotation_replay,
                                 in_flight, delivered_pending_rotation_commit,
                                 manual_required, acknowledged_pending_local_settlement,
                                 settled, unknown}
  attempt_id                 -- UUIDv7, Avibe-owned, minted by the rotation tool
                                 before the request is sent
  attempt_epoch              -- the epoch the lease was acquired under
                                 (pre-rotation / advanced / rollback /
                                 acknowledged_pending_local_settlement);
                                 verified at every provider call and at
                                 success settlement
  add_request_id             -- EverOS-owned, populated only from the
                                 response envelope (NULL until ack parsed)
  lease_owner                -- rotation worker identity (host pid + worker
                                 uuid) currently holding the lease; NULL
                                 when no lease is held
  lease_acquired_at_ms       -- wall-clock when the lease was taken
  lease_expires_at_ms        -- wall-clock when the lease expires (taken
                                 from the configured lease timeout;
                                 absent when lease_owner IS NULL)
  operation_id               -- journal operation_id of the rotation
                                 that produced this row
  audit_pointer              -- pointer to the audit row for any
                                 manual_required / acknowledged / settled
                                 decision
```

**`attempt_id`, `attempt_epoch`, `lease_owner`, `lease_acquired_at_ms`, and `lease_expires_at_ms` together form the lease metadata.** The rotation tool writes `attempt_id`, `attempt_epoch`, `lease_owner`, `lease_acquired_at_ms`, `lease_expires_at_ms`, and `replay_state = in_flight` in the same SQLite transaction that records the lease (this is the explicit "transaction that records the lease" the lifecycle relies on), sends `/add` with no client request-id, then on response parses `add_request_id` from the envelope and writes it in a second transaction. `attempt_id` and `add_request_id` are distinct identities with distinct lifetimes: `attempt_id` is Avibe-owned and used by Avibe's own audit / retry / resolution logic; `add_request_id` is EverOS-owned and only readable from the response envelope parsed in `core/memory/everos.py:193-203`. The pre-send write cannot carry `add_request_id` because EverOS has not yet returned one, and the existing sidecar route guard `core/memory/sidecar.py:221-228` rejects an added client request-ID field on the `/add` payload (the validator requires the exact key set `{session_id, app_id, project_id, messages}` and the message dict's exact key set `{sender_id, role, timestamp, content}`), so the plan cannot smuggle a client-supplied request-id into the payload under the no-EverOS-modification constraint.

Boot recovery uses `lease_owner` and `lease_expires_at_ms` to decide whether a live claim is still owned: `in_flight` rows whose `lease_owner` matches a live worker and whose `lease_expires_at_ms` is in the future stay `in_flight`; `in_flight` rows whose lease has expired (no live owner, or `lease_expires_at_ms < now`) are recovered to `manual_required` because we cannot prove the write was not committed. `attempt_epoch` is verified at every provider call and at success settlement: an `in_flight` row whose `attempt_epoch` does not match the live session's epoch is a stale-epoch lease and is recovered to `manual_required`. `unknown` rows have `lease_owner IS NULL` and `lease_expires_at_ms` in the past; boot recovery treats them as `manual_required` for the same reason. Boot recovery never auto-replays an `in_flight` row.
...
- **Quarantine-transfer crash reconciliation.** The fence-copy phase described above is **journaled**: the rotation journal records `quarantine_transfer_in_progress` with the row's `(source_message_digest, principal_id, epoch, project_ref, session_id)` tuple and a per-row state of `copied_to_replay`, `removed_from_source`, or `reconciled` at the moment of crash. Boot recovery scans the journal for any open transfer: when a row is found in `copied_to_replay` but not yet `removed_from_source`, recovery completes the removal (the replay-store copy is already durable). When a row is found in neither `copied_to_replay` nor `reconciled` (the replay-store write itself crashed mid-commit), recovery removes the partial replay-store row and lets the next replay attempt re-issue the copy from the snapshot-covered source. When a row is found in `removed_from_source` but not `reconciled`, recovery **refuses to select a durable owner until both copies are reconciled**: it writes a `quarantine_transfer_owner_selection_pending` marker into the journal keyed by `source_message_digest` and blocks any replay, capture, or clear terminal that would otherwise claim the row until boot reconciliation chooses one durable owner by `(source_message_digest, attempt_id)` determinism — the replay-store copy wins iff the replay-store copy's `attempt_id` is greater than or equal to the source's restored `attempt_id`; otherwise the source copy wins and the replay-store row is marked `superseded`. The selected owner is the only row that subsequent replay, settlement, or clear operations may act on; the other copy is deleted in the same journaled step. Until that selection is durable, the row is **off-limits**: no replay will issue an `/add` against it, no clear will delete it without also deleting its sibling, and no /get or projection will surface it. The journaled owner-selection marker is idempotent on `source_message_digest`: re-running boot recovery on the same row is a no-op once a `selected_owner` row is present.
...
- The rotation replay store is **not** wiped by `restart` of the live capture queue. `clear` of the live queue, however, is a destructive operator action that explicitly removes every Avibe-owned local Memory data (`docs/plans/memory-plugin-system.md:290-305`) and is the only sanctioned path to remove quarantined rows. The maintenance fence prevents concurrent access but does not make commits across the two separate SQLite files atomic — SQLite's WAL (`core/memory/store.py:1188`) only orders writes within a single database file, and an attached multi-database transaction cannot provide cross-file atomicity — so a crash between the live-queue delete and the replay-store delete would otherwise let boot recovery recreate memory the user just cleared. `clear` therefore uses a **two-phase journaled completion** instead of a single fenced transaction:
  1. **Phase 1 — write the clear plan.** A new `clear_in_progress` row is written into a third persistent journal (`state/memory/clear_journal.sqlite` or an equivalent append-only log) before any destructive commit. The journal records `clear_started_at_ms`, `live_queue_digest`, `replay_store_digest`, `live_queue_snapshot_path`, `replay_store_snapshot_path`, `provider_data_snapshot_path`, and the operator identity. **The journal SQLite file cannot itself contain the snapshot directory** (it is a regular `.sqlite` database file and a subdirectory `<journal_id>/` next to it would fail with `ENOTDIR` on creation), so the journal SQLite lives at `state/memory/clear_journal.sqlite` and the per-clear snapshots live in a sibling directory at `state/memory/clear-journal/<journal_id>/` holding `live_queue.sqlite`, `replay_store.sqlite`, and `provider_data.sqlite` — the journal SQLite row records the sibling path. Each snapshot file is taken via the same `sqlite3 backup` walk that powers the runtime backup set, written under the per-root maintenance lock and verified by checksum before any destructive commit proceeds. The journal write is itself idempotent (a second `clear` request sees the open plan and refuses to start a parallel one).
  2. **Phase 2 — execute the destructive deletes under the per-root maintenance lock.** Delete rows from the live capture queue in `state/memory/memory.sqlite`; then delete rows from `state/memory/rotation_replay.sqlite`; **then** run the owned provider-root cleanup that `_clear_provider_data_or_fail()` invokes (`core/memory/module.py:670-675`, via `_run_owned_provider_cleanup` and `_recreate_owned_provider_root`) so the Markdown, memcells, indexes, and other owned provider-root contents are removed by the same fenced transaction. Phase 2 records per-substep completion timestamps (`live_queue_phase2_completed_at_ms`, `replay_store_phase2_completed_at_ms`, `provider_data_phase2_completed_at_ms`) so resume knows which substep to re-run after a crash. All three substeps complete before the lock releases; a `clear` that completes without the provider-root cleanup would still report success while the user's actual memory remains available after restart, which is the silent-state-divergence failure mode the journaled completion exists to prevent. After phase 2 the provider root is recreated empty (per `_recreate_owned_provider_root`) so the next operator `enable` finds the slot ready.
  3. **Phase 3 — write `clear_completed_at_ms` into the same journal row, then drop the snapshot directory but keep an immutable completed-journal record.** Phase 3 marks the journal row `clear_completed_at_ms`, then drops only the **open-state marker** (`clear_in_progress` boolean) and the **disposable snapshot directory** at `state/memory/clear-journal/<journal_id>/`. The completed journal row itself is **not** dropped: it is retained as an immutable `clear_completed` record carrying `clear_started_at_ms`, `clear_completed_at_ms`, `clear_journal_id`, the operator identity, the live-queue / replay-store / provider-data digests recorded at phase 1, the per-substep phase-2 completion timestamps, the snapshot directory path that was used (for audit only — not for restore, since the snapshot itself has been deleted), and the deletion audit row pointer. A `clear_completed` row is append-only: no field is mutable, no row is ever deleted by the runtime. Retention is bounded by an explicit `clear_journal_retention_days` policy (default 365d) — after that window the row is eligible for archival to `state/memory/clear_journal_archive/clear-<journal_id>.jsonl` and then dropped from the journal SQLite, with the archive file itself retained indefinitely for compliance. The rationale is that the completed journal is the only durable evidence that the operator's destructive `clear` ran to completion across all three durable homes, and dropping it on success would let a later dispute (operator claimed the clear failed, user claimed the clear never ran, support saw a partial state) be answered only by the operator's hearsay. Phase 3 therefore **separates** the open-state marker (which is what blocks a parallel clear and which boot recovery consumes) from the completed audit row (which is what answers post-hoc questions).
  4. **Crash recovery.** On boot, the recovery path reads the clear journal. If an open `clear_in_progress` row exists, the boot refuses to feed any `pending_rotation_replay` rows from the replay store into the live-queue feed (replaying quarantined rows against a state that the user already cleared is exactly the failure mode the journal exists to prevent), and instead surfaces the open clear plan on the processing-record page with an explicit "recovery needed" indicator. An operator command decides:
     - `fenced clear --resume`: re-execute the destructive deletes against the recorded digests. **Divergence detection treats partial deletion as expected**, not as a failure: the live queue and replay store may have already lost the rows deleted before the crash, in which case the resume predicate accepts a per-store state that matches either (i) the recorded `live_queue_digest` / `replay_store_digest` (no destructive work completed yet), or (ii) the empty-store digest (destructive work completed for that store before the crash). A store that matches neither — e.g. unexpected rows added since `clear_started_at_ms`, or a row whose contents differ from the pre-clear row — surfaces the divergence on the processing-record page and refuses to silently complete. The journal records per-store phase completion (`live_queue_phase2_completed_at_ms`, `replay_store_phase2_completed_at_ms`) so resume knows which destructive delete to re-run; an empty digest on a store whose phase is already complete is a successful no-op. On a successful resume, phase 3 closes the journal normally.
     - `fenced clear --abort`: restore all three durable homes from the snapshot directory recorded in the journal — `state/memory/memory.sqlite`, `state/memory/rotation_replay.sqlite`, **and the owned provider-root contents** (Markdown, memcells, indexes, and any other cleared operational stores that `_clear_provider_data_or_fail()` removes in `core/memory/module.py:670-675` via `_run_owned_provider_cleanup` and `_recreate_owned_provider_root`). The provider-root snapshot is taken at phase 1 start, before any destructive work, via a parallel `sqlite3 backup` walk plus a recursive file copy under the per-root maintenance lock; the journal records `provider_data_snapshot_path` so abort can locate it. Abort restores all three homes to their pre-clear state, marks the journal row `clear_aborted_at_ms`, and drops the journal row. The snapshots make abort a real rollback rather than a best-effort guess — each home is replaced with the journal-persisted copy, each snapshot's checksum is re-verified at restore time, and any row that the journal's digest recorded as present at `clear_started_at_ms` is restored to its pre-clear state. **Snapshotting only the two Avibe databases is not acceptable**: an abort that restored only the SQLite files would leave the restored queue claiming a pre-clear state while the previously extracted Markdown / memcells / indexes remain deleted, which is exactly the silent-state-divergence failure mode this plan exists to prevent. Abort is **not** allowed after phase 3 has dropped the snapshot directory (a completed clear is not undoable through this path; the operator who issued the completed clear can only invoke it again with the same destructive semantics, never restore deleted rows). The journal is the single source of truth for crash-safe completion across all three durable homes; a `clear` that never wrote the open plan is treated as if `clear` never started (boot recovery proceeds normally).
  5. **Audit.** A deletion audit row is written before phase 1 closes, recording the operator, the timestamp, the live-queue count, the replay-store count, and the journal row id. The journal row carries the audit pointer and the snapshot directory path.
  The fence is the same per-root maintenance lock that powers rotation and the audited `operate` path; rotation rollback and operator `restart` are the only paths that may leave the replay store undeleted.
...
- A replay `/add` whose response cannot be confirmed (timeout, transport reset, or sidecar accepted-but-silent after the request was sent) is **not** marked `pending_rotation_replay` for retry. §10.1 treats write timeouts as ambiguous outcomes that auto-replay may not silently resolve; replay would risk duplicating an already-accepted memory. Such a row is marked `manual_required` with the audit pointer (the journal `operation_id`, the row's `attempt_id`, the row's `add_request_id` if the response partially parsed before the timeout, and the timeout evidence), and the rotation replay store is excluded from the live-queue feed. The row stays in the replay store until an audited `manual_required` resolution records operator intent and transitions the row to a follow-on state: case rf1 (later confirmed acknowledged) transitions `replay_state` to `delivered_pending_rotation_commit` with the confirmed `add_request_id`, then normal success-settlement runs once the rotation journal commits; case rf2 (confirmed `not_committed`) demotes the row to `pending_rotation_replay` for the next rotation; case rf3 (refuse and dispose) removes the row and records the operator decision in the audit log. The embedding fingerprint is not modified by any of these transitions — rf1 does not "pin a confirmed-acked fingerprint" because the fingerprint was already written in step 5 of §8.3 before any replay was attempted, and changing it would invalidate the new root's vector space. Boot recovery never replays a `manual_required` row.
- On rotation success, the rotation tool drains the replay store: rows that committed through the normal success-settlement path are removed. Rows that the success-settlement path itself rejected (rare; a row whose replay ack parsed but whose settlement path returned a failure) are **not** routed through a provider re-delivery — EverOS has already received the `/add`, so any subsequent `/add` would duplicate the memory in the rebuilt root. The retry path is **local-settlement-only**: the row's `replay_state` transitions to a new `acknowledged_pending_local_settlement` value (a non-replayable acknowledgement state that cannot issue another `/add`); the row keeps its confirmed `add_request_id` and the parsed `confirmed_watermark_ms` from the original ack envelope; the local SQLite success-settlement (`core/memory/store.py:491-500`) is retried under the per-root maintenance lock without re-issuing `/add`. If the local-settlement retry succeeds, the row transitions to `settled` and is removed from the replay store. If the local-settlement retry fails again, the row carries a leased `manual_required` reservation that pauses new capture `/add` for that principal until the reservation resolves — the same no-newer-first invariant that applies to a rollback drain applies here, because the row is an already-accepted capture for the same human user. The reservation is operator-visible on the processing-record page and follows the same audited resolution path as a fresh `manual_required` (rf1/rf2/rf3) — rf1 confirms the local settlement succeeded and removes the row with audit; rf2 confirms the row should be disposed because the provider already accepted it but local settlement cannot complete (operator decision); rf3 refuses the row (operator decision). The local-settlement retry keeps the same `attempt_id` so the audit chain is continuous and the provider-side acknowledgement is never re-attempted. The reservation across unrelated principals does not block each other. `manual_required` rows that were already `manual_required` before rotation success do not return to `pending_rotation_replay` automatically; they wait for the audited resolution.
- On rotation-failure terminal (snapshot restore verified), the rotation tool **actively drains** `pending_rotation_replay` rows against the restored old sidecar before reopening capture acceptance, in the same fenced transaction that restarts the old sidecar and reopens intake. The drain attempts each row against the restored sidecar in original order. **Any row whose drain fails is NOT left in the replay store while intake reopens**: doing so would let newer captures proceed against the restored old sidecar while previously-accepted captures from the same human user sit undelivered indefinitely, since no subsequent rotation or process restart is guaranteed. The failure handling depends on what kind of failure the drain observed:
  - **Transient failure (timeout, transport reset, capability probe retry).** The row remains `pending_rotation_replay` AND is **moved back into the live capture queue's retry feed ahead of any new captures**: the rotation tool writes a `replay_drain_failed` audit row with the per-row evidence and writes a leased `manual_required` reservation **scoped to the canonical provider session reference** `(principal_id, epoch, project_ref, session_id, target_generation)` of the failed row (not to the principal alone) so the worker pauses new capture `/add` for the affected session until the reservation is resolved. A reservation scoped to the principal alone would stall Memory delivery for every other session the human user participates in — including a Slack DM, a fresh Workbench session, and a different project — when one conversation has a stuck replay, which violates §5.5 and the acceptance §15 rule that `manual_required` leaves unrelated sessions unblocked. The session-scope key now matches the §9.1 bounded-wait target exactly, so a later `freshness=bounded` recall against the same session / generation will see the reservation and avoid being misled by a missing row. The reservation is operator-visible on the processing-record page. Intake reopens only after the per-session reservation is recorded; reservations across unrelated sessions (and across unrelated principals) do not block each other. The reservation follows the same audited resolution path as a fresh `manual_required` (rf1/rf2/rf3) — rf2 promotes the row back to `pending_rotation_replay` for the next rotation attempt, rf3 disposes it. The row stays ahead of new captures for the same session until the reservation clears, satisfying the no-newer-first invariant.
  - **Permanent failure (capability mismatch, sidecar rejection).** The row is transitioned directly to `manual_required` in the replay store with the per-row evidence, and intake reopens without delay because the row will not be replayed automatically. The audited resolution path is the same rf1/rf2/rf3. **Intake does not reopen for the same session until the audit decision is recorded** — the same session-scoped reservation used by the transient branch is held until rf1/rf2/rf3 resolves the row, so newer captures for the same session cannot reach EverOS first while the older row remains unresolved indefinitely. Resuming intake without the reservation would let an operator who has not yet audited the row see newer captures land first, which is the same no-newer-first violation the transient branch exists to prevent. The lock release that opens intake to other sessions / principals is independent; the reservation is a per-session lever, not a global one. The reservation is **session-scoped** for the same reason as the transient branch — the §5.5 invariant is observed even when the row is non-replayable, so a permanent failure in one session does not stall Memory delivery elsewhere.
  - **Ambiguous failure (provider accepted but acknowledgement lost).** The row is transitioned to `manual_required` with the existing §10.1 evidence; auto-replay is forbidden for the same reason. Intake reopens only after the per-session reservation is recorded, same as transient failure.
  Rows that deliver transition to normal success settlement. The drain runs under the per-root maintenance lock; capture acceptance stays paused for the duration; only after the drain completes and any necessary per-principal reservations are recorded does the lock release and intake reopen. This makes "rotation failure" a true rollback to the pre-rotation state: no quarantined row is dropped from the operator's responsibility, and no new capture for the same principal leapfrogs a still-pending replay.
...
RecallPolicy
  ...
  -- request-wide deadline (independent of freshness and agentic-step
     timeouts) --
  process_timeout_seconds: int (required, missing or zero is rejected;
      bounds the wall-clock duration of the entire request — primary
      run plus any declarations — and is the field the §9.2.1 sum-budget
...
- `freshness=bounded` requires three additional fields so the adapter can implement the §14 success criterion against the right session: `wait_scope` must be the trusted provider session reference (`principal_id`, `epoch`, `project_ref`, `session_id` per §5.3), `target_generation` must name the generation that bounded recall is waiting on, and `target_watermark_ms` must be the watermark the adapter must observe before returning success. The generic `filters` tree is **not** an acceptable substitute for these fields — a filter cannot identify a session. A `bounded` request that omits any of `wait_scope` / `target_generation` / `target_watermark_ms` is rejected at the adapter layer with the same fail-closed rule as the agentic budget fields; success defined as "flush confirmed successful" (§14) cannot be implemented without naming the generation whose flush must confirm. The observation source for the target generation / watermark / flush_state is the **Avibe-owned coordinator's durable state** (`memory_meta` plus the per-session durable generation / watermark / flush_state columns), not EverOS: the sidecar's `core/memory/sidecar.py:177-200` allowlist exposes only `/health`, `/add`, `/flush`, `/search`, and `/get`, and `MemoryRuntime.status_payload()` (`core/memory/runtime.py:542-558`) is an aggregate without per-session generations or watermarks. The plan does not modify EverOS, so a per-session `/status` projection is out of scope for this plan. The adapter observes through the same coordinator-owned Avibe durable state that the capture path already writes; the session fence that the capture path holds is the same fence the bounded wait observes under. The bounded predicate does **not** read the single live `generation` / `flush_state` columns — those columns advance as new captures arrive, and reading them after the target generation has already settled would still see a non-`settled` flush_state for the next generation and incorrectly return `timeout`. Instead, the coordinator writes a per-generation **flush settlement record** at the moment a generation's flush transitions to `settled`: the row stores `(generation, settled_at_ms, confirmed_watermark_ms, flush_state=settled, source=flush|natural_boundary)` and is appended to an append-only `session_flush_settlements` table (or an equivalent column family on `memory_meta`) — the row is never overwritten. **Natural-boundary extractions also produce a settlement record**: when `/add` returns `status=extracted` (EverOS has already written Markdown without an explicit flush), the coordinator writes a settlement row with `source=natural_boundary` and `confirmed_watermark_ms` set to the row's `provider_timestamp_ms`. Without this, a bounded-recall receipt targeting the naturally extracted generation could never find its exact settlement row and would time out even though EverOS has already written Markdown — the capture is durably committed, but the settlement record is missing. The natural-boundary record is written by the same worker that processes the `/add` ack, in the same SQLite transaction that marks the local row delivered, so a crash between the `/add` ack and the settlement write is recovered normally (the row is still `delivered` and the settlement write is idempotent on `(session_ref, generation)`). The **value the adapter compares to `target_watermark_ms` is `confirmed_watermark_ms`, not `settled_at_ms`**: the bounded wait is for *captures up to a watermark*, not for *settlements up to a timestamp*, and the settlement record persists the generation's confirmed add watermark at the moment the flush succeeded (the largest `provider_timestamp_ms` acknowledged by the flush) so that comparing `confirmed_watermark_ms >= target_watermark_ms` is the correct direction. Comparing `settled_at_ms <= target_watermark_ms` would invert the predicate (settlement normally happens after the watermark has been reached, so a successful flush would time out), and comparing `settled_at_ms >= target_watermark_ms` would be a clock-time comparison that drifts independently of the actual capture order. On every poll, the adapter looks up the row for `(session_ref, target_generation)` and returns success only when **all three** of the following are true for that exact generation row: (a) the row exists; (b) the row's `confirmed_watermark_ms >= target_watermark_ms`; (c) the row's `flush_state` is `settled`. A `flush_state` of `due` / `in_flight` / `unknown` / `manual_required` for the target generation is not a successful freshness result even when the live columns look right — §14 success is a confirmed flush for the *named* generation, not a watermark the provider has merely acknowledged for a *later* generation. When the deadline elapses with the target generation row missing or below the required threshold, the adapter returns `timeout`; when an unresolvable error blocks observation, it returns `unknown`. The same three-way predicate applies regardless of mode, including `agentic` (which observes bounded freshness on the same coordinator state before its LLM step).
...
- A single `RecallPolicy` body may carry multiple independently-budgeted mode runs through the explicit `declarations: list<RecallDeclaration>` field. The top-level scalar `mode` is the primary run and remains the contract for the caller-facing response; each entry in `declarations` is an additional independently-budgeted run that shares the same `limit` and `freshness` contract but has its own `RecallBudget`. JSON duplicate-key encoding is **not** used and would be silently dropped by ordinary parsers, so the multi-run shape is modeled only through the typed list field. The adapter executes declarations in declared order, each under its own budget; the caller sees the primary run's response and the declarations' results are surfaced only through their own metadata projection (no implicit merging, no implicit fallback between declarations). Each declaration's `limit` must satisfy `declaration_limit <= caller_limit`; an `agentic` declaration must carry the full §9.2.2 budget set; a declaration that uses `freshness=bounded` must carry `freshness_timeout_seconds`. The total wall-clock budget of the request is the sum of declaration budgets plus the primary run's budget; the adapter rejects a request whose total exceeds the caller's `process_timeout_seconds`. A request with no `declarations` field is a single-mode run; the existing scalar `mode` is the source of truth.
- **Fan-out ceilings.** To prevent a multi-declaration request from becoming an unbounded cost or latency vector, the adapter enforces fixed ceilings that the call site cannot raise: `max_declarations_per_request = 4` (primary run plus up to 3 declarations; declared in `config/v2_config.py` under `memory.recall.fan_out`, overrideable for self-hosted deployments but not by an individual request), `max_agentic_declarations_per_request = 1` (only one declaration per request may use `agentic` mode, regardless of how many declarations exist; additional declarations must use `keyword`, `vector`, or `hybrid` — combining two `agentic` declarations in one request is rejected with `fan_out_exceeded`), `max_declaration_total_budget_seconds = min(sum of declaration budgets, process_timeout_seconds)` (the adapter additionally bounds the aggregate wall-clock budget independently of `process_timeout_seconds` so a 4-declaration request with each declaration consuming most of its own budget cannot bypass the request-wide deadline), and a non-agentic aggregate retrieval budget of `max_non_agentic_declarations_results_per_request = 64` (sum of `max_results` across all non-agentic declarations; an agentic declaration does not contribute to this aggregate because its own `cost_budget_tokens` and `max_model_calls` ceilings apply). The fan-out ceilings are checked at the adapter boundary before any declaration is dispatched: a request whose declaration count, aggregate budget, or aggregate non-agentic result count exceeds the ceiling is rejected with `fan_out_exceeded` and no provider call is issued. These ceilings are part of the §9.2.1 contract and the same ceilings apply regardless of the call site (UI, CLI, another backend module).
...
2. **Principal-scoped list reads.** The list endpoint accepts a `principal_id` filter, defaults the filter to the principal derived from the requesting principal's verified UI key, and refuses to render a memcell whose `sender_ids_json` does not contain that principal — **including memcells whose `sender_ids_json` contains any sender that is not in the verified principal set**. The reader's existing single-principal check (`core/memory/everos_insight/reader.py:450-498`, `json_array_length(sender_ids_json) = 1`) does not by itself catch a memcell that mixes one verified sender with an unverified one, which would happen if a future capture path leaks a system-side or impersonated sender alongside the real sender. The list-read predicate is therefore strengthened to require that **every sender in `sender_ids_json` belongs to the verified principal set** the requesting browser principal owns, not just that the verified principal set intersects the sender set. A memcell whose `sender_ids_json` includes any sender outside that set is rejected with `sender_unverified` and never returned to the list view — even though the memcell may also include the legitimate sender, the presence of an unverified sender means the row's provenance cannot be trusted and the row is dropped until an audited reconciliation records the offending sender. The full internal view (un-redacted, all principals) is reachable only through an explicit admin capability that is not granted by `memory_admin_log_access`; the Cloud browser session therefore cannot enumerate memcells belonging to other principals even if it bypasses the redacted projection. The default is derived through the same canonical helper that all capture paths already use: `runtime.principal_for_user_key()` (`core/memory/store.py:208`), whose underlying `derive_principal_id` HMACs the literal input under the per-root `scope_key` (`core/memory/store.py:1490-1498`). HMAC over a fixed key is one-way and collision-resistant, so **distinct literal inputs yield distinct `u-…` principals**: `avibe:local` produces one principal, `avibe:remote:<subject>` produces another, and `slack:<user_id>` / `discord:<user_id>` / `telegram:<user_id>` / `lark:<user_id>` / `feishu:<user_id>` / `wechat:<user_id>` each produce their own principals. Lark is treated as its own platform literal — `core/memory/admission.py:29,99` derives a Lark capture's principal from the literal `lark:<user_id>`, and `lark` is in `IM_PLATFORMS` as a distinct entry from `feishu` (the international vs domestic split) — so a Lark capture's principal is `u-…` of `lark:<user_id>`, not of `feishu:<user_id>`. Pairing must therefore enumerate Lark separately so the alias map can bind a browser principal to a Lark capture. The browser principal and the IM principal of the same human user are **not** the same string, and the round-7 wording that claimed they were is wrong. The list-read path therefore enumerates the **principal set** that the requesting browser principal owns, not a single principal string:
...
- Profile switching to `/get` is no longer gated on a project-isolation gap because the profile scope is user-global (§14). The Phase 3 task is to add the owner-keyed `/get(memory_type="profile")` adapter path while keeping the search-literal fallback for hosts that prefer it, and to test the user-global contract end-to-end. **Migration of existing per-project profile rows must complete before the owner-only adapter ships.** The current reader addresses profiles at `avibe/{project_id}/users/{principal_id}/user.md`, while the new route deliberately supplies no reliable project filter; declaring the target scope user-global without consolidating the historical rows would let users receive an arbitrary project profile or lose visibility of the others after the cutover. The migration is journaled and runs once at startup before the owner-only adapter is enabled. The migration policy for a principal that already has profile rows under multiple project paths:
  1. **Inventory.** Read every existing `avibe/{project_id}/users/{principal_id}/user.md` for the principal and compute a per-row hash + size + `last_modified_at_ms`. The inventory is recorded in the same journal as the rotation journal so the migration is durable and replayable.
  2. **Selection rule.** Pick the single canonical row that becomes the owner-only profile: the row with the most recent `last_modified_at_ms` wins; ties are broken by the largest size, then by the lexicographically smallest `project_id` for determinism. The selection is recorded with a per-row audit pointer.
  3. **Archive, do not delete.** Every non-canonical row is moved to `state/memory/profile_archive/users/{principal_id}/{project_id}-{last_modified_at_ms}.user.md` — **outside the owned provider root** at `avibe/`. Archived profiles therefore cannot re-enter the provider-call surface, the memory log projection, the rotation snapshot, the snapshot's checksum walk, or the runtime backup set's projection scan, all of which only traverse the owned provider root's tree. Storing the archive inside `avibe/` would put dormant, possibly stale profile rows back into the projection scan on every boot, which is exactly the leak the migration exists to prevent. The archive directory is Avibe-owned state (not provider-owned state) and is the only durable surface for the archived bytes; the migration journal records the original `project_id`, hash, size, `last_modified_at_ms`, and the new archive path for each archived row. Restoring an archived row into the canonical path is an explicit operator action that re-runs the inventory + selection rule, not an automatic scan.
  4. **Promote canonical.** The canonical row's bytes are written into `avibe/users/{principal_id}/user.md` (the new owner-only path) under the per-root maintenance lock with the same `sqlite3 backup` / checksum verification the runtime backup set uses.
  5. **Audit.** A deletion-class audit row records the operator (here: the migration itself), the per-row inventory, the selection rule, the archived paths, and the new canonical path. The audit row is durable and survives restart.
  6. **Rollback.** If the migration fails partway, the journal carries a per-row `migrated_at_ms` so a rollback can move archived rows back to their original `avibe/{project_id}/users/{principal_id}/user.md` paths. The owner-only adapter stays disabled until the migration either completes cleanly or rolls back to the original per-project layout.
  Any future reopen of project-scoped profile is a separate decision with its own upstream dependency.
...
- **Closed-loop scenario catalog for IM alias pairing (§11.4).** The pairing flow above is the trust boundary between browser principal and IM principal and is not covered by an existing catalog. A new `tests/scenarios/memory_log/` catalog and a closed-loop scenario harness case are added before Phase 3 ships, with visible scenario IDs and case coverage: `memlog_pairing_challenge_success` (browser issues challenge to a real Slack/Discord/Telegram/Lark/Feishu/WeChat account, response verifies, alias row recorded, the corresponding IM capture's memcell becomes visible to the browser list view), `memlog_pairing_challenge_failure` (challenge response fails or expires, no alias row recorded, list view continues to hide the IM capture), `memlog_pairing_replay_rejected` (the same challenge nonce is replayed against a different browser principal, alias row refused because the verification_evidence_ref does not match the new requester's verified UI key), `memlog_pairing_revocation` (alias row is revoked by either side, list view no longer includes the IM principal for that browser principal, the next capture from that IM account becomes invisible to the browser list view), `memlog_pairing_lark_vs_feishu_split` (Lark pairing is attempted against an existing Feishu alias — refused — and vice-versa — also refused — because the alias map treats Lark and Feishu as distinct platform literals), `memlog_pairing_unverified_sender_dropped` (a memcell's `sender_ids_json` contains the verified IM principal AND a non-verified sender — list view rejects the row entirely until reconciliation), and `memlog_pairing_admin_view_separate` (an unprivileged Cloud browser session sees only its own alias rows; the admin capability surface that exposes the full alias map is a separate gate). Each case has a deterministic stub for the platform-side ownership challenge so the test does not depend on a live Slack/Discord/Telegram/Lark/Feishu/WeChat round trip. Provider-specific parsing / heuristics live in focused unit tests under `tests/scenarios/memory_log/parsing/`, and the catalog YAML is updated alongside the cases. The same closed-loop harness pattern is reused from `tests/scenarios/auth_setup/test_auth_setup_scenarios.py`.
...
- `clear` is a destructive operator action that explicitly removes every Avibe-owned local Memory data (`docs/plans/memory-plugin-system.md:290-305`) **and** removes `manual_required` rows from the rotation replay store. The plan's §8.3.1 establishes that the maintenance fence prevents concurrent access but does not make commits across the two separate SQLite files atomic — SQLite's WAL (`core/memory/store.py:1188`) only orders writes within a single database file, and an attached multi-database transaction cannot provide cross-file atomicity. The acceptance criterion therefore requires the **two-phase journaled completion** specified in §8.3.1: a `clear_journal` row records `clear_started_at_ms`, the live-queue and replay-store digests, and the operator identity before any destructive commit; the destructive deletes are issued under the maintenance lock; the journal row is marked `clear_completed_at_ms` and dropped; a mid-clear crash leaves the journal open and surfaces on the processing-record page, and boot recovery refuses to feed `pending_rotation_replay` rows into the live-queue feed until the open clear plan is resolved (resume against the recorded digests or abort with row restoration per §8.3.1). A bare "same fenced transaction" implementation is **not** acceptable here because the two databases are separate SQLite files and cannot be made atomic without a journal. **This is destructive clear, distinct from "clear-as-evidence"**, which is forbidden: §5.5 case rf3 already records operator intent through the audited `operate` path, not through `clear`; a `clear` that silently treated every `manual_required` row as resolved would destroy the evidence chain that proves the operator saw and decided the row. The destructive `clear` is the only path that may remove `manual_required` rows without producing an rf1/rf2/rf3 decision row, and it must produce its own audit row pointing at the `clear_journal` row that authorized it;
