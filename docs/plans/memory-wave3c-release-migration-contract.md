...
>
> Scope: Wave 3c Phase 0, Doc B; review begins only after Doc A merges
...
The existing release identity and one asset-complete finalizer remain the
publication owner. It stages and verifies both distributions, publishes Memory
before core, rechecks public availability, and only then removes the transition
release gate. Manifest ownership follows the distribution that owns each
artifact; no second tag, finalizer, or release identity is introduced.

Doc B depends normatively on the lifecycle and readiness invariants in Doc A:
`PackageLifecycleTransaction` owns mutation and recovery, while this document
defines release-family and migration rules that must produce resolver-satisfiable
targets. No implementation begins from this Phase 0 draft.
...
and this document. The retained branches are evidence only; no code is
cherry-picked.
...
- no product, test, catalog, workflow, release, manifest, or config change in
  this Phase 0 PR; and
- no implementation or PR-B review before Doc A merges and the owner approves
  the release gate.
...
express a real captured shape; the hard-dependency transition family does not.
...
The existing release workflow stages one asset-complete Draft and has one final
publication owner. The finalizer must perform these checks and ordering inside
...
and finalizer ordering evidence.
...
| #1741 single-finalizer staged publication ordering | Preserved and sharpened here with Memory-before-core and post-publication verification |
...
1. **Phase 0, Doc A:** approve and merge the lifecycle contract first.
2. **Phase 0, Doc B:** after Doc A merges, approve and merge this release and
   migration contract. Until then this branch has no PR and no Codex review.
3. **Implementation gates 2-4:** Doc A merge unlocks lifecycle implementation
   review; Doc B merge additionally unlocks release-family and migration work.
4. **Gate 5, release transition:** only after the transition wheelhouse,
   scenario 022/023, Doc A's 018-021 evidence, and the single finalizer's
   staged/public checks pass may the release gate be removed.
