...
# here. Acceptance moves that snapshot into ``messages`` under the same id.
message_deliveries = Table(
    ...
    Column("dispatch_text", Text, nullable=False),
    Column("dispatch_sha256", String, nullable=False),
    Column("dedupe_key", Text, nullable=True),
    Column("accepted_turn_id", String, nullable=True),
    ...
    ForeignKeyConstraint(
        ...
        deferrable=True,
        initially="DEFERRED",
        name="fk_message_deliveries_accepted_turn",
    ),
    ForeignKeyConstraint(
    ...
    UniqueConstraint("message_id", name="uq_message_deliveries_message"),
    UniqueConstraint("dedupe_key", name="uq_message_deliveries_dedupe"),
    UniqueConstraint("current_attempt_id", name="uq_message_deliveries_current_attempt"),
    CheckConstraint("priority in ('p0', 'p1', 'p3')", name="ck_message_deliveries_priority"),
    CheckConstraint(
        "state in ('reserved', 'queued', 'start_attempting', 'pending_steer', "
        "'steering', 'interrupt_waiting', 'reconciling_start', "
        "'reconciling_steer', 'accepted', 'retired')",
    ...
    CheckConstraint(
        "message_id is null or message_id = id",
        name="ck_message_deliveries_stable_message_id",
    ),
    CheckConstraint(
    ...
    Index("ix_message_deliveries_accepted_turn", "accepted_turn_id"),
