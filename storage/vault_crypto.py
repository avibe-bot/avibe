...
def _load_key_file(path: Path) -> bytes:
    """Read + validate an existing machine key, tightening its mode to 0600 if it's looser.

    This key decrypts every standard-tier secret and the feature relies on it being 0600. A key
    restored from backup or copied with 0644 would stay readable by other local users while
    everything kept working; repair it on read rather than silently trusting it.
    """
    key = path.read_bytes()
