...
class CodexAgent(BaseAgent):
    ...
    def _write_caller_env_script(self, request: AgentRequest) -> Path | None:
        from core.caller_context import (
            ensure_private_caller_context_dir,
        ...
        # This script exports the session's Memory CLI capability so reused
        # shells re-read a refreshed one (see 516008b2). At the default umask
        # that credential would land 0644 in a 0755 directory whose file names
        # are Agent session ids, so both are narrowed to the owning user.
        ensure_private_caller_context_dir(script_path.parent)
        lines = ["# Generated by Avibe. Sourced by Codex shell commands.\n"]
        for key, value in sorted(env.items()):
            lines.append(f"export {key}={shlex.quote(value)}\n")
        write_private_caller_context_file(script_path, "".join(lines))
        return script_path
