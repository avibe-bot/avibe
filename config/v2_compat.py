...
def _runtime_agent_cli_path(configured: str | None, default: str) -> str:
    ...
    if not is_desktop_managed_runtime():
        return selected
...
def to_app_config(v2: V2Config) -> AppCompatConfig:
    claude = ClaudeCompatConfig(
        ...
        system_prompt=None,
        cli_path=_runtime_agent_cli_path(v2.agents.claude.cli_path, "claude"),
    ...
    if v2.agents.codex.enabled:
        codex = CodexCompatConfig(
            enabled=True,
            binary=_runtime_agent_cli_path(v2.agents.codex.cli_path, "codex"),
            extra_args=[],
    ...
    if v2.agents.opencode.enabled:
        opencode = OpenCodeCompatConfig(
            enabled=True,
            binary=_runtime_agent_cli_path(v2.agents.opencode.cli_path, "opencode"),
            port=4096,
