...
def dotenv_candidates(root: Path | None = None) -> tuple[Path, Path]:
    checkout = root or workspace_root()
    return (
...
def discover_provider_settings(root: Path | None = None) -> ProviderSettings:
    for candidate in dotenv_candidates(root):
...
def sync_locked_environment(*, root: Path | None = None, uv_binary: str = "uv") -> Path:
    ...
    checkout = root or workspace_root()
    state = ensure_owner_directory(runtime_root(checkout))
...
def verify_locked_environment(python: Path | None = None) -> Path:
    """Reject host Python, user-site packages, and a mismatched provider wheel."""
    target = python or locked_environment_python()
    if not target.is_file() or not os.access(target, os.X_OK):
        raise HarnessError("locked_environment_missing")
    code = (
        "import importlib.metadata as metadata, site, sys; "
...
def child_environment(settings: ProviderSettings, *, everos_root: Path, child_home: Path, metrics_path: Path) -> dict[str, str]:
    """Build the only environment inherited by the provider child process."""
    ensure_owner_directory(child_home)
    xdg_config = ensure_owner_directory(child_home / ".config")
    xdg_data = ensure_owner_directory(child_home / ".local" / "share")
    xdg_cache = ensure_owner_directory(child_home / ".cache")
    xdg_state = ensure_owner_directory(child_home / ".local" / "state")
    env = {
        "ENV": "prod",
