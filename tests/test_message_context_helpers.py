...
def test_turn_sink_key_stays_fixed_when_session_binds_mid_turn():
    """Dispatch selects the sink before the backend can bind a Session row."""
    context = MessageContext(
        user_id="U1",
        channel_id="C1",
        platform="discord",
        platform_specific={"turn_sink_key": "discord::C1"},
    )
    context.platform_specific["agent_session_id"] = "ses_runtime_a"

    assert build_context_turn_sink_key(context) == "discord::C1"
