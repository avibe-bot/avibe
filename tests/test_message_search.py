...
def test_search_finds_status_by_default(isolated_state):
    engine = create_sqlite_engine()
    with engine.begin() as conn:
        scope_id = _seed_scope(conn)
        _seed_session(conn, scope_id, "ses_status", "Show status")
        _insert_msg(
            conn,
            scope_id,
            "ses_status",
            "agent",
            "Show Runtime failed to render",
            "2026-06-01T10:00:00Z",
            msg_type=messages_service.STATUS_TYPE,
        )

    with engine.connect() as conn:
        result = messages_service.search_messages(conn, query="render")

    match = result["sessions"][0]["matches"][0]
    assert (match["author"], match["type"]) == (
        "agent",
