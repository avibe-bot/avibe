...
async def test_native_orphan_shift_between_classification_and_wait(tmp_path, monkeypatch):
    if sys.platform != "darwin":
        pytest.skip("Darwin native fault")
    from psutil import _psosx

    child = await spawn(tmp_path)
    reference = psutil.Process(child.pid)
    stamp = reference.create_time()
    try:
        monkeypatch.setattr(_psosx, "boot_time", lambda: _psosx.INIT_BOOT_TIME + 1)
        assert psutil.Process(child.pid).create_time() == stamp + 1
        assert not await module._wait_for_identities_exit({child.pid: reference}, 0.1)
        reference.terminate()
        await child.wait()
        assert await module._wait_for_identities_exit({child.pid: reference}, 0.1)
    finally:
        await dispose(child)
...
async def test_reused_pid_stays_terminal_during_delayed_child_callback(monkeypatch):
    kernel = {}
