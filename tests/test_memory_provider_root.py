...
def _write_released_generated_controls(root: ProviderRoot) -> dict[str, bytes]:
    root.path.parent.mkdir(mode=0o700)
    root.path.mkdir(mode=0o700)
...
def _write_released_unsentinelled_root(root: ProviderRoot) -> None:
    controls = _write_released_generated_controls(root)
    for name, payload in controls.items():
        path = root.path / name
        path.write_bytes(payload)
        path.chmod(0o600)
...
@pytest.mark.parametrize(
    "root_entries",
    [
        {".everos.toml.tmp": "partial"},
        {"everos.toml": "complete"},
        {"everos.toml": "complete", ".ome.toml.tmp": "partial"},
    ],
    ids=("everos-temp", "everos-complete", "ome-temp"),
)
def test_provider_root_clear_recovers_released_partial_control_write(
    tmp_path: Path,
    ...
    metadata = _metadata()
    controls = _write_released_generated_controls(root)
    for entry_name, state in root_entries.items():
        control_name = (
            entry_name.removeprefix(".").removesuffix(".tmp")
            if entry_name.startswith(".")
            else entry_name
        )
        payload = controls[control_name]
        if state == "partial":
            payload = payload[:24]
        path = root.path / entry_name
        path.write_bytes(payload)
        path.chmod(0o600)
...
@pytest.mark.parametrize(
    ...
    [
        "mismatched_config",
        "mismatched_temporary_config",
        "partial_controls_with_provider_data",
        "unknown_child",
        "symlinked_index",
    ],
)
def test_provider_root_clear_refuses_unproven_unsentinelled_root(
    ...
    elif unsafe_evidence == "mismatched_temporary_config":
        temporary = root.path / ".ome.toml.tmp"
        temporary.write_bytes(b"foreign")
        temporary.chmod(0o600)
    elif unsafe_evidence == "partial_controls_with_provider_data":
        (root.path / "ome.toml").unlink()
