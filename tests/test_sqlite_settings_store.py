...
def test_settings_save_does_not_absorb_a_newer_external_revision(
    ...
    def save_then_race(state: SettingsState, *, user_context=None, baseline=None) -> str:
        committed_revision = original_save(state, user_context=user_context, baseline=baseline)
        external.save_state(
            SettingsState(
                channels={
