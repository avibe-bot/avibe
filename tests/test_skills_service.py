...
def test_runtime_catalog_filter_reuses_unified_listing_acl(monkeypatch) -> None:
    captured = {}
    ...
    allowed = skills.filter_accessible_runtime_skill_names(
        [
            {"name": "allowed", "scope": "project", "backends": ("codex",)},
            {
                "name": "denied",
                "scope": "global",
    ...
    assert captured["result"]["skills"] == [
        {"name": "allowed", "scope": "project", "agents": ["codex"]},
        {"name": "denied", "scope": "global", "agents": ["claude", "opencode", "codex"]},
    ]
    assert captured["scope"] == "all"
    assert captured["project_dir"] == "/project"
    assert captured["project_id"] == "project-1"
    assert captured["backends"] is None


def test_runtime_catalog_authorizes_only_the_resolved_candidate_backend(monkeypatch) -> None:
    captured = []

    def filter_listing(result, **_kwargs):
        captured.extend(result["skills"])
        return {
            "ok": True,
            "skills": [row for row in result["skills"] if row["agents"] == ["claude"]],
        }

    monkeypatch.setattr(skills, "_filter_skill_listing", filter_listing)
    monkeypatch.setattr(skills, "_project_id_for_skill_directory", lambda _path: None)

    allowed = skills.filter_accessible_runtime_skill_names(
        [
            {"name": "codex-winner", "scope": "global", "backends": ("codex",)},
            {"name": "claude-winner", "scope": "global", "backends": ("claude",)},
        ],
        backend="opencode",
        project_dir="/project",
        user_context={"sub": "remote-user"},
    ...
    assert captured == [
        {"name": "codex-winner", "scope": "global", "agents": ["codex"]},
        {"name": "claude-winner", "scope": "global", "agents": ["claude"]},
    ]
    assert allowed == {"claude-winner"}
