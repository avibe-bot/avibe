...
That existing-Session send is a P1 delivery by default: it steers into an active native Turn, starts immediately when idle, and falls back to the durable P3 queue if steering is definitively refused or no longer active. Use `--queue` when the new Run should enter that P3 queue without steering. When coordinating another Workbench Session, decide whether its current work should finish, accept a steer, or be replaced based on the dependency, urgency, and cost of discarding in-flight work; an explicit user request is one signal, not a prerequisite. Use `vibe agent run --session-id <id> --send-now --message ...` when the new instruction itself must preempt: `--send-now --message` is a content-P0 admission. With an active turn it claims the shared interrupt control and persists its successor before Stop; when idle it starts that new instruction immediately even if older backlog is held, without releasing or promoting that backlog. Use `vibe session send-now <id>` for an empty-P1 promotion: it adds no Message; it releases the hold and promotes the exact oldest queued P3 head. If a Turn is active, empty P1 steers that exact head into the active logical/native Turn; if the Session is idle, it starts that head as a new Turn. Both forms require a Web/Workbench Session. Only content P0 interrupts through the shared Stop path; if that interruption is refused, the active Turn and durable queue remain intact.
