...
- To provide an agent-facing representation, add `api/agent-markdown.ts` and export a GET handler that returns UTF-8 Markdown with `Content-Type: text/markdown; charset=utf-8`. A caller requesting the Show Page with `Accept: text/markdown` receives this handler response instead of the human-facing HTML app.
- The Markdown handler is page-owned and receives the requested SPA pathname in the server-injected `X-Avibe-Show-Path` header (`/` for the root route). It receives no visitor identity, Avibe Session/Project/Organization context, cookies, Authorization, CSRF credentials, or caller-supplied Show Runtime protocol headers. Public `/p/<share_id>/` Markdown is public data, so never include secrets or private information in it.
