...
class MemoryMaintenance:
    ...
    def _migrate_legacy(self, store: MemoryStore | None, *, lease_held: bool = False) -> None:
        ...
        try:
            if store is not None:
                current_epoch = store.ensure_meta().epoch
                self._intent.reconcile_legacy(current_epoch=current_epoch)
        ...
        except ClearIntentError as error:
            self._legacy_migration_deferred = False
            self._intent_error = error
            logger.warning("Memory Clear state could not be persisted; keeping Memory fenced")
        except Exception as error:
            self._legacy_migration_deferred = True
            logger.exception("Legacy Memory state reconciliation failed")
        finally:
    ...
    async def reconcile_pending(self, *, lease_held: bool = False) -> bool:
        ...
        if intent is None:
            return True
        if intent.state == "failed" and intent.error_code == LEGACY_RERUN_ERROR_CODE:
            # Legacy semantics are deliberately not interpreted. Only a new
            # user Clear may replace this fail-closed authority.
            return False
        ...
        async with self._runtime.exclusive_fence():
            ...
            try:
                ...
                if intent is None:
                    ...
                    meta = await self._run_maintenance_io(self._store.ensure_meta)
                    if (
                        intent.error_code == LEGACY_RERUN_ERROR_CODE
                        or meta.epoch not in {intent.pre_epoch, intent.target_epoch}
                    ):
                        intent = ClearIntent.new(operator_ref=operator_ref, pre_epoch=meta.epoch)
                    else:
                        intent = intent.deleting()
