...
_RELEASED_TEMP_CONTROL_FILES = {
    f".{name}.tmp": name for name in _RELEASED_CONTROL_PREFIXES
}
...
class ProviderRoot:
    ...
    def _require_released_unsentinelled_root(self) -> None:
        ...
        try:
            ...
            if root_fd is not None:
                os.close(root_fd)
        released_controls = set(_RELEASED_CONTROL_PREFIXES)
        temporary_controls = set(_RELEASED_TEMP_CONTROL_FILES)
        root_controls = names & (released_controls | temporary_controls)
        if not root_controls or not names.issubset(
            _RELEASED_UNSENTINELLED_ROOT_ENTRIES
        ):
            raise ProviderRootError(
                "memory provider root has unrecognized released-state entries"
            )
        provider_entries = names - released_controls - temporary_controls
        if provider_entries and (
            not released_controls.issubset(names) or names & temporary_controls
        ):
            raise ProviderRootError(
                "memory provider root has data beside incomplete controls"
            )
        ...
        for name, prefix in _RELEASED_CONTROL_PREFIXES.items():
            ...
            if not generated_payload.startswith(prefix):
                raise ProviderRootError(
                    "memory provider root generated controls do not match"
                )
            generated_payloads[name] = generated_payload
            ...
            if root_payload != generated_payload:
                raise ProviderRootError(
                    "memory provider root generated controls do not match"
                )

        for temporary_name, control_name in _RELEASED_TEMP_CONTROL_FILES.items():
            if temporary_name not in names:
                continue
            temporary_payload = self._read_private_control(
                self.path / temporary_name,
                f"memory provider root {temporary_name}",
            )
            if not generated_payloads[control_name].startswith(temporary_payload):
                raise ProviderRootError(
                    "memory provider root temporary control does not match"
                )
