...
import { isEditableFile, isEditableMeta, isRenderOnlyImage, previewableDoc, type DocPreviewKind } from '../../lib/filePreview';
import {
  contentUrl,
  downloadFile,
...
export const AppsFileBrowserPage: React.FC<{ windowed?: boolean; windowId?: string }> = ({ windowed = false }) => {
  ...
  const [preview, setPreview] = useState<{ path: string; name: string; kind: 'image' | DocPreviewKind } | null>(null);
  useEffect(() => {
    if (!preview) return;
  ...
  const open = async (e: FsEntry) => {
    ...
    // A raster image, or a rich document we can render read-only (DOCX / XLSX / PPTX / PDF) → preview
    // in the in-window overlay instead of downloading. Works on mobile too (the overlay is in-window,
    // unlike the desktop-only editor).
    if (isRenderOnlyImage(e)) {
      setPreview({ path: full, name: e.name, kind: 'image' });
      return;
    }
    const doc = previewableDoc(e);
    if (doc) {
      setPreview({ path: full, name: e.name, kind: doc });
      return;
    }
        ...
        <div className="absolute inset-0 z-20 flex flex-col bg-surface">
          ...
          <div className="min-h-0 flex-1">
            {preview.kind === 'image' ? (
              ...
              <FilePreview kind={preview.kind} path={preview.path} name={preview.name} />
            )}
          </div>
        </div>
      )}
