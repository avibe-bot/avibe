    ...
    <div className={clsx('space-y-4', containerClass)}>
      <div className={clsx('grid grid-cols-1 gap-3', accessGridCols)}>
        ...
        {/* Require bind (channels only): when On, only bound users can drive the
            agent here; unbound members are silently ignored. Same on/off switch
            semantics as require @mention above. */}
        {showRequireMention && (() => {
          const effective =
            value.require_bind === null || value.require_bind === undefined
              ? !!(globalConfig as any)?.[inheritsFromKey || '']?.require_bind
              : value.require_bind;
          return (
            <div className="space-y-1">
