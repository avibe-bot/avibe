...
export const SupplyGraph: React.FC<{
  ...
  return (
    <svg aria-hidden="true" className="pointer-events-none absolute inset-0 z-10 hidden size-full overflow-visible lg:block" viewBox={`0 0 ${drawing.width} ${drawing.height}`} preserveAspectRatio="none">
...
export const SupplyLegend: React.FC<{ relations: SupplyRelation[] }> = ({ relations }) => {
  ...
  return (
    <div className="model-hub-legend flex flex-wrap items-center justify-between gap-x-5 gap-y-2 px-0.5 py-2 text-[11px] text-muted">
      ...
      <span className="flex items-center gap-1.5 text-muted/55">
        <span className="hidden sm:inline">{t('settings.models.legend.note')}</span>
        <ModelHubInfoHint label={t('settings.models.legend.note')} content={t('settings.models.legend.note')} className="size-[20px] text-muted/55" align="end" />
      </span>
    </div>
  );
};
