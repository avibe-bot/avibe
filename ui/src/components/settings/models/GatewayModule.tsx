      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2 pt-3">
        {supply.kind === 'loading' && agents === undefined
          ...
          : supply.kind === 'unread'
            ...
            : <>
                {supply.kind === 'degraded' && supply.cause === 'read_failed' && <div className="mb-2 flex items-center justify-between gap-3 rounded-lg border border-destructive/25 bg-destructive/[0.08] px-3 py-2"><p className="text-[11px] text-destructive-ink">{readFailureCopy ?? t('settings.models.gateway.supply.unread')}</p><Button variant="outline" size="xs" onClick={onRetry}>{t('settings.models.gateway.retry')}</Button></div>}
