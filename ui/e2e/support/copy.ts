...
// its accessible name, and every name comes from `src/i18n/en.json`. Reading
// the shipped bundle instead of pasting the strings means a copy change breaks
// the test at the key, where the diff is legible, rather than at a locator that
...
export const copyOrNull = (key: string, vars?: Vars): string | null => {
  const count = vars?.count;
  const candidates = typeof count === 'number'
    ? [`${key}_${new Intl.PluralRules('en-US').select(count)}`, key]
    : [key];
  for (const candidate of candidates) {
    const template = resolve(bundle, candidate.split('.'));
 ...
 */
export const copy = (key: string, vars?: Vars): string => {
  const text = copyOrNull(key, vars);
  if (text === null) throw new Error(`i18n key missing from src/i18n/en.json: ${key}`);
  return text;
};
...
export const hub = (key: string, vars?: Vars): string => copy(`settings.models.${key}`, vars);

/** Nullable form of {@link hub}. */
export const hubOrNull = (key: string, vars?: Vars): string | null =>
  copyOrNull(`settings.models.${key}`, vars);
